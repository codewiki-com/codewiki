...
const DEFAULT_USER_AGENT = 'codewiki-linkcheck/1.0';
...
export async function checkLinks(
  ...
  for (const url of unique) {
    ...
    if (cached && isFresh(cached, now())) fresh.set(url, { ...cached, via: 'cache' });
  ...
  await Promise.all(
    Array.from({ length: workers }, async () => {
      for (let index = next++; index < pending.length; index = next++) {
        ...
        const status = await probe(url, { fetchImpl, timeoutMs, userAgent, now });
...
async function probe(url: string, options: ProbeOptions): Promise<LinkStatus> {
  ...
  return verdict(await request(url, 'GET', options), 'GET', options.now);
}

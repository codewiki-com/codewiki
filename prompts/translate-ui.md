...
- Use full-width punctuation (`，。：、；？“”`) in Chinese prose, quoting with “” rather than 「」; keep half-width punctuation inside Latin fragments, and keep the `·` separators that the English copy uses.
- Put no space between CJK characters and adjacent Latin letters, digits, placeholders or code (`标准深度约{count}`, `在AI时代精通编程`); the site sets `text-autospace`, so the browser adds the gap.

...
Legend: **[user]** = needs a decision or action from the owner · **[design]** = design lead (Fable) designs first · **[Opus]** / **[Codex]** = suggested implementer.
...
- [ ] **B3. Rewrite the "In the AI era" sections.** [Codex, bulk] Across all topics: no unevidenced "AI often does X"; state the failure modes to check for in that scenario, each with a counterexample or a failing test; keep the four-part structure. Update `prompts/editorial-standard.md` §"In the AI era" first.

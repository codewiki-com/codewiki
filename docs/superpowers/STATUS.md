...
- **Merged:** `feat-daily-kata` (daily kata card on the home page + 7-day rotation; screenshots reviewed; `ledgers/2026-09-04-daily-kata-report.md`; the shared-script budget was raised to 63,488 B). Follow-up noted: on 390 px the "+N more lines" chip overlays the last visible code line — move it under the peek on mobile in a later polish.
- **Merged:** `feat-pwa` (0f6d3fd; manifest + icons, `sw.js` built by an Astro integration with a content hash, shell precache 383 KiB, visited pages LRU 300, per-track "save offline", 27 MB runtimes on request, update toast, `/offline/` pages, `Cache-Control: no-cache` for `sw.js`; `ledgers/2026-09-04-pwa-report.md`). Unverified until deploy: iOS install, the `no-cache` header on Cloudflare, the runtimes download end to end.
- **Merged:** `content-titles-1..3` (ff85c09, 6c2a06f; 450 bilingual kata titles, all within 48/20 chars; the only untitled items are in the three python banks the fix round deletes). main after the merges: lint, check and 2,383 unit tests green (one interview tag renamed away from the `calibration` placeholder, d43f66b).
...
- **Remaining:** merge `fix-site-review` when its report lands (it merges main before its gate); then full gate on `main`; after each merge `pnpm lint && pnpm check && pnpm test`; then full gate on `main` (`pnpm build && pnpm check:links && pnpm test:e2e && pnpm exec lhci autorun`), then deploy prep (Cloudflare Pages needs the user's auth).
- **Deploy facts (2026-09-05):** no git remote yet; `gh` is logged in as tomchen; no `wrangler`. `dist` = 523 MB, 15,964 files (Cloudflare Pages limit 20,000 files, 25 MiB per file; largest file `llms-full.txt` 13.9 MB). Tier 2/3 content (661 topics) would push the file count past 20,000 — before that wave, cut Pagefind fragments (6,050 files; raise the chunk size) and/or OG images (2,262), or pick a host without the limit. Simplest launch path: push to GitHub, connect the repo in Cloudflare Pages (build `pnpm build`, output `dist`, Node 24, pnpm), then add the custom domain — needs the user at the dashboard.

...
- **Codex quota exhausted at 13:02Z (resets 2026-09-11 05:45, or buy credits).** Until then: code work falls back to Opus subagents; content polish is paused (15 batch-6 topics failed on the quota, attempts=1 — do not re-run `content:polish` until Codex is back, or the attempts burn out).

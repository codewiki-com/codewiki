...
- **Running — Codex (3, disjoint quiz directories, logs `.superpowers/sdd/titles/codex.log` per worktree):** `content-titles-1..3` add bilingual `title` to the 455 untitled review/spotbug katas (brief `.superpowers/sdd/titles/brief.md`); they skip the three python banks the fix round deletes.
...
- **Merge order (remaining):** titles-1..3 → fix-site-review (the fix round merges main before its gate); after each merge `pnpm lint && pnpm check && pnpm test`; then full gate on `main` (`pnpm build && pnpm check:links && pnpm test:e2e && pnpm exec lhci autorun`), then deploy prep (Cloudflare Pages needs the user's auth).

...
- **Running — Opus:** PWA offline on `feat-pwa` (report → `ledgers/2026-09-04-pwa-report.md`).
...
- **Merge order (remaining):** titles-1..3 → pwa → fix-site-review (the fix round merges main before its gate); after each merge `pnpm lint && pnpm check && pnpm test`; then full gate on `main` (`pnpm build && pnpm check:links && pnpm test:e2e && pnpm exec lhci autorun`), then deploy prep (Cloudflare Pages needs the user's auth).

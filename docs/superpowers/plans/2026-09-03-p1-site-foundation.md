...
**Who writes the Chinese strings:** the implementer writes `en.ts` and creates `zh.ts` with the same keys whose values are the English strings prefixed with `[zh] ` (so the key-parity and non-empty tests pass). The controller then runs Codex with `prompts/translate-ui.md` (created in this task, see Step 5) to replace every `[zh] …` value with natural Chinese UI copy. The implementer must not translate.

- [ ] **Step 5: Write `prompts/translate-ui.md`**: a Codex brief that says: read `src/i18n/en.ts` and `src/i18n/zh.ts`; replace every value starting with `[zh] ` in `zh.ts` with concise, natural Simplified Chinese UI copy for a developer audience (keep `{vars}` placeholders, keep product names and code identifiers in English, prefer 2–6 characters for nav labels, full-width punctuation, a space between CJK and Latin); touch no other file; run `pnpm vitest run tests/unit/i18n.test.ts` and print `TRANSLATE-UI DONE` when it passes.

- [ ] **Step 6: Run tests, expect pass. Commit** `git add -A && git commit -m "feat: i18n dictionaries and locale-aware URL helpers"`
...
- [ ] **Step 6: Placeholder content (implementer) and real sample content (Codex).** The implementer creates placeholder files so the collections are non-empty and the schema tests run: `python/closures.{en,zh}.mdx` and `javascript/event-loop.{en,zh}.mdx` with complete, schema-valid frontmatter (`status: reviewed`, `reviewed: 2026-09-03`, `verified: { version: "Python 3.14", date: 2026-09-03 }` / `Node 24`, `terms: [free-variable, enclosing-scope, late-binding, cell]`, `quiz: python/closures`, `prerequisites: [python/functions, python/scope-legb]`, `related: [python/decorators, python/functools]`) and a one-line body `Placeholder — replaced by Codex.`; glossary placeholders `free-variable.yaml`, `enclosing-scope.yaml`, `late-binding.yaml`, `cell.yaml`, `closure.yaml`, `event-loop.yaml`, `microtask.yaml` with `short: { en: "placeholder", zh: "占位" }`; quiz `python/closures.yaml` with one valid `mcq` placeholder item; path `python-from-zero.yaml` with 5 milestones using the topic ids from the Path mockup (`docs/design/mockups/Path.dc.html`; topics that do not exist yet are allowed; the page marks them "coming soon"). The implementer must not write article prose, glossary definitions or quiz questions beyond these placeholders.

The controller then dispatches Codex with `prompts/write-sample-topics.md` (implementer creates this brief in this step) which instructs Codex to: follow `prompts/editorial-standard.md`; write `python/closures.{en,zh}.mdx` in full (TL;DR with three cells, sections "What a closure is", "Late binding: the trap", a runnable fence ` ```python run title="make_counter.py" ` whose real output is `1 2 3` then `3`, a `> [!PITFALL]` callout, an "In the AI era" section, a `<Depth level="deep">` section "How CPython stores cells", `<Checkpoint id="python/closures" />`), the zh file as the faithful aligned translation; write `javascript/event-loop.{en,zh}.mdx` (~120 lines) with one runnable ` ```js run ` fence for `console.log(1); setTimeout(() => console.log(2)); Promise.resolve().then(() => console.log(3)); console.log(4);` whose output is `1 4 3 2`; write the seven glossary terms with real bilingual `short` definitions; write `python/closures.yaml` with 1 `predict` (the `[2, 2, 2]` puzzle) + 2 `mcq` + 1 `review`; keep every frontmatter field the implementer set; run `pnpm astro sync && pnpm test` and print `SAMPLES DONE`. Components used in the MDX (`TLDR`, `TLDRCell`, `Depth`, `Term`, `Checkpoint`) are implemented in Task 11; until then the MDX only needs to parse (`astro sync`).

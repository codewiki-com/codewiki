...
These four self-contained examples build through windows, staged queries, recursion, and null-safe exclusion logic. The output came from SQLite 3.45.1; it wasn't calculated by hand.
...
```sql run title="window_report.sql"
...
```sql run title="null_safe_anti_join.sql"

...
Models mix APIs from similarly named libraries, place a `styled()` declaration inside a React component, and invent prop-filtering options from another major version. They also turn every prop into a template interpolation, including high-cardinality values that should be custom properties. Server examples are often copied from an older router or rendering mode, so the page appears correct after navigation but arrives without complete styles on the first response.

**Ask your AI to check**

- Name the styling package, package version, framework rendering mode, and exact server integration used by this component.
- Classify every dynamic value as a finite variant or a high-cardinality instance value, then show where its CSS is emitted.
- Build for production and compare server and client class identifiers, rule order, and rendered DOM attributes.

**Review checklist**

- Search component bodies for style-factory calls and test that focused or stateful descendants are not remounted.
- Inspect the production CSS, the first server response, and hydration logs for missing, duplicated, or reordered rules.
- Check generated selectors in the browser's computed-style panel, including global CSS, layers, themes, and supported consumer overrides.

**Prompt vocabulary**

Use the exact terms `runtime CSS-in-JS`, `build-time extraction`, `static extraction`, `style injection`, `deterministic class name`, `style registry`, `prop forwarding`, `finite variant`, `high-cardinality value`, `CSS custom property`, `cascade layer`, and `hydration mismatch`. Ask for emitted HTML and CSS rather than stopping at the authoring snippet.

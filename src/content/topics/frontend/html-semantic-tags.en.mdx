...
Models often map every rectangle in a mockup to `<div>`, then simulate links and buttons with `onClick`; the opposite failure converts every wrapper to `<section>`. Generated pages also accumulate multiple visible `<main>` elements, several unnamed `<nav>` elements, repeated `banner` landmarks inside articles, and headings arranged by visual size instead of content hierarchy. Component conversions regularly drop a native button's `type`, causing it to submit unexpectedly when later placed in a form.

**Ask your AI to check**

- List every landmark's element, computed role, and accessible name, then flag duplicate or unnamed landmarks of the same type.
- Print every heading and level in DOM order, explaining which content relationship justifies each level change.
- Find every mouse event handler, verify that actions use buttons and navigation uses links, and trace keyboard activation, disabled state, and form submission.

**Review checklist**

- Confirm in the browser's Elements panel that the parsed DOM matches the template's intent and inspect whether invalid nesting was repaired.
- Check final names, roles, states, and landmark counts in the accessibility tree; do not treat class names or a screenshot as evidence.
- Complete skip navigation, menu opening, form submission, and back navigation using only a keyboard, confirming focus order matches visual order.

**Prompt vocabulary**

Use the exact terms `semantic HTML`, `implicit role`, `landmark region`, `accessible name`, `heading hierarchy`, `native control`, `sectioning content`, and `parsed DOM`. Ask the AI for element-selection reasons and browser verification steps; "make this accessible" is too vague to expose incorrect roles and interaction paths.

...
模型常把设计稿中的每个矩形都映射为 `<div>`，再用 `onClick` 模拟链接或按钮；另一种极端是把每个容器都改成 `<section>`。生成页面还容易出现多个可见 `<main>`、无名称的多个 `<nav>`、文章内部重复的 `banner` 地标，以及按视觉字号而不是内容层级排列的标题。组件转换尤其容易丢掉原生按钮的 `type`，导致它放进表单后意外提交。

**让 AI 检查**

- 列出页面中每个地标的元素、最终角色与可访问名称，并标出重复或无名称的同类地标。
- 按 DOM 顺序输出所有标题及级别，说明每次级别变化对应哪一层内容关系。
- 找出所有鼠标事件处理器，确认操作使用按钮、导航使用链接，并追踪键盘激活、禁用状态与表单提交路径。

**审查清单**

- 在浏览器 Elements 面板确认解析后的 DOM 与模板意图一致，并检查无效嵌套是否被改写。
- 在可访问性树中核对名称、角色、状态和地标数量，不把类名或截图当作证据。
- 只用键盘完成跳过导航、打开菜单、提交表单和返回上一页，确认焦点顺序与可见顺序一致。

**提示词词汇**

使用准确术语：`semantic HTML`（语义化 HTML）、`implicit role`（隐式角色）、`landmark region`（地标区域）、`accessible name`（可访问名称）、`heading hierarchy`（标题层级）、`native control`（原生控件）、`sectioning content`（分节内容）、`parsed DOM`（解析后的 DOM）。要求 AI 给出元素选择理由和浏览器验证步骤；“让页面更无障碍”不足以暴露错误的角色与交互路径。

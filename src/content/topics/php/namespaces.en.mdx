...
Models often import Java or JavaScript intuitions into PHP: they assume `use` loads a file, separate namespaces with dots or forward slashes, or put an import inside a method. They also concatenate short class names into strings without noticing that static imports do not rewrite dynamic values. When generating PSR-4 files, they frequently disagree on `Service` versus `Services` or the case of the class and filename; another subtle failure is editing closure capture `use ($value)` as though it were a namespace import.

**Ask your AI to check**

- List the complete compiled name for every class-like reference, and say whether it comes from an import, the current namespace, a fully qualified name, or a dynamic string.
- For every PSR-4 prefix in `composer.json`, calculate the exact path and case implied by the remainder of each class name.
- Check imports and fallback separately for classes, functions, and constants, and find every class name assembled from external input.

**Review checklist**

- Autoload every added or moved class through Composer on a filesystem with the same case sensitivity as production.
- Confirm that factories, container configuration, and callback strings use `Type::class` or a closed allowlist instead of assuming a short string resolves.
- Confirm every colliding short name has a clear alias and no file-level import has been mistaken for closure capture or loading.

**Prompt vocabulary**

Use the precise terms `namespace declaration`, `unqualified name`, `qualified name`, `fully qualified name`, `import alias`, `global fallback`, `dynamic class name`, and `PSR-4 prefix mapping`. Asking for “the resolved complete name and file path” exposes the break between the two mechanisms more reliably than asking an AI to “check the imports.”

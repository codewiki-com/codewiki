...
Models often add two individually valid traits to one class but omit adaptation for their same-named methods, causing a fatal error when autoloading reaches the class. They also mistake `as private alias` for renaming the original method and leave the original public entry point in the API. Other generated code assumes `$this->repository` exists or puts request state into a trait static property to "reuse a counter." Code learned from older PHP behavior may also assume a parent and a child that uses the trait again share static state, which is false in PHP 8.3.

**Ask your AI to check**

- List members contributed by the class, its parent, and every direct or nested trait; mark duplicate methods, properties, and constants, then name each final source.
- For every `insteadof` and `as`, state the original name, alias, visibility, and callable entry points instead of merely explaining the syntax.
- Find every `$this` access and static write inside traits, then identify the host contract, state owner, initialization site, and process lifetime.

**Review checklist**

- Run `php -l` and tests for every changed consumer so collisions are found before production autoloading.
- Invoke reused behavior through an interface and verify that a second implementation without the trait remains substitutable.
- On PHP 8.3, test static state across the parent, a child that only inherits, and a child that uses the trait again.

**Prompt vocabulary**

Use the exact terms `trait composition`, `method precedence`, `conflict resolution`, `insteadof adaptation`, `method alias`, `abstract trait method`, `host class requirement`, and `trait static property`. Ask for a member-source table and a state-ownership table; "reuse this code" is too vague to determine the public contract.

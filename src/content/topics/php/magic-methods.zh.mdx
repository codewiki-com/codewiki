---
title: 魔术方法
description: 理解 PHP 8.3 的魔术方法触发规则、安全分派、克隆与序列化契约，避免把隐式钩子变成难以检查的隐藏接口。
track: php
section: oop
...
<TLDR>
  <TLDRCell label="what">
    <Term id="magic-method">魔术方法（magic method）</Term>是由 PHP 在特定对象操作发生时调用的 `__*` 钩子。它们定义对象如何处理不可访问成员、字符串上下文、调用、克隆与序列化。
  </TLDRCell>
  <TLDRCell label="trap">
...
PHP 所说的<Term id="property-overloading">属性重载（property overloading）</Term>不是 Java 或 C++ 中按参数签名选择方法的重载。这里的含义是在读取、写入、检查或删除不可访问属性时动态处理操作。不可访问包括不存在的属性，也包括当前调用作用域不能访问的 `private` 或 `protected` 属性。
...
字符串上下文调用 `__toString(): string`。拥有该方法的类也会被 PHP 视为实现了 `Stringable`，但显式声明接口能让意图更清楚。对象后跟圆括号时调用 `__invoke()`，因此这类实例可以传给需要<Term id="callback">回调（callback）</Term>的 API。
...
<TryToBreak items={['把 nickname 设为空字符串', '读取未列入允许列表的 locale', '给 timezone 赋整数']} />
...
以整数分表示金额，避免示例把二进制浮点舍入混入魔术方法主题。`__invoke()` 的参数和返回类型仍是普通 PHP 类型契约，调用对象不会绕过类型检查。
...
模型常生成对所有名称返回 `null` 的 `__get()`，或在 `__set()` 中执行 `$this->{$name} = $value`，从而隐藏拼写错误并创建已弃用的动态属性。它还会用 `call_user_func_array([$service, $name], $arguments)` 建立无限制代理，把整个 `get_object_vars()` 结果序列化，或者把刷新缓冲区和提交事务塞进 `__destruct()`。这些实现通常能通过单一路径演示，却没有定义未知名称、可空字段、恶意方法名、嵌套对象复制和进程异常退出的行为。

**让 AI 检查**

- 列出每个魔术方法的准确触发操作，并给出一个不会触发它的对照操作。
- 从外部名称追踪到最终属性或方法，标出允许列表、类型检查、授权和拒绝路径。
- 枚举克隆与序列化状态，区分拥有对象、共享服务、秘密、临时缓存和恢复时必须重建的字段。

**审查清单**

- 用拼写错误、大小写变体、未知方法、额外实参和显式 `null` 测试后备钩子。
- 修改克隆中的每个嵌套可变对象，确认原对象只共享契约规定的引用。
- 检查序列化字节是否包含秘密，并证明关键清理在不调用析构器时也能完成。

**提示词词汇**

使用准确术语：`magic method`（魔术方法）、`property overloading`（属性重载）、`inaccessible member`（不可访问成员）、`fallback dispatch`（后备分派）、`dynamic property`（动态属性）、`shallow copy`（浅拷贝）、`deep copy`（深拷贝）、`serialization hook`（序列化钩子）和 `object injection`（对象注入）。要求模型提供触发矩阵、名称允许列表和状态所有权表，比「给这个类加魔术方法」更容易暴露隐藏契约。
...
- [PHP 手册：魔术方法](https://www.php.net/manual/en/language.oop5.magic.php)
- [PHP 手册：重载](https://www.php.net/manual/en/language.oop5.overloading.php)
- [PHP 手册：对象克隆](https://www.php.net/manual/en/language.oop5.cloning.php)
- [PHP 手册：对象序列化](https://www.php.net/manual/en/language.oop5.serialization.php)
- [PHP 手册：`unserialize()`](https://www.php.net/manual/en/function.unserialize.php)
- [PHP 手册：动态属性](https://www.php.net/manual/en/language.oop5.properties.php#language.oop5.properties.dynamic-properties)

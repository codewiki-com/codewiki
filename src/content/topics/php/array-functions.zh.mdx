...
模型常把不同语言的数组 API 混在一起：把 PHP 的 `array_reduce()` 写成“首项自动作为初始值”，假定 `array_filter()` 会重新编号，或把 `usort()` 当作返回新数组的函数。配置代码中还常见 `array_merge($user, $defaults)` 的覆盖方向写反，以及为了“深合并”机械换成 `array_merge_recursive()`，最终把重复标量变成数组。

**让 AI 检查**

- 标出每一步的键集合，并说明结果是列表还是映射。
- 对每个回调列出参数顺序、返回类型和空输入行为。
- 找出所有原地修改与宽松比较，并给出保留输入、使用严格比较的版本。

**审查清单**

- 用 `0`、`"0"`、`null`、空字符串和空数组测试筛选条件。
- 用重复字符串键、非连续整数键和嵌套数组测试合并政策。
- 用小于、相等、大于三组元素测试比较函数，并确认调用后原数组是否允许改变。

**提示词词汇**

提示中使用 `key preservation`（键保留）、`reindexing`（重新编号）、`initial accumulator`（初始累加器）、`strict comparison`（严格比较）、`in-place sort`（原地排序）、`stable sort`（稳定排序）和 `comparator contract`（比较函数契约）。再明确说明输出需要 PHP 列表还是映射；只说“处理这个数组”无法约束键语义。

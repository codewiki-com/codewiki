...
模型常把 `catch (Exception)` 生成为“捕获所有错误”，因此漏掉 PHP 8 中常见的 `TypeError` 和 `ValueError`。它也会生成空处理器、在每层记录后重新抛出、包装时遗漏 `$previous`，或在 `finally` 中 `return`。另一个真实风险是把异常消息直接塞进 JSON 响应，连同文件路径、SQL 或上游服务细节一起泄露。

**让 AI 检查**

- 列出每个 `try` 实际可能产生的 `Throwable` 类型，并说明每个 `catch` 为什么能恢复、转换或终止。
- 沿所有返回和抛出路径跟踪事务、锁、流与临时处理器，确认所有权和 `finally` 清理成对出现。
- 找出所有重新包装和用户响应，验证 `$previous` 得到保留，而内部消息、堆栈和秘密不会越过公开边界。

**审查清单**

- 用一个预期业务异常和一个 `TypeError` 测试同一入口，检查捕获范围是否符合设计。
- 断言回滚或资源释放在成功、异常和提前返回路径都执行一次，并且不会覆盖原结果。
- 检查日志去重、关联字段与脱敏；同一次失败只由拥有完整上下文的边界记录。

**提示词词汇**

使用准确术语：`Throwable hierarchy`（`Throwable` 层次）、`first matching catch`（第一个匹配的捕获块）、`multi-catch`（多类型捕获）、`exception propagation`（异常传播）、`exception wrapping`（异常包装）、`previous exception`（前一个异常）、`stack unwinding`（调用栈展开）、`recovery boundary`（恢复边界）和 `finally cleanup`（`finally` 清理）。要求模型给出类型与资源所有权表，比“添加错误处理”更容易暴露过宽捕获和清理缺口。

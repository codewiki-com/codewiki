...
模型经常为列表参数生成 `items=[]`，让测试顺序决定结果；也会在某个分支遗漏 `return`，把意外的 `None` 送进后续计算。修改接口时，生成代码还可能随意移动 `/` 或 `*`、把清晰的关键字形参替换成宽泛的 `**kwargs`，或者误把类型注解当成运行时验证。另一类常见错误是让本应返回数据的辅助函数直接打印、修改调用方容器或读取全局状态，从而隐藏副作用。

**让 AI 检查**

- 列出每个默认表达式的求值时机，并标出所有可变默认对象。
- 为每个返回路径写出实际返回类型，找出执行到函数末尾而隐式返回 `None` 的路径。
- 根据签名模拟位置、关键字、重复和未知实参的绑定，确认公开调用方式没有被意外破坏。

**审查清单**

- 连续调用函数两次，并检查第二次是否受到第一次留下的默认状态或全局状态影响。
- 比较调用前后的可变实参，确认每个就地修改都是契约的一部分。
- 用静态检查和运行时边界测试分别验证注解与真实输入，不把其中任何一个当成另一个的替代品。

**提示词词汇**

使用准确术语：`parameter`（形参）、`argument`（实参）、`argument binding`（实参绑定）、`positional-only parameter`（仅限位置形参）、`keyword-only parameter`（仅限关键字形参）、`mutable default argument`（可变默认实参）、`implicit None return`（隐式返回 `None`）、`side effect`（副作用）和 `first-class function`（一等函数）。要求 AI 给出“每条返回路径”和“调用前后对象状态”，比笼统地要求“检查这个函数”更容易暴露真实错误。
...
生成测试的 AI 容易照着实现复制同一个错误，尤其会用被测公式计算预期值。审查时应从需求或手工算出的独立样例得到预期结果，并故意加入能区分两种实现的输入。测试只有在预期值来源独立时，才提供新的证据。

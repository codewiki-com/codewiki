...
Models often describe `partial(client.request, timeout=5)` as enforcing a timeout while leaving the caller's override path open. They may import 3.14-only `Placeholder` when the deployed runtime is still 3.13, or incorrectly put a placeholder in keyword arguments. Wrapper code may add `@wraps` and claim full transparency while dropping keywords, losing the return value, or wrapping a coroutine with a synchronous function. Data pipelines frequently omit a `reduce()` initializer after a filter that can become empty, and generated comparators often return a Boolean.

**Ask your AI to check**

- Expand every partial into its final call, listing stored arguments, call-time arguments, overridden keywords, and every `Placeholder` fill position.
- Check runtime forwarding separately from the metadata chain for every wrapper, using `inspect.signature()` plus success, exception, and async paths.
- State the empty-input result of every `reduce()` and verify comparator sign and transitivity with less-than, equal, and greater-than cases.

**Review checklist**

- The target Python version supports every imported `functools` name, especially 3.14 `Placeholder` and `reduce(initial=...)`.
- Stored argument override rules match the authorization and configuration contract, and binding a mutable object isn't mistaken for taking a snapshot.
- Empty inputs, duplicate sort keys, unsupported comparison types, and wrapper exception paths have tests.

**Prompt vocabulary**

Use the exact terms `higher-order function`, `partial application`, `call-time keyword override`, `Placeholder filling`, `descriptor binding`, `wrapper metadata`, `reduction initializer`, `three-way comparator`, and `rich comparison`. Ask the model to expand the final call signature and boundary inputs; “simplify this function” is too vague to expose binding rules.

---
title: 浅拷贝与深拷贝
description: Python如何复制对象图、保留共享引用、正确处理循环，并为类和不可变风格记录定义字段替换行为。
track: python
section: objects
...
Python变量保存的是对象引用。`backup = settings`这样的赋值会把另一个名称绑定到同一个对象，并不会复制对象。如果任一名称访问并修改了可变对象，另一个名称也会看到变化。
...
`copy`模块提供3种相关操作。`copy.copy()`创建浅拷贝，`copy.deepcopy()`创建深拷贝，`copy.replace()`创建相同类型的对象并替换指定字段。第3种操作在Python 3.13中加入，也包含在这里验证的Python 3.14目标中。
...
这张表描述的是所有权，而不是相等性。刚构造完成时，这5种结果都可能与源对象比较相等。真正重要的差异要通过对象标识和后续修改来观察。
...
在每个入口和出口都复制，并不会自动形成防御式设计。这样做可能掩盖不清楚的所有权，调用者也无法判断API返回的是实时视图、浅快照，还是隔离的对象图。应在操作名称或文档中写明这项契约。
...
下面的示例依次展示普通容器、对象图拓扑、结构化替换和类自定义策略。所有输出均由Python 3.14实际生成。

### 赋值、浅拷贝与深拷贝

这个示例从同一个嵌套字典出发，分别采用3种处理方式。修改操作有意同时涉及一个顶层字段和两个嵌套列表，以便清楚显示边界。
...
> [!PITFALL]
> AI建议或快速重构可能把代码写成`working = template`，仿佛这样就创建了工作副本。两个名称仍指向同一个对象，因此就地更新会改变模板。
>
...
>
> **修复方法：** 在API边界定义所有权。如果类把值状态与服务或资源混在一起，应优先采用不可变输入、显式构造函数，或`clone_for_request()`这样的领域方法。
...
有些值会被有意复用，而不是重新构造。Python复制文档明确说明函数和类会原样返回，并排除了若干运行时对象与外部资源类型。类的复制策略应区分持久值状态和进程中的实时资源。
...
Python还会复用通过`copyreg`注册的归约函数，这套机制也用于支持pickle。对于已经围绕归约协议设计的类，这种联系很有用，但并不表示复制与序列化可以互换。序列化会跨越表示边界，复制则在当前进程中构造对象。
...
`copy.replace(obj, **changes)`的适用范围有意小于浅拷贝和深拷贝。在Python 3.14中，它支持由`namedtuple()`创建的具名元组、数据类，以及实现`__replace__()`的用户自定义类。未知字段会按照受支持类型的行为被拒绝。
...
- [Python 3.14数据模型：对象、值与类型](https://docs.python.org/3.14/reference/datamodel.html#objects-values-and-types)

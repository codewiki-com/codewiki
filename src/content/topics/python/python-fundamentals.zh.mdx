...
模型经常把 Python 画成“变量保存值”的语言，然后在重构时复用同一个列表或字典，意外修改调用方数据。
它也会用 `is` 比较字符串、用 `x or default` 抹掉有效的零值、生成可变默认参数，或假定类型注解会验证
JSON。另一种常见失误是用一个宽泛的 `except` 把真实编程错误变成空列表、`None` 或伪造的成功响应。

**让 AI 检查**

- 列出每次赋值、函数调用和容器修改涉及的对象别名，并说明修改对哪些调用方可见。
- 为每个条件区分 `None`、零、`False` 与空容器，指出短路表达式实际返回哪个操作数。
- 跟踪每个异常的抛出点、捕获类型和传播路径，确认处理器没有覆盖无关操作。

**审查清单**

- 用空输入、零值、缺少键、错误类型和嵌套可变对象运行边界测试。
- 检查 `is`、可变默认参数、宽泛异常处理，以及未经验证的反序列化数据。
- 比较函数调用前后的输入对象，确认修改与返回值契约一致。

**提示词词汇**

使用准确术语：`name binding`（名称绑定）、`object identity`（对象标识）、`mutable object`（可变对象）、
`rebinding`（重新绑定）、`truth value testing`（真值测试）、`short-circuit evaluation`（短路求值）、
`argument binding`（实参绑定）和 `exception propagation`（异常传播）。要求 AI 给出对象别名表与失败路径，
比笼统地说“检查 Python 代码”更容易暴露共享状态和错误边界。

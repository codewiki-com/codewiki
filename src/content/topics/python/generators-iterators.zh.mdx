...
模型常把列表推导式改成生成器表达式，却保留后面两次遍历、`len()` 或索引操作；类型看起来更「流式」，调用契约却已经改变。它也会生成 `with open(...)` 内直接返回生成器表达式的代码，或在日志语句里执行 `list(rows)` 后再把已耗尽的 `rows` 交给业务逻辑。另一类错误是在生成器中写 `yield next(source)`，没有处理输入耗尽，于是正常结束变成 `RuntimeError`；涉及文件时，模型还经常漏掉提前 `break` 后的显式关闭路径。

**让 AI 检查**

- 列出每个参数是可迭代对象还是迭代器、谁创建它、谁消费它，以及是否允许重复遍历。
- 标出所有隐式消费点，包括 `list()`、`sum()`、成员测试、解包、日志与调试代码，并给出执行后的剩余序列。
- 沿正常耗尽、提前 `break`、解析异常和显式 `close()` 四条路径跟踪资源与 `finally`。

**审查清单**

- 用单遍迭代器测试所有消费者，并确认没有第二次读取、`len()` 或索引假设。
- 让输入为空、只有一项、含无效项，并检查生成器是正常结束还是错误地抛出 `RuntimeError`。
- 在首项后停止消费，确认文件、游标或锁立即释放，而不是等待垃圾回收。

**提示词词汇**

使用准确术语：`iterable`（可迭代对象）、`iterator`（迭代器）、`iterator protocol`（迭代器协议）、`single-pass`（单遍）、`lazy evaluation`（惰性求值）、`generator frame`（生成器帧）、`exhaustion`（耗尽）、`StopIteration`（迭代结束异常）、`yield from delegation`（`yield from` 委托）、`resource ownership`（资源所有权）。要求模型输出消费点清单和资源生命周期表；只说「改成生成器以节省内存」不足以证明改动正确。

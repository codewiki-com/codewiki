...
Models produce `@handler.register(list[Order])` as if it were a valid runtime registration, or assume `handler(records, format)` examines both arguments. They also overlook that `bool` inherits from `int`, so a handler registered for integer amounts unexpectedly accepts flags. In generated plugin code, registrations can live in a module that no startup path imports; in generated class methods, `@classmethod` may wrap `@singledispatchmethod`, making `.register` unavailable in the class body.

**Ask your AI to check**

- List the actual dispatch position for every generic function and identify a runtime class for every registration instead of merely repeating annotations.
- For each sample call, show `type(first_arg)`, exact registrations, MRO candidates, ABC candidates, and the final `dispatch()` result.
- Check plugin import paths, duplicate type registrations, and variant signatures, then identify registry differences between startup environments.

**Review checklist**

- Exercise the default and inherited paths with an unregistered type, a subclass, `bool`, and a representative virtual subclass.
- Confirm that no `list[T]`, field value, or second argument is being treated as a runtime dispatch condition.
- Compare keyword parameters, exceptions, and return contracts across variants, and verify `singledispatchmethod` decorator order.

**Prompt vocabulary**

Use the exact terms `single dispatch`（单分派）, `generic function`（泛型函数）, `dispatch type`（分派类型）, `registered implementation`（已注册实现）, `method resolution order`（方法解析顺序）, `abstract base class`（抽象基类）, `virtual subclass`（虚拟子类）, and `dispatch ambiguity`（分派歧义）. Ask for a dispatch matrix and registration timeline; “implement function overloading” doesn't say which runtime value is inspected.

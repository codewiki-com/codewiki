...
模型常把 `(item)`、`(item,)` 和 `tuple(item)` 混为一谈，也会把“元组不可变”误写成“其中所有对象都不可变”。处理缓存或分组键时，生成代码可能直接把含列表的记录转成外层元组，直到第一次哈希才失败。另一类错误是在函数新增返回值后，只更新函数而遗漏固定长度解包的调用方，或者假定 `NamedTuple` 注解已经验证了外部数据。

**让 AI 检查**

- 列出每个元组的预期长度和每个位置的含义，并找出依赖固定长度解包的所有调用点。
- 沿着复合字典键递归检查元素的可哈希性，以及参与哈希的状态能否变化。
- 区分重新绑定元组、替换元组槽位和修改嵌套对象，并为实际发生的操作生成测试。

**审查清单**

- 用零个、一个和多个元素测试构造与解包，确认尾随逗号和星号目标的行为。
- 对每个返回元组覆盖所有分支，并检查调用方是否依赖旧的元素数量或顺序。
- 对来自外部系统的命名元组字段执行运行时验证，不把类型注解当成检查结果。

**提示词词汇**

使用准确术语：`tuple literal`（元组字面量）、`tuple packing`（元组打包）、`sequence unpacking`（序列解包）、`starred target`（星号目标）、`shallow immutability`（浅层不可变性）、`hashable elements`（可哈希元素）、`lexicographic comparison`（字典序比较）、`named tuple`（命名元组）。要求 AI 逐项说明数据形状、所有权和哈希条件，比“把它改成不可变”更容易得到可验证的结果。

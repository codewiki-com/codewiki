...
模型经常生成恒真的 `if value == "x" or "y"`，在 `while` 的 `continue` 前遗漏计数更新，或在遍历列表时原地删除元素。对于 `match`，它还可能把常量写成捕获名称、把一般映射模式放在具体模式之前，或假定映射模式会拒绝额外键。这些错误通常能运行，却选择了错误路径。

**让 AI 检查**

- 为每个条件列出能让它为真、为假以及位于边界上的输入，并指出被前面分支遮蔽的条件。
- 跟踪每条循环回边和退出边，确认状态是否推进，以及 `break`、`continue`、`return` 分别会到达哪里。
- 为每个 `case` 说明它捕获的名称、允许的额外数据和守卫求值时机，再检查分支从具体到一般排列。

**审查清单**

- 运行空集合、单项集合、边界值、未知状态，以及连续两个都应被删除或跳过的项目。
- 给每个 `while` 设置可观察的尝试上限，并覆盖所有 `continue` 与 `break` 路径。
- 检查布尔运算符返回的是哪个操作数，并测试 `match` 的默认分支与类型不匹配输入。

**提示词词汇**

使用准确术语：`truth value testing`（真值测试）、`short-circuit evaluation`（短路求值）、`iterable`（可迭代对象）、`progress invariant`（进度不变量）、`loop else`（循环 `else`）、`structural pattern matching`（结构模式匹配）、`capture pattern`（捕获模式）和 `guard`（守卫）。要求 AI 提供路径表或循环状态轨迹，比笼统地要求「检查逻辑」更容易暴露错误。

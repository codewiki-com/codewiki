---
title: Asynchronous concurrency with Python asyncio
...
Generated code often awaits every call immediately, so asynchronous-looking code remains fully
sequential. The opposite failure sends arbitrary input to `gather()` without a concurrency limit or
an overall deadline. Models also call synchronous SDKs inside `async def`, discard the return value
of `create_task()`, or continue after cancellation, causing loop stalls, orphaned tasks, and hangs
during shutdown.

**Ask your AI to check**

- Mark every call that can block the event loop and verify that the exact library method is async.
- For every created task, identify its owner, await site, exception path, and shutdown cancellation policy.
- Trace one child failure and one overall timeout through siblings, resource cleanup, and the caller's result.

**Review checklist**

- Check that I/O meant to overlap is scheduled before it is awaited and has an explicit concurrency limit.
- Check that every resource release is in `finally` or `async with`, with no swallowed cancellation.
- Check that the entry point, background-task set, and task groups form one complete ownership tree.

**Prompt vocabulary**

Use the precise terms `coroutine object`, `awaitable`, `cooperative scheduling`,
`structured concurrency`, `cancellation propagation`, `deadline`, `backpressure`, and
`bounded concurrency`. Ask the AI for a task tree and failure-propagation path; “make it async” is
too vague to expose lifecycle errors.

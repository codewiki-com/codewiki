...
Generated wrappers often add `*args, **kwargs` to “make the API flexible,” then read a few keys with `.get()` and silently accept every misspelling. Models also inject a keyword such as `timeout=30` while forwarding the caller's unchanged `**kwargs`, producing a duplicate-value `TypeError`. Another recurring failure is assuming that packing copies nested dictionaries, so generated tracing or retry code mutates caller-owned headers.

Generated code may forward unreviewed request dictionaries directly into SDKs. That can expose controls the outer API did not promise, move secrets into debug output, or make behavior change when the downstream library adds a parameter. A seemingly transparent decorator may also omit `@wraps` and precise `ParamSpec` typing, leaving introspection and static callers with different views of the function.

**Ask your AI to check**

- Classify every parameter by kind and show how each supported call binds to it.
- List wrapper-owned, forwarded, rejected, and sensitive keyword names, including the policy for duplicates.
- Trace the identity of every mutable object inside `args` and `kwargs`, and verify the public signature before and after decoration.

**Review checklist**

- Run calls with no extras, several extras, a misspelled keyword, a duplicate keyword, and a non-string `**` key.
- Confirm that each layer removes its own options and forwards only an explicit downstream allowlist.
- Inspect `inspect.signature()` and static types, then check that logs omit argument values that may contain secrets or personal data.

**Prompt vocabulary**

Use `positional-only parameter`, `positional-or-keyword parameter`, `variadic positional parameter`, `keyword-only parameter`, `variadic keyword parameter`, `argument unpacking`, `duplicate keyword`, `argument forwarding`, `ParamSpec`, and `Unpack[TypedDict]`. Ask for an “argument binding table” and an “option ownership policy”; a request to “handle all arguments” invites an unbounded catch-all.

---
title: lru_cache 函数缓存
description: 用 functools 缓存可重复的函数调用，并正确处理缓存键、淘汰、失效、并发重复计算与对象生命周期。
track: python
...
这里的 LRU 是“最近最少使用”（least recently used）。缓存达到 `maxsize` 后再加入新结果，会淘汰最长时间没有被命中的条目；一次命中也会更新该条目的新近程度。有限容量让长期运行的进程可以控制条目数量，但不能控制每个结果本身的大小。

缓存适合相同实参会反复出现、结果由这些实参决定，而且计算或读取成本值得复用的函数。递归动态规划、解析不可变配置文本和按稳定标识读取只读数据，都是常见入口。若每次调用都必须产生副作用、新对象或反映当前外部状态，就不应直接套用它。

`functools.cache` 是 `lru_cache(maxsize=None)` 的无界简写。它不执行 LRU 淘汰，因此代码更简洁，但键空间持续增长时也会持续保留实参与结果。选择两者的核心问题不是装饰器名称，而是结果能保存多久、由谁触发失效，以及键空间是否有界。
...
下面四个例子依次展示递归记忆化、LRU 淘汰、调用模式与类型区分，以及外部状态变化后的显式失效。所有输出都来自本地 `python3` 运行结果。
...
若 API 允许许多等价写法，可以在未缓存的公开函数中绑定并规范化输入，再调用签名更窄的私有缓存函数。不要依赖当前实现的私有键格式，也不要把 `typed=False` 理解成“所有相等值必然共用条目”。
...
> [!PITFALL]
> `@cache` 和 `lru_cache(maxsize=None)` 会一直保留每个不同调用的实参与结果。用户 ID、搜索文本或时间戳不断变化时，进程内存可随键空间增长。
...
**修复方法：** 让被缓存函数可安全重复执行，或在业务层实现按键的 single-flight 协调。并发测试应让两个调用在第一次计算完成前重叠，而不是只测试缓存已经预热后的命中。
...
模型常给数据库读取或 HTTP 请求直接加上 `@cache`，却没有生成写入后的失效路径、租户或权限维度，也没有限制键空间。它还会缓存 `async def`，从而保存协程对象而不是已等待的结果；第二次等待同一对象会失败。另一类错误是把“线程安全”改写成“每个键只计算一次”，或者返回可变字典后允许调用方原地修改。

**让 AI 检查**

- 列出函数结果依赖的每项状态，并标明它是否完整出现在缓存键中。
- 给出每种写入、配置切换和权限变化对应的失效触发点与测试。
- 检查键空间上界、返回值可变性、实例保留，以及并发未命中是否允许重复执行。

**审查清单**

- 先命中缓存，再改变底层状态，验证新鲜度与权限边界。
- 用不同调用形式、不同直接类型和不可哈希输入检查键行为。
- 让同一冷键并发执行，并监控 `hits`、`misses`、`currsize` 与清空事件。

**提示词词汇**

使用准确术语：`memoization`（记忆化）、`cache key`（缓存键）、`cache hit`（缓存命中）、`cache miss`（缓存未命中）、`LRU eviction`（LRU 淘汰）、`cache invalidation`（缓存失效）、`bounded cache`（有界缓存）、`single-flight`（同键请求合并）。要求模型给出键组成、状态所有权、失效矩阵和并发冷启动时间线；“加个缓存优化”无法表达这些约束。

<Depth level="deep">

## 在 cache、lru_cache 与不缓存之间选择
...
LRU 只根据访问顺序淘汰，不理解结果的成本、大小、租户或过期时间。一个很大的结果与一个很小的结果都只算一个条目。若业务需要按字节预算、TTL、逐键删除或跨进程一致性，应选择明确支持这些能力的缓存，而不是在装饰器周围堆叠隐式线程。

缓存是函数对象的一部分，默认只存在于当前 Python 进程内。多个工作进程各有自己的内容和统计，重启后内容消失。它既不是持久化层，也不会在多个主机之间传播失效。
...
布尔值是整数的子类，而且 Python 中不同数值类型可能相等并共享哈希。`typed=True` 可以让直接传入的 `False`、`0` 和某些其他数值类型按类型区分，但它仍不是通用序列化方案。先定义业务上哪些输入应视为同一个请求，再编码键规则。
...
版本化键把失效事件转成普通实参。例如，读取方调用 `_price(sku, catalog_version)`，写入成功后推进版本；旧条目仍会占据容量，直到 LRU 淘汰或全量清理，但不会再被新版本调用命中。这种方法容易测试，却需要可靠地传播版本。

按时间清空整个包装器不等于每条记录各自拥有 TTL。它会让所有键同时过期，还可能在请求处理中清掉刚写入的结果。若新鲜度契约以每条记录的写入时间为准，应使用能保存逐条截止时间并定义并发行为的实现。
...
若重复计算只浪费 CPU，可以接受这种语义并在容量与延迟测试中覆盖它。若重复调用会扣款、写数据库或发送消息，缓存本来就放错了层级；幂等键、事务约束或 single-flight 协调才负责业务上的唯一执行。
...
实例方法的第一个实参是 `self`，所以不同实例通常产生不同键。缓存不会自动知道两个实例代表同一个业务实体，也不会只提取其中的 ID。实例还必须可哈希；自定义相等性与哈希若会随对象状态变化，会破坏字典键契约。
...
另一种边界是让模块级缓存函数只接收不可变实体 ID 和版本，再由实例方法委托给它。这样生命周期和失效更明显，但前提是 ID 加版本足以决定结果。若结果依赖实例中的未表达状态，这种重构只会制造错误共享。

...
prerequisites: [python/functions, python/type-hints, python/pydantic]
related: [python/asyncio, python/pytest, python/django, python/celery]
...
FastAPI is an application framework, not a production network server. Deployment still requires an ASGI-compatible server and separate decisions about processes, proxies, timeouts, and observability boundaries.
...
Models often copy Pydantic v1 forms such as `@validator`, `.dict()`, and `regex=` from old tutorials while generating otherwise Pydantic v2 code. They also mark every handler `async def` but call a blocking ORM inside it, or omit `response_model` and return an internal model directly. Another recurring failure is to generate request validation without object-level authorization, dependency-override cleanup, or tests for `422`, `401`, and `403` paths.

**Ask your AI to check**

- List every path operation parameter's source, requiredness, and failure status, and identify whether the route template, a default, or FastAPI metadata determines it.
- Trace calls inside each `async def`, mark I/O boundaries that can't be awaited, and state whether each runs in a thread pool or on the event loop.
- Compare fields across input models, domain objects, and response models, identifying sensitive data that could cross the API boundary.

**Review checklist**

- Execute HTTP tests for valid, missing, malformed, and boundary values, checking both the status code and error `loc`.
- Test authorization with a valid resource identifier owned by another user or tenant, confirming that query scoping rejects access.
- Check that every dependency override and `yield` dependency restores state or releases resources on success, exception, and cancellation paths.

**Prompt vocabulary**

Use the exact terms `path operation`, `request body`, `response model`, `dependency graph`, `dependency override`, `request-scoped cache`, `blocking I/O`, `object-level authorization`, and `OpenAPI schema`. Ask for a parameter-source table and request-lifecycle trace; “write a FastAPI endpoint” is too vague to expose boundary errors.

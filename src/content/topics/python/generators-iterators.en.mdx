...
Models often replace a list comprehension with a generator expression while leaving two later traversals, a `len()` call, or indexing in place. The type looks more "streaming," but the calling contract has changed. They also return a generator expression directly from `with open(...)`, or run `list(rows)` in a log statement before handing the now-exhausted `rows` to business logic. Another common error is `yield next(source)` without handling exhaustion, which turns normal completion into `RuntimeError`; with files, generated code also tends to omit the explicit close path after an early `break`.

**Ask your AI to check**

- Classify every parameter as iterable or iterator, identify who creates and consumes it, and state whether repeated traversal is allowed.
- Mark every implicit consumption point, including `list()`, `sum()`, membership tests, unpacking, logs, and debug code, then show what remains afterward.
- Trace resources and `finally` through normal exhaustion, early `break`, parse failure, and explicit `close()`.

**Review checklist**

- Test every consumer with a single-pass iterator and confirm there is no second read, `len()`, or indexing assumption.
- Try empty, one-item, and invalid-item inputs, then check whether the generator ends normally or incorrectly raises `RuntimeError`.
- Stop after the first item and verify that files, cursors, or locks release immediately rather than waiting for garbage collection.

**Prompt vocabulary**

Use the exact terms `iterable`, `iterator`, `iterator protocol`, `single-pass`, `lazy evaluation`, `generator frame`, `exhaustion`, `StopIteration`, `yield from delegation`, and `resource ownership`. Ask for a consumption-point inventory and a resource-lifecycle table. "Change this to a generator to save memory" doesn't establish that the change is correct.

...
模型生成智能体宿主代码时，常把 `subprocess.run(..., shell=True)` 与模型输出直接拼接，或用字符串前缀判断路径仍在工作区。它也会只按工具名缓存永久批准，把 `pytest` 的输出中出现 `passed` 当成成功而忽略退出码，或者在失败后无限重试。另一类错误是把截断的日志重新概括成「无错误」，导致后续循环失去真正的失败原因。

**让 AI 检查什么**

- 列出每个工具可能产生的副作用，并逐项追踪参数验证、工作目录、网络访问和批准范围。
- 找出所有由模型文本触发的 shell、路径或 URL 操作，证明未经信任的数据不能突破工作区与权限边界。
- 追踪每条完成声明到原始退出码、测试报告和差异范围，说明失败、超时与输出截断会怎样传播。

**审查清单**

- 用绝对路径、`..`、符号链接、shell 元字符和超长输出测试工具边界。
- 确认取消、预算耗尽、工具失败和审批拒绝都有有限且可恢复的停止路径。
- 在干净的隔离工作区重跑验收命令，并人工审查实际差异而非智能体摘要。

**提示词词汇**

使用准确术语 `agent loop`（智能体循环）、`tool call`（工具调用）、`tool result`（工具结果）、`approval gate`（审批门）、`least privilege`（最小权限原则）、`workspace boundary`（工作区边界）、`stop condition`（停止条件）和 `verification evidence`（验证证据）。要求 AI 给出「工具、完整参数、副作用、批准规则、完成证据」的映射，比笼统地要求「让智能体更安全」更容易审查。

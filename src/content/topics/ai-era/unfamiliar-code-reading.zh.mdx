...
> [!PITFALL]
> 一次成功运行几乎不能说明缺失字段、空集合、边界值、异常、重试或重复调用的行为。貌似合理的模型常会把多个分支压成一段整洁叙述。
...
模型可能根据文件名虚构从路由到处理器的边，漏掉重写输入的中间件，或在描述生产持久化时选中了内存适配器。它常只跟踪正常路径，未检查重试、唯一约束或导入状态，就声称幂等性等不变量成立。面对宽泛的解释请求时，它还可能引用过期注释，同时遗漏隐藏在 `notify` 或 `record` 等辅助函数后的副作用。

**让 AI 检查**

- 给出每条控制流判断对应的确切注册位置、构造位置与调用点，并把推断而非观察到的边明确标出。
- 从外部输入开始，经过校验与规范化，一直追踪到每个副作用去向，生成字段传递表。
- 按顺序列出数据库、网络、文件系统、事件、缓存、日志、时钟、随机数和模块状态副作用，并说明每处失败后仍保留什么。

**审查清单**

- 确认建议的入口确实接入当前配置与运行时目标。
- 用一条有效跟踪、一个被拒绝的输入和一次相关依赖失败核对解释。
- 只把与改动有关的不变量写成聚焦测试，再检查最终副作用顺序和修改文件范围。

**提示词词汇**

使用准确术语：`entry point`（入口）、`call path`（调用路径）、`call site`（调用点）、`construction root`（组合根）、`data lineage`（数据传递脉络）、`trust boundary`（信任边界）、`side-effect inventory`（副作用清单）、`hidden mutable state`（隐藏可变状态）、`invariant`（不变量）、`failure boundary`（失败边界）、`characterization test`（特征测试）和 `evidence-backed claim`（有证据支持的判断）。要求提供源码路径、符号、不确定性标签和反例；「解释这个代码库」只会引出无法验证的导览。

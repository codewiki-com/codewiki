...
模型常用通行模式补全缺失语义。「验证输入」可能变成真假值检查并错误拒绝零；「保留 API」可能保留签名，却把抛出的错误改为 `null`；「让重试安全」可能会重试非幂等扣款。要求创建「服务」可能在只使用小型模块函数的代码库中生成新类与依赖层，而「简化」可能把策略与 I/O 合并，删除有价值的测试接缝。

**让 AI 检查**

- 改动前，为每个公开符号建立表格，列出调用方、接受的输入、返回结构、异常或错误值，以及副作用。
- 把每项不变量与失败要求转成具体谓词或示例，再找出未覆盖分支与并发转换。
- 对照所需结构边界、受保护的兼容维度和指定验收命令检查最终差异；把所有假设单独报告。

**审查清单**

- 根据指定接口、不变量、失败通道与副作用顺序，检查成功路径和反例路径。
- 搜索变更符号的所有使用方，包括关键字调用、配置字符串、序列化、反射与命令包装器。
- 确认测试证明的是可观察行为，而不是简单复制生成实现的内部步骤。

**提示词词汇**

使用准确术语：`public API`（公开 API）、`function signature`（函数签名）、`parameter`（形参）、`argument`（实参）、`call site`（调用位置）、`precondition`（前置条件）、`postcondition`（后置条件）、`invariant`（不变量）、`side effect`（副作用）、`failure mode`（失败模式）、`error channel`（错误通道）、`idempotent effect`（幂等效果）、`atomic transition`（原子转换）、`architecture boundary`（架构边界）、`dependency seam`（依赖接缝）、`pure helper`（纯函数辅助单元）、`breaking change`（破坏性变更）、`backward compatibility`（向后兼容）、`characterization test`（特征测试）与 `acceptance criterion`（验收条件）。术语必须与具名符号和示例配对；只有指向证据时，它才能缩小含义范围。

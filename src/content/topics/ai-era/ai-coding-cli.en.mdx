...
Models generating CLI wrappers often splice natural-language tasks into a `shell=True` string, turning quotes or command substitution into injection paths. They also use string prefixes to check path ownership, accept lookalike prefixes or symlink targets as workspace files, read only `stdout`, search for the word `passed`, or return `0` unconditionally and thereby wrap failure as success. Another failure is forwarding the shell's full environment into a session and then writing token-bearing diagnostics into an <Term id="execution-transcript">execution transcript</Term>.

**Ask your AI to check**

- List every process the wrapper starts, state whether arguments pass through a shell, explain how `cwd` is chosen, and trace nonzero exits, timeouts, and signals.
- Trace workspace containment checks, construct counterexamples with `..`, lookalike path prefixes, and symlinks, and prove that every write remains in scope.
- Inventory the environment variables and files sent to the model, subprocesses, and logs; identify where credentials or <Term id="prompt-injection">prompt injection</Term> can enter.

**Review checklist**

- Start the wrapper from an intentionally wrong directory and confirm that it fails closed instead of widening discovery.
- Make a test command write an error only to `stderr` and return nonzero; confirm that the final state is still failure.
- Compare the initial and final diff, checking for out-of-scope files, surprise commits, unexplained dependency updates, and hidden failed checks.

**Prompt vocabulary**

Use the exact terms `working directory`, `repository root`, `non-interactive mode`, `approval gate`, `least privilege`, `exit status`, `execution transcript`, and `fail closed`. Spell out `allowed paths`, `acceptance command`, and `non-goals`; these are more testable than saying “make changes carefully.”

...
Models often generate an impressive `AGENTS.md` by restating the README while omitting the one package-specific test and the generated-file boundary. They may copy Codex discovery rules into a Claude configuration, concatenate all nested guides without checking scope, or claim instructions enforce security. In MCP servers, recurring failures include `additionalProperties` left open, paths checked by string prefix, authorization performed only during `tools/list`, tool errors returned as success text, and one broad shell tool carrying production credentials.

**Ask your AI to check**

- Produce an instruction-source trace for two representative target files, including load order, conflicts, and rules that do not apply.
- Map every MCP tool to its input schema, output schema, side effects, resource authorization, size limit, timeout, and approval rule.
- Generate adversarial calls for traversal, cross-tenant identifiers, oversized results, repeated mutations, unknown fields, and denied approvals, then show the host-level verdict.

**Review checklist**

- Confirm every persistent instruction is repository-specific, scoped, current, and tied to observable evidence.
- Verify that tool schemas reject malformed shape and executors independently enforce semantic scope and caller authorization.
- Inspect the actual tools, credentials, network routes, approval decisions, and redacted results available in a fresh session.

**Prompt vocabulary**

Use the precise terms `instruction discovery`, `instruction precedence`, `working-directory scope`, `path-scoped rule`, `MCP tool declaration`, `inputSchema`, `outputSchema`, `resource-level authorization`, `least privilege`, `approval gate`, `structured tool error`, and `instruction-source trace`. Ask for a table of "guidance, enforceable control, and verification evidence"; "make the agent follow our rules" hides which layer must change.

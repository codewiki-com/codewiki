...
Models can invent upstream URLs or package versions, describe remembered code as original, and add a dependency without mentioning its transitive graph or install script. They may generate tests from the same faulty interpretation, claim those tests ran, update a checksum beside changed bytes, or accept any valid attestation without constraining repository, workflow, issuer, and subject digest.

**Ask your AI to check**

- Produce a claim-evidence matrix for behavior, source origin, artifact identity, every new resolved dependency, and license status; mark unsupported cells instead of inferring them.
- List copied or adapted blocks with immutable source revision, exact location, license, notice obligation, and local transformation; identify any origin that remains unresolved.
- Show the clean verification commands, working directory, runtime, exit status, and fresh output, then explain why each oracle is independent of the candidate implementation.

**Review checklist**

- Reproduce one normal case and one distinguishing counterexample from requirements or a trusted specification, not from generated expected values.
- Reconcile manifest, lockfile, package contents, SBOM, and distributed artifact across direct, transitive, optional, and downloaded components.
- Verify source and artifact records against immutable digests and explicitly trusted identities, then review license obligations and every unresolved exception.

**Prompt vocabulary**

Use the exact terms `code provenance`, `claim-evidence matrix`, `immutable source revision`, `subject digest`, `materials`, `provenance attestation`, `builder identity`, `trust policy`, `test oracle`, `dependency closure`, `lockfile integrity`, `SBOM`, `SPDX license identifier`, and `residual risk`. Ask for observed commands and unsupported claims; “verify everything” doesn't define an evidence boundary.

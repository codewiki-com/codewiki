...
上下文不完整时，模型可能修改生成的客户端而不是它的模式，因检索遗漏已有辅助函数而重复实现，或只更新一个调用点并留下损坏的间接使用方。上下文过期时，它可能覆盖队友编辑，引用补丁之前的测试结果，或保留用户已经替换的要求。大型日志转储还会引入另一种失败：不可信输出中类似指令的文字分散模型注意力，而真正的错误和仓库规则落在活动窗口之外。

**让 AI 检查什么**

- 生成上下文清单，列出每条路径的角色、选择原因、作用域和新鲜度标记。
- 列出待改符号的定义、直接与间接调用方、适用指令、契约、运行时选择点和测试。
- 分开记录观察事实与推断，再列出最新编辑后已经失效并需要重跑的观察。

**审查清单**

- 确认智能体读取的是当前目标和起约束作用的指令，而不只是检索片段或摘要。
- 沿每个变化的接口追踪使用方，并把每项正确性声明映射到当前命令输出或人工决定。
- 检查最终差异中是否出现意外路径、生成产物、弱化测试、携带秘密的上下文和未解决假设。

**提示词词汇**

使用这些确切术语：`context window`（上下文窗口）、`working set`（工作集）、`source of truth`（事实来源）、`authoritative source`（权威来源）、`context provenance`（上下文来源信息）、`freshness marker`（新鲜度标记）、`invalidation trigger`（失效触发条件）、`retrieval query`（检索查询）、`definition and references`（定义与引用）、`call site`（调用点）、`dependency edge`（依赖边）、`blast radius`（影响范围）、`raw observation`（原始观察）和 `context manifest`（上下文清单）。要求生成「路径、角色、原因、版本、更新触发条件」表；「读取相关文件」没有定义何谓相关，也没有说明新鲜度。

...
Multi-file edits generated in an AI IDE often update a definition but miss indirect callers, especially dynamic imports, string registries, test fixtures, and code-generation entry points. They also use removed APIs from stale indexed snippets, mix configuration from different packages in one repository, or edit tests to accommodate a wrong implementation. Another concrete failure is mistaking editor diagnostics for full acceptance: types turn green while concurrency order, authorization checks, migration direction, or browser behavior remains wrong.

Bad context leaves locally plausible code. A model can import the wrong same-named symbol, pull a test helper into production, or add a dependency at a monorepo root when the consuming package has its own manifest. Generated code also turns placeholder credentials, broad exception handlers, and silent fallback into apparent "complete error handling," hiding failure paths on demo inputs.

**Ask your AI to check**

- List the files, symbols, diagnostics, and project instructions this response actually used, and identify assumptions made without reading their source.
- Search every caller through public interfaces, dynamic references, configuration, and test fixtures, then explain why each unmodified hit is safe.
- Give the exact acceptance commands, working directories, and expected failure semantics, and report skipped, timed-out, or unexecuted checks.

**Review checklist**

- Confirm the changed-file scope and baseline, then read every diff hunk instead of substituting the conversation summary for source review.
- Run the repository's formatter, static checks, tests, and build in the correct package directory, and inspect exit status and skipped work.
- Inspect new dependencies, configuration, logging, network calls, and secrets to make sure they do not widen task permissions or data exposure.

**Prompt vocabulary**

Use exact terms: `editor context`, `explicit attachment`, `codebase retrieval`, `context window`, `edit scope`, `approval gate`, `diff hunk`, `stale preimage`, `workspace trust`, and `acceptance command`. Asking the AI to "list evidence, assumptions, and unexecuted checks" yields something more reviewable than asking it to "be careful."

...
> [!PITFALL]
> A diff can look locally safe while relying on an unscoped caller, permissive middleware, dangerous serializer, or privileged deployment identity. An explanation limited to the changed functions hides the controls that must exist elsewhere.
...
Fallbacks deserve their own threat model. Catching a policy timeout and permitting the action, changing a failed signature check into a warning, or substituting an environment default with broader authority can bypass the intended control. Decide explicitly which degraded behavior is safe and test it under dependency failure.

...
修复已复现缺陷、重构陌生代码、修改<Term id="api-contract">API 契约（API contract）</Term>或替换稳定接口背后的实现时，都会用到这种方法。当提示词允许多种有效实现，但可观察行为已经确定时，它尤其有用。
...
智能体经常生成只重放一个提示词示例的测试，直接在预期值中复制实现公式，或者模拟自己声称要测试的函数。它还可能不断更新快照和断言直到变绿，接受过于宽泛的错误类型，遗漏「失败时不得写入」，或把测试放进运行器无法发现的文件。另一类错误是针对可见用例优化：补丁特判一项具名输入，却让未测试的租户、边界、顺序或重试发生回归。

**让 AI 检查**

- 把每项改变的行为映射到测试名称、断言与执行层，并指出未覆盖的失败路径和副作用路径。
- 证明每项新回归测试会在有缺陷的基线上，或加入一个刻意缺陷时失败，并引用相关失败行。
- 报告确切测试命令、工作目录、发现与跳过的数量、退出码、最终差异状态，以及所有未执行的环境边界。

**审查清单**

- 检查测试改动中是否存在削弱的断言、宽松模拟、更新的快照、隐藏跳过项，以及运行器发现范围之外的文件。
- 根据改动的真实风险，用相邻边界、第二个身份或实例，以及一次协作者失败来挑战补丁。
- 在最终编辑后重跑聚焦测试和相关测试套件，再把每项完成声明映射到保留的证据或具名人工检查。

**提示词词汇**

使用准确术语：`behavioral contract`（行为契约）、`regression test`（回归测试）、`boundary-value case`（边界值用例）、`negative assertion`（否定断言）、`test double`（测试替身）、`test discovery`（测试发现）、`fault injection`（故障注入）、`mutation testing`（变异测试）、`red-green verification`（红绿验证）和 `independent test run`（独立测试运行）。要求输出风险到测试矩阵和反事实失败证据；「增加更多测试」没有说明测试套件必须拒绝什么。

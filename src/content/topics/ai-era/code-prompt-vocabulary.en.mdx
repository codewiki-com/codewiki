...
Models often complete missing semantics with common patterns. “Validate the input” can become truthiness checks that reject zero, “preserve the API” can keep the signature while changing thrown errors into `null`, and “make retries safe” can retry a non-idempotent charge. A request for a “service” may produce a new class and dependency layer even when the repository uses small module functions, while “simplify” may merge policy with I/O and remove a useful test seam.

**Ask your AI to check**

- Build a table of each public symbol, its callers, accepted inputs, return shape, exceptions or error values, and side effects before changing code.
- Translate every invariant and failure requirement into a concrete predicate or example, then identify uncovered branches and concurrent transitions.
- Compare the final diff with the requested structural boundary, protected compatibility dimensions, and named acceptance commands; report any assumption separately.

**Review checklist**

- Verify happy paths and negative paths against the stated interface, invariant, failure channel, and side-effect ordering.
- Search every consumer of changed symbols, including keyword calls, configuration strings, serialization, reflection, and command wrappers.
- Confirm tests prove observable behavior and don't merely copy the generated implementation's internal steps.

**Prompt vocabulary**

Use `public API`, `function signature`, `parameter`, `argument`, `call site`, `precondition`, `postcondition`, `invariant`, `side effect`, `failure mode`, `error channel`, `idempotent effect`, `atomic transition`, `architecture boundary`, `dependency seam`, `pure helper`, `breaking change`, `backward compatibility`, `characterization test`, and `acceptance criterion`. Pair the terms with named symbols and examples; the vocabulary narrows meaning only when it points to evidence.

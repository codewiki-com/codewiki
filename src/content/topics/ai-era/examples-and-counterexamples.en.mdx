...
Generated code often fits the visible examples with an overly broad shortcut: it uses a fixed threshold from one item, turns “non-empty” into truthiness, or applies an administrator rule before an archive restriction. Generated tests may repeat the same sample with different superficial data, calculate expected results from the production condition, or quietly bless behavior for omitted cases. Another common failure is changing both implementation and old expectations so that a regression disappears from the evidence.

**Ask your AI to check**

- List at least two plausible implementations that satisfy the current examples, then propose one distinguishing counterexample for each difference.
- Produce a dimension table covering partitions, exact boundaries, invalid forms, rule interactions, repeated actions, and forbidden side effects; label every unresolved cell.
- Run the approved cases against the pre-change and generated implementations, report which cases distinguish them, and show the exact assertion output.

**Review checklist**

- Confirm that every case has an independent literal result, stable failure, or reviewed invariant and a name explaining its purpose.
- Check adjacent boundary values and at least one case where two rules apply; vary one decision-bearing fact at a time where possible.
- Inspect omitted categories and effects explicitly, and reject any behavior the implementation or generated tests invented without approval.

**Prompt vocabulary**

Use `representative example`, `counterexample`, `input partition`, `boundary value`, `expected output`, `failure contract`, `decision table`, `test oracle`, `distinguishing case`, and `forbidden behavior`. Ask for a “minimal distinguishing set against named wrong rules,” not merely “more edge cases,” because the latter does not say which ambiguity each case must remove.

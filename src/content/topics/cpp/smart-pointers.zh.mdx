---
title: 智能指针
description: 用 unique_ptr 表达独占所有权，用 shared_ptr 与 weak_ptr 建模共享和观察关系，并避开控制块、循环引用与生命周期陷阱。
track: cpp
...
模型经常在没有所有权说明时把字段和形参全部改成 `shared_ptr`，或者从已有裸指针和 `this` 各构造一份新 `shared_ptr`。生成的图结构会让父子双方都持有强引用，异步 lambda 会捕获 `shared_from_this()` 后又存回对象自身，数组代码则可能混用 `unique_ptr<T>` 与 `new T[]`。另一类错误是在移动后继续解引用源 `unique_ptr`，或先检查 `weak_ptr::expired()`，随后跨越异步边界使用旧裸指针。

**让 AI 检查这些内容**

- 为每个动态对象列出强所有者、弱观察者、裸借用者、所有权转移点和最终销毁条件。
- 找出每个从裸指针或 `this` 构造 `shared_ptr` 的位置，并证明相同对象只对应一个控制块。
- 模拟最后一个外部强所有者离开、回调晚到和 `weak_ptr::lock()` 失败，说明每个分支会保活、跳过还是清理什么。

**审查清单**

- 确认默认使用 `unique_ptr`，每次 `std::move` 后都不再依赖旧值，每次 `release()` 都有立即接管者。
- 确认共享图中没有无意的强引用环，`enable_shared_from_this` 对象从一开始就由兼容的 `shared_ptr` 管理。
- 确认弱观察使用一次 `lock()` 得到的局部强指针，并单独审查对象状态与同一个指针变量的线程同步。

**提示词词汇**

使用精确词汇 `exclusive ownership`（独占所有权）、`shared ownership`（共享所有权）、`control-block identity`（控制块身份）、`strong reference cycle`（强引用环）、`non-owning observer`（非拥有观察者）、`ownership transfer`（所有权转移）、`moved-from state`（移动后状态）、`custom deleter`（自定义删除器）和 `atomic<shared_ptr>`。要求模型输出所有权图与析构条件，比笼统地要求「使用智能指针保证安全」更容易发现设计错误。
...
`owner_before` 和 `std::owner_less` 比较的是所有权关系，可用于按控制块身份组织关联容器。C++23 没有提供对应的所有权哈希接口，因此本主题的示例不能假定它存在。
...
- [C++23 工作草案：`shared_ptr`](https://timsong-cpp.github.io/cppwp/n4950/util.smartptr.shared)
- [C++23 工作草案：`weak_ptr`](https://timsong-cpp.github.io/cppwp/n4950/util.smartptr.weak)

---
title: C++ 运算符重载
description: 运算符重载让自定义类型使用自然的 C++ 表达式；理解候选查找、返回类型与比较契约，避免生成代码中常见的语义错误。
track: cpp
section: basics
...
运算符重载是为已有 C++ 运算符声明一个<Term id="operator-function">运算符函数（operator function）</Term>。表达式中至少有一个操作数是类或枚举类型时，编译器可以在内置运算之外选择这个函数。于是 `total + tax`、`version < candidate` 和 `scores[index]` 可以直接表达领域操作。
...
> [!PITFALL]
> 成员 `Quantity::operator*(double)` 可以支持 `quantity * 2.0`，却不能自动支持 `2.0 * quantity`。模型常根据第一种调用通过编译，就声称接口具有交换性。
...
模型容易按语法形状补齐运算符，却不先确定语义。常见结果包括：成员 `operator+` 意外修改 `*this`；只测试 `value * scalar` 就声称 `scalar * value` 可用；下标按值返回或缺少 `const` 重载；手写六个比较关系后出现 `a == b` 但 `a < b` 也为真；默认 `<=>` 把缓存字段纳入身份；以及用重载 `&&` 保护本应短路的右侧访问。另一类失败是加入过多隐式转换来消除编译错误，最后把原问题变成歧义或静默精度损失。

**让 AI 检查**

- 列出表达式的成员、非成员、内置与改写候选，并说明每个实参需要的转换以及最终选择。
- 为每个运算符写出「是否修改操作数、返回类别、相等与排序字段、异常和边界行为」契约表。
- 编译正反两种操作数顺序、`const` 对象调用、链式调用和预期失败的隐式转换，给出实际诊断。

**审查清单**

- 检查符号含义是否符合领域约定，并确认 `+`、`+=`、`==` 与排序之间没有语义分叉。
- 检查返回值或引用的生命周期、成员函数的 `const` 限定，以及隐藏友元能否通过 ADL 找到。
- 用边界值、混合类型、NaN、越界索引和带副作用的逻辑右操作数覆盖容易被样例漏掉的路径。

**提示词词汇**

使用准确术语：`operator function`（运算符函数）、`member operator`（成员运算符）、`non-member operator`（非成员运算符）、`hidden friend`（隐藏友元）、`argument-dependent lookup`（参数依赖查找）、`overload resolution`（重载决议）、`rewritten candidate`（改写候选）、`implicit conversion sequence`（隐式转换序列）、`three-way comparison`（三向比较）、`comparison category`（比较类别）与 `short-circuit evaluation`（短路求值）。要求模型给出候选集合和语义契约；只说「实现所有常用运算符」太容易得到一组能编译却彼此矛盾的函数。
...
<Term id="three-way-comparison">三向比较（three-way comparison）</Term>运算符 `<=>` 一次描述小于、等于或大于的关系，并可能表示无序。C++20 的改写候选允许 `<`、`>`、`<=` 和 `>=` 使用 `<=>`，`!=` 则可从 `==` 改写。这样能减少手写关系运算逐渐不一致的问题。
...
生成代码审查可以把这些检查整理成一张矩阵：操作数类型、左右顺序、对象常量性、预期候选、是否修改、返回类别与失败方式。矩阵中的空格比重复样例更有价值，因为真正的错误常藏在模型没有生成的那一种调用形状里。
...
- [C++23 草案：重载运算符](https://timsong-cpp.github.io/cppwp/n4950/over.oper)
- [C++23 草案：表达式中的运算符候选](https://timsong-cpp.github.io/cppwp/n4950/over.match.oper)
- [C++23 草案：默认比较运算符](https://timsong-cpp.github.io/cppwp/n4950/class.compare.default)

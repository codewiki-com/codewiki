...
Models often generate a getter returning `const T&` while its body returns a local variable, the by-value result of `optional.value_or(...)`, or a temporary such as `path.string()`. They also retain a reference to a container element in an asynchronous lambda and then insert into the vector; reallocation invalidates the reference before the callback runs. Generic wrappers show two opposite mistakes: omitting `std::forward` degrades an rvalue to an lvalue, while using `std::move` unconditionally can consume a caller's lvalue.

**Ask your AI to check**

- List every reference parameter, return, data member, and by-reference lambda capture, then name its ultimate owner, invalidating operations, and last use.
- For every `T&&`, state whether the template parameter is deduced there, the argument's value category, the collapsed type, and which overload `std::forward` or `std::move` selects.
- Build tests for owner-first destruction, vector reallocation, an lvalue passed through the wrapper, and a temporary argument; run those paths with compiler warnings and sanitizers.

**Review checklist**

- Confirm that every returned reference designates an object that survives the call and that its cv-qualification matches the interface.
- Check whether retained references cross threads, tasks, callbacks, or container invalidation points, and ensure non-owning access was not mistaken for ownership.
- Treat every named rvalue reference as an lvalue expression, then confirm that each wrapper neither copies nor moves a caller-owned object by mistake.

**Prompt vocabulary**

Use the exact terms `lvalue reference`, `rvalue reference`, `value category`, `reference initialization`, `temporary lifetime extension`, `dangling reference`, `reference collapsing`, `forwarding reference`, and `perfect forwarding`. Ask for an ownership/lifetime table and per-call overload resolution; "check whether these references are safe" is too vague.

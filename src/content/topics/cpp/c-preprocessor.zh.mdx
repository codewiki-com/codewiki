---
title: C 预处理器
description: C 预处理器在编译前展开宏、包含头文件并选择条件代码；掌握词元替换、配置边界与可验证的宏写法。
track: cpp
section: basics
...
<TLDR>
  <TLDRCell label="what">
    C 预处理器在编译器分析类型和表达式之前处理源文件。它执行文件包含、条件选择和宏替换，结果成为编译器看到的翻译单元。
  </TLDRCell>
  <TLDRCell label="trap">
...
C 预处理器读取源文件，在正式编译前处理以 `#` 引导的 <Term id="preprocessing-directive">预处理指令</Term>。`#include` 引入文件内容，`#if` 选择保留的词元，`#define` 创建 <Term id="macro">宏</Term>。预处理器不理解 C 对象的类型、作用域或运行时值。
...
> [!PITFALL]
> 宏不遵守 C 的块作用域或命名空间。短名字、以下划线开头的保留形式，以及重复的头文件保护名，都可能改写无关代码或静默跳过整个头文件。
...
> [!PITFALL]
> `typeof`、语句表达式、`, ##__VA_ARGS__` 和特定厂商 `#pragma` 可能在当前编译器工作，却不属于目标 C23 接口。模型常从旧项目复制这些写法而不标注方言。
...
模型常生成 `#if sizeof(void *) == 8`，但预处理常量表达式不能使用 `sizeof`；也会把 `#ifdef FEATURE` 用在值为 `0` 的开关上。另一些输出把带 `i++` 的实参传给重复求值宏，漏掉宏形参或整体表达式的括号，或者从旧 GNU 代码复制 `##__VA_ARGS__`。自动生成多个头文件时，模型还会复用同一个保护宏，使后包含的声明整块消失。

**让 AI 检查什么**

- 列出每个宏形参在替换列表中出现的次数，并标记所有带副作用的调用实参。
- 对每个条件编译名称说明它表达「是否定义」还是数值状态，并列出所有有效构建组合。
- 展示关键调用经 `gcc -E -P` 得到的展开结果，再指出使用的每项非标准扩展及其保护条件。

**审查清单**

- 用严格警告分别构建功能开启和关闭的配置，并运行两套测试，而不是只阅读 `#if` 分支。
- 确认公共头文件可单独包含，保护宏唯一，且没有普通外部定义或泄漏的辅助宏。
- 把值计算宏的每个调用当作词元替换审查，检查优先级、重复求值、控制流和空可变参数。

**提示词词汇**

使用准确术语 `object-like macro`（对象式宏）、`function-like macro`（函数式宏）、`replacement list`（替换列表）、`macro argument prescan`（宏实参预扫描）、`stringification`（字符串化）、`token pasting`（词元粘贴）、`include guard`（头文件防重复包含）、`conditional inclusion`（条件包含）和 `translation unit`（翻译单元）。要求给出「展开后的词元序列」和「配置构建矩阵」，比笼统地说「检查这个宏」更容易暴露问题。

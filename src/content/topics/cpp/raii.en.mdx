...
Models generate raw-handle classes with destructors but forget to delete compiler-generated copying. They also acquire a handle, call a configuration function that may throw, and wrap the handle only afterward. Other common output uses `std::shared_ptr` to hide unclear ownership and creates a reference cycle, pairs an array with `delete`, or chooses the wrong deleter for `FILE*`. Generated destructors may call a throwing `close()` directly, turning an ordinary error into termination during unwinding.

**Ask your AI to check**

- For each resource, list the acquisition point, sole release responsibility, every borrower, every transfer, and every exit path.
- Inspect the destructor, copy constructor, copy assignment, move constructor, and move assignment of each resource-owning class, explaining why each is defaulted, deleted, or custom.
- Inject a failure at every potentially throwing point after acquisition, then trace fully constructed objects, external side effects, and the final release count.

**Review checklist**

- Confirm that every successful acquisition immediately enters an owner and is released exactly once on normal return, early return, and exception.
- Confirm that borrows don't outlive the owner, moved-from objects remain safe to destroy, and shared ownership has no strong-reference cycle.
- Confirm that destructors and deleters don't let exceptions escape, with a separate explicit interface for close or commit failures that must be reported.

**Prompt vocabulary**

Use the exact terms `RAII`, `scope-bound resource management`, `ownership`, `borrowed handle`, `deterministic destruction`, `stack unwinding`, `move-only owner`, `moved-from state`, `non-throwing destructor`, and `strong exception guarantee`. Ask for an ownership table and exception-path trace instead of vaguely requesting "safer resource management."

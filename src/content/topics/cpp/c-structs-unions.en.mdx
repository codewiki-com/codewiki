...
Models often generate constructors whose tag and union member disagree, or add an enumerator to a write path without updating read and destruction branches. They also hard-code `sizeof` and offsets from a familiar x86-64 result, parse untrusted bytes through a `packed struct`, or generate `free` calls for both copies of an owning pointer. Such code often compiles; it fails after an ABI change, an unknown tag, or an error-cleanup path.

**Ask your AI to check**

- List every union read and write, prove that the discriminant matches at that point, and cover an unknown discriminant.
- Mark every layout, width, byte-order, and alignment assumption as a C guarantee, a target ABI rule, or an unverified guess.
- Trace ownership of every pointer member through initialization, struct copies, error returns, and destruction.

**Review checklist**

- Run construction, access, and cleanup tests for every enumerator, then add one invalid integer tag.
- Test truncated input, maximum lengths, and failed allocations under strict warnings, AddressSanitizer, and UndefinedBehaviorSanitizer.
- Print `sizeof`, `alignof`, and key `offsetof` results with the target compiler, and confirm that the external byte format does not depend on them.

**Prompt vocabulary**

Use the exact terms `structure type`, `union active member`, `tagged union`, `discriminant`, `padding byte`, `alignment requirement`, `object representation`, `shallow copy`, and `flexible array member`. Ask for the invariant between discriminant and payload and the width and byte order of every external field; “make this struct safe” is too vague.

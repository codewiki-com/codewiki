...
> [!PITFALL]
> A member `Quantity::operator*(double)` can support `quantity * 2.0`, but it can't automatically support `2.0 * quantity`. Models often see the first call compile and claim the interface is commutative.
...
Models tend to complete an operator set from its syntax before settling its meaning. Common results include a member `operator+` that mutates `*this`; a claim that `scalar * value` works after testing only `value * scalar`; a value-returning subscript or one without a `const` overload; six hand-written comparisons where `a == b` and `a < b` can both be true; a defaulted `<=>` that includes a cache field; and overloaded `&&` used to guard a right-side access that must not run. Another frequent patch is adding broad implicit conversions until a compile error disappears, replacing the original bug with ambiguity or quiet precision loss.

**Ask your AI to check**

- List the member, non-member, built-in, and rewritten candidates for the expression, including each argument conversion and the selected function.
- Write a contract table for every operator covering operand mutation, return category, equality and ordering fields, exceptions, and boundary behavior.
- Compile both operand orders, calls on `const` objects, chained calls, and implicit conversions that are supposed to fail, then show the actual diagnostics.

**Review checklist**

- Check that the symbol's meaning matches the domain and that `+`, `+=`, `==`, and ordering don't split into conflicting semantics.
- Check value and reference lifetimes, member `const` qualification, and whether ADL can find each intended hidden friend.
- Exercise boundary values, mixed types, NaN, out-of-range indexes, and a logical right operand with a side effect.

**Prompt vocabulary**

Use the exact terms `operator function`, `member operator`, `non-member operator`, `hidden friend`, `argument-dependent lookup`, `overload resolution`, `rewritten candidate`, `implicit conversion sequence`, `three-way comparison`, `comparison category`, and `short-circuit evaluation`. Ask for the candidate set and semantic contract; "implement the usual operators" is too vague and tends to produce a set of functions that compile but contradict one another.
...
Generated-code review can turn these cases into a matrix: operand types, left/right order, object constness, expected candidate, mutation, result category, and failure mode. Empty cells in that matrix matter more than repeated happy-path examples because operator bugs tend to hide in the call shape the model never generated.

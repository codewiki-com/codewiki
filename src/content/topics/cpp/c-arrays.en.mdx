...
Models often treat `void process(int data[16])` as a contract that truly receives 16 elements, then use `sizeof(data)` inside the function to derive its length. They also rewrite two-dimensional arrays as `int**`, emit variable-length arrays that standard C++23 does not support, or return a span into a local buffer. Generated string code frequently allocates only the visible character count and forgets `\0`, while loop generation readily changes `<` to `<=`.

**Ask your AI to check**

- List the complete type and extent of every array expression, and mark every array-to-pointer conversion.
- Give the value range of every subscript and the corresponding array extent, proving that every path satisfies `index < extent`.
- Trace the backing array's storage duration for every pointer, reference, and span, and verify that every multidimensional parameter retains the correct column type.

**Review checklist**

- Exercise empty, one-element, exactly-full, and out-of-range cases, and enable sanitizers in test builds.
- Check that function signatures carry both data and length, with no attempt to recover an extent by applying `sizeof` to a decayed pointer.
- Verify character-buffer capacity units, terminator space, and truncation behavior, and confirm that no returned view escapes its owner.

**Prompt vocabulary**

Use the exact terms `array extent`, `array-to-pointer conversion`, `one-past-the-end pointer`, `array reference`, `fixed extent span`, `row-major order`, `null terminator`, and `object lifetime`. Ask for type deduction and a bounds proof. “Check whether this array is safe” is too broad to reliably expose parameter adjustment or a dangling view.

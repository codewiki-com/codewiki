...
模型常把 `void process(int data[16])` 当成真正接收 16 个元素的契约，随后在函数内写 `sizeof(data)` 推算长度。它们也会把二维数组改写成 `int**`，生成标准 C++23 不支持的变长数组，或返回指向局部缓冲区的 span。字符串处理代码还常按可见字符数分配数组，漏掉 `\0`，而循环生成器很容易把 `<` 改成 `<=`。

**让 AI 检查**

- 列出每个数组表达式的完整类型和边界，并标出发生数组到指针转换的位置。
- 对每个下标给出它的取值范围和数组边界，证明所有路径都满足 `index < extent`。
- 沿每个指针、引用和 span 追踪底层数组的存储期，并检查二维形参是否保留正确的列类型。

**审查清单**

- 用空输入、单元素、恰好满容量和越界索引测试边界分支，并在测试构建中启用消毒器。
- 检查函数签名是否同时表达数据与长度，确认没有从已退化的指针使用 `sizeof` 恢复边界。
- 核对字符缓冲区的容量单位、终止符空间和截断行为，确认返回的所有视图都未逃逸其所有者。

**提示词词汇**

使用准确术语：`array extent`（数组边界）、`array-to-pointer conversion`（数组到指针转换）、`one-past-the-end pointer`（尾后指针）、`array reference`（数组引用）、`fixed extent span`（固定边界 span）、`row-major order`（行主序）、`null terminator`（空字符终止符）、`object lifetime`（对象生命周期）。要求 AI 给出类型推导和边界证明；“检查数组是否安全”太宽泛，容易漏掉形参调整和悬空视图。

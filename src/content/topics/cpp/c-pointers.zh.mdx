---
title: C 指针
description: C 指针保存对象地址；理解类型、解引用、数组边界、生命周期与限定符，才能写出契约明确且可检查的底层接口。
track: cpp
...
指针让函数间接访问调用方对象，也让代码表示数组区间、动态对象、链式数据结构和硬件映射。C 按值传递所有实参；把指针交给函数时，函数得到的是地址副本，却仍可通过该副本访问同一个目标对象。这是 C 模拟输出参数与借用接口的基础。
...
对象生命周期结束后，保存其地址的别名不会自动清空。这样的别名是<Term id="dangling-pointer">悬空指针（dangling pointer）</Term>，不得继续读取目标、写入目标或交给要求有效对象的 API。把某一个变量设为 `NULL` 只能保护那一个变量，无法修复其他副本。
...
下面四个程序依次展示单对象借用、半开数组区间、只读查找结果和二级指针输出。每个文件都用本地 GCC 13.3.0 按 `-std=c2x -Wall -Wextra -Wconversion -Wpedantic -Werror` 编译，再运行得到所示输出。
...
<TryToBreak items={['让 first 与 last 相等', '让阈值恰好等于一个元素', '把 last 改为 readings + 4']} />
...
模型常为缓冲区形参只生成 `T *data`，随后用 `sizeof(data)` 猜长度，或者把循环条件写成 `cursor <= end` 并解引用尾后位置。它也会返回局部数组元素的地址、在 `free` 后仅清空一个别名，或为了消除编译警告而强制去掉 `const`。另一类常见错误是用 `if (pointer)` 作为全部安全检查，却没有追踪地址来自哪个对象、该对象是否仍存活以及可访问多少个元素。

**让 AI 检查**

- 为每个解引用列出指针来源、目标类型、可访问元素范围和对象生命周期依据。
- 为每个指针加减、相减和顺序比较证明两个端点属于同一数组，并标出尾后位置是否被解引用。
- 列出每次指针复制、所有权转移与释放，说明失败路径上哪些别名仍然有效。

**审查清单**

- 用空范围、单元素范围、恰好到边界和越界输入测试所有指针循环，并在测试构建中启用 AddressSanitizer 与 UndefinedBehaviorSanitizer。
- 确认每个裸指针参数都有配套的可空性、长度单位、可变性、所有权和保留时长契约。
- 检查返回值与输出形参不指向局部对象，失败路径不会丢失原指针，也不会留下部分更新状态。

**提示词词汇**

使用准确术语：`object pointer`（对象指针）、`pointee`（所指对象）、`dereference`（解引用）、`null pointer`（空指针）、`dangling pointer`（悬空指针）、`one-past-the-end pointer`（尾后指针）、`pointer provenance`（指针来源关系）、`pointee constness`（目标常量性）和 `ownership transfer`（所有权转移）。要求模型输出逐次解引用证明与指针生命周期表；「检查这个指针」不足以暴露范围和别名错误。
...
裸指针通常表示借用，而不是自动表示拥有；这只是项目约定，不是 C 类型规则。将拥有者数量压缩到一个，并让借用范围短于释放点，可以显著减少悬空别名。需要共享所有权时，必须另外设计计数或资源管理协议。

测试只能发现被执行路径上的一部分错误。严格警告能发现不兼容转换和格式问题，AddressSanitizer 常能捕获越界与释放后使用，UndefinedBehaviorSanitizer 能发现部分未定义操作。它们无法证明所有生命周期和别名契约，所以仍需逐个审查来源、范围与释放路径。
...
- [SEI CERT C：ARR30-C 不形成或使用越界指针与数组下标](https://cmu-sei.github.io/secure-coding-standards/sei-cert-c-coding-standard/rules/arrays-arr/arr30-c/)

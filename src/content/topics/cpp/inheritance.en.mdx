...
Models often mistake similar fields or implementation reuse for public inheritance, then generate a derived
class that narrows valid inputs or breaks a base invariant. They also produce `std::vector<Base>`, by-value
callback parameters, or by-value factory results that slice objects. Intended overrides frequently miss a
`const`, reference qualifier, or exact parameter type and also omit `override`. In multiple-inheritance code,
a model may initialize a virtual base only in intermediate classes instead of making the most-derived class
responsible.

**Ask your AI to check**

- List every base conversion and mark whether it converts a reference or pointer, or constructs a new base
  object by value.
- Compare the complete signature of every derived function with its base virtual function, and require
  `override` on every intended override.
- Follow every destruction path and diamond path, checking the destructor policy, the number of base
  subobjects, and which most-derived class initializes each virtual base.

**Review checklist**

- Test every derived class with boundary inputs from the base contract and confirm that substitution adds no
  exception or stricter precondition.
- Search for polymorphic base types used by value, including function parameters, returns, container elements,
  and `catch` parameters.
- Compile calls and destruction through a base reference, a derived object, and an owning base pointer
  separately.

**Prompt vocabulary**

Use the exact terms `base subobject`, `public inheritance`, `implicit upcast`, `object slicing`,
`name hiding`, `substitutability`, `virtual destructor`, `virtual base`, and `most-derived class`. Asking the
AI to draw the subobject graph and mark every by-value boundary exposes more than a vague request to "check

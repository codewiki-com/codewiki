...
模型常为每个字段生成公开 setter，把本应由构造函数维持的关系拆散。它还会把初始化写进构造函数体、让初始化器顺序与声明顺序不一致，或给拥有裸指针的类只生成析构函数而遗漏复制控制。另一个具体信号是 getter 返回大容器的副本，同时漏掉 `const`，导致接口既限制调用位置又做无意复制。

**让 AI 检查**

- 列出每个构造函数完成后必须成立的不变量，并为每个修改状态的公开函数说明它如何保持这些规则。
- 按声明顺序追踪基类和成员初始化，指出任何依赖尚未初始化成员的表达式。
- 说明六个特殊成员函数分别是隐式生成、默认、删除还是自定义，并根据每个成员的所有权验证结果。

**审查清单**

- 用边界值和非法值调用每个构造与修改入口，确认失败不会留下可观察的半更新状态。
- 通过 `const` 引用调用所有观察函数，并检查返回的引用或视图是否可能比对象活得更久。
- 复制并移动对象后分别修改源与目标，再配合地址消毒器检查泄漏、悬空访问和重复释放。

**提示词词汇**

使用准确术语：`class invariant`（类不变量）、`access specifier`（访问说明符）、`member initializer list`（成员初始化器列表）、`declaration order`（声明顺序）、`converting constructor`（转换构造函数）、`const member function`（const 成员函数）、`special member function`（特殊成员函数）和 `Rule of Zero`（零法则）。要求模型逐项给出对象所有权和失败后的状态；「检查这个类」不足以暴露初始化顺序或隐式复制问题。

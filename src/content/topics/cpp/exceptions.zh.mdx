...
生成代码经常捕获 `std::exception` 的值，再用 `throw error;` 重新抛出，导致动态类型和诊断被切掉。它还会插入空的 `catch (...)` 让失败后的半成品继续流动，或用裸指针配合只覆盖成功路径的 `delete`。更危险的组合是先修改对象，再调用可能抛出的函数，同时声称提供强保证；以及把可能分配或调用用户回调的函数误标为 `noexcept`，使原本可捕获的异常直接触发终止。

**让 AI 检查**

- 列出从每个 `throw` 到预期处理边界的调用路径，并标出每层离开时销毁的对象和仍可观察的状态。
- 找出所有 `catch`、裸 `throw;`、`throw expression;` 与 `noexcept`，说明动态类型是否保留，以及异常能否越过声明边界。
- 对每个声称提供强保证的函数，按执行顺序列出可能抛出的操作和此前已经提交的修改。

**审查清单**

- 用异常注入或可控失败输入测试首次操作、中间操作和提交前的失败，核对资源数量与对象不变量。
- 确认异常通过 `const` 引用捕获，派生处理器位于基类之前，而且兜底处理器没有静默吞掉未知错误。
- 检查线程入口、回调、析构函数、`noexcept` 函数和 C ABI 边界，确保异常在允许的位置被转换或终止工作单元。

**提示词词汇**

使用准确术语：`exception object`（异常对象）、`handler matching`（处理器匹配）、`stack unwinding`（栈展开）、`catch by const reference`（通过 `const` 引用捕获）、`rethrow`（重新抛出）、`object slicing`（对象切片）、`basic guarantee`（基本保证）、`strong guarantee`（强保证）、`no-throw guarantee`（不抛出保证）和 `noexcept boundary`（`noexcept` 边界）。要求 AI 给出失败时间线和状态不变量，比笼统地要求“完善错误处理”更容易暴露缺陷。

...
Generated code often guesses `axis` from a variable name without validating the actual input shape. It also mixes `(n, 1)` and `(n,)`, creating an accidental `(n, n)` intermediate. Other recurring failures include using `*` for matrix multiplication, reallocating arrays inside a loop with `np.append()`, and standardizing an integer array in place until `UFuncOutputCastingError` appears. More quietly, generated code may modify a basic slice as if it were temporary and mutate an array still owned by the caller.

**Ask your AI to check**

- List every array's `shape`, `dtype`, axis meanings, and possible memory sharing, then trace how each property changes after every step.
- Run the expression with the smallest non-square input that can expose a wrong broadcast, reduction axis, or matrix multiplication hidden by equal dimensions.
- Find `np.append()` loops, global `np.random.seed()`, in-place integer division, and undocumented writes through views, then propose behavior-preserving replacements.

**Review checklist**

- Assert dimensionality, shape, permitted `dtype`, finite values, and the empty-array policy at the input boundary.
- Record the expected result shape and ownership for each reshape, broadcast, reduction, and advanced index.
- Run tests with a constant column, one row, empty input, `NaN`, extreme values, and a non-contiguous slice.

**Prompt vocabulary**

Use the exact terms `ndarray`, `shape`, `axis semantics`, `dtype promotion`, `broadcasting compatibility`, `basic indexing view`, `advanced indexing copy`, `contiguous array`, and `in-place operation`. Ask for a step-by-step shape table and ownership table. "Vectorize it" says nothing about allowed allocations, missing-data policy, or numerical error.

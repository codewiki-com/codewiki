...
Models often repeat chained-assignment and `inplace=True` patterns from older tutorials without accounting for pandas 3.0 enabling Copy-on-Write by default. They also wrap column-wise logic in `apply(axis=1)`, omit `validate` on joins, or apply `fillna(0)` after automatic index alignment produces `NaN`. Such code often passes tiny samples with sequential indexes but changes business meaning when it meets duplicate keys, shuffled indexes, nullable booleans, or unknown categories.

**Ask your AI to check**

- List the shape, index uniqueness, column `dtype` values, and missing counts before and after every selection, assignment, aggregation, and join.
- State the expected cardinality for every `merge()`, generate duplicate-key and unmatched-key tests, and verify invariant totals.
- Find chained assignment, row-wise `apply()`, implicit type inference, and unexplained `fillna()` calls, then propose semantics-preserving replacements.

**Review checklist**

- Run operations with shuffled, partially overlapping indexes and confirm whether the code pairs by label or by position.
- Test duplicate join keys, null join keys, and missing group keys, then check that row counts and totals obey the contract.
- Compare source and derived objects before and after assignment to confirm ownership under Copy-on-Write matches the API promise.

**Prompt vocabulary**

Use the exact terms `DataFrame`, `Series`, `index alignment`, `nullable dtype`, `boolean mask`, `named aggregation`, `join cardinality`, `merge validation`, and `Copy-on-Write`. Asking an AI to write the data contract and invariants is more likely to expose semantic errors than asking it to “optimize this pandas code.”

...
Generated code often splices pandas into Polars by using Boolean indexing, `axis=1`, `inplace=True`, or row-wise `apply`. Models also claim scan pushdown after `read_parquet().lazy()`, read a newly created alias inside the same `with_columns()`, or mistake `fill_null()` for `NaN` cleanup. Other snippets still use the older `collect(streaming=True)` form, while the current interface selects the streaming engine with `collect(engine="streaming")`.

**Ask your AI to check**

- List every input schema, join-key cardinality, and null policy, then state which invariants must be checked after `collect()`.
- Show the optimized query plan and confirm that file inputs use `scan_*` with the required predicate and projection pushdowns still visible.
- Find every Python user-defined function, alias dependency within one context, and output that relies on row order without an explicit sort.

**Review checklist**

- Run the pipeline on small data containing duplicate keys, unmatched keys, `null`, `NaN`, and an empty input.
- Confirm that `collect()` and `sink_*` appear only at execution boundaries visible to the caller.
- Compare schemas, row counts, and domain totals before and after transformations instead of checking only a few displayed rows.

**Prompt vocabulary**

Use the exact terms `expression context`, `LazyFrame`, `logical query plan`, `predicate pushdown`, `projection pushdown`, `join cardinality`, `schema validation`, and `streaming engine`. Asking an AI to mark materialization boundaries and data invariants produces a more reviewable result than asking it to "make this pandas code faster with Polars."

...
Generated plotting code often pairs `sorted(values)` with labels in their original order, or reuses a pre-aggregation index after a DataFrame aggregation. Models also connect lines after dropping missing values, autoscale every subplot independently, or truncate a bar baseline to `min(values)` and `max(values)`. Another specific failure is copying a sample palette without adding a second perceptible encoding for color.

**Ask your AI to check**

- Trace each input record through filtering, aggregation, and sorting to its final mark, label, and legend entry.
- List the domain, transformation, unit, and baseline for every axis and color channel, then compare all related panels.
- Run chart-preparation code with empty input, duplicate keys, missing values, a constant column, negatives, and extreme values.

**Review checklist**

- With a hand-checkable dataset, verify that mark count, order, labels, and values correspond one to one.
- At the final display size, inspect ticks, units, legends, direct labels, and every state cue that does not rely on color.
- Compare the chart with unaggregated data to check sample sizes, missing records, bin boundaries, and uncertainty descriptions.

**Prompt vocabulary**

Use the exact terms `analytical question`, `data grain`, `mark`, `visual channel`, `scale domain`, `zero baseline`, `shared normalization`, `missing-value gap`, and `uncertainty interval`. Asking for a “field-to-channel mapping table” and a “record-to-mark trace” is more testable than asking an AI to “make the chart more professional.”

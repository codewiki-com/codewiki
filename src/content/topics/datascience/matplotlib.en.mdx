...
Models often mix pyplot stateful calls with object-oriented calls inside one multi-panel function, so titles, legends, or colorbars land on the wrong `Axes`. They also normalize each panel separately, apply `set_xticklabels()` to automatic ticks, or omit `plt.close(fig)` in a batch loop. Another real failure is copying a notebook's `%matplotlib` magic into a `.py` file, which immediately causes a syntax error.

Generated plotting code can also "improve" the meaning out of the data. Common cases include truncating the y-axis without being asked, sorting categories without applying the same order to values, dropping NaNs silently, and placing different units on one axis. Code that runs is not proof that the picture still represents its input.

**Ask your AI to check**

- List the `Figure` and `Axes` that own every artist, and identify every call that depends on pyplot's current state.
- Verify that each group of x, y, color, and size arrays has matching lengths and that category sorting reorders every associated column through the same index.
- State the policies for axis ranges, missing values, color normalization, backend choice, save order, and figure cleanup, with an executable assertion for each.

**Review checklist**

- Use a small, known dataset to check artist counts, data order, axis units, ranges, and shared color scales.
- Run the save path without a display; confirm that files are nonempty and correctly sized and that failures leave no figures open.
- Compare the generated figure against the task line by line, looking for unrequested aggregation, sorting, filtering, or visual exaggeration.

**Prompt vocabulary**

Use the exact terms `Figure`, `Axes`, `Axis`, `Artist`, `object-oriented API`, `current Axes state`, `backend`, `layout engine`, `colormap normalization`, `shared Normalize`, and `headless rendering`. Ask the model to state object ownership and map data fields to artists. "Make it look professional" exposes none of those constraints.

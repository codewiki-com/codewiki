...
生成的绘图代码常把 `sorted(values)` 与原顺序标签组合，或在 DataFrame 聚合后误用聚合前的索引。模型也会把缺失值删除后跨缺口连线，让每个子图独立自动缩放，或者用 `min(values)` 到 `max(values)` 截断柱形基线。另一个具体问题是复制示例色板，却没有为颜色增加第二种可感知编码。

**让 AI 检查**

- 逐个追踪输入记录经过筛选、聚合、排序后对应到哪个标记、标签和图例项。
- 列出每个轴与颜色通道的领域、变换、单位和基线，并比较所有相关面板。
- 用空输入、重复键、缺失值、常量列、负数和极端值运行图表准备代码。

**审查清单**

- 用一个可手算的小数据集核对标记数量、顺序、标签和值是否一一对应。
- 在最终显示尺寸下检查刻度、单位、图例、直接标签和颜色之外的状态提示。
- 对照未聚合数据检查样本量、缺失记录、分箱边界和不确定性说明。

**提示词词汇**

使用准确术语：`analytical question`（分析问题）、`data grain`（数据粒度）、`mark`（标记）、`visual channel`（视觉通道）、`scale domain`（尺度领域）、`zero baseline`（零基线）、`shared normalization`（共享归一化）、`missing-value gap`（缺失值断点）、`uncertainty interval`（不确定性区间）。要求 AI 输出“字段到通道的映射表”和“记录到标记的追踪”，比要求它“把图做得更专业”更可验证。

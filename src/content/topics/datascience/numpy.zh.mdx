...
生成代码经常根据变量名猜 `axis`，却不验证真实输入形状；它也会把 `(n, 1)` 与 `(n,)` 混用，制造意外的 `(n, n)` 中间结果。另一些常见错误是用 `*` 代替矩阵乘法、在 `np.append()` 循环中反复分配数组，或对整数数组执行原地标准化并触发 `UFuncOutputCastingError`。更隐蔽的问题是修改一个基本切片，以为它是临时副本，结果把调用者持有的原数组一起改掉。

**让 AI 检查**

- 为每个数组列出 `shape`、`dtype`、轴语义和是否可能共享内存，并跟踪每一步后的变化。
- 用最小的非方形输入运行表达式，确认广播结果、聚合轴与矩阵乘法不会因两个长度偶然相等而蒙混过关。
- 找出循环里的 `np.append()`、全局 `np.random.seed()`、整数原地除法和未说明的视图写入，并给出保持行为的替代实现。

**审查清单**

- 在入口断言维度、形状、允许的 `dtype`、有限值与空数组策略。
- 对每次 `reshape`、广播、聚合和高级索引，记录预期输出形状与所有权。
- 用常数列、单行、空输入、`NaN`、极值和非连续切片运行测试。

**提示词词汇**

使用准确术语：`ndarray`（多维数组）、`shape`（形状）、`axis semantics`（轴语义）、`dtype promotion`（数据类型提升）、`broadcasting compatibility`（广播兼容性）、`basic indexing view`（基本索引视图）、`advanced indexing copy`（高级索引副本）、`contiguous array`（连续数组）和 `in-place operation`（原地操作）。要求 AI 给出逐步的形状表和所有权表；「把它向量化」无法说明允许的内存分配、缺失值策略或数值误差。

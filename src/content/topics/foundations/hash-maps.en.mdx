...
Models often replace a scan with a map but choose the wrong key, such as using a freshly constructed object for lookup against identity-based keys. They generate `JSON.stringify()` as a universal composite key even though property order, omitted values, numeric edge cases, and normalization may differ. Another common failure is a teaching implementation that overwrites on collisions, copies buckets without rehashing, or recursively resizes while reinserting because its size accounting is wrong.

Generated performance explanations also turn expected behavior into a guarantee. They may claim every operation is `O(1)`, ignore the cost of hashing long input, or use an attacker-controlled deterministic hash in a public service. For mutable key types, models frequently generate `equals()` and `hashCode()` from different fields or include fields that can change after insertion.

**Ask your AI to check**

- State the exact key equality rule, hash inputs, canonicalization steps, and whether any participating field can mutate.
- Construct two unequal keys that collide, two separately created keys that should be equal, and keys immediately around every resize threshold.
- Trace a miss, an update, a collision, deletion if supported, and resizing while checking size and reachability invariants.

**Review checklist**

- Verify that equal keys always hash alike and that collisions retain and compare full candidate keys.
- Distinguish expected or amortized bounds from worst-case work, including key hashing and resize latency.
- Check absent-versus-stored-value behavior, untrusted input limits, and the ownership of mutable keys and values.

**Prompt vocabulary**

Use the precise terms `hash function`, `key equality`, `hash-equality contract`, `collision resolution`, `separate chaining`, `open addressing`, `probe sequence`, `load factor`, `rehashing`, `amortized O(1)`, `worst-case O(n)`, `canonical key`, and `hash-flooding resistance`. Ask for an invariant table covering size, bucket ownership, and key reachability instead of asking only to “make lookup fast.”

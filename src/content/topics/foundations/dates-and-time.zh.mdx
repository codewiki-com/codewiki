...
模型经常把所有时间值都压成时间戳，给简单型文本直接追加 `Z`，或者在本应转换时调用 `replace(tzinfo=...)`。生成的重复日程代码常常保存当前偏移、每天增加 `86_400` 秒、接受不存在的本地时间，而且从不测试重叠的第二次出现。超时辅助函数也会因为 `datetime.now()` 的输出更熟悉而拿它当计时来源。

**让 AI 检查什么**

- 把每个字段归类为时间点、民用日期、民用日期时间、指定时区日程、经过时长或单调截止值。
- 跟踪每次解析、`replace`、`astimezone`、算术操作与序列化；说明每一步取得或丢弃了什么上下文。
- 根据指定时区的真实空缺与重叠生成边界测试，再加入月末、闰日、负区间与时钟调整。

**审查清单**

- 每个表示时间点的边界都规定语法、偏移、精度、范围与整数单位。
- 每次本地时间解析都指明 IANA 时区，以及空缺与重叠策略。
- 每个时长或截止值都使用合适的时钟；日历运算既断言本地字段，也断言经过时间。

**提示词词汇**

使用准确术语：`instant`（时间点）、`civil time`（民用时间）、`aware datetime`（感知型日期时间）、`naive datetime`（简单型日期时间）、`UTC offset`（UTC 偏移）、`IANA time zone`（IANA 时区）、`tzdata version`（时区数据版本）、`DST gap`（夏令时空缺）、`DST overlap`（夏令时重叠）、`fold`（折叠标记）、`calendar arithmetic`（日历运算）、`elapsed duration`（经过时长）、`wall clock`（挂钟）和 `monotonic clock`（单调时钟）。要求提供时间值类型表与转换边界追踪；「正确处理日期」无法形成可测试契约。

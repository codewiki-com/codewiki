...
Generated socket code often assumes `recv(4096)` returns one complete request, or mistakes two consecutive `send()` calls for two boundary-preserving TCP messages. Models also take only the first `getaddrinfo()` result and hard-code `AF_INET`, which works on a developer machine while breaking dual-stack and fallback addresses. Worse variants allocate directly from an untrusted length, decode network input with `pickle`, or replay a write that may already have executed after a timeout.

Another cluster of errors concerns lifetime. A generated server starts an unlimited thread or task for every connection, with no concurrency limit, deadline, or cancellation path. Exception branches then forget to close accepted sockets. It looks like a complete echo server, but slow clients can exhaust descriptors, tasks, or memory.

**Ask your AI to check**

- Enumerate partial completion, timeout, EOF, reset, and cancellation paths for every `send`, `recv`, `connect`, and `accept`, and name the owner that closes each resource.
- Derive the incremental parsing states from the protocol, then test half a header, half a payload, several frames in one read, a zero-length frame, and an oversized declaration.
- Trace the policy from DNS candidates to connection establishment, covering IPv6, IPv4, one overall deadline, and aggregation of failures.

**Review checklist**

- Put hard bounds on input size, connection count, in-flight work, buffered queues, and operation time, then trigger each bound in tests.
- Never treat transport acknowledgment as business commit; every automatic retry has an explicit idempotency basis.
- Split and merge bytes in tests, simulate slow reads, half-close, reset, and total candidate-address failure, and verify that descriptors are eventually released.

**Prompt vocabulary**

Use the exact terms `byte stream`, `datagram boundary`, `message framing`, `partial read/write`, `network byte order`, `operation deadline`, `half-close`, `backpressure`, and `idempotent retry`. Ask for a byte-level input sequence and a resource-ownership table; "write a robust server" is too vague to expose the omissions.

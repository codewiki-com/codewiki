...
Generated code often memoizes `hostname -> address` with no TTL, collapses every resolver error into an empty list, or retries `NXDOMAIN` indefinitely. It may call a DNS-specific API where system lookup semantics are required, silently skip IPv6, or use only the first returned address. Security-sensitive variants validate a resolved address and later resolve the hostname again when connecting, leaving a DNS-rebinding gap.

Models also invent DNS guarantees. Common examples are treating record order as priority, interpreting TTL as a refresh interval every client obeys simultaneously, or claiming DNS-over-HTTPS authenticates zone data end to end. Generated rollout scripts may update an `A` record without inspecting `AAAA`, aliases, delegations, DNSSEC, negative caches, or existing connection pools.

**Ask your AI to check**

- Trace the exact lookup API to its resolver source and list whether it uses system hosts, search domains, DNS servers, and per-process caching.
- Enumerate `NOERROR` with data, NODATA, `NXDOMAIN`, `SERVFAIL`, refusal, timeout, malformed reply, and DNSSEC failure paths, including retry and cache behavior.
- Prove that every connection uses a currently validated candidate under one deadline and that IPv6, IPv4, TTL expiry, and address changes are tested.

**Review checklist**

- Preserve question name, record type, response code, TTL, resolver identity, and candidate order in observable diagnostics without logging sensitive full query streams.
- Bound alias/referral hops, retries, lookup time, cache size, negative lifetime, and stale serving; keep tenant or network views isolated.
- Test record rotation, mixed reachable and unreachable candidates, NODATA versus `NXDOMAIN`, transient resolver failure, and pooled connections surviving a DNS change.

**Prompt vocabulary**

Use the exact terms `stub resolver`, `recursive resolver`, `authoritative server`, `delegation`, `glue record`, `resource-record set`, `CNAME chain`, `positive cache`, `negative cache`, `TTL`, `NXDOMAIN`, `NODATA`, `SERVFAIL`, `split-horizon DNS`, `DNSSEC validation`, `Happy Eyeballs`, and `DNS rebinding`. These terms force a prompt to distinguish roles, failure classes, cache scope, and the handoff from resolution to connection establishment.

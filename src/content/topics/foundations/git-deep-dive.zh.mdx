...
模型经常把 `git add . && git commit` 当作无条件收尾步骤，从而纳入密钥、构建产物或用户未授权的修改。处理冲突或回滚请求时，它也可能直接建议 `reset --hard`、`clean -fd` 或强制推送，却没有先区分工作树、索引、本地引用和远端引用。另一些答案会假定当前分支叫 `main`、远程叫 `origin`、reflog 条目固定，或把 `--force-with-lease` 描述成绝对安全。

**让 AI 检查**

- 根据 `git status --short --branch`、`git diff`、`git diff --cached` 和 `git log --graph --decorate --oneline --all`，逐项说明建议命令会改变哪一层。
- 列出命令执行前后会移动的本地与远端引用，并判断哪些提交将失去可达路径。
- 为破坏性建议给出非破坏性预检、恢复引用和验证命令，同时说明 reflog 无法覆盖哪些情况。

**审查清单**

- 命令没有依赖未验证的分支名、远程名、路径、reflog 序号或干净工作树假设。
- 暂存与提交范围经过明确差异审查，不包含秘密、生成文件或任务外修改。
- 所有历史重写都标明协作边界、租约条件、备份引用以及失败后的停止点。

**提示词词汇**

使用准确术语：`object database`（对象数据库）、`blob`（内容对象）、`tree object`（树对象）、`commit graph`（提交图）、`reference`（引用）、`symbolic ref`（符号引用）、`index`（索引）、`merge base`（合并基点）、`reachability`（可达性）、`reflog`（引用日志）、`detached HEAD`（分离的 HEAD）。要求 AI 说明状态转移与引用前后值，比只问安全的 Git 命令更容易发现前提错误。

...
**What generated code gets wrong here.** Models often treat `navigator.language` as a currency and time zone, use `count === 1` as a universal plural rule, or generate matchers that split BCP 47 tags on `'-'`. They also translate and concatenate word fragments, use unstable English source text as keys, swallow every missing key behind `|| 'English text'`, and pass translations to `innerHTML` for rich content. Generated RTL patches commonly set `dir` but retain physical CSS properties and mirror every icon.

**Ask your AI to check.** Give it the supported locales, catalog structure, and product fallback policy, then ask for missing keys and placeholder type differences across every catalog. Ask it to trace each date, amount, and count from storage to rendering and flag implicit time zones, currencies, or plural conditions. Finally ask it to generate tests with `ar`, `fr`, `zh-Hant`, and a pseudo-locale, while you verify translations and product rules yourself.

**Review checklist.**

- Every locale-sensitive output can identify where its locale, time zone, currency, or unit comes from, and server and client share the same resolution result.
- Messages store complete semantic units; catalog keys, placeholders, and plural branches pass static validation; variables render safely as text.
- RTL tests cover logical properties, dynamic bidirectional fragments, focus order, and icon meaning, not only right-aligned text in screenshots.

**Prompt vocabulary.** Precise terms reduce the model's room to guess: internationalization (i18n), localization (l10n), locale negotiation, BCP 47 language tag, plural category, named placeholder, fallback chain, pseudo-localization, right-to-left (RTL), bidirectional isolation, and CSS logical properties.

...
Models often splice commands from different distribution tutorials: they use `apt` while assuming a RHEL path, hard-code an interface as `eth0`, append the same mount to `/etc/fstab` repeatedly, or execute unpinned remote content with `curl | sudo sh`. They also generate `chmod -R 777`, unquoted variables, broad `pgrep -f` plus `kill -9` matches, and “health checks” that inspect output but ignore exit status. These mistakes can pass on an empty test machine and still damage a stateful host, restricted container, path containing spaces, or partially failed run.

**Ask your AI to check**

- List the distribution, init system, user identity, namespaces, paths, and external commands the script assumes, and give a runtime validation for each assumption.
- For every write, deletion, recursive permission change, and process termination, expand the exact target and explain idempotency, failure state, and rollback.
- Trace ownership and cleanup of every child process and file descriptor across success, error, timeout, cancellation, and signal paths.

**Review checklist**

- Run the script as a least-privileged user in a container or virtual machine that matches production, and verify that it does not depend on an interactive terminal.
- Exercise empty arguments, filenames containing spaces or newlines, repeated execution, a full disk, and an interrupt during each mutation.
- Confirm that logs retain original exit statuses and standard error without recording tokens, environment secrets, or complete sensitive paths.

**Prompt vocabulary**

Use the precise terms `system call`, `path resolution`, `effective UID`, `file descriptor ownership`, `process reaping`, `graceful termination`, `mount namespace`, and `cgroup limit`. Ask for a five-part operation plan—preconditions, read-only inspection, exact mutation, verification, and rollback—instead of “write a Linux fix script.”

...
模型经常为计算所得测量值生成严格相等比较，把 `Number.EPSILON` 当成通用容差，或者用 `Math.round(value * 100) / 100` 实现十进制结算。它们还可能因为公式不会 `throw` 就省略有限结果检查，并按实数结合律重构求和。只含较小正值的测试会漏掉溢出、消减、不安全整数、`NaN` 和负零。

**让 AI 检查**

- 跟踪每个数值输入、中间值与输出的表示方式，包括单位、最大量级和发生舍入的位置。
- 根据比较所用算法推导绝对与相对误差界限，不要按惯例选择容差。
- 为消减、溢出、次正规下溢、`NaN`、无穷、带符号零和 `Number.MAX_SAFE_INTEGER` 生成边界测试。

**审查清单**

- 确认二进制浮点数符合领域要求；精确十进制金额和大整数标识符要使用合适的表示方式。
- 根据书面的数值约定检查每个相等比较、舍入步骤、归约顺序和有限值边界。
- 测试接近零、普通和最大量级，并覆盖非有限结果及量级悬殊的操作数。

**提示词词汇**

使用准确术语：`binary64`（binary64 格式）、`significand`（有效数字）、`unit in the last place (ULP)`（末位单位）、`round to nearest, ties to even`（就近舍入，中点取偶）、`absolute tolerance`（绝对容差）、`relative tolerance`（相对容差）、`catastrophic cancellation`（灾难性消减）、`subnormal`（次正规数）、`overflow`（溢出）、`underflow`（下溢）、`NaN`、`signed zero`（带符号零）和 `safe integer`（安全整数）。要求模型给出误差预算和中间量级表；「修复精度」没有说明数值约定。

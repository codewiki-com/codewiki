...
模型常通过关闭校验来消除 `UNABLE_TO_VERIFY_LEAF_SIGNATURE`，或者连接到 IP 时没有保留 `servername`，结果造成主机名错误，继而引出不安全的绕过方案。它还可能在添加一个私有 CA 时替换平台信任根，信任对端提供的根，用 `endsWith()` 解析证书名称，或者让同一条 TLS 套接字跨越不兼容的 authority。生成的重试代码也常把每种握手错误都当作临时问题，并在结果不明确时重新发送非幂等操作。

**让 AI 检查**

- 把 DNS 名称、已选地址、SNI、主机名校验输入、HTTP authority 和代理目标作为不同值追踪。
- 列出证书链构建输入，包括叶证书、有序中间证书、信任锚、有效时间、扩展密钥用途和名称约束。
- 按 DNS、传输、TLS 协商、证书链校验、服务身份和应用协议分类失败，不得提出绕过校验的方案。

**审查清单**

- 确认每条生产路径都启用对端校验，并且没有环境变量在暗中削弱校验。
- 测试受信正确名称、未知根、错误名称、过期叶证书、缺失中间证书和证书轮换重叠。
- 检查连接池、重试、客户端证书和明文日志是否遵守 authority 与终止边界。

**提示词词汇**

使用准确术语：`trust anchor`（信任锚）、`certificate path validation`（证书路径校验）、`Subject Alternative Name (SAN)`（主题备用名称）、`reference identity`（参考身份）、`hostname verification`（主机名校验）、`Server Name Indication (SNI)`（服务器名称指示）、`ALPN`（应用层协议协商）、`CertificateVerify`、`Finished`、`ephemeral key share`（临时密钥份额）、`forward secrecy`（前向保密）和 `0-RTT replay`（0-RTT 重放）。要求模型输出分阶段失败表和明确的负面测试；「让 HTTPS 工作」没有说清安全契约，容易诱发绕过方案。

...
Models often produce a recognizable base case but fail to decrease every branch: binary search keeps the midpoint, a parser retries the same cursor, or graph traversal marks nodes after visiting neighbors. They also generate elegant recursive walkers for JSON, AST, or directory input without a depth budget, cycle detection, or error policy. Another common failure is naive recursive Fibonacci or path counting presented with an incorrect `O(n)` claim because the model counts depth instead of total calls.

Generated backtracking code deserves special attention. A model may reuse one mutable `path`, omit cleanup on early return or exception, or memoize results under an incomplete key. These defects often survive happy-path tests because the first explored branch succeeds.

**Ask your AI to check**

- Name the base-case set and a well-founded termination measure, then show the measure before and after every recursive edge.
- Trace active frames and returns for the smallest input, one ordinary input, and a cycle or maximum-depth input.
- Count total calls separately from maximum call depth, and propose an explicit-stack version with the same traversal order.

**Review checklist**

- Exercise empty, singleton, chain-shaped, cyclic, and rejected inputs that the stated domain permits.
- Verify every branch preserves the call contract, strictly decreases the measure, and restores mutated path state.
- Bound total work and simultaneous depth independently; require iteration when external input can exceed the depth budget.

**Prompt vocabulary**

Use `base case`, `recursive case`, `well-founded measure`, `strict decrease`, `structural induction`, `call frame`, `maximum call depth`, `recursion limit`, `cycle detection`, `memoization`, `backtracking invariant`, and `explicit stack`. Ask for a call tree when work branches, a frame trace when order matters, and a worst-case shape rather than a representative input.

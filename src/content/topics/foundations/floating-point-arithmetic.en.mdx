...
Models commonly generate strict equality for computed measurements, paste `Number.EPSILON` as a universal tolerance, or implement decimal settlement with `Math.round(value * 100) / 100`. They may omit finite-result checks because the formula has no `throw`, and they often refactor sums under real-number associativity. Tests made only from small positive values then miss overflow, cancellation, unsafe integers, `NaN`, and negative zero.

**Ask your AI to check**

- Trace the representation of every numeric input, intermediate, and output, including units, maximum magnitude, and where rounding occurs.
- Derive absolute and relative error bounds for the comparison instead of choosing a tolerance by convention.
- Generate boundary tests for cancellation, overflow, subnormal underflow, `NaN`, infinities, signed zero, and `Number.MAX_SAFE_INTEGER`.

**Review checklist**

- Confirm that binary floating-point matches the domain; exact decimal amounts and large integer identifiers use an appropriate representation.
- Inspect every equality, rounding step, reduction order, and finite-value boundary against a written numeric contract.
- Run tests across near-zero, ordinary, and maximum magnitudes, including nonfinite results and mixed-scale operands.

**Prompt vocabulary**

Use the exact terms `binary64`, `significand`, `unit in the last place (ULP)`, `round to nearest, ties to even`, `absolute tolerance`, `relative tolerance`, `catastrophic cancellation`, `subnormal`, `overflow`, `underflow`, `NaN`, `signed zero`, and `safe integer`. Ask for an error budget and intermediate-magnitude table; “fix the precision” does not identify a numeric contract.

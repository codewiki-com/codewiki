---
title: 'HTTP 语义'
description: '综合运用方法、状态码、响应字段、缓存策略、幂等性和连接边界等要素，设计并诊断 HTTP 请求与响应交互。'
track: 'foundations'
section: 'networking'
difficulty: 'beginner'
...
<TLDR>
  <TLDRCell label="what">
    HTTP 语义定义请求和响应的含义：方法表达意图，状态码报告结果，字段限定消息的处理方式。
  </TLDRCell>
  <TLDRCell label="trap">
...
HTTP 语义是一套赋予交互含义的共同规则。请求说明客户端想对目标资源执行什么操作，响应说明该请求如何被处理，还可以携带资源状态的某种<Term id="representation">表示（representation）</Term>。
...
<TryToBreak items={['无效的 JSON 内容', '两个并发的首次写入', '超过服务限制的请求体']} />
...
> [!PITFALL]
> 把 `{ "success": false }` 等 JSON 字段放在 `200 OK` 内，会向通用客户端、缓存、网关与可观测工具隐藏认证失败、冲突、资源缺失和过载等结果。
...
> [!PITFALL]
> 带有较长新鲜度寿命且缓存键不完整的个性化响应，可能被共享缓存复用于另一个用户。仅仅使用 Cookie 不能代替明确的响应缓存策略。
...
生成式客户端经常重试所有网络异常，却不检查方法语义、主体可重现性或幂等约定。生成式服务端常为全部结果返回 `200`，在 `204` 或 `304` 中发送内容，把实体标签当普通字符串比较，遗漏 `Vary`，或把认证数据公开缓存。模型还会把 HTTP/1.1 连接字段混入 HTTP/2 代码，并推断连接重置足以证明应用已经回滚。

**让 AI 检查**

- 为每种方法建立表格，列出安全性、幂等性、请求体可重放性、重试触发条件、尝试上限和截止时间行为。
- 枚举所有响应状态，并根据 RFC 9110 与 RFC 9111 检查其内容规则、必需字段、可缓存性和调用方动作。
- 追踪一次新鲜命中、一次陈旧验证、一次验证器变化、一次写入响应丢失和一次连接重置，不要推断无法观察到的应用结果。

**审查清单**

- 对每项操作重复发送相同请求，同时确认预期状态与允许存在的响应差异。
- 测试缓存变体、认证身份、`304` 元数据合并，以及已存内容缺失的情况。
- 检查协议边界是否存在歧义帧结构、转发的逐跃点字段、无界重试，以及混为一谈的连接与操作标识符。

**提示词词汇**

使用准确术语：`safe method`（安全方法）、`idempotent method`（幂等方法）、`selected representation`（选定表示）、`conditional request`（条件式请求）、`strong entity tag`（强实体标签）、`freshness lifetime`（新鲜度寿命）、`cache key`（缓存键）、`Vary`、`end-to-end field`（端到端字段）、`hop-by-hop field`（逐跃点字段）、`message framing`（消息帧结构）和 `ambiguous commit`（提交结果不确定）。要求模型给出状态约定与重试决策表；「让这个端点符合 REST」过于模糊，无法暴露相关 HTTP 保证。

...
生成代码经常把整洁的递归树遍历直接用于图形 API 数据，却不验证父节点唯一性，也不增加已访问状态。模型还会生成使用 `shift()` 的 BFS、等到出队后才做标记、覆盖父节点，或者用显示标签充当标识。常见的依赖辅助函数会把 DFS 发现顺序当作「拓扑排序」，并静默接受环。

**让 AI 检查这些内容**

- 说明边是否有向、每个方向的含义、顶点如何标识，以及权重是否相等。
- 在自环、菱形结构、断开的顶点和不存在的目标上跟踪算法；逐步展示栈或队列状态。
- 说出访问不变量，根据所选表示推导时间与空间上界，并指出运行时栈或输入规模限制。

**审查清单**

- 确认表示形式、方向、标识、邻居顺序和缺失顶点行为符合领域契约。
- 检查已访问状态的标记时机与所有权，以及路径、环或拓扑顺序所需的父节点或完成状态。
- 运行对抗性样例：空图、起点等于终点、环、共享目标、断开的连通分量和深链。

**提示词词汇**

使用准确术语：`rooted tree`（有根树）、`directed graph`（有向图）、`vertex identity`（顶点标识）、`edge direction`（边方向）、`adjacency list`（邻接表）、`depth-first search (DFS)`（深度优先搜索）、`breadth-first search (BFS)`（广度优先搜索）、`visited set`（已访问集合）、`discovery time`（发现时间）、`completion time`（完成时间）、`frontier`（前沿）、`parent map`（父节点映射）、`unweighted shortest path`（无权最短路径）和 `topological sort`（拓扑排序）。应要求给出不变量和经过跟踪的反例；「遍历这些数据」没有说明关键契约。

...
模型经常把用户值插入一整段命令字符串，引用失败后又添加 `eval`，或者写出 `for file in $(find ...)`，导致空格与元字符变成语法。它还会把 `set -euo pipefail` 当成固定仪式，却不测试其依赖上下文的行为；在日志命令之后才读取 `$?`；或者生成 `rm -rf "$dir"/*`，却不证明 `dir` 的取值。另一种常见错误是用 `source` 加载原本只应包含数据的配置文件，从而把文件作为 Shell 代码执行。

**让 AI 检查**

- 对每条外部命令，追踪源文本经过参数展开、单词拆分、路径名展开与引号删除后形成最终参数向量的过程。
- 列出所有能影响路径、选项、命令名称、环境值或加载文件的输入，并说明其校验规则与权限边界。
- 枚举简单命令与管道的非零状态，说明每个状态在哪里被捕获、忽略、转换或返回。

**审查清单**

- 使用空格、空字符串、通配符、换行与前导连字符名称测试参数边界。
- 用数组替换生成的命令字符串，拒绝任何没有解释的 `eval`、`bash -c` 或 `source` 边界。
- 强制每条命令与每个管道阶段失败，再验证状态传播、诊断信息、清理逻辑与修改范围。

**提示词词汇**

使用准确术语：`argument vector`（参数向量）、`parameter expansion`（参数展开）、`double-quoted expansion`（双引号内展开）、`word splitting`（单词拆分）、`pathname expansion`（路径名展开）、`command substitution`（命令替换）、`option terminator`（选项终止符）、`exit status`（退出状态）、`pipeline status`（管道状态）、`pipefail`、`subshell`（子 Shell）与 `environment inheritance`（环境继承）。要求模型输出展开追踪与失败状态表；「让这条 Shell 命令更安全」没有指出解析边界或权限边界。

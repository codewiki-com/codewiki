...
Models often repeat “arrays are fast for reads, linked lists are fast for writes” without specifying indices, node handles, or lookup cost. Generated linked queues commonly fail to clear `tail` after the last dequeue, while generated doubly linked lists update only one direction. Another failure is replacing an array with linked objects after seeing an `O(n)` insertion, even though the real workload is traversal-heavy and now allocates one object per element.

**Ask your AI to check**

- Produce an operation table that separates locating a position from mutating it, with preconditions for every complexity claim.
- Trace empty, one-node, head, tail, and middle mutations, then list the endpoint, length, and bidirectional-link invariants after each step.
- Compare array, ring-buffer, and linked-node memory models for the actual element type, and label runtime assumptions that require measurement.

**Review checklist**

- Confirm the dominant access pattern uses the representation's cheap path rather than paying a hidden scan or shift.
- Run boundary tests for invalid indices, empty transitions, the last node, duplicate handles, and repeated removal.
- Profile representative sizes for allocation count, retained memory, traversal time, and resize latency before accepting a rewrite.

**Prompt vocabulary**

Use the exact terms `contiguous storage`, `random access`, `dynamic array`, `capacity`, `geometric growth`, `amortized append`, `singly linked list`, `doubly linked list`, `head`, `tail`, `node handle`, `pointer rewiring`, `cache locality`, and `allocation overhead`. Ask for an operation-and-precondition table; “which data structure is faster” does not define a workload.

...
Models frequently produce a Boolean comparator, call mutating `sort()` on caller-owned input, or write a binary-search loop whose equality branch returns an arbitrary duplicate. They also repeat sorting inside a request loop while claiming the operation is `O(log n)`, or reuse a stale sorted snapshot after records change. Generated tests often contain one present target in the middle and miss empty arrays, absent values, duplicate runs, and endpoints.

**Ask your AI to check**

- State the ordering contract: key extraction, direction, null policy, tie-breakers, stability, and whether the result must be deterministic across input orders.
- Write the binary-search loop invariant and show that each branch strictly shrinks the half-open interval.
- Account for construction, mutation, and query costs across the real workload, and identify who owns refresh or invalidation.

**Review checklist**

- Test empty and one-element inputs, targets below and above the range, absent interior targets, and duplicate keys at both ends.
- Confirm that sorting and searching use the same comparator or key normalization and that equal-key semantics match the requested result.
- Verify whether input mutation is permitted, whether derived order can become stale, and whether repeated sorting occurs on a hot path.

**Prompt vocabulary**

Use the exact terms `linear search`, `stable sort`, `comparator contract`, `total order`, `tie-breaker`, `lower bound`, `upper bound`, `half-open interval`, `loop invariant`, `in-place sort`, and `sorted snapshot`. Ask for the query/update mix and duplicate policy; “make lookup faster” leaves the data contract unspecified.

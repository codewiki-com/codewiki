...
<TryToBreak items={['发送第二条命令', '收到回复前断开连接', '发送 IPC 序列化器无法表示的值']} />

</Depth>

消息会按照 IPC 序列化契约传输数据，不会把 `parentState` 暴露给子进程。`message` 事件是通信边界，`exit` 事件是生命周期边界。生产代码还要处理 `error`、异常退出和响应截止时间。
...
模型经常为每项输入创建一个工作线程，用 `value += 1` 修改共享计数器，再把 `setTimeout()` 当作等待工作结束。它还可能在收到第一个工作线程消息后就兑现父 Promise，遗漏异常退出处理，或者误以为 `unref()` 会取消线程。另一类常见错误是把线程推荐为执行不可信代码的安全边界，或者只给写入操作加互斥锁，却把依赖同一不变量的读取留在锁外。

**让 AI 检查**

- 为每个可变对象、文件描述符、通道、进程与线程制作所有权表，并注明由谁关闭或等待它。
- 画出创建、发布、更新、取消与最终读取之间的先发生关系，找出每个先检查后操作序列。
- 根据峰值输入与部分失败计算最大存活工作单元数、排队项数、栈内存、继承句柄和关闭时长。

**审查清单**

- 运行能感知竞态的测试与对抗性调度用例，再检查测试无法证明的每个共享不变量。
- 覆盖空输入、创建失败、工作线程崩溃、超时、取消、部分结果，以及工作阻塞时关闭系统。
- 确认并发有明确上限，而且隔离方式符合故障、信任与资源边界要求。

**提示词词汇**

使用准确术语：`process isolation`（进程隔离）、`thread`（线程）、`address space`（地址空间）、`scheduler`（调度器）、`runnable`（可运行）、`blocked`（阻塞）、`context switch`（上下文切换）、`race condition`（竞态条件）、`critical section`（临界区）、`mutex`（互斥锁）、`atomic read-modify-write`（原子读取修改写回）、`memory visibility`（内存可见性）、`happens-before`（先发生）、`join`（等待线程结束）、`cancellation`（取消）、`bounded worker pool`（有界工作池）和 `lock ordering`（加锁顺序）。要求生成所有权表与生命周期状态图；「让它并发」没有指定必要契约。
...
<Term id="race-condition">竞态条件</Term>不只是「两次写入同时发生」。先检查权限有效再打开路径，可能与重命名竞争；先检查队列非空再移除元素，可能与另一个消费者竞争。应把决策与操作作为一个不变量保护，或者使用把两者合并为原子步骤的 API。
...
- [Node.js v24 文档：子进程](https://nodejs.org/docs/latest-v24.x/api/child_process.html)
- [Node.js v24 文档：工作线程](https://nodejs.org/docs/latest-v24.x/api/worker_threads.html)
...
- [Linux 手册：调度概览](https://man7.org/linux/man-pages/man7/sched.7.html)
- [Linux 手册：POSIX 线程](https://man7.org/linux/man-pages/man7/pthreads.7.html)

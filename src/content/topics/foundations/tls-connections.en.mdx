...
Models often silence `UNABLE_TO_VERIFY_LEAF_SIGNATURE` by disabling verification, or connect to an IP without preserving `servername`, causing either a hostname failure or an unsafe workaround. They may replace the platform trust roots when adding one private CA, trust a peer-supplied root, parse certificate names with `endsWith()`, or reuse one TLS socket across incompatible authorities. Generated retry code also tends to label every handshake error transient and can resend non-idempotent work after an ambiguous failure.

**Ask your AI to check**

- Trace DNS name, selected address, SNI, hostname-verification input, HTTP authority, and proxy destination as separate values.
- Enumerate chain-building inputs: leaf, ordered intermediates, trust anchors, validity time, Extended Key Usage, and name constraints.
- Classify failures by DNS, transport, TLS negotiation, chain validation, service identity, and application protocol without proposing a verification bypass.

**Review checklist**

- Confirm peer verification is enabled on every production path and no environment variable silently weakens it.
- Test a trusted correct name, unknown root, wrong name, expired leaf, missing intermediate, and certificate rotation overlap.
- Verify connection pooling, retries, client certificates, and plaintext logs respect authority and termination boundaries.

**Prompt vocabulary**

Use `trust anchor`, `certificate path validation`, `Subject Alternative Name (SAN)`, `reference identity`, `hostname verification`, `Server Name Indication (SNI)`, `ALPN`, `CertificateVerify`, `Finished`, `ephemeral key share`, `forward secrecy`, and `0-RTT replay`. Ask for a phase-by-phase failure table and explicit negative tests; “make HTTPS work” invites a bypass without naming the security contract.

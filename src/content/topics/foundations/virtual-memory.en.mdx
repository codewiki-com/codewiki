...
Models often equate allocation size with RSS, call every page fault a disk read, or label a growing virtual arena as a leak. Low-level generated code may omit multiplication-overflow checks before `mmap()`, ignore `MAP_FAILED`, keep a pointer after `munmap()`, or use `MAP_FIXED` over an existing range. Operational suggestions are just as risky: disabling swap, dropping caches, or raising a container limit can hide an unbounded cache without explaining ownership.

**Ask your AI to check**

- Build a table for every large range with its owner, virtual size, committed or touched pages, backing type, permissions, sharing mode, cleanup path, and applicable cgroup or process limit.
- Separate allocation failure, minor fault, major fault, protection fault, `SIGSEGV` or `SIGBUS`, and OOM kill; name the evidence that distinguishes each branch on the target operating system.
- Trace size arithmetic and lifetime around every allocator, `mmap()`, `fork()`, `mprotect()`, and `munmap()` call, including error, cancellation, and concurrent access paths.

**Review checklist**

- Exercise zero, one-page, boundary, oversized, partially touched, and concurrent inputs under the real memory limit.
- Compare virtual mappings, RSS or proportional accounting, private dirty memory, fault rates, reclaim, swap I/O, and latency on one timeline.
- Confirm that every mapping and allocation has checked arithmetic, permissions no broader than required, a single owner, and a defined release point.

**Prompt vocabulary**

Use the exact terms `virtual address space`, `page table entry`, `resident set size`, `proportional set size`, `minor page fault`, `major page fault`, `protection fault`, `demand paging`, `copy-on-write`, `page cache`, `anonymous memory`, `working set`, `memory cgroup`, and `OOM kill`. Ask for a mapping-to-backing table and a fault decision tree; "optimize memory" is too vague to expose the relevant layer.

...
生成代码常把 `hostname -> address` 永久记忆化，把所有解析错误压成空列表，或无限重试 `NXDOMAIN`。它可能在需要系统查找语义时调用 DNS 专用 API，悄悄跳过 IPv6，或者只使用第一个返回地址。涉及安全时，更危险的写法会先验证解析地址，连接时再解析一次主机名，留下 DNS 重绑定空隙。

模型还会虚构 DNS 保证。常见例子包括把记录顺序当作优先级，把 TTL 解释为所有客户端同时遵守的刷新周期，或声称 DNS-over-HTTPS 可以端到端认证区域数据。生成的发布脚本可能只更新 `A` 记录，而不检查 `AAAA`、别名、委派、DNSSEC、否定缓存和现有连接池。

**让 AI 检查这些内容**

- 从具体查找 API 追踪到解析器来源，列出它是否使用系统 hosts、搜索域、DNS 服务器和进程内缓存。
- 枚举带数据的 `NOERROR`、NODATA、`NXDOMAIN`、`SERVFAIL`、拒绝、超时、畸形响应和 DNSSEC 失败路径，包括重试与缓存行为。
- 证明每次连接都在同一截止时间内使用刚刚通过验证的候选地址，并测试 IPv6、IPv4、TTL 过期和地址变化。

**审查清单**

- 在可观测诊断中保留问题名称、记录类型、响应码、TTL、解析器身份和候选顺序，但不要记录敏感的完整查询流。
- 限制别名或转介跳数、重试次数、查找时间、缓存大小、否定生存时间和过期数据供应；隔离租户或网络视图。
- 测试记录轮换、可达与不可达候选混合、NODATA 与 `NXDOMAIN` 的差别、暂时性解析器故障，以及 DNS 变化后仍存活的池化连接。

**提示词汇**

使用精确术语 `stub resolver`（存根解析器）、`recursive resolver`（递归解析器）、`authoritative server`（权威服务器）、`delegation`（委派）、`glue record`（胶水记录）、`resource-record set`（资源记录集）、`CNAME chain`（CNAME 链）、`positive cache`（肯定缓存）、`negative cache`（否定缓存）、`TTL`（生存时间）、`NXDOMAIN`、`NODATA`、`SERVFAIL`、`split-horizon DNS`（分割视图 DNS）、`DNSSEC validation`（DNSSEC 验证）、`Happy Eyeballs`（快乐眼球算法）和 `DNS rebinding`（DNS 重绑定）。这些词能迫使提示区分各类角色、故障类别、缓存作用域，以及从解析到建立连接的交接。

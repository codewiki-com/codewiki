...
Models often treat `git add . && git commit` as an unconditional finishing step, capturing secrets, build output, or user changes outside the task. When asked to resolve a conflict or roll work back, they may immediately suggest `reset --hard`, `clean -fd`, or a force push without separating the working tree, index, local refs, and remote refs. Other answers assume the branch is `main`, the remote is `origin`, a reflog ordinal is stable, or `--force-with-lease` is absolutely safe.

**Ask your AI to check**

- From `git status --short --branch`, `git diff`, `git diff --cached`, and `git log --graph --decorate --oneline --all`, explain exactly which layer each proposed command will change.
- List every local and remote reference that moves before and after the command, then identify commits that will lose a reachability path.
- For every destructive suggestion, provide a non-destructive preflight, a recovery reference, and verification commands, and state which losses reflog cannot recover.

**Review checklist**

- Commands do not depend on unverified branch names, remote names, paths, reflog ordinals, or a clean-working-tree assumption.
- The staging and commit scope has an explicit diff review and excludes secrets, generated files, and out-of-task changes.
- Every history rewrite states its collaboration boundary, lease condition, backup reference, and failure stop point.

**Prompt vocabulary**

Use precise terms: `object database`, `blob`, `tree object`, `commit graph`, `reference`, `symbolic ref`, `index`, `merge base`, `reachability`, `reflog`, and `detached HEAD`. Ask the AI for a state transition and before-and-after ref values rather than merely asking for a safe Git command; that wording exposes false assumptions.

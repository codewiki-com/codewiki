...
Models often generate one response object with many optional fields, then use `data!` or `error!` to bypass missing values; that loses the correlation between discriminant and payload. They also write `JSON.parse(raw) as Event`, making apparently exhaustive branches depend on unvalidated data, or return `A & B` after spreading only `A` because they assume the type operation created `B` fields. When property names overlap, generated code tends to add more assertions instead of checking whether the intersection produced `never`.

**Ask your AI to check**

- List the discriminant and member-specific required fields for every union member, then construct one invalid state the current type mistakenly permits.
- Find every `as`, non-null assertion, and `any`, and explain whether it bypasses external-data validation or union narrowing.
- Compute the overlapping keys and resulting property types of every intersection, then compare them with the real object-construction path.

**Review checklist**

- Test every valid member, an unknown discriminant, a missing payload, and fields with the wrong primitive types at the boundary.
- Confirm that reliable narrowing precedes every member-specific access and that each closed union proves exhaustiveness with `never`.
- Confirm that every runtime value of an intersection really has both sides' fields and that no assertion hides an overlapping-property conflict.

**Prompt vocabulary**

Use the exact terms `union type`, `intersection type`, `discriminated union`, `discriminant property`, `narrowing`, `exhaustiveness checking`, `never`, `structural typing`, and `conflicting property`. Ask for a member matrix, boundary validation, and an overlapping-key analysis; “make the types safer” is too vague to produce a reviewable result.

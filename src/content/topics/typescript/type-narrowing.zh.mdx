---
title: 类型收窄
description: TypeScript 如何根据控制流收窄联合类型与 unknown，并用穷尽检查、可信谓词和稳定的局部绑定防止类型错误。
track: typescript
section: type-system
...
<TLDR>
  <TLDRCell label="what">
    <Term id="control-flow-narrowing">控制流收窄（control-flow narrowing）</Term>让 TypeScript 根据检查、赋值和可达路径，缩小值在当前位置的可能类型。
  </TLDRCell>
  <TLDRCell label="trap">
...
类型收窄不会修改数据，也不会把类型信息带入 JavaScript。`typeof`、相等比较和属性读取会在运行时执行，接口与联合类型则会被<Term id="type-erasure">类型擦除（type erasure）</Term>。因此，外部数据必须经过真实检查，不能靠类型注解变得可信。
...
<TryToBreak items={['空字符串', 'NaN', '无效的 Date', 'null']} />
...
模型经常在 `JSON.parse()` 后直接添加 `as Payload`，或者生成一个只检查判别字段存在的 `value is Payload` 谓词。它还会用 `if (count)` 同时处理缺失值与零，把可辨识联合的 `default` 分支写成通用回退，并在 `await` 或回调之后继续使用之前对可变属性得出的结论。这些代码往往能通过类型检查，因为断言和谓词正是在告诉编译器「相信我」。

**让 AI 检查**

- 列出每次收窄使用的具体运行时证据，并说明成立与不成立分支各自剩余哪些类型。
- 找出从守卫到使用点之间对变量、属性及其别名的所有写入，并检查每个异步或回调边界。
- 为每个自定义谓词生成真假两侧的反例，再确认联合类型新增成员时 `never` 检查会失败。

**审查清单**

- 用 `0`、`""`、`false`、`null`、`undefined` 和错误对象形状测试边界。
- 确认外部值从 `unknown` 开始，且类型断言没有替代运行时验证。
- 检查每个状态分支的专属载荷、穷尽性，以及回调捕获的是否为稳定局部值。

**提示词词汇**

使用准确术语：`declared type`（声明类型）、`observed type`（观察类型）、`control-flow narrowing`（控制流收窄）、`reachability`（可达性）、`type guard`（类型守卫）、`discriminated union`（可辨识联合）、`exhaustiveness check`（穷尽性检查）、`type predicate`（类型谓词）、`assertion function`（断言函数）。要求模型输出分支后的剩余联合成员和写入点清单；「让类型更安全」不足以约束结果。

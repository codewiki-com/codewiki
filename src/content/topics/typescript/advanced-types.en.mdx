...
Generated code often writes `DeepPartial<T>` or `DeepReadonly<T>` with one object branch for functions, arrays, `Date`, and `Map`.
It also adds broad assertions after `Object.keys()`, `JSON.parse()`, or dynamic assignment, wrapping an unverified implementation in a precise-looking API.
Another frequent failure is accidental conditional distribution tested only with a single type, so the defect appears only when a real union arrives.

**Ask your AI to check**

- List the proof obligation for every `as`, `any`, and non-null assertion, and identify the runtime check or invariant that supports it.
- Instantiate each conditional type with a single-member union, a mixed union, `never`, `unknown`, and optional properties; explain whether it distributes.
- Compare `keyof` with the runtime enumeration and state whether number keys, symbol keys, inherited properties, non-enumerable properties, and accessors are supported.

**Review checklist**

- Run a validator at every untrusted-data boundary before admitting the value to a derived domain type.
- Write assignability tests for type-level results and output plus failure-path tests for the corresponding runtime helper.
- Confirm that every recursive utility has explicit terminal cases, a constrained input domain, and a stated policy for built-in objects.

**Prompt vocabulary**

Use the terms `indexed access type`, `conditional type`, `distributive conditional type`, `mapped type`, `key remapping`, `template literal type`, `naked type parameter`, and `type erasure`.
Ask the AI to show the compile-time relationship, runtime implementation, and assertions crossing between them separately; “make the types safer” is too vague to expose the boundary.

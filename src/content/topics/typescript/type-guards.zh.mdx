---
title: 类型守卫
description: 用运行时证据安全地收窄联合类型与 unknown，并写出可信的自定义类型谓词和断言函数。
track: typescript
section: type-system
...
<TLDR>
  <TLDRCell label="what">
    <Term id="type-guard">类型守卫（type guard）</Term>是能让 TypeScript 在某条控制流路径上缩小值类型范围的运行时检查。
  </TLDRCell>
  <TLDRCell label="trap">
...
类型守卫就是这份证据。`typeof value === "string"` 不只计算一个布尔值；TypeScript 还会在成立分支中把 `value` 视为 `string`。这种根据分支、赋值和可达性持续更新「当前可能类型」的过程叫作<Term id="control-flow-narrowing">控制流收窄（control-flow narrowing）</Term>。

守卫没有给 JavaScript 加入新的运行机制。`typeof`、`instanceof`、`in`、相等比较和属性比较都照常运行，编译器只是理解其中一部分表达式的含义。接口和联合类型会被<Term id="type-erasure">类型擦除（type erasure）</Term>，因此外部输入仍然需要真实的运行时检查。
...
> [!PITFALL]
> `instanceof` 依赖运行时构造函数和原型链，因此反序列化对象、另一个 realm 创建的对象或重复加载库产生的实例可能失败。接口本身在运行时不存在，不能放在 `instanceof` 右侧。
...
生成器很容易先写出接口，再把 `JSON.parse()` 结果直接断言成该接口；代码能编译，却把静态声明冒充成运行时验证。另一个常见错误是生成过宽的 `isUser()`，只检查对象或一个判别字段，同时承诺所有嵌套字段都安全。模型还会把 `filter(Boolean)`、真值检查或 `instanceof` 当成通用解法，从而误删合法的 `0`，或拒绝结构正确的跨边界对象。

**要求 AI 检查**

- 列出每个类型谓词承诺的字段与领域约束，并指出函数体中对应的运行时证据。
- 为每个守卫生成真假两侧的反例，至少覆盖 `null`、数组、缺字段、错原始类型和边界数字。
- 找出所有 `as`、非空断言、`filter(Boolean)`、`in` 与 `instanceof`，说明它们能证明什么以及不能证明什么。

**审查清单**

- 外部值是否先以 `unknown` 进入，并在获得领域类型前经过真实检查。
- 谓词的假分支是否安全，守卫返回后依赖的可变对象是否可能被别名修改。
- 测试是否同时执行守卫与类型检查，并覆盖每个联合成员和非法判别值。

**提示词术语**

使用准确术语：`type guard`（类型守卫）、`control-flow narrowing`（控制流收窄）、`type predicate`（类型谓词）、`assertion function`（断言函数）、`discriminated union`（可辨识联合）、`exhaustiveness check`（穷尽性检查）、`runtime validation`（运行时验证）和 `type assertion`（类型断言）。要求生成器给出「运行时证据到静态收窄」的对应表，比笼统地要求「提高类型安全」更容易发现虚假的证明。

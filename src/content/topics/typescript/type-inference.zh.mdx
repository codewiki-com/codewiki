...
模型常把公开返回注解当作冗余语法删除，使新增的 `undefined`、错误对象或过宽字段进入生成的 `.d.ts`。它也会把内联回调抽成无注解常量，触发隐式 `any` 后再用显式 `any` 压掉诊断。配置代码中，模型可能把 `satisfies` 改成 `as Config`，把本应暴露的键名错误变成未经证明的信任声明。

泛型辅助函数是另一类高发位置。模型可能让一个 `T` 同时代表主值、回退值和转换结果，候选冲突后得到过宽联合；发现调用不通过时，又添加显式类型实参或双重断言。对于 `JSON.parse` 返回的 `any`，生成代码还会把无报错错误描述成“TypeScript 已经推断并验证结构”。

**让 AI 检查**

- 为修改前后的导出模块生成 `.d.ts`，逐项解释公开签名变化是否属于预期契约。
- 在 TypeScript 6 的 `--strict --noEmit` 下列出每个 `any` 的来源，区分显式 `any`、隐式 `any` 和第三方声明传播。
- 为每个泛型调用标出各类型参数从哪些实参收集候选，并说明约束、上下文返回类型与断言如何影响结果。

**审查清单**

- 检查局部推断与公开注解的边界，确保实现重构不会意外改变调用方类型。
- 检查空集合、抽取后的回调和无上下文对象字面量，确认没有隐式 `any` 或不必要拓宽。
- 检查 `as`、非空断言与 `JSON.parse` 路径，确认具体类型来自真实静态或运行时证据。

**提示词词汇**

使用 `type inference`（类型推断）、`contextual typing`（上下文类型推断）、`literal widening`（字面量拓宽）、`best common type`（最佳公共类型）、`generic type argument inference`（泛型类型实参推断）、`implicit any`（隐式 `any`）、`declaration emit`（声明输出）和 `inferred public API`（推断出的公开 API）。要求模型给出检查器计算的类型与生成的 `.d.ts`；“减少类型噪声”没有说明哪些边界必须稳定。

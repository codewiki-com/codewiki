...
Generators often write an interface and cast `JSON.parse()` straight to it. The code compiles, but it passes a static declaration off as runtime validation. Another common failure is an overbroad `isUser()` that checks only an object or one discriminant while promising that every nested field is safe. Models also reach for `filter(Boolean)`, truthiness, or `instanceof` as universal fixes, which can discard a valid `0` or reject a structurally correct boundary object.

**Ask your AI to check**

- List every field and domain constraint promised by each type predicate, then point to the corresponding runtime evidence in its body.
- Generate counterexamples for both outcomes of every guard, including `null`, arrays, missing fields, wrong primitives, and boundary numbers.
- Find every `as`, non-null assertion, `filter(Boolean)`, `in`, and `instanceof`, and explain what each proves and doesn't prove.

**Review checklist**

- External values enter as `unknown` and pass a real check before receiving a domain type.
- The predicate's false branch is sound, and mutable objects can't be changed through an alias after validation.
- Tests execute the guards as well as the type checker, covering every union member plus invalid discriminants.

**Prompt vocabulary**

Use the exact terms `type guard`, `control-flow narrowing`, `type predicate`, `assertion function`, `discriminated union`, `exhaustiveness check`, `runtime validation`, and `type assertion`. Asking for a table that maps runtime evidence to static narrowing exposes false proofs more readily than a vague request to “improve type safety.”

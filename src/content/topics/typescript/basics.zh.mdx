...
模型经常给每个局部变量重复添加显而易见的注解，却在真正的数据边界写下 `any` 或 `JSON.parse(raw) as DomainType`。
它还会把 `as` 当成转换，把非空断言当成空值处理，或者创建所有字段都可选的宽泛接口来消除报错。
另一个具体故障是修改可辨识联合时只添加新成员，不更新所有 `switch`，导致某些状态静默落入默认行为。

**让 AI 检查**

- 列出每个 `any`、类型断言和非空断言，并说明支持它的运行时证据以及失败路径。
- 从每个网络、文件、环境变量和 JSON 入口追踪数据，确认它以 `unknown` 进入并在获得领域类型前完成验证。
- 对每个联合类型枚举成员，并证明分支处理穷尽，同时测试 `null`、`undefined`、空数组和错误字段类型。

**审查清单**

- 在严格模式下运行类型检查，确认没有为了消除错误而扩大成 `any` 或全可选对象。
- 用不可信、缺字段和类型错误的输入执行边界代码，确认失败发生在首次使用之前。
- 修改一个联合成员或函数返回类型，确认编译器能指出所有需要同步更新的使用方。

**提示词词汇**

使用准确术语：`type annotation`（类型注解）、`type inference`（类型推断）、`literal widening`（字面量拓宽）、`union type`（联合类型）、`control-flow narrowing`（控制流收窄）、`discriminated union`（可辨识联合）、`type assertion`（类型断言）、`type erasure`（类型擦除）和 `runtime validation`（运行时验证）。
要求模型区分“编译器已经证明”“开发者作出断言”和“运行时已经验证”；只说“让它类型安全”无法暴露这三种保证之间的差别。

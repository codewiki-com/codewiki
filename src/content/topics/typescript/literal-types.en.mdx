...
Models often generate `{ method: "GET" }`, discover that the property widened, and add `as "GET"` or `as RequestConfig` at every call site.
That turns one declaration-site problem into scattered trust claims and makes union refactors easier to miss.
Another common error asserts the result of `JSON.parse` and then describes it as "validated."

Generated state models also widen the tag to `string` or make every payload field optional, breaking the association between tag and payload.
Sometimes a model creates the right union but leaves a permissive `default` in its consumer, silently returning an empty string after a state is added.
Maintaining a runtime options array separately from a handwritten union is another frequent source of drift.

**Ask your AI to check**

- List the inferred type of every object-literal field and identify fields widened from exact literals to primitive types.
- Find every assertion from `string`, `unknown`, `any`, or a `JSON.parse` result to a literal union, and state the corresponding runtime proof.
- Simulate adding one member to each discriminated union and confirm that every `switch` fails through a `never` exhaustiveness check.

**Review checklist**

- Confirm that allowed values have one source; when a runtime list is needed, derive the type from `as const` data.
- Distinguish the intent of annotations, `satisfies`, `as const`, and ordinary assertions, and remove assertions that merely silence errors.
- Test every external boundary with an unknown string, a missing tag, and an object whose tag conflicts with its payload.

**Prompt vocabulary**

Use `literal type`, `literal union`, `literal widening`, `contextual typing`, `const assertion`, `discriminant`, `discriminated union`, and `exhaustiveness checking with never`.
Ask the model to explain static assignability separately from runtime validation; "make the types stricter" doesn't say whether you need an annotation, preserved inference, or a boundary check.

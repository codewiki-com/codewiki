...
模型常把 `const config: Config = …` 机械改成 `const config = … satisfies Config`，却没有检查后续赋值；布尔值或判别字段变窄后，更新代码会编译失败。它们也会生成 `JSON.parse(raw) satisfies Config`，让 `any` 绕过检查；或者把有限注册表写成 `Record<string, Entry>`，使拼错的键看起来合法。另一类错误是无条件添加 `as const`，导致只读元组无法传给需要可变数组的 API。

**让 AI 检查**

- 列出每个 `satisfies` 左侧表达式的实际推断类型，并与目标类型逐属性对照。
- 找出来自 `any`、`unknown`、JSON、环境变量和类型断言的数据，说明哪一步执行了真实运行时验证。
- 检查目标中的键集合是否本应封闭，并跟踪声明后的每次赋值是否需要更宽类型。

**审查清单**

- 用 TypeScript 6 在严格模式下运行类型检查，同时保留用于证明非法情况的 `@ts-expect-error` 测试。
- 对注册表故意漏掉一个必需键、拼错一个键，并确认编译器都能拒绝。
- 给运行时解析器输入 `null`、错误判别值、缺失字段和错误字段类型，确认它们不会进入领域逻辑。

**提示词词汇**

使用 `satisfies operator`（`satisfies` 操作符）、`contextual typing`（上下文类型推断）、`assignability`（可赋值性）、`inferred expression type`（表达式推断类型）、`excess property check`（多余属性检查）、`finite key union`（有限键联合）、`runtime validation`（运行时验证）和 `const assertion`（`const` 断言）。要求模型分别写出“目标契约”和“结果类型”；只说“保持类型安全”不足以暴露 `any` 与开放索引签名。

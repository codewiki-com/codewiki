...
<TLDR>
  ...
  <TLDRCell label="trap">
    `T` 只存在于类型检查阶段；`as T`、`any` 或未经验证的 JSON 都能让泛型签名承诺运行时没有证明的结果。
  </TLDRCell>
  <TLDRCell label="fix">
...
你会在数组方法、Promise、集合、React 组件属性、API 客户端和库函数中遇到泛型。
标准库中的 `Array<T>`、`Map<TKey, TValue>` 与 `Promise<T>` 都用类型参数记录容器和所含值的关系。
业务代码中的分页响应、仓储接口和事件映射也常需要同一种表达能力。

泛型是一项静态机制。
它能让检查器拒绝不一致的调用，并让编辑器保留准确的补全信息，却不会自动检查网络响应或在 JavaScript 中创建类型对象。
如果运行时值来自 JSON、数据库驱动或无类型依赖，仍要先验证它。
...
名称应反映角色：很短、局部且关系明显时，`T` 与 `U` 足够；公开 API 涉及多个角色时，`TInput` 和 `TResult` 更容易读懂诊断。
...
公共 API 若把默认值写成 `any`，调用者在省略参数后会失去检查，因此 `unknown`、领域默认类型或不设默认值通常更诚实。

检查器大致沿着下面的路径处理泛型调用。
这张图描述静态阶段；最后生成的 JavaScript 不携带 `T`。
...
先让编译器推断，只有在错误信息或 API 边界确实需要时才固定类型实参。
...
类型参数会被擦除，所以 `parseJson<T>` 无法只凭 `T` 判断 JSON 是否有效。
...
> [!PITFALL]
> `<T extends { id: string }>` 只检查经过类型检查的调用；它不会检查来自 JSON 的对象，也不会在生成的 JavaScript 中留下守卫。

约束回答的是“函数体可以依赖哪些静态成员”。
它不回答“这个运行时值是否真的满足业务规则”。
即使结构正确，空字符串、格式错误或无权访问的 ID 仍可能不合法。
...
只有推断缺少信息、需要固定返回契约或调用空集合 API 时才显式传入类型实参。
用 `@ts-expect-error` 为必须拒绝的调用写负向类型测试。

### 忽略只读输入与 TSX 语法

> [!PITFALL]
> 只读取数组的工具若声明为 `T[]`，会拒绝只读数组并允许函数体误改数据；在 `.tsx` 中，单参数泛型箭头函数的 `<T>` 还可能被解析成 JSX。

这两个问题都常见于模型把普通函数机械改写成泛型箭头函数时。
类型关系也许正确，调用面和解析环境却变差。

**修复方法：** 只读输入写成 `readonly T[]` 或 `ReadonlyArray<T>`。
在 TSX 中把单个类型参数写成 `<T,>`，或使用 `function` 声明消除歧义。
...
模型很容易把“做成通用工具”理解为添加 `<T>`，然后用 `as T` 填平实现无法证明的地方。
常见结果包括返回 `Promise<T>` 的 JSON 客户端却不验证响应、用 `Record<string, any>` 扩大约束、给只出现一次的参数添加类型参数，以及在 `Object.keys()` 后把所有字符串强转成 `keyof T`。
另一类错误是把 `Map.get()`、数组查找或缓存读取的 `undefined` 用非空断言删掉，泛型签名因此比运行时行为更乐观。

**让 AI 检查**

- 列出每个类型参数出现的位置，说明它连接了什么关系；若只出现一次，提出具体类型或 `unknown` 的替代签名。
- 找出泛型实现中的 `any`、`as T`、双重断言和非空断言，并为每一处指出运行时证据或删除方案。
- 对每个 `T` 的创建与外部数据入口，确认代码传入了构造器、验证器或模式，并给出失败路径测试。

**审查清单**

- 用类型测试覆盖推断结果、必须拒绝的调用、空集合和显式类型实参，确认公共签名没有意外变宽。
- 在运行时测试错误 JSON、缺失键和查找失败，确认 `undefined` 与验证错误没有被断言隐藏。
- 检查只读输入、TSX 解析环境和类型擦除后的 JavaScript，确认类型层承诺有真实实现支撑。

**提示词词汇**

使用准确术语：`generic`（泛型）、`type parameter`（类型参数）、`type argument`（类型实参）、`generic constraint`（泛型约束）、`type inference`（类型推断）、`keyof constraint`（`keyof` 约束）、`indexed access type`（索引访问类型）、`type erasure`（类型擦除）、`runtime witness`（运行时见证）。
要求 AI 分开说明“检查器证明了什么”和“运行时验证了什么”；“让它类型安全”不足以暴露 `as T` 伪造的契约。

<Depth level="deep">

## 类型擦除与运行时见证

<Term id="type-erasure">类型擦除（type erasure）</Term>意味着类型参数、接口和类型别名不会成为 JavaScript 值。
...
`identity<string>` 与 `identity<number>` 最终调用的是同一份 JavaScript 逻辑。
...
空数组、无参数工厂和只出现在返回位置的类型参数往往缺少足够信息，这时才可能需要显式类型实参或更好的 API 形状。
...
这个所有权问题比一律使用单字母 `T` 更能决定 API 是否清楚。
...
- [TypeScript 手册：泛型](https://www.typescriptlang.org/docs/handbook/2/generics.html)
- [TypeScript 手册：`keyof` 类型运算符](https://www.typescriptlang.org/docs/handbook/2/keyof-types.html)
- [TypeScript 手册：索引访问类型](https://www.typescriptlang.org/docs/handbook/2/indexed-access-types.html)
- [TypeScript 手册：函数](https://www.typescriptlang.org/docs/handbook/2/functions.html)

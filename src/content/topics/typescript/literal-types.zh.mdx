...
> [!PITFALL]
> 把 `as const` 当成运行时冻结或输入验证。它不会调用 `Object.freeze`，也不能证明从网络或 JSON 得到的字符串属于某个联合。
...
模型经常先生成 `{ method: "GET" }`，发现属性被拓宽后，又在每个调用点添加 `as "GET"` 或 `as RequestConfig`。
这会把声明位置的问题变成散落的信任声明，重构联合时也更容易漏改。
另一个常见错误是对 `JSON.parse` 结果做断言，却把它描述成“已经验证”。

生成的状态模型也常把标签写成 `string`，或者把所有载荷字段改成可选，导致标签与载荷失去关联。
有时模型虽然创建了正确联合，却在消费者中留下宽泛 `default`，新增状态后静默返回空字符串。
运行时选项数组与手写联合分别维护，也是很常见的漂移来源。

**让 AI 检查**

- 列出每个对象字面量字段的推断类型，并指出哪些字段从精确字面量拓宽成了原始类型。
- 找出所有从 `string`、`unknown`、`any` 或 `JSON.parse` 结果到字面量联合的断言，给出对应的运行时证明。
- 为每个可辨识联合模拟新增一个成员，确认所有 `switch` 都会因 `never` 穷尽性检查而失败。

**审查清单**

- 确认允许值只有一份来源；需要运行时列表时，从 `as const` 数据派生类型。
- 区分注解、`satisfies`、`as const` 与普通类型断言的意图，删除只为压掉错误而存在的断言。
- 用未知字符串、缺失标签和标签与载荷不匹配的对象测试每个外部边界。

**提示词词汇**

使用 `literal type`（字面量类型）、`literal union`（字面量联合）、`literal widening`（字面量拓宽）、`contextual typing`（上下文类型推断）、`const assertion`（const 断言）、`discriminant`（判别字段）、`discriminated union`（可辨识联合）和 `exhaustiveness checking with never`（使用 `never` 的穷尽性检查）。
要求模型分别说明静态可赋值性与运行时验证；“让类型更严格”无法说明需要的是注解、推断保留还是边界检查。

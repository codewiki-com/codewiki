...
description: 通过 Cell 的值替换与 RefCell 的运行时借用，理解 Rust 内部可变性、借用冲突、重入回调和共享所有权的边界。
track: rust
...
<TryToBreak items={['让计数器从 u32::MAX 开始', '连续替换两次 String', '提前调用 into_inner() 移走 Cell']} />
...
> [!PITFALL]
> `Cell::get()` 要求 `T: Copy`，但这个约束不属于 `Cell<T>` 类型本身。生成代码常因此把 `Cell<String>` 改成 `RefCell<String>`，无端增加动态借用状态和 panic 路径。
...
> [!PITFALL]
> 迭代 `self.callbacks.borrow()` 的结果时，守卫通常覆盖整个循环。任何回调只要重新注册、删除回调或调用另一个借用同一 cell 的方法，就可能触发运行时 panic。
...
> [!PITFALL]
> 为了绕过一个编译期借用错误而包装 `Rc<RefCell<_>>`，会同时引入共享所有权、运行时 panic 和可能的强引用环。原本清楚的状态所有者也会变得难以辨认。
...
> [!PITFALL]
> AI 常把本地状态改成 `Rc<RefCell<T>>`，随后又把句柄捕获进要求 `Send` 的线程或多线程异步任务。继续叠加 `clone()` 不能补上 `Send` 或 `Sync`，也不能提供同步。
...
> [!PITFALL]
> 返回守卫能避免复制，却把内部动态借用状态暴露给调用方。调用方若把守卫保存在较长作用域中，后来一个看似无关的 `&self` 方法也可能 panic。
...
模型常用 `Rc<RefCell<_>>` 一次性压过所有权错误，却不说明谁拥有状态；随后它会在持有 `borrow_mut()` 时调用回调，让回调重入同一对象并 panic。它还会声称 `Cell<T>` 只能保存 `Copy` 类型，把本可整体替换的 `String` 升级成 `RefCell<String>`；或者把 `Rc<RefCell<_>>` 捕获进 `tokio::spawn`，再用更多 `clone()` 试图修复 `Send` 错误。另一种隐蔽失败是把 `try_borrow_mut().ok()` 的失败直接忽略，导致更新偶发丢失。

**让 AI 检查**

- 列出每个 `Cell`、`RefCell`、`Rc` 与 `Arc` 的所有者、线程边界，以及每个 `Ref`／`RefMut` 的最后使用点。
- 找出持有借用守卫时发生的函数调用、回调、格式化、迭代和错误转换，并检查它们是否会再次借用同一 `RefCell`。
- 对每个 `try_borrow*()` 说明冲突为何是允许的运行时状态，以及丢弃或重试失败是否会破坏业务不变量。

**审查清单**

- 用重入回调测试注册、删除和嵌套调用，并断言动态借用冲突不会变成未说明的 panic。
- 核对 `Cell<T>` 是否可以用整体替换完成任务，以及 `RefCell<T>` 返回的守卫是否泄漏到过长作用域。
- 验证共享状态的线程模型、`Send`／`Sync` 要求、`Rc` 强引用环和所有失败路径。

**提示词词汇**

使用准确术语：`interior mutability`（内部可变性）、`runtime borrow checking`（运行时借用检查）、`borrow guard`（借用守卫）、`shared borrow`（共享借用）、`exclusive borrow`（独占借用）、`reentrant callback`（重入回调）、`guard scope`（守卫作用域）、`strong reference cycle`（强引用环）、`thread boundary`（线程边界）。要求模型输出守卫时间线与所有权图；「修复 RefCell」不足以暴露冲突来自哪次调用。

<Depth level="deep">

## `UnsafeCell<T>` 与安全边界

<Term id="unsafe-cell">`UnsafeCell<T>`</Term> 是 Rust 语言认可的内部可变性底层原语。通过共享的 `&UnsafeCell<T>` 可以调用 `get()` 取得原始 `*mut T`，但解引用和写入仍属于 `unsafe` 操作。它只关闭编译器对「共享引用指向的数据不可变」的部分假设，不会自动证明引用有效、访问不重叠或没有数据竞争。
...
- [Rust 标准库：`Cell<T>`](https://doc.rust-lang.org/std/cell/struct.Cell.html)
- [Rust 标准库：`RefCell<T>`](https://doc.rust-lang.org/std/cell/struct.RefCell.html)
- [Rust 标准库：`UnsafeCell<T>`](https://doc.rust-lang.org/std/cell/struct.UnsafeCell.html)
- [《Rust 程序设计语言》：`RefCell<T>` 与内部可变性](https://doc.rust-lang.org/book/ch15-05-interior-mutability.html)

...
模型经常看到 `E0373` 就添加 `move`，却没有检查捕获变量是否仍是短生命周期引用；代码看似更拥有数据，返回或 `spawn` 时仍失败。
另一类补丁把整个上下文 `clone()` 进闭包，或者为了满足误写的 `Fn` 约束，把本可使用 `FnMut` 的局部状态改成 `Arc<Mutex<_>>`，无端改变并发和失败语义。

生成的 API 还会从同一个 `-> impl Fn` 函数的不同分支返回不同闭包类型，或把 `Box<dyn Fn()>` 送进线程后才发现缺少 `Send`。
模型也会根据 `move` 猜调用 trait，并错误地复用一个真正会移出捕获值的 `FnOnce` 闭包。
这些问题通常是所有权与调用次数没有先写成契约，而不是语法本身复杂。

**让 AI 检查**

- 列出每个闭包的精确捕获路径、捕获方式，以及闭包存在期间外部变量受到的限制。
- 根据闭包体证明它实现 `Fn`、`FnMut`、`FnOnce` 中的哪些 trait，并对照调用方实际调用次数。
- 检查每个返回或存储边界的生命周期、`Send`、`Sync` 与动态分发要求，指出每次克隆和装箱的原因。

**审查清单**

- 编译一个会重复调用回调的测试，再编译一个会消费捕获值的测试，确认 trait 约束既不太强也不太弱。
- 在线程任务或长生命周期回调中追踪每个引用的来源，确认 `move` 后保存的是真正拥有型值，而不是短借用。
- 检查异构回调集合是否有意使用函数指针、trait 对象或枚举，并验证所需的 `Send`、`Sync` 和生命周期边界。

**提示词词汇**

使用准确术语：`closure capture mode`（闭包捕获方式）、`capture path`（捕获路径）、`move closure`（`move` 闭包）、
`Fn` / `FnMut` / `FnOnce` call traits（闭包调用 trait）、`static dispatch`（静态分发）、`trait object`（trait 对象）、
`non-capturing closure coercion`（无捕获闭包强制转换）和 `object lifetime bound`（对象生命周期边界）。
要求 AI 给出捕获表和调用次数；只说「修复闭包生命周期」通常会换来没有解释的 `move`、`clone()` 或 `'static`。

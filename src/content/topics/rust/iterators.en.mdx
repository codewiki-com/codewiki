...
Models often treat `iter()`, `iter_mut()`, and `into_iter()` as stylistic alternatives, causing use-after-move errors
or cloning a whole collection just to compile. They also generate side effects inside a `map` with no consumer,
silently delete bad input with `filter_map(|value| value.parse().ok())`, and assume `zip` rejects unequal lengths.
Another recurring error returns an `impl Iterator` borrowing a local collection; the compiler rejects it, but the model may keep adding irrelevant `'static` annotations.

**Ask your AI to check**

- List the exact `Item` type at every pipeline stage and state whether each element is owned, `&T`, or `&mut T`.
- Mark the consumer that drives evaluation, every short-circuit point, and the closures that may never execute.
- Check whether parse errors, unequal `zip` inputs, and integer overflow follow the API contract explicitly.

**Review checklist**

- Use the source collection after iteration to confirm the chosen entry point did not move it or clone it excessively.
- Test empty input, a first-item failure, a middle failure, and unequal input lengths to verify termination behavior.
- Check whether laziness, filtering, short-circuiting, or repeated consumption changes the count and order of side effects.

**Prompt vocabulary**

Use the exact terms `Iterator trait`, `IntoIterator`,
`iterator adapter`, `consuming adapter`,
`lazy evaluation`, `short-circuiting`, `Item type`,
`borrowed iteration`, and `fallible collect`.
Ask for step-by-step ownership and `Item`-type tracing; “make the loop more Rust-like” is too vague to expose these failures.

...
Models often add `clone()` mechanically at an ownership error, turning one move into a full buffer copy on every loop iteration. They also widen read-only parameters from `&str` to `String`, forcing each caller to surrender ownership. Other concrete failures include logging a complete struct after partially moving one field, or writing `drop(&guard)` and assuming a lock has been released. Around callbacks and threads, generated code may clone shared handles without identifying which final path can destroy the resource.

**Ask your AI to check**

- Produce an owner timeline for every non-`Copy` value, marking each move, borrow, clone, and drop.
- Explain for every `.clone()` whether it clones data, a handle, or another resource, and why the copy must be independent.
- Find every early return, panic, `process::exit`, explicit `drop`, and reference cycle, then state whether cleanup remains reachable.

**Review checklist**

- Match `T`, `&T`, and `&mut T` parameters to whether each function takes, reads, or mutates the value.
- Test caller behavior after a move, mutation of two independent clones, and the expected destruction points.
- Inspect partial moves, lock guards, and long-lived shared handles to confirm that remaining values stay valid and resources can be released.

**Prompt vocabulary**

Use the exact terms `owner`, `move semantics`, `use after move`, `shared borrow`, `mutable borrow`, `Copy trait`, `explicit clone`, `partial move`, `drop order`, and `destructor reachability`. Ask the model for an owner timeline and clone inventory; “fix the borrow checker” is too vague to expose responsibility changes.

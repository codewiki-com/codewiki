---
title: 迭代器
description: 理解 Rust 迭代器的所有权语义、惰性适配器、短路消费、错误传播，以及自定义 Iterator 的正确契约。
track: rust
section: basics
...
Rust 的 `Iterator` trait 只有一个必须实现的方法 `next`，并通过关联类型 `Item` 声明产出类型。
...
如果需要并行处理，那是 Rayon 等其他抽象负责的事情，不是标准 `Iterator` 自动提供的能力。
...
阅读泛型 API 时，应看它要求 `Iterator` 还是 `IntoIterator`，以及实际的 `Item` 类型。
...
Rust 文档常把这类方法称为 consuming adapters，因为它们取得迭代器的所有权，
...
>
> **修复：** 长度相等属于业务契约时，在配对前比较可用的集合长度，或在 API 边界采用能表达同长约束的数据结构。只有「以较短侧为准」确实符合需求时才直接使用 `zip`。

## AI 时代

**生成代码在这里常犯的错**

模型经常把 `iter()`、`iter_mut()` 和 `into_iter()` 当成风格替换，导致移动后使用错误，
或为了通过编译而克隆整个集合。它还会生成没有消费者的 `map` 副作用，
用 `filter_map(|value| value.parse().ok())` 静默删除坏输入，并假定 `zip` 会拒绝长度不一致。
另一类常见问题是返回借用局部集合的 `impl Iterator`；编译器会拒绝它，但模型可能用不必要的 `'static` 标注继续兜圈。

**请 AI 检查**

- 列出管道每一步准确的 `Item` 类型，并说明元素是拥有型值、`&T` 还是 `&mut T`。
- 标出驱动求值的消费者、可能短路的位置，以及哪些闭包可能根本不会执行。
- 检查解析错误、`zip` 长度差与整数溢出是否按接口契约显式处理。

**审查清单**

- 在迭代后继续使用原集合，确认选择的入口没有意外移动或过度克隆。
- 用空输入、首项失败、中途失败和长度不等的输入测试终止行为。
- 检查所有副作用是否会因惰性、过滤、短路或重复消费而改变次数与顺序。

**提示词词汇**

使用精确词汇 `Iterator trait`（迭代器 trait）、`IntoIterator`（转为迭代器）、
`iterator adapter`（迭代器适配器）、`consuming adapter`（消费型适配器）、
`lazy evaluation`（惰性求值）、`short-circuiting`（短路）、`Item type`（元素类型）、
`borrowed iteration`（借用迭代）和 `fallible collect`（可失败收集）。
要求模型给出逐步的所有权与 `Item` 类型跟踪，比「把循环改得更 Rust」更能暴露问题。
...
闭包类型也由编译器生成，并不需要堆分配成 trait object。

函数只在内部使用管道时，通常让编译器推断完整类型最清楚。
API 需要返回管道时，`impl Iterator<Item = T> + '_` 可以隐藏具体适配器名字，
...
但会导致错误预分配、糟糕性能或违反更强 trait 的逻辑契约。
...
- [Rust 1.98 标准库：`Iterator`](https://doc.rust-lang.org/1.98.0/std/iter/trait.Iterator.html)
- [Rust 1.98 标准库：`IntoIterator`](https://doc.rust-lang.org/1.98.0/std/iter/trait.IntoIterator.html)
- [Rust 1.98 标准库：`FromIterator`](https://doc.rust-lang.org/1.98.0/std/iter/trait.FromIterator.html)

...
Models infer a module tree from directory appearance, declare a child at the wrong parent, or generate both `feature.rs` and `feature/mod.rs`. They also substitute `use` for a missing `mod`, use `crate::` from a binary to reach its library, or recursively add `pub` to eliminate privacy errors. Another generated-refactor failure is moving internal files while casually changing public export paths, creating needless downstream breakage.

**Ask your AI to check**

- Starting at every crate root, list the logical path and source file for every `mod` declaration, and report duplicate or unattached files.
- For every added `pub`, `pub(crate)`, `pub(super)`, and `pub use`, name the intended caller and justify the narrowest usable scope.
- Run `cargo check --all-targets`, then inspect the library API from paths visible outside the package instead of checking only internal unit tests.

**Review checklist**

- Identify which crate owns every library, binary, example, and integration-test target in the package, and where each path starts.
- Confirm that `use` only shortens a name and has not been mistaken for a module declaration or visibility grant.
- Compare public re-export paths before and after the refactor, and reject visibility widened solely to make a compiler error disappear.

**Prompt vocabulary**

Use the exact terms `Cargo package`, `crate`, `crate root`, `module tree`, `absolute path`, `restricted visibility`, `public re-export`, and `API facade`. Ask for three artifacts—a target inventory, module tree, and public-path list—instead of vaguely requesting a “cleaner file structure”; the concrete vocabulary exposes boundary mistakes.

...
Generated code often treats `HashMap` debug output as stable, or deduplicates through `HashSet` and then claims to preserve input order.
After a borrow-checker error, it may scatter `.clone()` calls and reallocate `String` keys that could be borrowed; another common pattern calls `contains_key` and then indexes the map, while using direct indexing for untrusted map keys or vector positions.
Models also place an expensive or side-effecting constructor inside `or_insert(...)` without accounting for eager argument evaluation.

**Ask your AI to check**

- List the semantics each collection must preserve: input order, key order, uniqueness, range lookup, and the end from which elements are removed.
- Trace ownership before and after every `.clone()`, and say whether `get`, `iter`, or a borrowed key lookup can replace it.
- Check both the present and absent path of every `entry` update, including whether default-value construction must be lazy.

**Review checklist**

- Test empty collections, duplicate keys, missing keys, and out-of-range indices; match every failure path to the function contract.
- Inspect external output, snapshots, and test assertions for dependence on incidental hash-collection iteration order.
- Compare parameter and return types to each borrow, move, and clone, confirming that every ownership conversion is intended.

**Prompt vocabulary**

Use exact terms in prompts: `iteration order`, `insertion order`, `key order`, `borrowed lookup`, `Entry API`, `occupied entry`, `vacant entry`, `capacity`, and `reallocation`.
Don't ask for "the fastest collection" without stating whether you need stable output, range queries, uniqueness, or borrowed versus owned elements.

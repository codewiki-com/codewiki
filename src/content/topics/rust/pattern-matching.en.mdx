...
Models often append `_ => unreachable!()` to an enum match because it makes today's code compile, but it turns a future variant into a production panic. They also put an outer variable's bare name directly in a pattern, mistaking a new binding for a comparison, and clone an entire structure after a field-move error without examining the ownership intent.

Models also mishandle guards and convenience forms. They may treat a guarded `Some(x)` as coverage for every `Some`, or rewrite an error that must be handled as `if let Ok(x)` with no `else`. Generated parsers also use `[first, ..]` to accept extra input even when the protocol requires an exact argument count.

**Ask your AI to check**

- List the value set covered by every `match` arm, including gaps or masking caused by `_`, variable bindings, ranges, and guards.
- Annotate the inferred type and binding mode of every pattern binding, then state whether each field is copied, borrowed, mutably borrowed, or moved.
- Write the failure path for every `if let`, `while let`, and `let-else`, confirming that no error is silently discarded.

**Review checklist**

- Add a temporary enum variant and confirm matches meant to be exhaustive fail to compile instead of reaching `unreachable!()`.
- Test range endpoints, a false guard, an empty slice, extra elements, and parse failures, then verify arm order.
- Use the original value after matching to expose accidental field moves and clones that only hide ownership mistakes.

**Prompt vocabulary**

Use the exact terms `scrutinee`, `match arm`, `exhaustiveness`, `refutable pattern`, `binding mode`, `match ergonomics`, `match guard`, `or-pattern`, `at binding`, and `partial move`. Ask for a pattern-coverage table and a binding-ownership table; "make this more Rust-like" is too vague to expose these failures.

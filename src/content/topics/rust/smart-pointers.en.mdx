...
Models tend to treat `Arc<Mutex<_>>` as a universal compilation fix and then carry the lock guard across a callback or `.await`. They also generate `Arc<RefCell<T>>` and claim the inner value is now thread-safe, or store strong handles on both ends of parent-child and subscription relationships. Custom wrappers often get an indiscriminate `Deref`, a direct `.drop()` call, logging of sensitive inner values, or `Box::leak` used to fake a `'static` lifetime.

**Ask your AI to check**

- State the specific requirement solved by every `Box`, `Rc`, `Arc`, `Weak`, and mutability wrapper, and mark all thread boundaries.
- Draw every strong and weak edge, then prove that each destructor remains reachable after external root handles disappear.
- List every implicit dereference, explicit `drop()`, `Box::leak`, and guard lifetime that crosses a callback or `.await`.

**Review checklist**

- Remove wrappers with no independent semantics, then recheck the `Send`, `Sync`, and mutable-access requirements.
- Expire weak handles and destroy the final strong handle; observe both upgrade results and resource cleanup.
- Check that `Drop` performs only infallible fallback cleanup and that sensitive values aren't logged, cloned, or leaked.

**Prompt vocabulary**

Use the exact terms `owning pointer`, `shared ownership`, `strong reference cycle`, `weak back edge`, `deref coercion`, `drop glue`, `guard lifetime`, and `interior mutability`. Ask for an ownership graph and a proof of destructor reachability; "fix the borrow checker error" is too vague to constrain wrapper choice.

---
title: 智能指针
description: Rust 智能指针把间接访问与所有权、共享和析构规则组合起来；理解 Deref、Drop 与包装类型的真实边界。
track: rust
...
<TryToBreak items={['删除一个 Box 层', '把空计划加入枚举', '在计算后移动 release']} />
...
> [!PITFALL]
> 给 `UserId`、`ValidatedPath` 或 `Secret` 实现 `Deref<Target = String>`，会让字符串的全部方法隐式成为包装器 API。调用方可能绕过领域操作，代码也更难看出何时发生自动解引用。
...
模型倾向于把 `Arc<Mutex<_>>` 当作通用编译修复，然后让锁守卫跨过回调或 `.await`。它也会生成 `Arc<RefCell<T>>` 并声称内部值已经线程安全，或者在父子和订阅关系两端都保存强句柄。自定义包装器中常见的另一类错误是随意实现 `Deref`、直接调用 `.drop()`、记录敏感内部值，或用 `Box::leak` 伪造 `'static` 生命周期。

**让 AI 检查**

- 为每个 `Box`、`Rc`、`Arc`、`Weak` 和可变性包装层写出它解决的具体需求，并标出线程边界。
- 画出所有强边与弱边，证明外部根句柄消失后每个析构函数仍可达。
- 列出所有隐式解引用、显式 `drop()`、`Box::leak` 和跨回调或 `.await` 的守卫生命周期。

**审查清单**

- 删除没有独立语义的包装层，并重新检查 `Send`、`Sync` 与可变访问条件。
- 让弱句柄过期、让最后一个强句柄销毁，再观察升级结果和资源清理。
- 检查 `Drop` 是否只做不可失败的兜底清理，敏感值是否会被日志、克隆或泄漏。

**提示词词汇**

使用准确术语：`owning pointer`（拥有型指针）、`shared ownership`（共享所有权）、`strong reference cycle`（强引用环）、`weak back edge`（弱反向边）、`deref coercion`（解引用强制转换）、`drop glue`（析构胶水）、`guard lifetime`（守卫生命周期）、`interior mutability`（内部可变性）。要求模型给出所有权图和析构可达性证明；「修好 borrow checker 错误」不足以约束包装器选择。

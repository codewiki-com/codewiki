...
模型常先在栈上构造带内部裸指针的 `Self`，再调用 `Box::pin`，导致指针从一开始就悬空。它也会为了满足 `Unpin` 约束而手写 `impl Unpin`，或用 `get_unchecked_mut()` 返回固定字段的普通可变引用，使后续 `mem::replace` 可以移动该字段。异步代码中，另一类错误是给每个 future 无条件 `Box::pin`，或者从旧示例复制尚不能在稳定版使用的负实现 `impl !Unpin`，既增加不必要的分配，又可能直接编译失败。

**让 AI 检查**

- 为每个自引用值给出「放入最终位置、创建 `Pin`、写入内部地址、开始使用」的严格时间线，并确认地址没有在中间变化。
- 列出所有 `get_unchecked_mut`、`map_unchecked_mut`、`new_unchecked` 与手写 `Unpin`，为每处说明调用方必须维持的具体不变量。
- 对每个 future 说明它是否为 `Unpin`、谁在第一次 `poll` 前固定它，以及能否用局部 `pin!` 代替堆分配。

**审查清单**

- 确认移动的每个对象究竟是指针句柄、整个目标，还是结构体中的某个字段。
- 测试构造失败、析构和替换路径，确保内部地址只在固定后建立，并一直有效到析构完成。
- 拒绝只为消除编译错误而添加的 `unsafe`、`Unpin` 实现或 `Box::pin`，要求它们对应明确的地址不变量或存储需求。

**提示词词汇**

使用准确术语：`pinned pointee`（固定目标）、`pointer handle`（指针句柄）、`address-sensitive value`（地址敏感值）、`pinning invariant`（固定不变量）、`structural pinning`（结构性固定）、`pin projection`（固定投影）、`drop guarantee`（析构保证）、`safe pinning constructor`（安全固定构造器）和 `Unpin auto trait`（`Unpin` 自动 trait）。要求模型输出固定状态时间线和逐字段投影表；「让它通过借用检查」不足以审查 `unsafe` 契约。

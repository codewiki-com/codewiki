...
Models often wrap every value in `Arc<Mutex<_>>` to silence ownership errors, then carry a mutex guard across a callback or `.await`. They also swap `Rc` for `Arc` while missing that an inner `RefCell` remains non-`Sync`, store strong back edges in trees and subscriber registries, or assume uniqueness remains true after checking `Arc::strong_count() == 1`. Another concrete error is treating `Box<T>` as `Pin<Box<T>>` and generating internal self-pointers that dangle after a move.

**Ask your AI to check**

- List the ownership direction of every `Rc`, `Arc`, and `Weak` edge, and mark every possible strong-reference cycle.
- Prove `T: Send + Sync` for each `Arc<T>`, and trace whether any lock guard crosses a callback, blocking operation, or `.await`.
- Find every `Weak::upgrade`, `get_mut`, `make_mut`, and reference-count check, then explain what happens if the target changes after the check.

**Review checklist**

- Drop external root owners and verify that weak back edges expire and destruction remains reachable.
- Recheck every wrapper against the thread boundary and mutation need, removing any `Arc`, `Mutex`, or `Box` with no semantic role.
- Run concurrency and re-entrancy tests around shared state, confirming that no unknown code executes inside a critical section.

**Prompt vocabulary**

Use the exact terms `single ownership`, `shared ownership`, `strong count`, `weak count`, `non-owning back edge`, `atomic reference counting`, `interior mutability`, `lock guard lifetime`, and `strong reference cycle`. Ask the model for an ownership graph with thread boundaries; “fix the borrow checker error” is too vague to expose unnecessary wrappers and incorrect strong edges.

...
Models often add one `'a` to every reference after a compiler error, accidentally making auxiliary parameters restrict a return value. They also conflate `'static`, `move`, and ownership. At stored-callback or thread boundaries, a model may suggest `Box::leak()`, broad `clone()` calls, or a self-referential struct built from raw pointers and unproven `unsafe`; more subtly, it may bind a callback to an outer lifetime where `for<'a>` is needed, leaving the API usable for only one borrow.

**Ask your AI to check**

- List the unique data source of every returned reference and say whether each input genuinely restricts the result.
- Draw the order of reference creation, last use, conflicting access, and owner destruction, then mark the smallest region NLL can infer.
- For every `'static` or `for<'a>`, say who chooses the quantified lifetime and check whether a `move` capture still contains a short-lived reference.

**Review checklist**

- Compile a call that destroys an auxiliary parameter before using the result to test that the signature is not over-constrained.
- Check that threads, tasks, and stored callbacks move genuinely owned data instead of moving short borrows or leaking memory.
- Trace sources through borrowed fields, trait objects, and closures, and confirm that no `unsafe` code forges a lifetime.

**Prompt vocabulary**

Use the exact terms `lifetime`, `borrow region`, `non-lexical lifetimes`, `lifetime elision`, `outlives bound`, `reborrow`, `'static reference`, `'static type bound`, and `higher-ranked trait bound`. Ask for a data-origin table and region constraints; “fix the borrow-checker error” does not reveal whether the model is widening the API or hiding an ownership problem.

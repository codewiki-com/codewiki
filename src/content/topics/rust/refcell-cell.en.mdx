...
> [!PITFALL]
> AI often changes local state into `Rc<RefCell<T>>` and later captures the handle in a thread or multithreaded async task that requires `Send`. More calls to `clone()` cannot add `Send` or `Sync`, and they provide no synchronization.
...
Models often use `Rc<RefCell<_>>` to suppress ownership errors without saying who owns the state. They then hold `borrow_mut()` while calling a callback, letting reentry panic on the same object. They also claim that `Cell<T>` can store only `Copy` types, promote replaceable `String` state to `RefCell<String>`, or capture `Rc<RefCell<_>>` in `tokio::spawn` and add more `clone()` calls to chase the `Send` error. A quieter failure turns `try_borrow_mut()` into `.ok()` and ignores the error, intermittently losing updates.

**Ask your AI to check**

- List the owner and thread boundary for every `Cell`, `RefCell`, `Rc`, and `Arc`, and mark the last use of every `Ref` or `RefMut`.
- Find function calls, callbacks, formatting, iteration, and error conversion performed while a guard is live, then check whether they can borrow the same `RefCell` again.
- For every `try_borrow*()`, explain why conflict is an allowed runtime state and whether dropping or retrying the failure can break a domain invariant.

**Review checklist**

- Test registration, removal, and nested calls with a reentrant callback, and assert that dynamic conflicts cannot become undocumented panics.
- Check whether `Cell<T>` can express the operation as whole-value replacement and whether any `RefCell<T>` guard escapes into an unnecessarily wide scope.
- Verify the thread model, `Send`/`Sync` requirements, strong `Rc` cycles, and every failure path for shared state.

**Prompt vocabulary**

Use the exact terms `interior mutability`, `runtime borrow checking`, `borrow guard`, `shared borrow`, `exclusive borrow`, `reentrant callback`, `guard scope`, `strong reference cycle`, and `thread boundary`. Ask for a guard timeline and ownership graph; "fix the RefCell" is too vague to expose which call creates the conflict.

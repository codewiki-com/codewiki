...
生成代码经常把 `HashMap` 的调试输出当成稳定结果，或者用 `HashSet` 去重后声称保留了输入顺序。
它也容易在借用检查器报错后到处添加 `.clone()`，把本可借用的 `String` 键重新分配；另一种常见写法是 `contains_key` 后再索引映射，并用直接索引处理不可信键或向量位置。
对于默认值，模型还会把昂贵或有副作用的构造器放进 `or_insert(...)`，没有意识到实参会提前求值。

**让 AI 检查**

- 列出每个集合必须保留的语义：输入顺序、键顺序、唯一性、范围查询和从哪一端取出元素。
- 标出每次 `.clone()` 前后的所有权路径，并说明能否改为 `get`、`iter` 或借用形式的键查找。
- 为每次 `entry` 更新检查键存在与缺失两条路径，并确认默认值构造是否必须延迟。

**审查清单**

- 用空集合、重复键、缺失键和越界索引运行测试，确认每条失败路径都符合函数契约。
- 检查任何对外输出、快照和测试断言是否依赖哈希集合的偶然迭代顺序。
- 对照参数与返回类型，确认代码何处借用、何处移动，以及克隆是否真的是所需的所有权转换。

**提示词词汇**

提示中应使用准确术语：`iteration order`（迭代顺序）、`insertion order`（插入顺序）、`key order`（键顺序）、`borrowed lookup`（借用查找）、`Entry API`（条目 API）、`occupied entry`（已占用条目）、`vacant entry`（空缺条目）、`capacity`（容量）和 `reallocation`（重新分配）。
不要只说“选最快的集合”；应说明是否需要稳定输出、范围查询、唯一性，以及函数要借用还是取得元素所有权。

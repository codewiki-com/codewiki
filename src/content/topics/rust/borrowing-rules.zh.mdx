...
Rust 区分<Term id="shared-reference">共享引用（shared reference）</Term> `&T` 与<Term id="mutable-reference">可变引用（mutable reference）</Term> `&mut T`。
...
<Term id="borrow-checker">借用检查器（borrow checker）</Term>在编译时拒绝这些重叠关系，安全 Rust 因而不会把检查推迟到偶然触发的运行时路径。
...
模型常生成“先从集合借用元素，再向集合 `push`，最后继续使用元素引用”的流程；
它也会尝试同时用两个运行时索引取得可变元素，或让异步闭包跨 `.await` 持有 `&mut`，导致后续访问长期被锁住。
另一类失败是用 `.clone()`、`Rc<RefCell<_>>` 或 `Arc<Mutex<_>>` 消音，而没有说明复制成本、运行时借用 panic、锁范围或跨线程约束。

**让 AI 检查**

- 列出每个引用的来源、最后一次使用，以及该区间内所有可能冲突的读取、写入和集合扩容。
- 对每个 `.clone()`、`RefCell` 和锁说明它解决的是所有权、内部可变性还是并发问题，并给出不使用它时的接口方案。
- 检查每个返回引用和跨 `.await` 引用来自哪个所有者，证明所有者在完整使用区间内有效。

**审查清单**

- 先测试空集合、相同索引、不同索引与会触发扩容的输入，再判断修复是否只覆盖了顺利路径。
- 查看函数签名是否准确区分 `&T`、`&mut T` 与 `T`，并确认调用方在调用后是否仍需要原值。
- 沿控制流标记借用的最后使用点；
对切片拆分、重新借用和锁守卫分别检查范围是否最小且可解释。

**提示词词汇**

使用准确术语：`shared reference`（共享引用）、`mutable reference`（可变引用）、`exclusive access`（独占访问）、`aliasing`（别名）、`reborrow`（重新借用）、`non-lexical lifetimes`（非词法生命周期）、`last use`（最后使用点）、`disjoint mutable borrows`（不重叠可变借用）。
要求 AI 给出“借用区间与冲突访问表”，比笼统地要求“修复 borrow checker”更容易暴露错误的所有权假设。

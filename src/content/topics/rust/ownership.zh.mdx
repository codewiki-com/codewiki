...
模型经常在所有权报错处机械添加 `clone()`，把一次移动改成循环中的整份缓冲区复制；也会把只读参数从 `&str` 扩大成 `String`，迫使每个调用方交出所有权。另一类错误是部分移动结构体字段后继续记录整个结构体，或写出 `drop(&guard)` 并误以为锁已经释放。涉及回调和线程时，生成代码还可能克隆共享句柄，却没有说明最终哪个路径能触发资源析构。

**让 AI 检查这些事项**

- 为每个非 `Copy` 值列出所有者时间线，标明每次移动、借用、克隆与丢弃。
- 逐个解释 `.clone()` 克隆的是数据、句柄还是其他资源，并说明副本为何必须独立。
- 找出所有提前返回、panic、`process::exit`、显式 `drop` 与引用环，说明清理是否可达。

**审查清单**

- 根据函数是否取得、读取或修改值，核对参数分别使用 `T`、`&T` 或 `&mut T`。
- 让测试覆盖移动后的调用方行为、两个独立克隆的修改，以及期望发生的析构时点。
- 检查部分移动、锁守卫和长期共享句柄，确认剩余值始终有效且资源能够释放。

**提示词词汇**

使用准确术语 `owner`（所有者）、`move semantics`（移动语义）、`use after move`（移动后使用）、`shared borrow`（共享借用）、`mutable borrow`（可变借用）、`Copy trait`、`explicit clone`（显式克隆）、`partial move`（部分移动）、`drop order`（丢弃顺序）与 `destructor reachability`（析构可达性）。要求模型输出所有者时间线与克隆清单；只说“修复 borrow checker”不足以暴露责任变化。

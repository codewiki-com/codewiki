...
模型常在「简化」时把 `apply` 换成 `run`，却没有重算链上下一步接收的类型；也会把 `value?.let` 改成 `value.let`，让原本跳过的空值进入块中。
另一类错误出现在生成的 Android 配置或 DSL 中：多层隐式 `this` 把属性写到最近接收者，代码仍可能通过编译。
它还会把发送消息、写数据库等不可重复操作塞进 `also`，并忽略调用方重试和异常传播。

**让 AI 检查**

- 给每个作用域函数列出上下文对象、块内引用方式、lambda 结果类型与整个调用类型。
- 标出每个 `?.` 的短路范围、每个 `return` 的目标，以及所有嵌套 `this` 或 `it` 的静态类型。
- 列出 `also`、`apply` 块中的外部副作用，并说明异常、重试和重复调用时的行为。

**审查清单**

- 用空接收者、非空但转换为空的接收者分别测试可空链。
- 展开嵌套接收者的标签和参数名，确认每个读取与赋值的对象。
- 沿链记录每一步的静态类型，并对不可重复副作用做失败注入。

**提示词词汇**

使用准确术语：`scope function`（作用域函数）、`context object`（上下文对象）、`lambda receiver`（lambda 接收者）、`lambda argument`（lambda 参数）、`lambda result`（lambda 结果）、`context-object return`（返回上下文对象）、`safe-call short-circuit`（安全调用短路）、`implicit receiver`（隐式接收者）、`labeled this`（带标签的 `this`）、`non-local return`（非局部返回）。
要求模型输出「接收者－结果类型表」和逐步求值路径，比「改成更地道的 Kotlin」更容易发现语义变化。

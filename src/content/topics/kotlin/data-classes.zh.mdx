...
> [!PITFALL]
> `Array` 的 `equals()` 使用引用语义，所以含 `Array` 属性的两个数据类实例不会仅因数组元素相同而自动相等。模型经常把 `ByteArray` 放进数据类，然后生成一个看似合理但失败的值相等测试。
...
模型常把普通类机械改成数据类，却不先定义相等性边界：瞬时 UI 状态被留在类体，或数据库实体的可变字段进入哈希计算。它们也会把 `copy()` 描述成深拷贝，在状态更新器里复用 `MutableList`，导致旧状态随新状态一起变化。

另一类错误来自“看起来像值”的字段。生成的二进制载荷模型常使用 `ByteArray` 并假定数据类会按元素比较；认证模型则把令牌放进主构造函数，又在调试日志中直接插值整个对象。两种代码都能编译，但测试和安全边界不符合意图。

**让 AI 检查**

- 列出主构造函数和类体中的每个属性，并说明它是否应参与相等性、哈希、显示、解构和复制。
- 沿每次 `copy()` 跟踪引用，标出仍与原实例共享的可变集合、数组和嵌套对象。
- 找出所有作为哈希键、使用数组属性或可能被完整写入日志的数据类，并为每处生成一个失败优先的测试。

**审查清单**

- 用两个独立构造的实例测试 `==`，并在修改允许变化的字段后复测哈希集合查找。
- 复制实例后修改每个嵌套可变对象，确认共享或隔离行为符合所有权约定。
- 检查 `toString()` 和日志调用，确保构造函数中的敏感字段不会以原值出现。

**提示词词汇**

使用准确术语：`data class`（数据类）、`primary-constructor property`（主构造函数属性）、`structural equality`（结构相等性）、`referential equality`（引用相等性）、`hash-code contract`（哈希码契约）、`shallow copy`（浅拷贝）、`componentN order`（组件顺序）和 `mutable key`（可变键）。要求 AI 给出“属性参与矩阵”和“复制后的引用别名图”，比笼统地要求“检查这个 DTO”更容易暴露语义错误。

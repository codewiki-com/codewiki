...
Models often silence a non-exhaustive compilation error by adding `else` instead of finding the missing variant's business meaning. They also generate direct subclasses into different architecture packages or modules, or make a direct subclass `open` while claiming every concrete descendant is known at compile time. For UI state, another recurring error is a singleton with mutable progress, request IDs, or failure details, causing concurrent operations to share state.

**Ask your AI to check**

- List every direct subtype of each sealed type, its package and module, and whether it is `final`, `sealed`, or `open`.
- Find every `when` whose subject has that sealed type and explain which existing or future variant each `else` can conceal.
- For each `object` or `data object` variant, check for occurrence-specific data and trace the sharing scope of every mutable property.

**Review checklist**

- Add a temporary variant and recompile, confirming that handlers needing a business decision actually fail.
- Verify that direct subtypes share a package and module, and that any open indirect extension is deliberate.
- Test state-command combinations, generic payload-free branches, and concurrent state ownership separately.

**Prompt vocabulary**

Use the exact terms `sealed class`, `sealed interface`, `direct subclass`, `indirect subclass`, `exhaustive when`, `smart cast`, `final leaf`, `open extension point`, and `bottom type`. Ask for a hierarchy-boundary table and a coverage matrix for every `when`; “make state handling robust” is too vague to expose branches hidden by `else`.

---
title: Kotlin 基础
description: 从只读引用、静态类型与空安全出发，掌握 Kotlin 的表达式、控制流和函数调用边界。
track: kotlin
section: basics
difficulty: beginner
...
<TLDR>
  <TLDRCell label="what">
    Kotlin 是静态类型语言。它用 `val` 与 `var` 区分引用能否重新赋值，并把可空性写进类型。
  </TLDRCell>
  <TLDRCell label="trap">
...
Kotlin 的基础语法围绕一个简单目标展开：让程序的约束直接出现在代码里。名称能否重新赋值由 `val` 或 `var` 表达，值能否为 `null` 由类型末尾的 `?` 表达，分支可以直接产生结果。编译器因此能在程序运行前拒绝一批不成立的状态。

Kotlin 是静态类型语言。每个表达式都有编译期类型，但局部变量通常不必重复写出类型，因为编译器会做<Term id="type-inference">类型推断（type inference）</Term>。推断不会把语言变成动态类型：`val count = 3` 一旦推断为 `Int`，之后就不能把字符串赋给 `count`。
...
`String` 不接受 `null`，而<Term id="nullable-type">可空类型（nullable type）</Term> `String?` 接受。可空接收者不能直接访问普通成员；代码必须先证明值非空，使用安全调用 `?.`，或者用 Elvis 操作符 `?:` 提供另一条路径。
...
> [!PITFALL]
> Java 代码中的 `long total = count` 可以自动扩宽，但 Kotlin 的 `val total: Long = count` 不会编译。直接改成 `Int` 又可能缩小可表示范围。
...
> [!PITFALL]
> `==` 检查结构相等并安全处理 `null`，`===` 检查两个引用是否指向同一对象。字符串驻留或 JVM 装箱缓存可能让错误的 `===` 在部分测试数据上碰巧通过。
...
> [!PITFALL]
> 模型常为枚举或密封层级的 `when` 自动补上 `else`。代码暂时更短，但新增状态后会继续编译，并可能走一个从未为新状态设计的默认分支。
...
> [!PITFALL]
> `if`、`when`、Elvis 与作用域函数都能嵌套成一个表达式。生成代码有时把日志、状态修改、提前返回和结果计算塞在一起，使执行顺序难以审查。
...
模型会把 Java 习惯带进 Kotlin，例如写 `new`、三元运算符或依赖数字自动扩宽。它们也常把所有局部量声明成 `var`，把可空值后面统一接 `!!`，或者误以为 `val MutableList` 不能修改。为快速消除 `when` 的编译错误而添加 `else`，则会破坏枚举与密封类型原本提供的新增状态检查。

另一类错误来自表面上“更地道”的压缩：生成代码把多个安全调用、Elvis、作用域函数和非局部返回连成一行。代码可能通过编译，却把“缺失”“格式错误”和“权限不足”合并为同一个默认值。审查时要追踪每条失败路径，而不是只确认没有出现 `NullPointerException`。

**让 AI 检查**

- 列出每个 `var` 的全部赋值点，并说明它为什么不能改成 `val`。
- 标出每个可空值从何处进入、在哪里被收窄，以及每个 `!!` 对应的已验证不变量。
- 对每个枚举或密封类型的 `when` 删除 `else` 后重新编译，确认是否遗漏具体状态。

**审查清单**

- 使用边界值测试范围、整数除法、数值转换和可能溢出的乘法。
- 分别测试 `null`、空字符串、格式错误和值合法这几条输入路径。
- 新增一个枚举成员并编译，确认所有需要响应新状态的位置都会失败。

**提示词词汇**

使用准确术语：`read-only reference`（只读引用）、`mutable object`（可变对象）、`type inference`（类型推断）、`nullable type`（可空类型）、`smart cast`（智能转换）、`safe call`（安全调用）、`Elvis operator`（Elvis 操作符）、`exhaustive when`（穷尽 `when`）、`structural equality`（结构相等）和 `referential equality`（引用相等）。要求 AI “逐条说明失败路径和静态类型”，比“让代码更 Kotlin 化”更容易得到可核对的结果。
...
- [Kotlin 基础语法概览（官方文档源码）](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/basic-syntax.md)
- [Kotlin 数字类型（官方文档源码）](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/numbers.md)
- [Kotlin 空安全（官方文档源码）](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/null-safety.md)
- [Kotlin 条件与循环（官方文档源码）](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/control-flow.md)
- [Kotlin 函数（官方文档源码）](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/functions.md)

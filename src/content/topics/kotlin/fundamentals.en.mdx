...
> [!PITFALL]
> Models often add `else` automatically to a `when` over an enum or sealed hierarchy. The code gets shorter, but it keeps compiling after a new state appears and may send that state down a fallback that was never designed for it.
...
Models carry Java habits into Kotlin, writing `new`, a ternary operator, or code that depends on numeric widening. They also declare every local as `var`, append `!!` to each nullable value, or assume a `val MutableList` can't change. Adding `else` just to silence an incomplete `when` is another common shortcut, and it removes the new-state check that enums and sealed types were meant to provide.

Another failure comes from compression that looks superficially idiomatic. Generated code chains safe calls, Elvis operators, scope functions, and non-local returns into one line. It may compile while merging “absent,” “bad format,” and “not authorized” into the same fallback. Review every failure path instead of merely confirming that no `NullPointerException` appears.

**Ask your AI to check**

- List every assignment to each `var` and explain why the binding can't be a `val`.
- Mark where every nullable value enters, where it is narrowed, and the validated invariant behind each `!!`.
- Remove `else` from every `when` over an enum or sealed type, recompile, and identify any missing concrete state.

**Review checklist**

- Test ranges, integer division, numeric conversions, and potentially overflowing multiplication at their boundaries.
- Exercise `null`, empty text, malformed input, and valid input as separate paths.
- Add an enum member and compile to confirm that every place requiring new-state behavior fails.

**Prompt vocabulary**

Use the exact terms `read-only reference`, `mutable object`, `type inference`, `nullable type`, `smart cast`, `safe call`, `Elvis operator`, `exhaustive when`, `structural equality`, and `referential equality`. Asking an AI to “state every failure path and static type” produces something you can check; “make it more Kotlin-like” doesn't.

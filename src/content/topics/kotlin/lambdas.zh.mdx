---
title: Lambda 表达式
description: Lambda 表达式把行为表示为有静态类型的值；这里讲清函数类型、捕获、接收者、返回控制与 JVM 边界。
track: kotlin
...
Lambda 表达式（lambda expression）是用花括号写出的函数字面量。它没有声明名称，却能生成一个可以调用的值。Kotlin 把函数作为<Term id="first-class-function">一等函数（first-class function）</Term>处理，因此这个值可以存入属性和集合、作为实参传递，也可以由另一个函数返回。
...
Lambda 可以访问定义位置可见的局部变量。函数值与这些外部绑定共同形成<Term id="closure">闭包（closure）</Term>。Kotlin 允许 lambda 读取并修改捕获的 `var`，所以同一作用域创建的多个 lambda 可能共享一份不断变化的状态。
...
<Term id="lambda-with-receiver">带接收者的 lambda（lambda with receiver）</Term>把一个对象作为隐式 `this`，适合表达层级构建或在受限对象上执行一组操作。它仍是普通函数值，不会增加解析器，也不会自动限制嵌套接收者。复杂 DSL 的作用域控制属于 `@DslMarker` 与专门的 DSL 设计。

函数式接口（functional interface）使用 `fun interface` 声明，并且只有一个抽象方法。<Term id="sam-conversion">SAM 转换（SAM conversion）</Term>可以把签名匹配的 lambda 转成这种名义接口的实例；Kotlin/JVM 也支持 Java 单抽象方法接口。接口可以拥有名称、非抽象成员和独立扩展，普通函数类型则只表达输入与结果。
...
`selectIds` 决定何时以及对哪些元素调用谓词；lambda 只提供判断规则。第一处调用把函数值放在圆括号内，第二处把最后一个 lambda 移到括号外，两者传递的是同一种 `(Order) -> Boolean` 契约。
...
裸 `return` 能穿过 `forEach`，是因为标准库在调用点内联了这个 lambda。把同样的 lambda 传给普通非内联函数会编译失败；编译器不能让稍后保存或间接调用的回调返回一个已经不存在的调用帧。
...
> [!PITFALL]
> 把 lambda 中的裸 `return` 一律理解为“只返回 lambda”，会导致提前退出外层函数；反过来，把它放进普通回调又可能直接编译失败。
...
> [!PITFALL]
> 嵌套 lambda 都使用 `it` 时，内层参数会遮蔽外层参数，生成代码很容易对错对象读取属性或执行更新。
...
> [!PITFALL]
> 多个函数形参都带默认值时，尾随 lambda 绑定最后一个形参，可能让本来表示成功处理的代码进入失败处理位置。

**修复方法：** 对角色不同的回调写具名实参，例如 `onSuccess = { ... }` 与 `onFailure = { ... }`。设计新 API 时，避免让多个同形状回调仅靠位置区分；名义接口或事件对象可以让含义进入类型和名称。

> [!PITFALL]
> 把函数类型、`typealias` 与 `fun interface` 当成同一个类型，会破坏重载、扩展或 Java 互操作边界。
...
模型常把传给非内联回调的 `return@forEach` 改成裸 `return`，或者反过来在需要退出外层函数时机械地添加标签。它还会在循环外创建一个捕获可变计数器的 lambda 并注册给所有监听器，用嵌套 `it` 混淆事件与元素，或把需要调用挂起 API 的参数声明成普通 `() -> Unit`。这些错误分别表现为编译失败、提前返回、状态串扰、更新错对象和错误的协程边界。

**让 AI 检查**

- 列出每个函数类型、接收者、参数和结果，并指出普通函数类型与 `suspend` 函数类型是否匹配。
- 标出每个 lambda 捕获的变量、状态所有者、调用次数、调用线程和预期生命周期。
- 为每个 `return` 写出精确目标，并验证接收函数的 `inline`、`noinline` 或 `crossinline` 约束。

**审查清单**

- 交错调用由同一处生成的多个回调，确认需要独立的状态没有被共享。
- 用空集合、首项命中、末项命中与未命中输入测试带标签和非局部返回。
- 展开嵌套 lambda 的参数名，并检查尾随 lambda 实际绑定的形参名称。

**提示词词汇**

使用准确术语：`function type`（函数类型）、`higher-order function`（高阶函数）、`expected type`（预期类型）、`closure capture`（闭包捕获）、`trailing lambda`（尾随 lambda）、`lambda with receiver`（带接收者的 lambda）、`labeled return`（带标签返回）、`non-local return`（非局部返回）、`SAM conversion`（SAM 转换）。要求模型给出“捕获－调用－返回时间线”，比“简化这个 lambda”更容易暴露所有权和控制流错误。
...
- [Kotlin 高阶函数与 lambda 文档源文件](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/lambdas.md)
- [Kotlin 内联函数文档源文件](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/inline-functions.md)
- [Kotlin 函数式接口文档源文件](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/fun-interfaces.md)
- [Kotlin 函数引用文档源文件](https://raw.githubusercontent.com/JetBrains/kotlin-web-site/master/docs/topics/reflection.md)

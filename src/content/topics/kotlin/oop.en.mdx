...
Models often move Java templates into Kotlin word for word: they add `new`, empty constructors, and companion objects that imitate `static`, or make every property a public `var`. To make mocking easy, they may mark an entire hierarchy `open` without an override contract. For reuse, they make a service inherit a repository or logger, turning a "has a" relationship into an "is a" relationship.

The subtler failures are about lifetime. A generated base class calls an open function from `init`, and the derived override immediately reads an uninitialized property. A generated `object` stores a current user, mutable list, or test counter, causing concurrent requests and test cases to share state. The code compiles, and a single happy-path test may still pass.

**Ask your AI to check**

- List every `open`, `abstract`, `override`, and `final override` member, including its replacement contract and whether overrides may call `super`.
- Draw the construction order and mark every call from a property initializer or `init` block that can dispatch at runtime.
- Classify each relationship as "is a" or "has a", then list all public state mutation paths and the invariants they must preserve.

**Review checklist**

- Try to violate each invariant from outside the class and confirm visibility and public methods reject the invalid state.
- Exercise at least two implementations through an interface type, including override boundaries and failure paths.
- Interleave two object instances and two test cases to find shared mutable state in companion objects or `object` declarations.

**Prompt vocabulary**

Use the exact terms `encapsulation`, `class invariant`, `subtype polymorphism`, `runtime dispatch`, `open member`, `final override`, `abstract class`, `interface default implementation`, `composition over inheritance`, and `object declaration`. Ask for an "open-point inventory, construction dispatch path, and state ownership table"; "improve the OOP design" is too vague to audit.

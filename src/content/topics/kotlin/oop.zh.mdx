...
模型经常把 Java 模板逐字搬进 Kotlin：添加 `new`、无意义的空构造函数和模拟 `static` 的伴生对象，或者把属性全部做成公开 `var`。为了便于 mock，它还可能把整棵类层级设为 `open`，却没有定义覆盖契约；为了复用，又让服务继承仓库或日志器，把「有一个」误写成「是一个」。

更隐蔽的失败发生在生命周期上。生成的基类会在 `init` 中调用开放函数，派生覆盖随即读取尚未初始化的属性；生成的 `object` 则保存当前用户、可变列表或测试计数，让并发请求和测试用例共享状态。代码能编译，单次快乐路径测试也可能通过。

**让 AI 检查**

- 列出每个 `open`、`abstract`、`override` 和 `final override` 成员，说明允许替换的契约与调用 `super` 的规则。
- 画出构造顺序，并标记属性初始化器与 `init` 块中所有可能运行时分派的调用。
- 把每条关系分类为「是一个」或「有一个」，并列出所有公开状态修改路径和必须保持的不变量。

**审查清单**

- 尝试从类外破坏每个不变量，确认可见性和公开方法会阻止非法状态。
- 通过接口类型运行至少两个实现，并测试覆盖实现的边界与失败路径。
- 交错运行两个对象实例和两个测试用例，检查伴生对象或 `object` 中是否泄漏共享可变状态。

**提示词词汇**

使用准确术语：`encapsulation`（封装）、`class invariant`（类不变量）、`subtype polymorphism`（子类型多态）、`runtime dispatch`（运行时分派）、`open member`（开放成员）、`final override`（最终覆盖）、`abstract class`（抽象类）、`interface default implementation`（接口默认实现）、`composition over inheritance`（组合优于继承）和 `object declaration`（对象声明）。要求模型输出「开放点清单、构造分派路径和状态所有权表」，比「改进 OOP 设计」更容易核对。

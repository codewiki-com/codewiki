...
Models often put every visible property into one mutable builder, then patch `build()` with `lateinit`, `!!`, or empty strings. They omit `@DslMarker` from one nested receiver, claim that the marker forbids explicit outer access, or hand the builder's collections straight to the result. In SQL or HTML DSLs, generated code also mistakes type-correct string interpolation for safe encoding.

**Ask your AI to check**

- List the receiver type of every lambda with receiver and identify which receiver binds each unqualified name when scopes nest.
- Find every mutable reference shared between the builder and result, then provide a test that mutates each side after construction.
- Separate static constraints, build-time validation, and output encoding, and name every domain rule for which no layer is responsible.

**Review checklist**

- Compile at least one invalid nesting example that should fail, confirming that the DSL marker covers the entire receiver hierarchy.
- Call the entry point with missing values, boundary values, and untrusted text; inspect both messages and encoding behavior.
- Mutate original inputs and the builder after producing a result, and confirm that the result doesn't change unexpectedly.

**Prompt vocabulary**

Use the exact terms `internal DSL`, `lambda with receiver`, `implicit receiver`, `receiver tower`, `trailing lambda`, `DSL marker`, `staged builder`, and `result boundary`. Ask for a "receiver-resolution table" and a "builder-to-result alias graph". Those requests expose concrete faults better than "make this DSL safer."

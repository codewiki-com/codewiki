...
Kotlin 还支持<Term id="extension-property">扩展属性（extension property）</Term>。
...
模型经常为一个基类和多个子类分别生成同名扩展，然后把它们当作多态方法。
测试若只用具体类型变量会全部通过，一旦值进入基类参数或集合，静态解析就选择基类扩展。
另一种常见错误是生成与现有成员或标准库扩展同名的声明，实际调用目标与注释不一致。

生成的扩展属性还常携带隐藏成本或隐藏状态。
模型会在 getter 中解析 JSON、访问磁盘或维护全局缓存，也会给 `Type?` 扩展的调用点
机械地加上 `?.`，从而跳过函数内部的回退逻辑。这些问题大多不会表现为语法错误。

**让 AI 检查**

- 列出每个扩展调用的接收者声明类型、可见候选和最终选择，并标出成员优先的调用。
- 对所有可空接收者调用分别推演接收者为 `null` 时，直接调用与 `?.` 调用的结果。
- 搜索扩展属性中的 I/O、分配、全局可变状态和可能抛出的操作，再建议函数或显式存储边界。

**审查清单**

- 用具体类型、基类类型和接口类型分别调用同名扩展，确认静态解析符合契约。
- 检查扩展名称、导入和依赖新增成员，确保没有遮蔽、歧义或重新编译后的行为漂移。
- 对 `null`、空集合与边界输入运行示例，并确认扩展没有假装拥有接收者的私有权限或新状态。

**提示词词汇**

使用准确术语：`extension function`（扩展函数）、`extension property`（扩展属性）、
`extension receiver`（扩展接收者）、`declared receiver type`（接收者声明类型）、
`static dispatch`（静态分派）、`member precedence`（成员优先）、
`nullable receiver`（可空接收者）和 `dispatch receiver`（分派接收者）。
要求 AI 输出“接收者类型与候选表”，比笼统地要求“检查这个扩展”更容易暴露解析错误。

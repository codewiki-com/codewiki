...
Models often generate an `object` delegate with a `private var value` and reuse it for several properties, causing data to leak across fields and owners. Another frequent mistake puts database writes or coroutine launches in an `observable` callback and then claims the assignment is transactional. If the callback fails, the new in-memory value is already present.

Thread mistakes are harder to spot. Generated code treats `lazy(NONE)` as a blanket optimization, or selects `PUBLICATION` while the initializer registers listeners, writes files, or increments a billing counter. Concurrent first access can repeat those side effects even though every reader eventually observes the same published value.

Generators also overuse map delegation. They cast an unvalidated deserialization result to `Map<String, Any?>` and assume missing keys, `null`, numeric representation, and property renames are covered by static typing. Those failures have merely moved to property access time.

**Ask your AI to check**

- List the number of instances produced by every expression on the right of `by`, and classify its state as per-property, per-owner, or global.
- Trace callback timing, thread, exception paths, and external side effects for every `observable`, `vetoable`, and `lazy` initializer.
- For every map delegate, list exact key names, runtime value types, missing-value policy, and rename migration, then generate failure-first tests.

**Review checklist**

- Interleave reads and writes across two properties and two owners to expose accidental mutable-delegate sharing.
- Make initializers and change callbacks throw, then inspect the property value, retry behavior, and external side effects.
- Trigger first access from two threads and use a counter to verify the chosen `LazyThreadSafetyMode` contract.

**Prompt vocabulary**

Use the exact terms `delegated property`, `property delegate`, `getValue/setValue operator`, `thisRef`, `KProperty metadata`, `delegate instance lifetime`, `safe publication`, and `provideDelegate binding time`. Ask for an "owner-property-delegate instance graph" and a "read/write timeline"; those requests expose sharing and timing mistakes better than "check this Kotlin property."

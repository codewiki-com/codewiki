...
模型常把所有可见属性塞进一个可变构建器，再用 `lateinit`、`!!` 或空字符串凑出 `build()`。它还会漏掉某个嵌套接收者上的 `@DslMarker`，误称标记会禁止显式外层访问，或让 `build()` 把内部集合原样交给结果。生成 SQL 或 HTML DSL 时，类型正确的字符串插值也经常被误当成已经安全编码。

**让 AI 检查**

- 列出每个带接收者 lambda 的接收者类型，并说明嵌套时每个未限定名称会绑定到哪一个接收者。
- 找出构建器与结果之间共享的所有可变引用，并给出构建完成后修改任一侧的测试。
- 把静态约束、构建期校验和输出编码分别列出，指出没有任何一层负责的领域规则。

**审查清单**

- 编译至少一个应当失败的非法嵌套示例，确认 DSL 标记覆盖了整个接收者层级。
- 用缺失值、边界值和不可信文本调用入口，检查错误消息与编码行为。
- 构建结果后继续修改原始输入与构建器，确认结果没有意外变化。

**提示词词汇**

使用准确术语：`internal DSL`（内部 DSL）、`lambda with receiver`（带接收者的 lambda）、`implicit receiver`（隐式接收者）、`receiver tower`（接收者塔）、`trailing lambda`（尾随 lambda）、`DSL marker`（DSL 标记）、`staged builder`（分阶段构建器）、`result boundary`（结果边界）。要求 AI 提供「接收者解析表」和「构建器－结果别名图」，比笼统地要求「让 DSL 更安全」更容易发现具体错误。

---
title: 空安全
description: 用可空类型、智能转换、安全调用与 Elvis 运算符表达缺失值，并在 Java 互操作和生成代码边界避免运行时空指针异常。
track: kotlin
section: basics
difficulty: beginner
...
<TLDR>
  <TLDRCell label="what">
    Kotlin 把可否为 `null` 写进类型：`T` 表示非空值，`T?` 表示值也可能缺失。
  </TLDRCell>
  <TLDRCell label="trap">
    `?.` 只会安全地传播 `null`，不会判断缺失是否符合业务规则；`!!` 则把检查推迟到运行时。
  </TLDRCell>
  <TLDRCell label="fix">
    在输入边界保留可空性，随后用智能转换、Elvis 提前返回或显式结果类型，把已验证的数据收窄为非空领域值。
  </TLDRCell>
</TLDR>

## 是什么，为什么存在

Kotlin 空安全（null safety）把缺失值作为静态类型信息。普通的 `String` 不能保存 `null`，<Term id="nullable-type">可空类型</Term> `String?` 才能保存字符串或 `null`。调用方因此能从函数签名看出缺失是否属于契约，而不必依赖注释或等到解引用时才发现问题。
...
<Term id="elvis-operator">Elvis 运算符</Term> `?:` 在左侧非空时返回左侧，否则才计算右侧。右侧可以是默认值，也可以是 `return` 或 `throw`，因为它们在 Kotlin 中可以出现在表达式位置。`val user = lookup(id) ?: return NotFound` 很适合在函数入口结束缺失分支，并让后续的 `user` 保持非空。
...
> [!PITFALL]
> 检查可变属性后再读取它，既可能无法智能转换，也可能因自定义 getter 或并发修改而观察到另一个值；自动生成的修复常直接追加 `!!`。
...
> [!PITFALL]
> 从 Java 得到的平台类型可以被当作非空值使用，但 Java 实现仍可能返回 `null`；代码看起来没有可空类型，却会在赋值检查或成员调用处失败。
...
模型常根据一份始终带值的 JSON 样例，把可选字段生成为非空属性，随后在反序列化或映射代码里补 `!!`。它也会把长安全调用链结尾统一成 `?: emptyList()`，从而把服务缺失、解析失败和合法空结果混在一起；或用 `filterNotNull()` 让批量记录数量悄悄减少。Java 互操作代码里，模型还会把平台类型直接推断为非空局部变量，并假定未注解的第三方方法遵守样例行为。

**让 AI 检查这些内容**

- 列出每个 `?`、`?.`、`?:`、`as?` 和 `!!` 的位置，并说明该处 `null` 的具体领域含义和失败路径。
- 沿着反序列化、数据库和 Java 调用边界追踪可空性，确认第一次转换为非空值之前完成了哪些运行时校验。
- 找出 `orEmpty()`、`filterNotNull()` 和通用默认值导致数量、顺序、错误原因或审计行为变化的位置。

**评审清单**

- 用字段缺失、显式 `null`、空白文本、错误运行时类型和含空元素的集合测试每个外部输入边界。
- 把所有 `!!` 当作待证明的断言，确认其不变量由同一控制流、同步边界或可信注解保证。
- 对每条安全调用链逐段置空，验证得到的是预期领域结果，而不是被默认值掩盖的故障。

**提示词汇**

使用准确术语 `nullable type`（可空类型）、`non-null type`（非空类型）、`safe call`（安全调用）、`Elvis operator`（Elvis 运算符）、`smart cast`（智能转换）、`not-null assertion`（非空断言）、`platform type`（平台类型）、`nullability annotation`（可空性注解）和 `definitely non-nullable type`（确定非空类型）。要求模型给出“可空性来源到验证点的路径”，比笼统地要求“让代码空安全”更容易暴露错误假设。
...
Java 源码通常没有 Kotlin 类型系统所需的完整可空性信息。Kotlin 编译器把这类 Java 表达式表示为 <Term id="platform-type">平台类型</Term>（platform type），诊断和 IDE 中常显示为 `String!` 一类形式；这个感叹号不是可以写进 Kotlin 类型声明的语法。
...
未写上界的 Kotlin 类型参数默认上界是 `Any?`。因此 `fun <T> keep(value: T): T` 可以用 `String?` 作为 `T`，函数体不能假定 `value` 非空。需要拒绝可空类型实参时，声明 `T : Any`，正如示例中的 `requireAll`。
...
泛型擦除也不会在运行时恢复元素可空性。一个外部框架可能把含 `null` 的数据交给静态声明为 `List<String>` 的边界，尤其是通过 Java、反射或不安全转换时。对不可信数据的结构和元素仍需在进入领域模型前验证。

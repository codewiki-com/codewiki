...
模型经常为了消除“不穷尽”的编译错误直接补上 `else`，而不是找出缺失变体的业务含义。它也会按架构目录把直接子类生成到不同包或模块，或者把直接子类标为 `open`，却仍声称所有具体后代在编译期已知。生成界面状态时，另一个常见错误是把携带进度、请求编号或错误详情的分支写成带可变属性的单例，导致同时进行的操作共享状态。

**让 AI 检查**

- 列出密封类型的每个直接子类型、所在包、所在模块及其 `final`、`sealed` 或 `open` 状态。
- 找出所有以该密封类型为主题的 `when`，说明每个 `else` 会掩盖哪个现有或未来变体。
- 对每个 `object` 或 `data object` 变体检查是否存在每次事件独有的数据，并追踪任何可变属性的共享范围。

**审查清单**

- 在加入一个临时变体后重新编译，确认需要业务决定的处理点确实失败。
- 验证直接子类型都在同包同模块，并检查开放的间接扩展是否出于明确设计。
- 对状态与命令的组合、泛型空载荷分支和并发状态所有权分别编写测试。

**提示词词汇**

使用准确术语：`sealed class`（密封类）、`sealed interface`（密封接口）、`direct subclass`（直接子类）、`indirect subclass`（间接子类）、`exhaustive when`（穷尽 `when`）、`smart cast`（智能转换）、`final leaf`（最终叶节点）、`open extension point`（开放扩展点）、`bottom type`（底类型）。要求模型输出层级边界表和所有 `when` 的覆盖矩阵；“让状态处理更健壮”无法暴露被 `else` 隐藏的分支。

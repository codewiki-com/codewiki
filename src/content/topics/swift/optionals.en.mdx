...
Models readily repair "optional must be unwrapped" diagnostics by inserting `!`, especially around dictionary reads, `first`, URL construction, and test fixtures. They also write `Int(raw) ?? 0`, treat every decoding failure as a missing field, or claim that a `nil` from `account?.profile?.email` proves the email field itself was absent. With `[String: Value?]`, generated code may flatten `Value??` and silently remove the "key does not exist" state.

**Ask your AI to check**

- List every `!`, implicitly unwrapped property, and `as!`, and name the executable precondition that proves each value or type assertion.
- For every `??`, enumerate why the left side can be `nil` and decide whether the fallback hides malformed input or a missing record.
- Mark the failure states merged by each optional chain and nested optional, then say whether callers must distinguish them.

**Review checklist**

- Run tests with a missing key, malformed text, an empty collection, and `nil` at every link in each optional chain.
- Check whether `nil`, an empty string, zero, `false`, and an empty collection have distinct domain meanings.
- Confirm that APIs needing failure reasons don't return only `nil`, and that code needing three states doesn't flatten them accidentally.

**Prompt vocabulary**

Use precise terms: `optional value`, `wrapped type`, `optional binding`, `optional chaining`, `nil-coalescing operator`, `force unwrap`, `implicitly unwrapped optional`, and `nested optional`. Ask the model to state the source and business meaning of every `nil`; "make this code stop crashing" isn't enough to judge whether a default or flattening operation is correct.

...
模型经常生成一个名为 `Atomic` 的包装器，只给 getter 与 setter 分别加锁，然后在调用处继续使用 `+=`；名称掩盖了丢失更新。它也会生成过度泛化的 `UserDefaults` 包装器，用 `try?` 抹掉编码错误并把认证令牌打印到日志。另一个常见错误是把包装器改成类来「避免复制」，却没有指出外围结构体副本从此共享状态。

Swift 6 中还有一类迁移错误：模型看到 `wrappedValue` 标记为 `@MainActor`，就推断包含该属性的整个类型也自动获得 `@MainActor`。SE-0401 已在 Swift 6 语言模式移除这种向外推断；需要隔离的类型或成员必须明确表达。模型还会根据其他框架里同名的 `$property` 猜测投影类型，而不查看当前包装器定义。

**让 AI 检查**

- 把每个被包装属性展开成 `_property`、`property` 与可选的 `$property`，并指出实际调用的构造器和投影类型。
- 列出包装器的复制、错误、持久化和并发语义，再给出能证伪每项保证的最小测试。
- 检查每个复合读改写操作的同步边界，并确认 Swift 6 actor 隔离来自显式声明而不是包装器向外推断。

**审查清单**

- 同时测试初始值与后续赋值，包括边界值、无效值和失败是否可观察。
- 复制外围值后分别修改两个副本，并核对后备存储应当共享还是独立。
- 对持久化和并发包装器测试损坏数据、升级迁移、复合更新、隔离与敏感日志。

**提示词词汇**

使用准确术语：`property wrapper`（属性包装器）、`wrapped value`（包装值）、`projected value`（投影值）、`backing storage`（后备存储）、`init(wrappedValue:)`、`wrapper composition`（包装器组合）、`compound read-modify-write`（复合读改写）和 `outward actor isolation inference`（actor 隔离向外推断）。要求模型给出语法展开与状态所有权表；「让这个属性更安全」不足以定义契约。

---
title: Swift 基础
description: 通过可运行程序掌握 Swift 的静态类型、不可变与可变绑定、安全转换、控制流、函数、集合和值语义。
track: swift
section: basics
difficulty: beginner
...
Swift 数组和字典具有<Term id="value-semantics">值语义（value semantics）</Term>。
...
> [!PITFALL]
> Swift `String` 不是 UTF-16 码元、字节或可用整数索引的字符数组。
...
生成的 Swift 代码常像语言会自动插入转换一样混用 `Int` 和 `Double`，或在没有考虑单位与精度时随意添加强制转换来消除诊断。
模型还会强制解包字典查找与解析结果，直接把外部整数当作数组索引，并通过 `??` 把解析失败替换成 `0`，于是格式错误的输入会变成看似有效的数据。
生成的测试还会固定某次观察到的集合或字典顺序，生成的字符串代码则常照搬其他语言的整数下标。

**让 AI 检查**

- 列出类型会影响精度、单位、重载选择或空集合的每个推断声明，只在标注能澄清契约时提出添加建议。
- 跟踪每个可选值从来源到解包或提供默认值的过程，并说明缺失、格式错误与超出范围是否应该具有相同含义。
- 找出每个集合与字符串下标，证明索引对该集合确实有效，并确认所有可观察顺序都已显式规定。

**审查清单**

- 使用目标 Swift 语言模式编译；拒绝只靠强制解包、宽泛默认值或随意数值转换来消除错误的修改。
- 测试空集合、重复键、缺失的字典字段、格式错误的数字、边界整数以及由多个标量组成的字符。
- 修改数组、字典、字符串与嵌套引用值的副本，确认所有权和值行为符合预期。

**提示词词汇**

使用准确术语：`static type`（静态类型）、`type inference`（类型推断）、`explicit conversion`（显式转换）、`optional binding`（可选绑定）、`nil-coalescing`（空合运算）、`collection index`（集合索引）、`Hashable key`（可哈希键）、`value semantics`（值语义）、`copy-on-write`（写时复制）、`String.Index`（字符串索引）和 `extended grapheme cluster`（扩展字素簇）。
当多种无效情况需要区别处理时，要求 AI 给出输入状态表；「让这段 Swift 更安全」太含糊，无法表达所需语义。
...
Swift `Character` 表示一个扩展<Term id="grapheme-cluster">字素簇（grapheme cluster）</Term>，其中可以包含一个或多个 Unicode 标量。

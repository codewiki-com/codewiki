...
Models treat `[weak self]`, `@escaping`, or `[index]` as closure templates without checking the actual storage graph or Swift's loop-binding semantics. They also generate an `@Sendable` closure that captures and mutates an ordinary local `var`, producing diagnostics under Swift 6 strict concurrency checking, or describe `[service]` as an object copy and miss its strong reference. Generated completion handlers also omit exactly-once protection, calling back twice—or not at all—across success, error, and cancellation paths.

**Ask your AI to check**

- List the declaration, creation time, read time, and strong, weak, or unowned ownership for every capture.
- Draw storage edges among objects, tasks, queues, and closures, and identify which parameters truly require `@escaping`.
- Trace callback counts through success, error, cancellation, and repeated triggers, and check that `@Sendable` captures satisfy sendability and isolation.

**Review checklist**

- Invoke the closure after changing its outer variables to confirm whether shared state or a creation-time snapshot is intended.
- Release captured objects and trigger retained callbacks to verify there is no cycle, trap, or silent loss of required work.
- Compile concurrent closures in the target Swift language mode and fix data-race diagnostics instead of suppressing them with `@unchecked Sendable`.

**Prompt vocabulary**

Use the exact terms `closure expression`, `capture context`, `capture list`, `nonescaping closure`, `escaping closure`, `autoclosure`, `strong reference cycle`, `sendable closure`, and `actor isolation`. Ask for a creation, storage, invocation, and release timeline; “optimize this callback” doesn't identify the semantics you want checked.

...
模型经常为了“避免复制”把纯数据记录改成类，却没有证明身份或共享修改是需求，于是一个调用点的更新会泄漏到其他持有者。
它也会复制包含类属性的结构体并声称得到了完整快照，或者把 `let` 类引用描述成深层不可变。
另一类常见错误是依赖合成成员初始化器作为公共 API，随后新增属性便让调用代码失效；在 Swift 6 严格并发检查下，生成的共享可变类还可能跨隔离边界产生诊断。

**让 AI 检查**

- 列出每个自定义类型需要的是值相等还是稳定身份，并指出继承、`deinit` 或共享修改的真实使用点。
- 沿每次赋值和传参追踪哪些位置复制值、哪些位置复制引用，再检查结构体内所有类、闭包和缓冲区字段。
- 用 Swift 6.3.3 编译完整调用点，并检查合成初始化器访问级别、`let`/`var` 绑定以及严格并发诊断。

**审查清单**

- 交错修改原值与副本，验证每个字段的共享或独立行为符合契约。
- 对类分别测试 `===` 身份与 `==` 内容相等，不把两者混用。
- 检查类的拥有者与生命周期，并确认值快照没有隐藏的可变引用。

**提示词词汇**

使用准确术语 `value semantics`（值语义）、`reference semantics`（引用语义）、`reference identity`（引用恒等性）、`aliasing`（别名）、`memberwise initializer`（成员初始化器）、`mutating method`（可变方法）、`shallow copy`（浅拷贝）和 `copy-on-write`（写时复制）。
要求模型给出赋值与修改轨迹、嵌套引用清单和所有权图；“帮我选择结构体或类”不足以暴露语义假设。

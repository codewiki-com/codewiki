...
模型遇到关联类型编译错误时，常把 `<T: Protocol>` 改成 `any Protocol`，随后加入 `as!` 恢复被擦除的信息。它还会让 `-> some P` 的不同分支返回不同结构体，把可定制方法只放进协议扩展，或给整个泛型类型添加仅一个方法需要的约束。这些修改可能让局部错误消失，却改变了选择权、分派或失败时机。

**让 AI 检查**

- 为每个泛型参数、关联类型、`some` 和 `any` 标明谁选择具体类型，以及哪些位置必须保持同类型。
- 列出协议扩展中的每个成员，说明它是否也是协议要求，并分别预测具体、泛型与存在类型调用的实现。
- 找出所有 `as!`、`Any` 和手写擦除包装器，说明编译期证明在哪里丢失，以及能否用约束恢复。

**审查清单**

- 用两种不同遵循类型编译调用点，并确认应保持的输入输出关系仍由签名表达。
- 覆盖空集合、越界索引与异构存储，确认没有强制解包或强制转换承担正常控制流。
- 通过实际 API 边界测试默认实现与定制实现，不只直接调用具体类型。

**提示词词汇**

使用准确术语：`generic parameter`（泛型参数）、`generic constraint`（泛型约束）、`associated type`（关联类型）、`same-type requirement`（同类型要求）、`opaque result type`（不透明返回类型）、`existential type`（存在类型）、`protocol requirement`（协议要求）、`type erasure`（类型擦除）和 `static dispatch`（静态分派）。要求模型输出「具体类型选择权表」与「协议成员分派表」；「让接口更灵活」无法说明要保留哪些类型证明。

...
模型常用 `try?` 和默认值让类型检查通过，却把损坏数据伪装成缺失数据；也会把测试里的 `try!` 带进生产路径。另一类失败是生成一个 `catch { print(error); return fallback }`，既向调用方报告成功，又让原始错误只存在于可能泄密的日志中。重试代码还可能捕获全部错误，把无效输入或 `CancellationError` 当成瞬时故障反复执行。

**让 AI 检查什么**

- 列出每个 `try` 的失败类型，并说明该位置选择恢复、转换还是继续传播的依据。
- 找出所有 `try?`、`try!`、宽泛 `catch` 和默认返回值，说明它们丢失了哪些错误信息。
- 逐条跟踪成功、每种领域失败、依赖错误与取消路径，检查重试是否只覆盖真正可重试的类别。

**审查清单**

- 对每个可抛出步骤注入失败，确认部分副作用、清理与返回状态符合契约。
- 检查错误转换是否保留可操作的类别，同时避免在日志和用户提示中泄露敏感关联值。
- 用目标 Swift 语言模式编译，确认类型化 `throws`、穷尽处理与函数类型转换没有被宽化或强制转换掩盖。

**提示词词汇**

使用精确术语 `error propagation`（错误传播）、`typed throws`（类型化 throws）、`exhaustive catch`（穷尽捕获）、`error translation`（错误转换）、`underlying error`（底层错误）、`rethrows`（重抛约束）、`recovery boundary`（恢复边界）和 `partial side effect`（部分副作用）。要求模型给出每条失败路径的控制流与状态变化；“加上错误处理”无法表达恢复契约。

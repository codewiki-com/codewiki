...
生成代码经常看到一份 JSON 样本就把所有字段声明为必需，或者反过来把所有字段改成可选并用 `try?` 隐藏契约错误。它也会假设属性默认值参与合成解码、把 `.convertFromSnakeCase` 当作任意键名的逆变换、遗漏日期策略，或生成一个捕获所有异常并返回空模型的辅助函数。涉及属性包装器时，模型还常只实现包装器的 `init(from:)`，却忽略缺失键会先在键控容器层失败。

**让 AI 检查**

- 列出每个字段对缺失、显式 `null`、错误类型和未知枚举值的预期行为，并指出实现它的具体解码调用。
- 用真实契约逐项核对 `CodingKeys`、键转换策略、日期单位与时区，标出无法往返的字段。
- 为每条失败路径给出预期 `DecodingError` 类别和 `codingPath`，同时检查日志是否泄露原始载荷。

**审查清单**

- 分别测试最小有效载荷、额外未知字段、缺失必需字段、显式 `null` 和错误标量类型。
- 用固定日期与未知枚举值测试向前兼容行为，并确认编码端与解码端策略成对。
- 检查解码后的验证边界，确认范围、大小、权限和跨字段不变量没有交给 `Codable` 假定完成。

**提示词词汇**

使用准确术语：`synthesized conformance`（合成一致性）、`CodingKeys mapping`（`CodingKeys` 映射）、`keyed container`（键控容器）、`single-value container`（单值容器）、`missing key`（缺失键）、`explicit null`（显式 `null`）、`date decoding strategy`（日期解码策略）、`codingPath`（编码路径）和 `transport model validation`（传输模型验证）。要求 AI 产出字段兼容性矩阵，比“让解析更健壮”更容易暴露宽松默认值与静默失败。

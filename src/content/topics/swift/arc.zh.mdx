...
生成代码常把 `[weak self]` 当作所有异步闭包的固定模板，却不分析闭包是否由 `self` 保存，结果让必要工作悄悄丢失。相反，模型也会在属性闭包中直接使用 `self`，或把 `self.method` 作为处理器传递，隐藏对象到闭包再回到对象的强引用环。为了去掉 `?`，它还可能把关系改成 `unowned`，但没有证明取消、导航或外部缓存不会让引用方活得更久。

**让 AI 检查**

- 列出每个类实例、闭包、任务和订阅之间的保存边，并把每条边标为 strong、weak 或 unowned。
- 为每个 `unowned` 写出目标必须更长寿的不变量，再寻找一个能违反它的公开调用序列。
- 跟踪成功、错误和取消路径，确认每个长期保存的回调或令牌都在所有路径断开。

**审查清单**

- 删除最后一个外部强引用后，用弱探针或 `deinit` 测试确认对象图能释放。
- 检查方法引用、嵌套闭包和任务体中的隐式强捕获，而不只搜索 `[weak self]`。
- 在每个 `await`、延迟回调和取消点之后，验证生成代码所假设的生命周期仍然成立。

**提示词词汇**

使用准确术语：`automatic reference counting`（自动引用计数）、`strong reference`（强引用）、`weak reference`（弱引用）、`unowned reference`（无主引用）、`strong reference cycle`（强引用循环）、`capture list`（捕获列表）、`zeroing weak reference`（自动置空弱引用）、`ownership graph`（所有权图）和 `lifetime invariant`（生命周期不变量）。要求 AI 输出保存关系图和成功、错误、取消三条释放时间线；“加上 weak self”并没有说明期望的所有权。

...
Generated code often sees one JSON sample and makes every field required, or makes every field optional and hides contract failures with `try?`. It also assumes declaration defaults affect synthesis, treats `.convertFromSnakeCase` as an inverse for arbitrary names, omits a date strategy, or creates a helper that catches every error and returns an empty model. With property wrappers, models often implement only the wrapper's `init(from:)` and miss that an absent key can fail first at the keyed-container layer.

**Ask your AI to check**

- List the intended behavior of every field for absence, explicit `null`, a wrong type, and an unknown enum value, and name the exact decoding call that implements it.
- Compare `CodingKeys`, key strategies, date units, and timezones against the real contract field by field; identify anything that can't round-trip.
- Give the expected `DecodingError` case and `codingPath` for every failure path, then check that logs don't reveal the raw payload.

**Review checklist**

- Test the smallest valid payload, extra unknown fields, a missing required field, explicit `null`, and the wrong scalar type separately.
- Test forward compatibility with a fixed date and an unknown enum value, and confirm that encoder and decoder strategies are paired.
- Find the post-decoding validation boundary; verify that ranges, sizes, permissions, and cross-field invariants weren't assumed to be `Codable`'s job.

**Prompt vocabulary**

Use the exact terms `synthesized conformance`, `CodingKeys mapping`, `keyed container`, `single-value container`, `missing key`, `explicit null`, `date decoding strategy`, `codingPath`, and `transport model validation`. Ask for a field compatibility matrix. “Make parsing robust” is too vague to expose permissive defaults and silent failures.

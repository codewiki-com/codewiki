...
模型会把 `[weak self]`、`@escaping` 或 `[index]` 当成闭包模板，而不检查实际存储关系和 Swift 的循环绑定语义。它还会让一个 `@Sendable` 闭包捕获并修改普通局部 `var`，在 Swift 6 严格并发检查下产生诊断；或者把 `[service]` 误说成对象副本，漏掉仍然存在的强引用。自动生成的完成回调也常缺少“恰好一次”保护，在成功、错误和取消路径上重复调用或完全不调用。

**让 AI 检查**

- 列出每个闭包捕获的声明、创建时机、读取时机，以及强、弱或无主所有权。
- 沿对象、任务、队列和闭包的存储边画出引用图，并指出哪些参数真的需要 `@escaping`。
- 按成功、错误、取消和重复触发路径跟踪回调次数，同时检查 `@Sendable` 捕获是否满足可发送性与隔离要求。

**审查清单**

- 在外层变量改变后调用闭包，确认需要的是共享值还是创建时快照。
- 释放闭包捕获的对象并触发延迟回调，确认不会循环保留、崩溃或静默丢掉必做工作。
- 在目标 Swift 语言模式下编译并发闭包，修复数据竞争诊断，而不是用 `@unchecked Sendable` 压制它们。

**提示词词汇**

使用准确术语：`closure expression`（闭包表达式）、`capture context`（捕获上下文）、`capture list`（捕获列表）、`nonescaping closure`（非逃逸闭包）、`escaping closure`（逃逸闭包）、`autoclosure`（自动闭包）、`strong reference cycle`（强引用循环）、`sendable closure`（可发送闭包）、`actor isolation`（actor 隔离）。要求 AI 给出创建、存储、调用和释放的时间线；“优化这个回调”无法说明你要核对的语义。

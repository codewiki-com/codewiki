...
When a model encounters an associated-type error, it often changes `<T: Protocol>` to `any Protocol`, then adds `as!` to recover the erased information. It also returns unrelated structures from branches of `-> some P`, places a customizable method only in a protocol extension, or constrains an entire generic type for one method's need. These edits can silence a local error while changing selection rights, dispatch, or failure timing.

**Ask your AI to check**

- For every generic parameter, associated type, `some`, and `any`, identify who selects the concrete type and which positions must remain the same type.
- List every protocol-extension member, state whether it is also a requirement, and predict the implementation called through concrete, generic, and existential types.
- Find every `as!`, `Any`, and hand-written erasure wrapper; explain where compile-time proof was lost and whether a constraint can restore it.

**Review checklist**

- Compile call sites with two different conformers and confirm the signature still expresses every required input-output relationship.
- Cover empty collections, out-of-range indexes, and heterogeneous storage without using forced unwraps or casts for normal control flow.
- Test default and customized implementations through the real API boundary, not only by calling the concrete type directly.

**Prompt vocabulary**

Use the exact terms `generic parameter`, `generic constraint`, `associated type`, `same-type requirement`, `opaque result type`, `existential type`, `protocol requirement`, `type erasure`, and `static dispatch`. Ask for a concrete-type selection table and a protocol-member dispatch table; "make the interface more flexible" doesn't say which type proofs must survive.

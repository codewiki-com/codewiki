...
模型常把其他语言的“扩展”规则套到 Swift：在扩展中加入存储属性、给类扩展方法写 `override`，或声称具体类型的同名方法总能覆盖协议扩展成员。它还会把约束堆到类型声明上，给标准库类型添加过于通用的名字，或用 `@retroactive` 消除警告却不分析遵循冲突。

**让 AI 检查**

- 列出每个扩展成员依赖的状态与约束，指出哪些声明错误地需要新存储，哪些约束可以缩小范围。
- 对每个协议扩展方法说明它是否是协议要求，并分别推导通过具体类型、泛型参数和 `any Protocol` 调用的实现。
- 找出类型与协议分别属于哪个模块；若两者都不是当前模块所有，说明追溯遵循的冲突与迁移方案。

**审查清单**

- 编译所有扩展，并测试空集合、边界索引以及不满足条件约束的类型组合。
- 搜索外部类型上的公共成员与协议遵循，检查命名碰撞、访问级别和上游演进风险。
- 为依赖多态行为的成员同时运行具体值与协议类型测试，确认静态分派没有改变结果。

**提示词词汇**

使用准确术语：`Swift extension`（Swift 扩展）、`computed property`（计算属性）、`constrained extension`（受约束扩展）、`conditional conformance`（条件遵循）、`protocol requirement`（协议要求）、`default implementation`（默认实现）、`static dispatch`（静态分派）、`protocol witness`（协议见证）和 `retroactive conformance`（追溯遵循）。要求模型给出静态类型与模块所有权，不能只说“用扩展让代码更整洁”。

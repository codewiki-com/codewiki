...
生成式工具常把每个方法拆成 `Service`、`Manager` 和 `Repository`，再为只有一个实现的类生成一一对应的 `IThing` 接口；文件变多了，变更原因仍然纠缠。它们也会根据 TypeScript 形状生成“兼容”实现，却在某个客户类型上抛出 `NotSupported`、返回 `null` 或改变错误语义。另一个常见问题是业务层接口直接复制某个数据库 SDK 的查询对象，使构造函数注入存在，依赖倒置却不存在。

**让 AI 检查**

- “列出这个模块的变更参与者，并用实际调用方或提交证据说明哪些行为应共同变化。”
- “写一组基类型契约测试，覆盖有效输入范围、后置条件、不变量和允许的失败，再对每个实现运行。”
- “输出业务模块到适配器的导入图，标出由低层供应商类型泄漏进高层接口的每个位置。”

**审查清单**

- 对每个新接口，确认至少有真实客户端需求、明确契约，以及独立变化或替换的理由。
- 在不修改调用者的前提下运行所有实现的共同契约测试，包括边界输入与错误路径。
- 检查高层模块的导入、构造参数和异常类型，确认没有具体数据库或 SDK 类型穿过边界。

**提示词词汇**

使用准确术语：`reason to change`（变更原因）、`actor`（变更参与者）、`extension point`（扩展点）、`behavioral subtyping`（行为子类型）、`precondition`（前置条件）、`postcondition`（后置条件）、`invariant`（不变量）、`role interface`（角色接口）、`source-code dependency`（源码依赖）和 `composition root`（组合根）。不要只要求“应用 SOLID”；应指出要保护的调用者、变化轴与可观察契约。

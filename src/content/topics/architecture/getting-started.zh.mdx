...
4. 受影响对象，例如 API、订单流程、数据副本或值班团队。
5. 预期响应，包括系统应完成、拒绝、降级或恢复的动作。
6. 响应判据，也就是用什么观察结果判断设计是否合格。

例如，支付提供方结果未知时，订单 API 不应报告「支付失败」或再次创建一笔全新扣款。系统应返回可查询的处理中状态，并让同一订单的重试复用支付幂等键。这个场景没有假装知道尚未测量的恢复时间，但已经约束了状态模型和接口。
...
决定完成后，用<Term id="architecture-decision-record">架构决策记录（Architecture Decision Record，ADR）</Term>保存背景、备选方案、选择和后果。ADR 解释为什么采用某种结构，当前架构说明则描述系统现在是什么样子。两者相互链接，但承担不同职责。

### 选择边界与交互

边界应围绕不同的变化原因、数据所有权或故障策略建立。如果定价规则和 HTTP 框架的升级节奏不同，让领域规则依赖一个小接口，通常比让它直接调用框架对象更容易演进。若两个模块必须在同一事务中频繁修改同一数据，把它们过早拆成网络服务反而会制造协调成本。

依赖方向说明谁可以知道谁。稳定的领域规则通常不应导入数据库驱动、Web 框架或消息客户端；外层适配器可以依赖内层策略，并通过接口提供外部能力。这条规则不是审美偏好，它让业务规则能在没有网络和数据库的情况下测试。
...
产物越少越好，但不能少到无法做决定。小型单体可能只需要一张上下文图、几条依赖规则和少量 ADR；跨区域系统则需要更明确的部署、数据和恢复视图。文档数量应由风险决定，不由模板决定。
...
下面三个示例使用 Node 24 的 JavaScript。它们从可替换的边界开始，再把依赖方向和故障行为写成可执行检查；所有输出都来自本地运行。

### 把业务策略留在边界内

第一个例子让结账策略只依赖两个注入的能力：查询价格和保存订单。策略不知道价格来自内存、数据库还是 HTTP，也不知道订单怎样持久化。
...
下一段脚本给模块标注层次，并检查内层是否反向依赖外层。示例故意让领域模块 `order` 依赖 PostgreSQL 适配器，因此报告一个违规关系。
...
>
> **修复：** 给每张图标明范围和更新时间，把稳定规则自动化，并定期从依赖图、部署清单和遥测反查文档。决定改变时新增 ADR，不要改写旧决定来迎合现状。

## AI 时代

**生成代码在这里常犯的错**

模型常把熟悉的目录模板当成已经成立的架构，凭名称声称模块「解耦」，却让领域代码直接导入 ORM、HTTP 客户端或另一个模块的表。它还会在没有证据时生成精确的吞吐量、可用性和恢复承诺，给每个失败统一添加重试，并漏掉幂等键、超时上限、所有权和部分成功。要求它「改成微服务」时，它往往复制数据模型和同步调用链，只把进程内故障改成网络故障。

**要求 AI 检查**

- 「从入口开始列出所有传递依赖，找出领域模块指向框架、数据库和外部客户端的反向依赖，并给出文件证据。」
- 「为每个网络调用列出超时、重试次数、幂等键、重复响应与未知结果的处理；没有代码证据的项标为缺失。」
- 「把架构说明中的组件和数据所有权与实际导入、数据库权限、部署清单逐项比对，只报告能定位到仓库的差异。」

**审查清单**

- 每个新边界是否隔离了已说明的变化、所有权或故障，而不是只增加一层转发？
- 生成的同步调用、缓存、队列和重试是否写清超时、一致性、幂等性与恢复责任？
- 质量或容量声明是否来自可重复的测试与遥测，未知信息是否仍明确标为未知？

**提示词汇**

使用 `quality attribute scenario`（质量属性场景）、`architecture boundary`（架构边界）、`dependency direction`（依赖方向）、`data ownership`（数据所有权）、`failure containment`（故障隔离）、`idempotency boundary`（幂等边界）、`architecture decision record`（架构决策记录）和 `fitness function`（适应度函数）。这些词要求模型说明约束与证据，比「让架构更好」或「应用最佳实践」更精确。
...
可逆决定可以保留选项，等证据更充分时再收窄。例如，在模块边界稳定之前保持进程内调用，通常比提前分成服务更容易调整。相反，外部 API、持久化格式和跨团队数据所有权会形成承诺点，需要兼容策略和迁移路径。
...
好的适应度函数只负责一个可解释约束，并在失败时给出可行动信息。「领域层不得导入适配器」可以定位到具体依赖；「架构质量分数必须大于 80」则把不同风险压成一个无法解释的数字。检查规则本身也需要版本化和评审。
...
新证据可能说明实现偏离了正确决定，也可能说明决定本身已经不合适。这两种情况要分开处理：前者修复代码并保留 ADR，后者创建新 ADR 说明背景变化，再安排迁移。直接改写旧记录会丢失理解旧代码所需的因果链。
...
- [Azure 应用架构基础](https://learn.microsoft.com/en-us/azure/architecture/guide/)
- [C4 模型：系统上下文图](https://c4model.com/diagrams/system-context)
- [ADR GitHub 组织与资源](https://adr.github.io/)

...
可以让智能体把架构声明变成可执行检查。假设一份 ADR 规定账单模块拥有发票数据：智能体可以从发票请求的入口一路追踪导入关系、数据库授权和部署清单，再为其他模块写入账单表的行为建立一个失败的架构测试。它还可以实现维持现状与移动边界的小型方案，并对两者运行同一延迟或故障场景。最终产物是一项有测量依据的选择，以及能够暴露后续漂移的稳定检查。

...
模型会根据仓库中的现状倒推出一个听起来合理的 ADR，却把猜测写成历史事实。它还常把用户的一句偏好扩写成“团队已接受”的决定，编造基准数据或备选方案，或者为了让文档与新实现一致而直接重写已接受的旧记录。生成脚本也可能只检查标题和状态，放过空白占位符、断开的取代链接和没有负面后果的记录。

**让 AI 检查**

- 把 ADR 中的每个事实、假设和预测分别列出，并为事实指出仓库内可核对的证据。
- 将代码变更与当前生效的 ADR 逐项对照，标出偏离之处，但不要自动改写任何已接受记录。
- 检查所有 `Superseded by` 与 `Supersedes` 链接是否双向、存在且无环，并报告孤立记录。

**审查清单**

- 确认状态反映真实共识；任何人名、日期、指标和组织约束都有来源。
- 确认备选方案是当时可行的真实选项，负面后果没有被措辞隐藏。
- 确认实现偏离会创建新提案或明确告警，而不是悄悄修改历史。

**提示词词汇**

使用准确术语：`architecture decision record`（架构决策记录）、`decision driver`（决策驱动因素）、`considered option`（备选方案）、`trade-off`（取舍）、`decision outcome`（决策结果）、`validation`（验证）、`supersedes`（取代）和 `decision log`（决策日志）。要求模型“区分仓库证据、明确假设与推测”，比“替这个设计写一份 ADR”更能阻止它伪造决策历史。

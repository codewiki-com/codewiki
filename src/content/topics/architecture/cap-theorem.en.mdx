---
title: 'CAP Theorem: Consistency and Availability During Partitions'
...
Models often generate a `ping()` health check and let a “healthy” node promote itself; one-way packet loss and pauses let both sides disagree about health and create split brain. They also call the fastest-replica path “strongly consistent,” return stale values through `Promise.race()`, or check only `W + R > N` before claiming linearizability. Recovery code commonly uses `Date.now()` for last-write-wins while ignoring clock drift, unknown outcomes from failed writes, and external side effects that already ran.

**Ask your AI to check**

- Enumerate observable success, failure, timeout, and partition results for every read and write, and mark which outcomes remain unknown.
- Produce an execution history with one-way packet loss, an old leader, and a membership change, then check for two writers or stale reads.
- Prove that conflict merge is commutative, associative, and idempotent, and identify business invariants that it cannot merge automatically.

**Review checklist**

- Replay every timed-out write with a stable idempotency key and confirm that it cannot duplicate charges, messages, or resources.
- Record complete invocation and completion histories under fault injection, then check the claimed linearizability or session guarantee.
- Confirm that quorums, terms, versions, membership configuration, and recovery all refer to the same consistency boundary.

**Prompt vocabulary**

Use the exact terms `linearizability`, `network partition`, `partition mode`, `quorum intersection`, `unknown outcome`, `split brain`, `conflict resolution`, `eventual convergence`, and `externalized side effect`. Ask the AI for an operation history and invariants instead of vaguely asking it to “make the database satisfy CAP.”

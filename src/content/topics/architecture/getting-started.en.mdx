...
Use an agent to make an architecture claim executable. Suppose an ADR says billing owns invoice data: the agent can trace the invoice request from its entry point through imports, database grants, and deployment manifests, then create a failing architecture test for an unrelated module that writes the billing tables. It can also implement small status-quo and moved-boundary variants and run the same latency or failure scenario against both. The outcome is a measured choice plus a stable check that reveals later drift.

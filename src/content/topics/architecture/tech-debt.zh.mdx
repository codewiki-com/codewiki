...
模型常把局部重复直接抽成通用框架，却没有检查这些代码是否会共同变化，结果新增抽象本身成为债务。它也会根据文件长度或 `TODO` 数量编造债务优先级，为并不存在的团队速度和事故成本生成精确数字，或者用「现代化」名义重写遗留模块，却漏掉空值、舍入、重试和副作用顺序等隐含行为。另一种失败是保留新旧两条路径和特性开关，却不生成删除条件，使迁移脚手架永久留在系统中。

**请 AI 检查**

- 找出本次改动新增的依赖方向、兼容分支和临时开关，并为每项给出可验证的删除条件。
- 从测试、版本历史和事故记录中区分观察事实与估算；没有证据的数字必须标成未知，而不是补写。
- 对重构前后的边界输入、错误类型、副作用顺序和外部调用次数生成差异测试，并列出尚未覆盖的行为。

**审查清单**

- 逐项追溯债务声明到代码、构建、事故或变更记录，拒绝仅凭坏味道名称得出的优先级。
- 在接受生成的抽象前检查真实共同变化；若两个调用方只是现在长得相似，不要强迫它们共享策略。
- 确认迁移代码有负责人、退出证据和删除步骤，并测试旧路径停用后的构建与运行结果。

**提示词词汇**

使用精确术语 `technical debt`（技术债务）、`debt principal`（债务本金）、`debt interest`（债务利息）、`change hotspot`（变更热点）、`characterization test`（特征测试）、`exit criteria`（退出条件）和 `incremental remediation`（增量偿还）。要求模型输出「证据—假设—决定—验证」表，比笼统地要求「清理技术债务」更容易暴露无依据的判断。

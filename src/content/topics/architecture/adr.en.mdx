...
An agent can draft an ADR alongside a proposed implementation, linking each option to existing decisions, relevant commits, and measurements. For a messaging-system change, ask it to compare two viable services against the project's residency requirement and a representative workload, using a small prototype when evidence is missing. The resulting record should make the chosen tradeoff and its evidence easy to revisit; an agent can then keep the implementation, validation checks, and supersession links consistent as that decision evolves.

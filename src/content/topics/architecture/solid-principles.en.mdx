...
Generators often split every method into a `Service`, `Manager`, and `Repository`, then create a one-to-one `IThing` interface for a class with one implementation; there are more files, but the reasons to change remain tangled. They also generate implementations that match a TypeScript shape but throw `NotSupported`, return `null`, or change error semantics for one customer type. Another recurring failure copies a database SDK's query objects into a business-layer interface, so constructor injection exists while dependency inversion does not.

**Ask your AI to check**

- “List this module's change actors, and use real callers or commit evidence to explain which behavior should change together.”
- “Write a base-type contract suite covering the valid input range, postconditions, invariants, and allowed failures, then run it against every implementation.”
- “Produce an import graph from business modules to adapters and mark every low-level vendor type that leaks into a high-level interface.”

**Review checklist**

- For every new interface, confirm a real client need, an explicit contract, and a reason based on independent change or substitution.
- Run shared contract tests against every implementation without changing the caller, including boundary inputs and error paths.
- Inspect imports, constructor parameters, and exception types in high-level modules for concrete database or SDK types crossing the boundary.

**Prompt vocabulary**

Use the exact terms `reason to change`, `actor`, `extension point`, `behavioral subtyping`, `precondition`, `postcondition`, `invariant`, `role interface`, `source-code dependency`, and `composition root`. Do not ask only to “apply SOLID.” Name the caller, axis of change, and observable contract that must be protected.

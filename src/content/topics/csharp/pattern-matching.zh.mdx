---
title: 模式匹配
description: 用 C# 模式匹配安全地检查运行时类型与数据形状，并掌握分支顺序、空值、穷尽性和列表模式边界。
track: csharp
section: types-linq
...
<TryToBreak items={['传入空字符串', '传入金额为负数的订单', '传入一个普通 object 实例']} />
...
> [!PITFALL]
> 属性模式会读取成员，子模式的检查顺序不应成为业务依赖。getter 若执行 I/O、推进游标或修改状态，匹配结果和调用次数就难以预测，异常也会从 getter 直接传播。
...
> [!PITFALL]
> `IEnumerable<T>` 只承诺枚举，不满足列表模式需要的计数和索引协议。列表模式也检查固定位置与长度，不会像 `Contains` 或 LINQ 查询那样搜索任意位置。
...
模型会把 `>= 18 and < 65` 生成成几乎恒真的 `>= 18 or < 65`，把一般类型分支放在具体属性分支之前，或添加 `_` 来消除警告却吞掉未知消息。它还常把列表模式用于只有 `IEnumerable<T>` 的输入，遗漏 `null` 和未定义枚举数值，或者把模式匹配写进 `Expression<Func<T, bool>>`；当前 C# 表达式树不支持这种语法，代码会在编译期失败。

**让 AI 检查**

- 为每个关系与逻辑模式生成边界真值表，并给出能进入每个分支的最小输入。
- 按文本顺序列出各 `switch` 分支覆盖的值集合，标出遮蔽、空值、未知运行时类型和未定义枚举值。
- 在 C# 14 下编译，并分别测试空数组、单元素数组、额外元素与只有枚举能力的输入类型。

**审查清单**

- 检查窄模式是否位于宽模式之前，且 `when` 方法无副作用并有真假两条测试路径。
- 确认 `null`、未知类型、非法状态组合和未定义枚举数值都有刻意设计的结果。
- 核对列表输入确实可计数、可索引；若先物化流，检查大小上限与单遍消费契约。

**提示词词汇**

使用准确术语：`declaration pattern`（声明模式）、`property pattern`（属性模式）、`positional pattern`（位置模式）、`relational pattern`（关系模式）、`logical pattern`（逻辑模式）、`list pattern`（列表模式）、`slice pattern`（切片模式）、`case guard`（分支守卫）、`subsumption`（涵盖关系）和 `exhaustiveness`（穷尽性）。要求模型输出分支覆盖表和边界反例；只说「简化这个 switch」不足以暴露遮蔽与失败路径。

...
Models tend to generate a happy path that compiles while missing intermediate expression types and control-flow boundaries. Typical results include doing integer division before assigning to `double`, using `var` to hide an unintended inference, reading an output after `TryParse` fails, treating `!` as a null check, and writing loops that fail on empty arrays or the final index. They also call a side-effecting method twice—once in a condition and once for the result—so validation and use can observe different states.

**Ask your AI to check**

- List the operand type, result type, conversion point, and overflow behavior of every arithmetic subexpression, not only the destination variable's type.
- Draw the reachable paths through each `if`, `switch`, and loop, including zero iterations and every early `continue`, `break`, `return`, and invalid input.
- Find all external inputs, `!` operators, casts, and repeated method calls, then propose tests that trigger null, malformed text, boundary values, and side effects.

**Review checklist**

- Run the code with empty input, one item, maximum values, malformed text, and an unknown case; confirm that every path has a defined result.
- Inspect assignments from right to left and confirm that intermediate expressions don't truncate, overflow, or run twice before conversion to the destination type.
- Compare method signatures with call sites and confirm that parameter ranges, failure modes, return states, and ownership aren't hidden only in comments.

**Prompt vocabulary**

Use the exact terms `static type`, `type inference`, `expression type`, `statement`, `definite assignment`, `short-circuit evaluation`, `control-flow path`, `loop invariant`, `entry point`, and `method contract`. Ask for a per-expression type table and a per-path test matrix; “check the basic syntax” is too vague to expose code that compiles but calculates the wrong result.

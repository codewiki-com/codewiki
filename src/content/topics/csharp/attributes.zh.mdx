---
title: C# 特性
description: 理解 C# 特性如何把声明式信息写入元数据，掌握目标、参数、继承与反射读取规则，并避开生成代码中的错误假设。
track: csharp
section: types-linq
...
<TLDR>
  ...
  <TLDRCell label="trap">
    写上 `[Something]` 不会自动执行行为；目标选错、继承查询不一致或用单值 API 读取多用特性，都会让元数据看似存在却不起作用。
  </TLDRCell>
  <TLDRCell label="fix">
...
特性信息会进入程序集元数据，而不是像注释那样在编译后消失。代码可以通过<Term id="reflection">反射（reflection）</Term>读取它，编译器、测试运行器、序列化器和 Web 框架也可以成为消费者。特性只描述事实或意图，真正的效果来自读取并执行规则的消费者。
...
`inherit` 参数和 `AttributeUsage.Inherited` 共同影响继承查询。<Term id="attribute-inheritance">特性继承（attribute inheritance）</Term>不是把记录复制到派生声明；它是反射 API 在查询时沿基类或重写链补充结果的规则。直接元数据读取仍会显示记录实际存放的位置。
...
模型会根据名字发明一个看似合理但目标框架从未读取的特性，或导入同名但属于错误命名空间的类型。它还常把属性目标与幕后字段目标混淆，在 `AllowMultiple` 开启后继续使用单值读取，并假设 `inherit: true` 会遍历接口和所有成员种类。扫描器代码则容易在请求热路径上反复反射，或让特性构造函数偷偷访问服务。

**让 AI 检查**

- 为每个特性列出完整类型名、允许目标、目标框架中的消费者，以及该消费者实际使用的版本化 API。
- 对每个反射查询说明它读取的元数据实体、是否包含继承结果，以及零个、一个和多个匹配项的行为。
- 区分 `GetCustomAttributes` 的实例化路径与 `CustomAttributeData` 的元数据路径，并标出构造函数、命名 setter 和外部 I/O 的执行点。

**审查清单**

- 通过真实框架入口测试特性产生的行为，不要只断言方括号或反射记录存在。
- 对自动属性、参数、返回值和重写成员，确认特性附着位置与消费者查询位置一致。
- 用缺失、重复、继承和非法配置用例检查消费者，并确认扫描不在每次请求中重复执行。

**提示词词汇**

使用准确术语：`attribute class`（特性类）、`attribute target`（特性目标）、`positional argument`（位置实参）、`named argument`（命名实参）、`AttributeUsage`（特性使用契约）、`AllowMultiple`（允许多次应用）、`Inherited`（允许继承查询）、`attribute instantiation`（特性实例化）和 `CustomAttributeData`（自定义特性元数据）。要求 AI 给出“声明—元数据实体—消费者 API—可观察行为”映射，比“给它加个 annotation”更容易暴露缺失的执行链。
...
直接调用 `GetCustomAttribute<T>()` 则要求运行时加载特性类型并创建对象。随后，消费者得到方便的强类型属性，但也跨过了执行代码的边界。面对不受信任或依赖不完整的程序集时，这个区别是架构选择，不只是 API 风格。

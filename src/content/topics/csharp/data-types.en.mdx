...
Models often repeat the "struct on stack, class on heap" slogan and then change a class into a struct, introducing silent copies and changing identity semantics. When fixing nullable warnings, they tend to add `!` in bulk instead of tracing null state from deserialization, database, or API boundaries. Money code also commonly parses through `double` before converting to cents, or attempts to unbox a numeric object directly as a different numeric type.

**Ask your AI to check**

- List whether every assignment, argument pass, and collection read copies a complete value or an object reference, and identify all mutable structs.
- Trace nullable flow from external input, explain the validated invariant behind each `!`, and name the test that covers the `null` path.
- Expand the intermediate type, `checked` boundary, and culture of each numeric expression, distinguishing numeric conversion from unboxing.

**Review checklist**

- Mutate through two aliases or two copies and confirm that sharing or independence matches the domain model.
- Test every parsing and conversion boundary with `null`, malformed text, culture differences, and boundary numbers.
- Follow paths through `object`, interface conversions, and nongeneric APIs to confirm boxing and the exact unboxing type.

**Prompt vocabulary**

Use the exact terms `value type`, `reference type`, `copy semantics`, `reference identity`, `nullable value type`, `nullable reference type`, `null-state analysis`, `boxing`, `unboxing`, `narrowing conversion`, and `checked context`. Ask the model for the intermediate type of a specific expression. "Check type safety" is too vague to expose an integer that overflows early or an unboxing type mismatch.

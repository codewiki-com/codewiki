...
模型很爱给每个方法套一层 `try/catch (Exception)`，记录 `Message` 后又写 `throw ex;`，于是同一次失败被重复记录，堆栈还被截短。它也常把 `decimal.Parse`、字典索引和文件探测写进异常驱动的正常分支，或者在 `finally` 中执行会失败的远程日志调用，覆盖原异常。处理异步代码时，生成结果经常只查看 `await Task.WhenAll` 抛出的一个异常，漏掉组合任务中其余失败。

**让 AI 检查**

- 列出每个 `catch` 能匹配的异常类型，并说明处理器采取了什么恢复动作；标出只记录后继续或返回默认值的分支。
- 找出所有 `throw ex`、没有 `innerException` 的包装，以及依赖 `Message` 文本的判断，并给出保留堆栈与结构化信息的改法。
- 对每个 `async` 调用绘制任务所有权，区分失败、调用方取消和超时，并说明多个并发任务失败时哪里保存全部异常。

**审查清单**

- 用成功、已知可恢复失败、未知失败和取消分别走过边界，确认只有该边界负责的一类被消费。
- 检查资源在正常返回与异常路径上都释放，并故意让清理失败一次，确认它不会悄悄覆盖主要诊断。
- 从最终日志或响应追溯到原始抛出方法，确认异常类型、内部异常和堆栈仍然存在，而且没有泄露敏感值。

**提示词词汇**

使用准确术语：`exception propagation`（异常传播）、`stack unwinding`（堆栈展开）、`exception filter`（异常筛选器）、`catch boundary`（捕获边界）、`bare rethrow`（无操作数重新抛出）、`stack trace preservation`（堆栈跟踪保留）、`inner exception`（内部异常）、`exception aggregation`（异常聚合）和 `cancellation semantics`（取消语义）。要求模型逐个说明处理器为何能恢复；“加上错误处理”通常只会得到更宽的 `catch`。

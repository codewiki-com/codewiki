...
A model may notice the covariance of `IEnumerable<Dog>` and then generate an invalid assignment from `List<Dog>` to `List<Animal>`. It may use `dynamic` to make `T + T` compile, or add the broad `INumber<T>` constraint when only one operator is required. Other generated code returns `default(T)` for a cache miss and conflates a valid `0` with failure. Another common mistake adds `where T : new()` to construct entities with missing dependencies, while assuming `notnull` performed runtime validation.

**Ask your AI to check**

- List every type parameter position in parameters, returns, fields, and delegates, then state whether it should be invariant, covariant, or contravariant.
- For each `where` constraint, identify the exact member used by the implementation and the statement that would fail if the constraint were removed.
- Trace every `default(T)`, `object` cast, boxing conversion, and use of `dynamic`, including how each failure path is reported to the caller.

**Review checklist**

- Exercise generic boundaries with a value type, reference type, nullable input, and empty sequence.
- Confirm that comparers, equality, and variance direction come from the public contract rather than a concrete type that happens to compile.
- Check type arguments supplied by reflection construction, dependency injection registration, and serialization code against both constraints and runtime value validation.

**Prompt vocabulary**

Use the exact terms `generic`, `type parameter`, `type argument`, `constructed type`, `generic constraint`, `method type inference`, `invariance`, `covariance`, `contravariance`, `variance annotation`, `static abstract interface member`, and `allows ref struct anti-constraint`. Ask for a type-position table and a constraint-usage table. “Make it more generic” doesn't identify a safe API.

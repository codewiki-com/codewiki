...
**What generated code gets wrong here.** Models often treat `Span<T>` as storable and emit a local that crosses `await` or a class field. They also call `ToArray()` or `ToString()` after every slice, quietly restoring the allocations the refactor was meant to remove. The dangerous variant rents an array, returns `Memory<T>`, and immediately gives the array back, or exposes the entire pooled capacity when the caller should see only `written` bytes.

**Ask your AI to check.** Ask it to label the backing owner, valid range, and last use of every view. Ask it to list each materialization call and justify whether that copy establishes a necessary ownership boundary. Ask it to inspect every `await`, `yield`, pool return, and `Dispose()` for aliases that remain live afterward.

**Review checklist.**

- Every `Memory<T>` or span traces to a live owner, and valid data length is never confused with capacity.
- No `Span<T>` crosses a suspension point, is captured, or enters an ordinary heap object; pooled memory isn't returned before its last consumer finishes.
- `ReadOnly*` means read-only access through this view only. Code that needs a stable snapshot copies explicitly and explains why.

**Prompt vocabulary.** Use `backing storage`, `aliasing`, `ref safety`, `escape analysis`, `owner / consumer / lease`, and `valid data length versus capacity` to describe the constraints. When asking an AI to change a signature, say `Span<T> for synchronous access` and `Memory<T> across await` precisely.

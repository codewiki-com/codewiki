---
title: 记录类型
description: 用 C# 记录类与记录结构体建模数据，并明确处理相等性、复制、可变性与继承行为。
track: csharp
section: types-linq
...
<Term id="init-accessor">`init` 访问器（init accessor）</Term>允许在对象构造阶段和 `with` 初始化器中赋值，之后拒绝普通赋值。这只提供浅层不可变性。若一个 `init`-only 属性指向 `List<T>`，调用方在构造后仍能修改该列表。
...
<TryToBreak items={['空 SKU', '数量为零', '两个只有字母大小写不同的值']} />
...
模型常把 DTO 改成记录类型后直接称其不可变，同时继续暴露 `List<T>` 成员。它会把 `with` 当成对象图的递归克隆，还会生成 `Normalized = Query.Trim()` 一类缓存属性，使其在后续副本修改 `Query` 后过期。对于缓存键，生成的记录还经常遗漏租户或授权范围，使用按引用比较的数组，或者加入能在插入后改变哈希的公开 setter。

生成式重构还会把实体类改成记录类型，却不检查调用方是否依赖引用身份。另一个常见错误是为了所谓的分配优化，把 `record` 改成 `record struct`，同时忽略全零无效值、字段复制、装箱与普通位置属性自带的 setter。这些决定都不能只靠语法得出。

**让 AI 检查**

- 列出参与相等与哈希的每个输入，并说明各成员类型实际采用的相等行为和可变性。
- 沿 `with` 操作向下跟踪一层引用，标出共享对象、替换成员、复制的缓存值，以及不会重新执行的验证。
- 找出所有字典键、ORM 实体、消息基类与结构体默认值用法，并为身份、查找稳定性、未知派生类型和零值提出测试。

**审查清单**

- 验证两个独立构造的逻辑值是否按预期相等，覆盖数组、列表、大小写、规范化与派生运行时类型。
- 复制后修改每个可达引用，确认共享或隔离行为符合所有权契约。
- 对记录结构体执行 `default`，并在所有允许的修改后执行哈希查找；拒绝任何会让键漂移的状态。

**提示词词汇**

使用准确术语：`record class`（记录类）、`record struct`（记录结构体）、`readonly record struct`（只读记录结构体）、`value equality`（值相等性）、`reference identity`（引用身份）、`init-only property`（仅初始化属性）、`with expression`（`with` 表达式）、`shallow copy`（浅拷贝）、`copy constructor`（复制构造函数）、`EqualityContract` 与 `hash stability`（哈希稳定性）。要求模型给出相等成员表和对象所有权图；「用记录把它变成不可变」没有说明真正重要的决定。

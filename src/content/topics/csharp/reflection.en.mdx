...
Models often generate `GetMethod(name)!` without parameter types or handling inherited namesakes. Generated object mappers then equate `CanWrite` with a public setter and ask `Convert.ChangeType` to guess nullable, enum, and culture-specific conversions; the sample works for strings and breaks on the real model. Plugin scanners frequently omit abstract types, open generics, unavailable constructors, duplicate keys, and `ReflectionTypeLoadException`, then rescan the assembly on every request.

Another common failure arrives at publication time. Generated code builds type or member names from strings while the project enables trimming or Native AOT; the model then adds `UnconditionalSuppressMessage` without proving that the members remain rooted. When dynamic invocation fails, it may log only the outer `TargetInvocationException` message, making the actual application exception disappear from diagnostics.

**Ask your AI to check**

- List the declaring type, name, full signature, and `BindingFlags` for every member lookup, and explain the failure for zero or multiple matches.
- Trace every string from configuration, network input, or a plugin manifest, showing the allowlist it crosses before reaching `Type`, `MemberInfo`, `Activator`, or `Invoke`.
- Run the project's actual trimmed or Native AOT publish command, examine every warning, and provide the member-retention evidence for each suppression.

**Review checklist**

- Test resolution and binding with overloads, inherited members, a non-public setter, `null`, and an argument of the wrong type.
- Confirm that target exceptions retain their original type and stack, while reflection infrastructure errors are classified separately.
- Check scan and cache lifetimes, concurrent access, capacity bounds, and behavior in the trimmed publish artifact.

**Prompt vocabulary**

Use precise terms: `runtime type`, `metadata`, `MemberInfo`, `BindingFlags`, `overload resolution`, `late binding`, `TargetInvocationException`, `trim analysis`, and `Native AOT`. Ask the model to output candidate sets, selection rules, and failure phases; “invoke it dynamically with reflection” doesn't define a safe contract.

...
模型在重构属性时，经常把验证 setter 换成自动属性，让非法状态重新变得可写；也会生成 `Name = value` 这种自递归访问器。为消除可空警告，它们可能同时添加 `required` 和 `null!`，表面上安静了，却没有建立任何运行时不变量。另一个真实故障是把 `private set` 或 `init` 误称为“不可变”，同时公开 `List<T>`、数组或可变子对象。

**让 AI 检查**

- 列出每个属性访问器的读取、写入、抛出、分配与 I/O，并检查是否直接或间接再次访问同一属性。
- 沿构造函数、对象初始化器、反射与实际序列化路径验证每个 `required` 成员，分别说明编译时保证与运行时验证。
- 标出所有 `init`、私有 setter 和只读属性可到达的可变对象，并说明谁可以修改它们。

**审查清单**

- 用有效值、边界值、`null`、空白值和失败后再次读取来测试自定义 setter。
- 对 getter 计数并重复读取，确认没有隐藏的网络、磁盘、时间依赖或意外状态变化。
- 尝试从公开属性修改集合及其元素，确认实际所有权符合 API 声明。

**提示词词汇**

使用准确术语：`property`（属性）、`property accessor`（属性访问器）、`backing field`（幕后字段）、`auto-implemented property`（自动实现的属性）、`field-backed property`（字段支持属性）、`init accessor`（`init` 访问器）、`required member`（必需成员）、`object initializer`（对象初始化器）、`computed property`（计算属性）和 `deep immutability`（深度不可变）。要求 AI 给出写入路径与对象所有权图；“让模型不可变”不足以说明调用方还能修改哪些引用对象。

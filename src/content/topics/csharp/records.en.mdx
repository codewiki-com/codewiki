...
Models often turn a DTO into a record and call the result immutable while leaving `List<T>` members exposed. They use `with` as if it recursively cloned an object graph, or generate a cached property initializer such as `Normalized = Query.Trim()` that becomes stale when `Query` changes in a later copy. For cache keys, generated records frequently omit tenant or authorization scope, use arrays whose equality is reference-based, or add a public setter that can change the hash after insertion.

Generated refactors also switch a class entity to a record without checking whether callers depend on reference identity. Another common mistake is changing `record` to `record struct` as a supposed allocation optimization, then overlooking zero-initialized invalid values, field copying, boxing, and the ordinary positional setters. None of those decisions follows from syntax alone.

**Ask your AI to check**

- List every equality and hash-code input, then state the actual equality behavior and mutability of each member type.
- Trace a `with` operation one reference edge deep, marking shared objects, replaced members, copied cached values, and validations that do not rerun.
- Find every use as a dictionary key, ORM entity, message base, or struct default, and propose tests for identity, lookup stability, unknown derived types, and zero values.

**Review checklist**

- Verify that two independently constructed logical values compare as intended, including arrays, lists, casing, normalization, and derived runtime types.
- Mutate every reachable reference after copying and confirm that sharing or isolation matches the ownership contract.
- Exercise `default` for record structs and perform hash lookup after all permitted mutations; reject any state that makes the key drift.

**Prompt vocabulary**

Use the exact terms `record class`, `record struct`, `readonly record struct`, `value equality`, `reference identity`, `init-only property`, `with expression`, `shallow copy`, `copy constructor`, `EqualityContract`, and `hash stability`. Ask for an equality-member table and an object-ownership diagram; "make this immutable with records" leaves the important decisions unspecified.

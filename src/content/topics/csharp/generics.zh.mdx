---
title: 泛型
description: 用类型参数表达可复用且类型安全的 C# API，掌握约束、类型推断、变体与运行时构造泛型的真实边界。
track: csharp
section: types-linq
...
<TryToBreak items={['空数组', '包含重复值的数组', '目标值为 null']} />
...
> [!PITFALL]
> 约束越多不代表 API 越安全。多余的 `class`、`IComparable<T>` 或 `new()` 会拒绝本可正常工作的类型，还可能让调用方误以为实现依赖这些能力。
...
模型会看到 `IEnumerable<Dog>` 的协变转换，随后错误地生成 `List<Dog>` 到 `List<Animal>` 的赋值；也会用 `dynamic` 让 `T + T` 通过编译，或加上宽泛的 `INumber<T>` 约束却只需要一个运算符。另一些生成结果用 `default(T)` 表示缓存未命中，把合法的 `0` 与失败混在一起；还有代码用 `where T : new()` 直接创建依赖不完整的实体，并误以为 `notnull` 已完成运行时校验。

**让 AI 检查**

- 列出每个类型参数在参数、返回值、字段和委托中的位置，并说明它应当不变、协变还是逆变。
- 为每个 `where` 约束指出实现中实际使用的成员，并给出删除该约束后会失败的具体语句。
- 跟踪所有 `default(T)`、`object` 强制转换、装箱和 `dynamic`，说明每条失败路径怎样向调用方报告。

**审查清单**

- 用值类型、引用类型、可空输入和空序列分别测试泛型边界。
- 确认比较器、相等规则与变体方向来自公开契约，而不是碰巧通过编译的具体类型。
- 检查反射构造、依赖注入注册与序列化代码传入的类型实参是否满足约束，并验证实际值。

**提示词词汇**

使用准确术语：`generic`（泛型）、`type parameter`（类型参数）、`type argument`（类型实参）、`constructed type`（构造类型）、`generic constraint`（泛型约束）、`method type inference`（方法类型推断）、`invariance`（不变）、`covariance`（协变）、`contravariance`（逆变）、`variance annotation`（变体标注）、`static abstract interface member`（静态抽象接口成员）、`allows ref struct anti-constraint`（`allows ref struct` 反约束）。要求模型输出类型位置表和约束使用表；只说「让它更通用」无法确定安全的 API。

...
模型容易生成“能编译的主路径”，却漏掉中间表达式类型和控制流边界。常见结果包括先做整数除法再赋给 `double`、用 `var` 掩盖意外推断、在 `TryParse` 失败后仍读取输出值、把 `!` 当成空值检查，以及写出对空数组或最后一个索引失败的循环。它还可能重复调用有副作用的方法，一次用于条件、一次用于结果，从而让检查与实际使用看到不同状态。

**让 AI 检查**

- 列出每个算术子表达式的操作数类型、结果类型、转换位置和溢出行为，而不只报告最终变量类型。
- 为每个 `if`、`switch` 和循环画出可达路径，标明零次迭代、提前 `continue`、`break`、`return` 与失败输入。
- 找出所有外部输入、`!`、强制转换和重复方法调用，并给出能触发空值、格式错误、边界值与副作用的测试。

**审查清单**

- 使用空输入、单项输入、最大值、无效文本和未知分支运行代码，确认每条路径都有定义明确的结果。
- 从右向左检查赋值，确认中间表达式在转换到目标类型之前没有截断、溢出或重复求值。
- 核对方法签名与调用点，确认参数范围、失败方式、返回状态和所有权没有只藏在注释中。

**提示词词汇**

使用准确术语：`static type`（静态类型）、`type inference`（类型推断）、`expression type`（表达式类型）、`statement`（语句）、`definite assignment`（明确赋值）、`short-circuit evaluation`（短路求值）、`control-flow path`（控制流路径）、`loop invariant`（循环不变量）、`entry point`（入口点）和 `method contract`（方法契约）。要求 AI 给出逐表达式类型表与逐路径测试矩阵；“检查基础语法”不足以暴露能编译但结果错误的代码。

...
Models produce the nearly-always-true `>= 18 or < 65` instead of `>= 18 and < 65`, place a general type arm before a specific property arm, or add `_` to silence a warning while swallowing unknown messages. They also apply a list pattern to an input typed only as `IEnumerable<T>`, omit `null` and undefined enum values, or put pattern matching inside an `Expression<Func<T, bool>>`; current C# expression trees do not support that syntax, so the code fails at compile time.

**Ask your AI to check**

- Generate a boundary truth table for every relational and logical pattern, with the smallest input that enters each arm.
- List the value set covered by every `switch` arm in text order, marking shadowing, null, unknown runtime types, and undefined enum values.
- Compile under C# 14 and separately test an empty array, a one-element array, extra elements, and an input type that supports only enumeration.

**Review checklist**

- Check that narrow patterns precede broad ones and that every `when` method is side-effect free with both true and false test paths.
- Confirm deliberate outcomes for `null`, unknown types, illegal state combinations, and undefined enum values.
- Verify that list input is truly countable and indexable; if a stream is materialized first, check its size limit and single-pass contract.

**Prompt vocabulary**

Use the exact terms `declaration pattern`, `property pattern`, `positional pattern`, `relational pattern`, `logical pattern`, `list pattern`, `slice pattern`, `case guard`, `subsumption`, and `exhaustiveness`. Ask for an arm-coverage table and boundary counterexamples; “simplify this switch” is too vague to expose shadowing and failure paths.

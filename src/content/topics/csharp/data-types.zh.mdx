---
title: C# 数据类型
description: 理解 C# 值类型与引用类型的复制语义、可空性、数值转换和装箱规则，并避开生成代码中常见的类型错误。
track: csharp
section: basics
...
C# 的类型系统是统一的：任何值都可以作为 `object` 处理。引用类型只需复制引用；值类型则通常需要<Term id="boxing">装箱（boxing）</Term>，也就是创建一个包含该值的对象。统一模型让非泛型 API 可以接收任意值，但复制和分配成本不会因此消失。
...
对于引用类型，<Term id="nullable-reference-type">可空引用类型（nullable reference type）</Term>注解表达 API 的空值意图。`string` 表示调用方不应提供 `null`，`string?` 表示 `null` 是预期状态。两者在运行时仍是相同的引用类型；编译器依靠流分析发出警告，并不会自动插入运行时检查。
...
模型经常复述“结构体在栈、类在堆”的口诀，然后据此把类改成结构体，造成静默副本和身份语义变化。处理可空警告时，它们常批量插入 `!`，而不是追踪值从反序列化、数据库或 API 边界进入后的空状态。金额代码也常先用 `double` 解析再转成分，或者在装箱值上直接尝试另一种数值类型的拆箱。

**让 AI 检查**

- 列出每次赋值、参数传递和集合读取是在复制完整值还是复制对象引用，并标出所有可变结构体。
- 从外部输入追踪 nullable flow，说明每个 `!` 依赖什么已验证的不变量，以及哪个测试覆盖 `null` 路径。
- 展开每个数值表达式的中间类型、`checked` 边界和区域性，并区分数值转换与拆箱。

**审查清单**

- 用两个别名或两个副本执行一次修改，确认共享与独立行为符合领域模型。
- 用 `null`、格式错误、区域性差异和边界数值测试每个解析与转换入口。
- 搜索到 `object`、接口转换和非泛型 API 的路径，确认装箱以及准确的拆箱类型。

**提示词词汇**

使用准确术语：`value type`（值类型）、`reference type`（引用类型）、`copy semantics`（复制语义）、`reference identity`（引用标识）、`nullable value type`（可空值类型）、`nullable reference type`（可空引用类型）、`null-state analysis`（空状态分析）、`boxing`（装箱）、`unboxing`（拆箱）、`narrowing conversion`（缩小转换）和 `checked context`（已检查上下文）。要求模型给出具体表达式的中间类型；“检查类型安全”不足以暴露整数先溢出或拆箱类型不匹配。

---
title: 集合
description: 根据访问模式、相等性、顺序与并发需求选择 C# 集合，并避开可变键、只读视图和复合操作中的错误。
track: csharp
section: types-linq
...
| 始终按比较器排序 | `SortedDictionary<TKey, TValue>` / `SortedSet<T>` | 查找和更新通常是 O(log n) |
| 已知节点附近频繁插入 | `LinkedList<T>` | 找到节点本身仍是 O(n) |
...
`List<T>` 以连续存储支持 O(1) 索引访问。尾部还有容量时，`Add` 只写入新位置；容量不足时需要分配更大的存储并复制元素，所以连续追加的单次成本会波动，但整体具有<Term id="amortized-complexity">摊还 O(1)（amortized O(1)）</Term>成本。

`Dictionary<TKey, TValue>` 与 `HashSet<T>` 建立在<Term id="hash-table">哈希表（hash table）</Term>上。它们先用哈希码缩小候选范围，再以相等性确认命中。查找通常接近 O(1)，但它依赖比较器质量、负载和输入，不能当成对所有一次调用的最坏情况保证。
...
字典与哈希集合使用 `IEqualityComparer<T>`。未显式提供时，它们使用 `EqualityComparer<T>.Default`，后者依据类型的相等性实现工作。领域规则不同，例如 SKU 不区分大小写时，应在构造集合时传入明确的<Term id="equality-comparer">相等比较器（equality comparer）</Term>。
...
商品先进入 `List<Product>`，因为录入顺序有意义。字典提供按 SKU 的第二条访问路径，并在构造时固定大小写规则。
...
模型会看到“中间插入 O(1)”就把 `List<T>` 改成 `LinkedList<T>`，却在每次插入前调用 O(n) 的 `Find`。它们也常生成 `ContainsKey` 加索引器的双重查找，把可变记录直接当作哈希键，或者返回由私有 `List<T>` 支撑的 `IReadOnlyList<T>` 并声称结果不可变。并发代码中，模型尤其容易假设 `GetOrAdd` 的工厂只运行一次，从而重复发送消息、创建资源或写审计记录。

**让 AI 检查**

- 为每个集合列出最高频的读取、插入、删除与排序路径，并给出包含查找节点在内的完整复杂度。
- 标出每个字典和集合使用的相等比较器，以及键进入集合后仍可变化的所有字段。
- 区分每个返回集合是实时视图、浅快照还是不可变值，并指出所有跨线程复合操作与可重复执行的工厂。

**审查清单**

- 用重复键、大小写变体、缺失键和键字段突变测试所有哈希集合边界。
- 在提供方修改底层数据后再次读取返回值，验证调用方得到的可见性符合 API 契约。
- 对并发复合操作施加竞争，统计工厂执行次数，并确认副作用与不变量仍然正确。

**提示词词汇**

使用准确术语：`access pattern`（访问模式）、`amortized complexity`（摊还复杂度）、`equality comparer`（相等比较器）、`hash code stability`（哈希码稳定性）、`enumeration order`（枚举顺序）、`read-only view`（只读视图）、`immutable snapshot`（不可变快照）、`structural modification`（结构修改）和 `atomic compound operation`（原子复合操作）。不要只要求“选择最快的集合”；应给出操作频率、顺序要求、数据所有权与并发边界。
...
扩容倍率属于运行时实现细节，不能写进业务正确性。稳定的结论是：不扩容的尾部添加通常是 O(1)，扩容那次是 O(n)，一串追加操作按<Term id="amortized-complexity">摊还复杂度（amortized complexity）</Term>分析。已知近似元素数时，可以在构造函数中传入容量或调用 `EnsureCapacity`，减少复制次数。

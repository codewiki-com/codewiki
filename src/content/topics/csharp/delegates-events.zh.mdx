...
模型会生成不存在的 `Func<void>`，或者为了“保持事件模式”把必须等待的 I/O 放进 `async void` 处理器。另一种常见错误是直接调用多播 `Func<Task>` 并只等待返回的最后一个任务，前面处理器的失败因此无人观察。循环生成回调时，它还可能捕获同一个可变索引；清理代码则常用一段新 lambda 去移除旧 lambda，导致订阅者继续被保留。

**让 AI 检查**

- 列出每个委托的准确运行时类型、参数、返回值、目标对象和捕获变量。
- 对每条多播调用画出调用顺序，并说明某个处理器同步抛出或返回失败 `Task` 时，哪些后续工作会运行。
- 配对每个 `+=` 与对应的 `-=`，确认两边使用相同的命名方法或已保存委托实例。

**审查清单**

- 在循环结束后调用生成的全部回调，用不同输入确认每个回调捕获了预期值。
- 对多播委托分别测试零个、一个和多个目标，并让中间目标失败。
- 对返回 `Task` 的处理器确认发布方等待所有任务，并按契约传播取消与多个异常。

**提示词词汇**

使用准确术语：`delegate type`（委托类型）、`method group conversion`（方法组转换）、`invocation list`（调用列表）、`delegate identity`（委托身份）、`captured variable`（捕获变量）、`multicast return value`（多播返回值）、`async void handler`（`async void` 处理器）、`event accessor`（事件访问器）。要求 AI 给出订阅、调用、失败与移除的时间线；“检查回调”无法暴露任务观察和委托身份问题。

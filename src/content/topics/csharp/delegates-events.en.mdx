...
Models generate the nonexistent `Func<void>`, or preserve an "event pattern" by putting I/O that must finish into an `async void` handler. Another common bug invokes a multicast `Func<Task>` directly and awaits only its last task, leaving failures from earlier handlers unobserved. Generated callback loops may capture one mutable index, while cleanup code uses a new lambda to remove an old one and leaves the subscriber retained.

**Ask your AI to check**

- List each delegate's exact runtime type, parameters, return value, target object, and captured variables.
- Draw the order for every multicast call, then state what runs if one handler throws synchronously or returns a failed `Task`.
- Pair every `+=` with its `-=`, confirming both sides use the same named method or stored delegate instance.

**Review checklist**

- Invoke all generated callbacks after their loop ends, using distinct inputs to prove each callback captured the intended value.
- Exercise multicast delegates with zero, one, and several targets, including a failing middle target.
- For `Task`-returning handlers, confirm the publisher awaits every task and propagates cancellation and multiple failures according to its contract.

**Prompt vocabulary**

Use the exact terms `delegate type`, `method group conversion`, `invocation list`, `delegate identity`, `captured variable`, `multicast return value`, `async void handler`, and `event accessor`. Ask for a timeline covering subscription, invocation, failure, and removal. "Check the callbacks" is too vague to expose task-observation and delegate-identity bugs.

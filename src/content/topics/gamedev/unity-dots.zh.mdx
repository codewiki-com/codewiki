...
模型很容易拼接不同 DOTS 世代的语法，例如给 `ISystem` 生成旧版 `Entities.ForEach`，或继续使用已被 `LocalTransform` 取代的 `Translation`。它还常漏写 `partial`、把 `ScheduleParallel()` 的返回句柄丢掉、在 Job 中调用 `EntityManager`，或者创建 ECB 后既不交给 ECB 系统也不手动回放。

另一类错误更隐蔽：把 `[BurstCompile]` 当成已经成功编译的证明，向 Job 塞入托管字段；把 `[EntityIndexInQuery]` 用作便宜索引，却忽略它会计算查询基准索引的成本；用并行 ECB 的排序键表达业务顺序；在每帧用添加与移除组件实现短暂状态。代码可能通过普通 C# 编译，却在 Unity 源码生成、Jobs 安全检查或回放时失败。

**让 AI 检查**

- 列出每个系统和 Job 对组件的读写集合，画出 `state.Dependency` 的传入、返回与首次完成位置。
- 标出所有结构变更，说明它由哪个 ECB 记录、在哪个系统回放，以及结果最早在哪个系统可见。
- 按 Entities 1.4.8 与 Burst 1.8.30 文档核对每个 API，并列出阻止 Burst 编译的托管类型或调用。

**审查清单**

- 在 Jobs Debugger 与安全检查开启时运行边界场景，并确认没有未注册的读写冲突。
- 检查所有 `Schedule` 与 `ScheduleParallel` 返回值、所有 ECB 的所有者及回放点。
- 在目标 Player 构建中确认 Burst 状态，并用相同工作负载比较帧时间、主线程等待和分配。

**提示词词汇**

使用准确术语：`archetype`（原型）、`chunk`（块）、`component access mode`（组件访问模式）、`job dependency`（Job 依赖）、`structural change`（结构变更）、`EntityCommandBuffer playback`（实体命令缓冲区回放）、`enableable component`（可启用组件）、`Burst-compatible HPC#`（兼容 Burst 的 HPC#）。提示中写出 `Unity 6.6`、`Entities 1.4.8` 和 `Burst 1.8.30`，比只说「写 DOTS 代码」更能避免版本拼接。

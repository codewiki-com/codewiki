...
Models often describe a coroutine as an "async thread," then put synchronous network calls or expensive loops inside it. They also generate `WaitForSeconds` for pause menus, start another flow on every click, or incorrectly claim that `enabled = false` stops coroutines automatically. Another frequent error is starting a child coroutine without yielding it, so the parent reports completion too early.

**Ask your AI to check**

- Identify the owner, retained handle, and stop path for every `StartCoroutine` call.
- Label every wait as scaled time, unscaled time, a frame boundary, or completion of an external operation.
- Trace nested coroutines and distinguish fire-and-forget startup from sequential `yield return StartCoroutine(...)`.

**Review checklist**

- Exercise the flow with `Time.timeScale = 0`, with the owner disabled, and after the owner is reactivated.
- Trigger the entry point twice in quick succession and verify that an old instance cannot overwrite newer state.
- Search between yields for blocking calls, unbounded loops, and expensive per-frame predicates.

**Prompt vocabulary**

Use the exact terms `iterator-based coroutine`, `yield instruction`, `scaled time`, `unscaled time`, `coroutine handle`, `owner lifecycle`, `replacement policy`, and `nested coroutine`. Ask for a timeline of startup, first yield, each resumption, and stopping; "make it run asynchronously" is too vague to expose incorrect assumptions.

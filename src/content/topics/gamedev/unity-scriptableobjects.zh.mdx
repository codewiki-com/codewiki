...
模型很容易把 ScriptableObject 当成带 Inspector 的全局单例，于是把当前生命值、积分或事件监听器直接写进资源。它还会用 `new` 创建实例，在运行时代码中调用 `AssetDatabase`，或把 `GetInstanceID()` 当作存档 ID。这些代码在短演示里看似连贯，一旦出现第二个消费者、Player 构建或跨启动存档，就会失效。

训练材料的版本差异会带来另一类错误。旧答案经常生成 `List<KeyValuePair>` 风格的字典替代物，却不检查 Unity 6.6 的原生字典支持；另一些答案又把新支持扩大到所有嵌套集合。模型也常把 `OnEnable` 解释为每次进入一局游戏都会运行一次，忽略 Fast Enter Play Mode 和资源实际加载边界。

**让 AI 检查**

- 为每个 ScriptableObject 字段标出作者数据、运行时状态或缓存，并追踪所有写入方。
- 找出 `new`、`CreateInstance`、`Instantiate` 和 `AssetDatabase.CreateAsset` 的每个调用，说明实例是否落盘以及由谁销毁。
- 按 Unity 6.6 规则检查字典、嵌套集合和 `[SerializeReference]`，再执行保存、重载与 Player 构建验证。

**审查清单**

- 给两个消费者同一资源，改变一个消费者的状态，确认另一个消费者和源资源都不变。
- 检查存档只包含稳定业务 ID 和值，不包含对象实例 ID、编辑器路径或直接资源对象。
- 在启用与禁用脚本域重载的配置下各跑一次 Play Mode 测试，并检查临时对象、缓存和订阅的清理。

**提示词词汇**

提示中使用准确术语：`authoring asset`（编辑期资源）、`shared asset reference`（共享资源引用）、`runtime state owner`（运行时状态拥有者）、`transient ScriptableObject`（临时 ScriptableObject）、`stable content ID`（稳定内容 ID）、`domain reload`（脚本域重载）、`serialized Dictionary`（序列化字典）。要求 AI 给出字段所有权表和创建/销毁时间线，比「用 ScriptableObject 重构」更容易暴露状态边界错误。

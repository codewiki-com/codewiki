...
Models readily turn a ScriptableObject into a global singleton with an Inspector, placing current health, score, or event listeners directly in the asset. They also construct instances with `new`, call `AssetDatabase` from runtime code, or use `GetInstanceID()` as a save ID. The code can look coherent in a small demo and then fail with a second consumer, a Player build, or a save loaded in another process.

Version drift in training material causes another class of mistakes. Older answers generate `List<KeyValuePair>`-style dictionary substitutes without checking Unity 6.6's native dictionary support; newer answers sometimes stretch that support to every nested collection. Models also describe `OnEnable` as a once-per-game-session hook, ignoring Fast Enter Play Mode and the asset's actual loading boundary.

**Ask your AI to check**

- Classify every ScriptableObject field as authoring data, runtime state, or cache, and trace every writer.
- Find every use of `new`, `CreateInstance`, `Instantiate`, and `AssetDatabase.CreateAsset`; state whether each object persists and who destroys it.
- Apply Unity 6.6 rules to dictionaries, nested collections, and `[SerializeReference]`, then run save-reload and Player-build checks.

**Review checklist**

- Give two consumers one asset, mutate one consumer's state, and verify that the other consumer and source asset stay unchanged.
- Check that save records contain stable domain IDs and values, not object instance IDs, editor paths, or direct asset objects.
- Run Play Mode tests with script-domain reload both enabled and disabled, checking cleanup of transient objects, caches, and subscriptions.

**Prompt vocabulary**

Use the exact terms `authoring asset`, `shared asset reference`, `runtime state owner`, `transient ScriptableObject`, `stable content ID`, `domain reload`, and `serialized Dictionary`. Ask for a field-ownership table and a creation/destruction timeline; "refactor this with ScriptableObjects" is too vague to expose state-boundary mistakes.

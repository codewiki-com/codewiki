---
title: Object 静态方法
description: 理清对象自有属性的枚举、浅拷贝、属性描述符与完整性级别，准确使用 JavaScript 的 Object 静态方法。
track: javascript
section: core
...
<TryToBreak items={['把 name 改为不可枚举属性', '增加第二个 Symbol 属性', '让对象直接拥有 inheritedRole']} />
...
> [!PITFALL]
> `Object.keys()` 会忽略不可枚举属性、Symbol 属性和继承属性。调试器能显示某个字段，并不表示该字段会出现在 `Object.keys()`、对象展开或 JSON 输出中。
...
> [!PITFALL]
> `Object.assign(defaults, input)` 把 `defaults` 当作目标并原地修改。模块级默认配置随后会携带前一次请求的数据，而且后来的来源还可能触发目标上的 setter。
...
> [!PITFALL]
> `Object.freeze()` 不会冻结嵌套对象，也不会让 getter 变成固定值。冻结访问器属性后，getter 和已有 setter 仍可运行并读写对象之外的状态。
...
模型经常生成 `Object.assign(DEFAULTS, input)`，无意中让请求共享被污染的默认配置；也会用
`{ ...input }` 加几次 `delete` 充当安全投影，使未来新增的凭据字段自动泄漏。另一类常见错误是对
不可信对象调用 `input.hasOwnProperty()`，或把 `Object.freeze({ ...value })` 描述成深度不可变。
当需求提到「完整复制」时，生成代码还常在不说明契约的情况下切换 `Object.assign()`、JSON 往返
与 `structuredClone()`，忽略原型、描述符、getter 副作用和支持的数据类型各不相同。

**让 AI 检查**

- 为每个枚举或复制操作列出「自有／继承、可枚举／不可枚举、字符串／Symbol」选择矩阵。
- 标出每个 `Object.assign()` 的目标对象、覆盖顺序、共享的嵌套引用，以及可能执行的 getter 或 setter。
- 用 `Object.getOwnPropertyDescriptors()` 和对象标识测试，验证冻结、复制与安全投影的实际边界。

**审查清单**

- 给输入加入继承键、不可枚举键、Symbol 键和名为 `hasOwnProperty` 的自有键，再检查结果。
- 在调用前后比较默认对象与嵌套对象的标识和值，确认没有跨请求修改或意外共享。
- 检查返回字段是否来自允许列表，并确认 getter、setter 与不可配置属性不会制造隐藏副作用。

**提示词词汇**

使用准确术语：`own property`（自有属性）、`inherited property`（继承属性）、
`enumerable property`（可枚举属性）、`symbol key`（Symbol 键）、`property descriptor`（属性描述符）、
`data property`（数据属性）、`accessor property`（访问器属性）、`shallow copy`（浅拷贝）和
`integrity level`（完整性级别）。要求模型输出键选择矩阵和对象标识关系，不要只说「安全地复制对象」。

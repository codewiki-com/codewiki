...
Generated date code commonly accepts locale-shaped input, checks `if (!date)`, and immediately calls `toISOString()`. It also mixes local constructors with UTC getters, adds a hard-coded offset to imitate a named zone, divides by `86_400_000` to count calendar days, or calls a mutating setter on an argument. Timer code may use `Date.now()` as though the system wall clock were monotonic.

**Ask your AI to check**

- State whether every value is an instant, calendar-only value, wall-clock time, named-zone schedule, or elapsed duration.
- Trace the input grammar, missing-offset behavior, invalid-date path, method family, and mutation ownership at every boundary.
- Test month ends and the relevant zone's daylight-saving transitions, then show both ISO output and elapsed milliseconds.

**Review checklist**

- Every external timestamp has an explicit contract and is rejected before an invalid value reaches storage or formatting.
- Each operation consistently uses UTC, the host local zone, or one named zone; no fixed offset pretends to be a zone.
- Calendar operations do not mutate caller-owned objects, and duration measurements do not assume the wall clock is monotonic.

**Prompt vocabulary**

Use `instant`, `epoch milliseconds`, `canonical ISO timestamp`, `UTC offset`, `named time zone`, `local calendar arithmetic`, `elapsed duration`, `DST gap`, `DST overlap`, `invalid Date`, and `monotonic clock`. These terms force the generated design to separate values that vague requests such as “add a day” collapse together.

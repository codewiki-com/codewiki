...
模型会在写入模块和读取模块中分别生成 `Symbol('id')`，只看描述就误判两者是同一键。它也会把 Symbol 属性称为“不可枚举”或“私有”，随后用 `Object.keys()` 做允许列表检查、用对象展开复制输入，于是未检查的可枚举 Symbol 键进入结果。另一类故障是给日志生成 `` `${key}` ``，当代理或反射代码真的收到 Symbol 键时，日志先抛错并改变原操作行为。

模型还会为了“跨模块一致”把所有键机械替换为 `Symbol.for()`，甚至把用户输入作为注册表键。这样既扩大了碰撞面，也没有解决 JSON 或消息传输；生成的测试若只覆盖字符串键，就看不到边界差异。

**让 AI 检查**

- 列出每个 Symbol 的创建位置、所有者和消费者，并说明消费者如何取得同一个标识。
- 为每次枚举、复制、合并和序列化列出它选择的自有／继承、可枚举／不可枚举、字符串／Symbol 键集合。
- 对每个内置 Symbol 钩子给出触发操作、允许的返回类型、状态所有权和失败测试。

**审查清单**

- 用描述相同但标识不同的两个 Symbol 测试键是否被错误重建。
- 在输入中加入可枚举 Symbol、不可枚举 Symbol 和继承字符串键，再检查复制与验证边界。
- 对 Symbol 键运行日志、异常路径、JSON 输出和反射代码，确认不会丢失检查或意外抛错。

**提示词词汇**

使用准确术语：`Symbol identity`（Symbol 标识）、`description`（描述）、`property key`（属性键）、`registered symbol`（注册 Symbol）、`global symbol registry`（全局 Symbol 注册表）、`well-known symbol`（内置 Symbol）、`enumerable own symbol key`（可枚举自有 Symbol 键）和 `protocol hook`（协议钩子）。要求 AI 输出键选择矩阵与标识所有权表；只说“用 Symbol 隐藏元数据”会掩盖可见性和边界问题。

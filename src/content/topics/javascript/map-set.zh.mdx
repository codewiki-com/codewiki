...
模型经常从接口中的 `{ id }` 临时构造对象键，误以为 `Map` 会按字段匹配；也会用 `get() || fallback` 抹掉零与 `false`，或用 `Set` 对对象数组做无效的内容去重。另一些生成结果会调用并不存在的 `map.map()`、`set.filter()`，重复实现 Node 24 已有的集合运算，或者把 `Map` 直接交给 `JSON.stringify()`。为了“防止内存泄漏”而机械换成 `WeakMap` 时，代码又常保留字符串键、需要枚举缓存，或依赖垃圾回收触发业务清理。

**让 AI 检查**

- 列出每个 `Map` 键和 `Set` 成员的规范表示，并说明相等依据是 SameValueZero、对象标识还是业务字段。
- 为每次 `get()` 标出缺失、明确的 `undefined`、零、`false` 和空字符串路径，再验证默认值操作符。
- 跟踪每个集合的迭代、序列化和生命周期需求，确认弱集合没有承担枚举、容量控制或确定性清理。

**审查清单**

- 用同一引用和同形新对象分别测试对象键，按业务 `id` 去重时再加入重复字段对象。
- 检查遍历中的所有 `set()`、`add()`、`delete()` 和 `clear()`，确认实时迭代不会漏项、重访或不终止。
- 在真实目标 Node 版本运行集合方法，并对每个跨进程边界写出可往返的显式线格式。

**提示词词汇**

使用准确术语：`keyed collection`（键控集合）、`SameValueZero`（同值零相等）、`object identity`（对象标识）、`insertion order`（插入顺序）、`set composition`（集合组合）、`set-like protocol`（类集合协议）、`weak key`（弱键）、`non-registered symbol`（未注册符号）和 `explicit serialization format`（显式序列化格式）。要求 AI 给出键规范与所有权表，比“用 `Map` 优化这段代码”更容易暴露语义错误。

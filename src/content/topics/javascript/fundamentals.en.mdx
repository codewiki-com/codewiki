...
terms: [lexical-environment, type-coercion, temporal-dead-zone, short-circuiting]
...
<Term id="short-circuiting">Short-circuit evaluation</Term> decides whether the right expression runs. `left && right` stops when the left side is falsy, `left || right` stops when it is truthy, and `left ?? right` evaluates the right side only when the left is `null` or `undefined`. The distinction matters for valid values such as `0`, `false`, and an empty string.

...
生成的日期代码常常接受区域格式输入，用 `if (!date)` 检查，随后立即调用 `toISOString()`。它还可能混用本地构造函数与 UTC getter，通过增加固定偏移来假装处理指定时区，用 `86_400_000` 做除数计算日历天数，或直接对参数调用可变 setter。计时器代码也可能把 `Date.now()` 当作单调递增的系统时钟。

**让 AI 检查**

- 说明每个值究竟是时间点、纯日历值、墙上时钟时间、指定时区日程，还是经过时长。
- 跟踪每个边界处的输入语法、缺少偏移时的行为、无效日期路径、方法家族和修改权限。
- 测试月末和相关时区的夏令时切换，并同时给出 ISO 输出与经过的毫秒数。

**审查清单**

- 每个外部时间戳都有明确契约；无效值进入存储或格式化流程前就会被拒绝。
- 每项操作始终使用 UTC、宿主本地时区或某个指定时区，不会用固定偏移冒充时区。
- 日历操作不会修改调用方所有的对象，时长测量也不会假设挂钟单调递增。

**提示词词汇**

使用准确术语：`instant`（时间点）、`epoch milliseconds`（纪元毫秒数）、`canonical ISO timestamp`（规范 ISO 时间戳）、`UTC offset`（UTC 偏移）、`named time zone`（指定时区）、`local calendar arithmetic`（本地日历计算）、`elapsed duration`（经过时长）、`DST gap`（夏令时空缺）、`DST overlap`（夏令时重叠）、`invalid Date`（无效 Date）和 `monotonic clock`（单调时钟）。这些术语会迫使生成的设计区分不同值，避免用“增加一天”这样的模糊要求把它们混为一谈。

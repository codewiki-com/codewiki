...
Models often infer an overly deep parameter pattern from one sample JSON object without protecting optional intermediate objects. They also mistake defaults for replacements for `null`. Another real failure is excluding the currently known password property with object rest and sending everything else to a log or response. Generated renames can treat the source key as a local variable, or an assignment expression can omit its required outer parentheses.

**Ask your AI to check**

- List every `undefined` and `null` case for each nested pattern, and identify which level throws first.
- Produce a trigger matrix for every default expression that includes missing, `undefined`, `null`, `0`, `false`, and an empty string.
- Trace the key set and nested references copied by every object rest property, and confirm that a new sensitive field cannot be forwarded automatically.

**Review checklist**

- Run boundary tests with a missing field, explicit `undefined`, explicit `null`, and the wrong type.
- Check every `{ sourceKey: localName }` mapping and confirm later code reads the binding that was actually created.
- Build public output from allowed fields and test whether mutating a nested value in the copy affects the source.

**Prompt vocabulary**

Use the exact terms `destructuring pattern`, `binding pattern`, `assignment pattern`, `default initializer`, `rest element`, `computed property key`, `iterator closing`, and `shallow copy`. Ask the AI to distinguish “property missing” from “property value is `null`,” and state whether an output object must be allowlisted or denylisted.

...
它也常机械地添加 `await`，从而把独立 I/O 串行化；或者只用 `async` 包裹 CPU 密集工作，却没有加入真正的暂停点或 worker 边界。针对 Node 的回答还可能把定时器、poll、check、next-tick 回调与 Promise 作业压缩成一个并不存在的队列。
...
要求 AI 说明哪些顺序由语言保证，哪些由 Node 文档保证，哪些只是在某次运行中观察到的结果。对于性能敏感的代码，还应询问每个回调内同步工作的上限，以及预期的取消或背压策略。
...
当 Node 设计需要阶段级或延迟级精度时，再加上 `poll phase`（轮询阶段）、`check phase`（检查阶段）、`process.nextTick queue`（next-tick 队列）、`cooperative yielding`（协作式让出）、`event-loop delay`（事件循环延迟）与 `backpressure`（背压）。

...
> [!PITFALL]
> 生成代码常从 TypeScript 或打包项目复制 `import "./config"`、`@/services` 或任意裸说明符。构建时可能成功，直接交给浏览器或 Node 24 却会解析失败。
>
...
> [!PITFALL]
> `import client from "pkg"`、`import { client } from "pkg"` 和 `const client = require("pkg")` 并不保证得到同一形状。模型经常根据变量名补出不存在的具名导出，或在互操作边界多取一次 `.default`。
>
...
模型会把一个工具链的解析习惯带到另一个宿主，例如给浏览器生成无扩展名相对导入，或把 TypeScript 路径别名当作 Node 包入口。它还会根据旧版包示例猜测默认导出，在静态导入改成 `await import()` 后立即解构实时值，或者用请求参数拼接插件路径。遇到循环依赖时，模型常把一侧改成动态导入，却漏掉新增的 Promise 边界；遇到测试污染时，又用随机查询参数强制重载，导致模块状态和副作用实例持续增加。

**让 AI 检查**

- 列出每个文件在部署环境中的模块格式、解析依据和完整解析结果，标明哪些规则来自浏览器、Node 24 或构建工具。
- 为每条导入列出实际导出形状、默认与具名入口、实时绑定要求，并画出强连通循环中的顶层读取顺序。
- 枚举每个动态说明符的所有可能值、输入信任边界、失败类型和缓存标识，确认它来自固定允许列表。

**审查清单**

- 在实际部署运行时执行冷启动入口，并覆盖 `package.json` 的每个公开条件与子路径。
- 检查循环中的顶层访问、模块级可变状态所有者，以及同一说明符是否只求值一次。
- 拒绝由外部输入拼出的说明符，并分别测试加载、求值和调用失败。

**提示词词汇**

使用准确术语：`module specifier`（模块说明符）、`module graph`（模块图）、`module namespace object`（模块命名空间对象）、`named export`（命名导出）、`default export`（默认导出）、`live binding`（实时绑定）、`re-export`（再导出）、`dynamic import`（动态导入）、`top-level await`（顶层 `await`）、`conditional exports`（条件导出）、`temporal dead zone`（暂时性死区）。要求模型输出文件格式矩阵、说明符解析表和循环求值时间线；只问「这个 import 对吗」不足以暴露宿主差异。

...
Models often generate a universal `curry` based on `fn.length`, then apply it to a function with defaults or rest parameters, causing early or immediate execution. Another common failure is an `async` stage inside a synchronous `pipe`, so the next stage receives a Promise; models also capture `this` from the wrong partial-call layer while repairing detached methods. Generated data pipelines frequently replace `toSorted()` with `sort()`, keeping the output correct while unexpectedly mutating the input.

**Ask your AI to check**

- List each target's source parameters beside its runtime `fn.length`, and identify the call that completes collection.
- State the input shape, output shape, and plain-value-or-Promise behavior of every composed stage.
- Trace every method receiver and each array operation's mutation behavior, including shared references.

**Review checklist**

- Run tests for unary, grouped, default, rest, and extra arguments.
- Test success and rejection with a real Promise, confirming that later stages receive fulfillment values rather than Promise objects.
- Freeze or copy the input before running the pipeline, then check `this`, array order, and nested objects for unintended changes.

**Prompt vocabulary**

Use the exact terms `currying`, `partial application`, `arity`, `function composition`, `unary function`, `point-free style`, `receiver`, and `Promise normalization`. Ask for the helper contract and a stage-by-stage type table before requesting code; "optimize this functional code" doesn't constrain grouping, receivers, or error channels.

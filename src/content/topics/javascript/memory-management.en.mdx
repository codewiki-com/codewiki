...
Models often produce an unbounded module-level `Map` that permanently retains request IDs, Promises, or full responses. They also register anonymous listeners or `setInterval()` callbacks in components without returning cleanup handles. Other generated “fixes” assign `null` to a local while ignoring the actual retaining path, or use `WeakRef` and `FinalizationRegistry` as deterministic resource management. A cache may even be called LRU while evicting only by insertion order, contradicting how callers expect hot entries to behave.

**Ask your AI to check**

- List the owner, key space, capacity or TTL, and deletion path for every long-lived collection.
- Match each listener, timer, observer, and subscription with symmetric cleanup on success, cancellation, and partial initialization failure.
- Write the retaining path from each suspect object to a GC root, distinguishing the JavaScript heap, `ArrayBuffer` external memory, and process RSS.

**Review checklist**

- Repeat creation and teardown with different inputs and confirm that live-object counts eventually reach a stable plateau.
- Check whether a closure captures an entire request, component, or service container when it only needs one small value.
- Confirm weak references do not decide correctness and external resources do not depend on finalizer execution.

**Prompt vocabulary**

Use the exact terms `reachability`, `GC root`, `retaining path`, `strong reference`, `weak collection`, `ephemeron`, `heap snapshot`, and `explicit disposal`. Ask for an ownership table, a lifecycle timeline, and a boundedness argument. “Optimize memory” is too vague to make a model inspect the long-lived references that matter.

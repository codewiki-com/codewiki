...
> [!PITFALL]
> 生成 handler 时直接返回 `target[key]`，在普通数据属性测试中看起来正确，却改变了 getter、setter 和继承链中的 `this`。用 `Reflect.get(target, key, target)` 也会产生同样问题。
...
> [!PITFALL]
> `set` trap 在没有修改目标时仍返回 `true`，会让赋值看似成功并丢失数据；无条件返回 `false` 又会在严格模式中把普通赋值变成 `TypeError`。模型常为了满足“必须返回布尔值”而生成固定结果。
...
模型经常生成 `return target[key]` 而丢掉 `receiver`，或让 `set` 无条件返回 `true`，于是继承访问器读错对象，失败写入却被报告为成功。它还会只实现 `get` 和 `set` 就宣称获得“安全私有属性”，遗漏 `defineProperty`、删除、枚举与目标别名。另一组故障来自未经边界测试的 `ownKeys`：虚拟键与真实键重复，或者目标冻结后仍报告额外键。

模型也常把同一个通用日志代理套在 `Map`、`Date` 或使用私有字段的类实例上，运行时才遇到 incompatible receiver 或私有品牌错误。为了修复，它可能把所有函数都绑定到目标，这又改变方法标识、`this` 和链式 API 的返回边界。日志代码若用 `` `${key}` `` 处理属性键，还会在某些 `Symbol` 键上抛错并破坏原操作。

**让 AI 检查**

- 列出调用方使用的每种对象操作，并把它映射到 trap、`Reflect` 默认行为和可能的绕过路径。
- 对每个 trap 给出 `receiver`、返回类型与相关不变量，再用不可配置属性和不可扩展目标验证结论。
- 搜索 `Map`、`Set`、`Date`、类私有字段、`Symbol` 键与目标别名，说明代理是否改变品牌检查、对象标识或访问边界。

**审查清单**

- 用继承对象和 getter／setter 验证 `receiver`，并断言失败写入没有被伪装成成功。
- 分别测试赋值、属性定义、删除、存在性、字符串键、`Symbol` 键与全部自有键。
- 冻结或禁止扩展目标后重复测试，再覆盖撤销前后和带品牌检查的内置对象。

**提示词词汇**

使用准确术语：`proxy trap`（代理陷阱）、`target`（目标）、`handler`（处理器）、`receiver`（接收者）、`forwarding`（转发）、`property descriptor`（属性描述符）、`proxy invariant`（代理不变量）、`internal slot`（内部槽）、`revocable proxy`（可撤销代理）、`brand check`（品牌检查）。要求模型输出操作矩阵和不变量测试；“给对象加一层代理”不足以定义正确行为。

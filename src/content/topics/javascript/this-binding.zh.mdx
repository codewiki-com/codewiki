---
title: this 绑定
description: JavaScript 根据调用方式确定 this；掌握方法调用、显式绑定、箭头函数与构造调用，避免回调和重构改变接收对象。
track: javascript
section: core
...
<TLDR>
  <TLDRCell label="what">
    <Term id="this-binding">this 绑定</Term>决定普通函数本次调用中的 `this` 值。通常要看调用表达式，而不是函数写在哪里。
  </TLDRCell>
  <TLDRCell label="trap">
...
现代 JavaScript 经常用闭包和显式参数代替 `this`，这并不让 `this` 过时。类方法、平台回调和既有库仍然依赖其调用约定。正确选择取决于 API 是否需要“同一操作作用于不同接收对象”这一能力。
...
下面四个示例依次展示属性调用与显式绑定、回调中的方法丢失、箭头函数的词法 `this`，以及绑定函数参与构造时的行为。所有输出均由本地 Node 24 执行对应文件得到。
...
<TryToBreak items={['在严格模式下把 formatJob 保存到变量后直接调用', '把原始值作为 call 的 thisArg', '尝试用 call 替换 bound 保存的接收者']} />
...
箭头包装器本身不依赖自己的 `this`，而是通过词法变量 `meter` 发起完整的方法调用。它也控制了只把 `value` 转发给方法，不会把 `map()` 额外提供的索引和数组误传给下游 API。
...
构造出的 `session` 仍按目标函数 `Session` 的 `prototype` 参与 `instanceof` 检查。绑定函数可以用来预填构造参数，但这种技巧会隐藏真实构造器；公开 API 通常用命名工厂或类的静态方法更清楚。
...
**修复：** 在实际调用点检查点号、方括号、`call()`、`apply()`、`bind()` 或 `new`。若 API 必须独立传递方法，只绑定一次并保存结果，或者传入显式接收对象的包装器。
...
### 假设所有回调 API 都提供同一种 this

> [!PITFALL]
> 回调由接收它的 API 调用；有的使用普通调用，有的接受 `thisArg`，还有的规定特定接收者。把在一个 API 中有效的方法直接搬到另一个 API，可能改变 `this` 和额外实参。

**修复：** 阅读回调 API 的调用契约，并用一个会访问接收者字段的测试验证。箭头回调会忽略 API 提供的 `thisArg`；若要使用 `thisArg`，必须传普通函数。
...
模型常把 `repository.save(record)` 重构成 `const { save } = repository; save(record)`，没有发现属性调用已变成普通调用。它也可能把对象方法改成箭头函数以“保留 this”，结果捕获模块顶层的 `this`；或分别在注册和清理代码中生成 `.bind(this)`，导致监听器无法移除。另一类错误是把 `items.map(service.normalize)` 当作等价简写，既丢失服务接收者，又意外传入索引和数组。

**要求 AI 检查**

- 列出每个读取 `this` 的函数，并写出所有实际调用形式及预期接收者。
- 找出所有方法解构、方法赋值、回调传递和 `.bind()` 调用，追踪函数标识是否需要用于清理。
- 分别模拟严格模式、箭头函数、绑定函数和 `new` 路径，标出 `call()` 无法覆盖的绑定。

**审查清单**

- 脱离对象调用每个被传递的方法，确认失败方式或绑定策略符合契约。
- 检查对象方法是否被误写成箭头函数，以及箭头回调是否错误忽略了 `thisArg`。
- 检查注册与注销是否使用同一个函数引用，并覆盖构造调用和显式对象返回分支。

**提示词汇**

使用准确术语 `call site`（调用点）、`receiver`（接收者）、`method extraction`（方法提取）、`implicit binding`（隐式绑定）、`explicit binding`（显式绑定）、`bound function`（绑定函数）、`lexical this`（词法 `this`）、`constructor call`（构造调用）和 `callback identity`（回调标识）。要求模型输出“调用形式—接收者”表与注册/清理的函数标识追踪；只说“检查 this”不足以暴露重构造成的变化。
...
严格普通函数保留调用者提供的 `this` 值。直接调用得到 `undefined`；`fn.call(null)` 得到 `null`；原始值也保持原始值。类的构造器和方法始终使用严格语义，JavaScript 模块也自动处于严格模式。

非严格普通函数会转换接收者。`undefined` 或 `null` 会被替换为 `globalThis`，字符串、数字、布尔值等原始值会被包装为对象。依赖这些转换会让代码随脚本类型和函数来源改变，因此不应把它们当作业务逻辑。

顶层 `this` 也由宿主和脚本类型决定。浏览器经典脚本、浏览器模块和 Node CommonJS 包装器的顶层结果并不相同。顶层箭头函数只会捕获该环境已经提供的值，所以不要用“它总是 `window`”或“它总是 `undefined`”来推理。
...
访问器也从访问表达式取得 `this`。继承来的 getter 在 `child.value` 中以 `child` 为接收者，而不是以定义 getter 的原型为接收者。`Reflect.get(target, key, receiver)` 的第三个参数可以显式控制访问器看到的接收者，代理转发代码遗漏它时可能破坏行为。
...
普通 `async` 方法在进入时按调用点确定 `this`，越过 `await` 后仍使用同一次调用的接收者。`await` 不会自己导致“this 丢失”。真正的问题通常发生在方法进入之前已经被提取，或方法内部又把另一个普通函数作为回调传出。

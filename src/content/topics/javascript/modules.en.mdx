...
> [!PITFALL]
> Imports such as `import "./config"`, `@/services`, or arbitrary bare specifiers copied from TypeScript or bundled projects may build successfully yet fail when run directly by a browser or Node 24.
>
...
> [!PITFALL]
> `import client from "pkg"`, `import { client } from "pkg"`, and `const client = require("pkg")` don't promise the same shape. Inferring a named export from the variable name or adding one `.default` too many breaks the interop boundary.
>

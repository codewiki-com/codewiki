...
JavaScript 社区中的 `curry` 辅助函数常允许 `fn(a)(b, c)` 或 `fn(a, b)(c)`。这种接口同时提供了柯里化和分组偏应用，使用起来方便，却不再是严格的逐个参数形式。阅读库代码时应先查它对分组、占位符、多余实参和完成条件的约定。
...
柯里化与组合经常一起出现，因为组合链中除入口外的阶段通常只接收一个值。先用柯里化或偏应用固定配置，就能把多参数操作变成合适的单输入阶段。不过，这只是便利的搭配；二者都可以独立使用，JavaScript 标准库也没有内置通用的 `curry`、`compose` 或 `pipe`。
...
> [!PITFALL]
> 同步 `pipe` 遇到异步阶段后，会把 Promise 直接交给下一函数。下一函数若读取业务字段，常见结果是 `undefined` 或与真实原因相隔数层的类型错误。
...
> [!PITFALL]
> 很长的 point-free 链会藏住每一步从对象到数组、再到字符串的变化。`sort()`、`reverse()` 或修改对象字段的阶段还可能让看似纯粹的转换改变调用方数据。
...
模型经常生成依赖 `fn.length` 的万能 `curry`，随后把它用于带默认参数或剩余参数的函数，导致过早执行或立即执行。另一个常见错误是同步 `pipe` 中混入 `async` 阶段，使下一阶段收到 Promise；模型还会在修补方法调用时从错误的一层捕获 `this`。生成的数据管道也常把 `toSorted()` 改成 `sort()`，输出正确却意外修改输入。

**让 AI 检查**

- 列出每个目标函数的源码形参与运行时 `fn.length`，说明管道在哪次调用完成收集。
- 为每个组合阶段写出输入形状、输出形状，以及它返回普通值还是 Promise。
- 跟踪每个方法的接收者与每个数组操作的修改行为，并指出共享引用。

**审查清单**

- 运行逐个实参、分组实参、默认参数、剩余参数和多余实参测试。
- 用真实 Promise 测试成功与拒绝路径，确认后续阶段收到履约值而不是 Promise 对象。
- 冻结或复制输入后运行管道，检查 `this`、数组顺序和嵌套对象是否被意外改变。

**提示词词汇**

使用准确术语：`currying`（柯里化）、`partial application`（偏应用）、`arity`（参数数量）、`function composition`（函数组合）、`unary function`（单输入函数）、`point-free style`（point-free 风格）、`receiver`（接收者）、`Promise normalization`（Promise 规范化）。要求模型先写辅助函数契约和逐阶段类型表；只说优化这段函数式代码，无法限定分组、接收者和错误通道。
...
以 `Promise.resolve(input)` 开始的异步管道会吸收普通值和 Promise。每次 `.then(step)` 还会把同步抛出的异常转换成拒绝，所以链尾可以用一次 `await` 和 `try...catch` 观察统一结果。这种统一不等于错误已经分类，业务仍要决定哪些失败可以重试。
...
Point-free 写法适合已经有清楚名称和稳定签名的函数。若必须反复回看 `prop`、`flip`、`uncurry` 等适配器才能理解方向，显式写出参数通常更短。可读性取决于读者能否看见数据变化，不取决于源码中是否出现形参。
...
- [MDN JavaScript 指南：函数](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Functions)

...
> [!PITFALL]
> A generated handler that returns `target[key]` looks correct in tests using ordinary data properties, but it changes `this` inside getters, setters, and prototype chains. `Reflect.get(target, key, target)` has the same problem.
...
> [!PITFALL]
> A `set` trap that returns `true` without changing the target makes an assignment appear successful while losing data. Always returning `false` instead turns ordinary assignment into `TypeError` in strict mode. Models often generate a fixed result merely to satisfy the Boolean return contract.
...
Models often emit `return target[key]` and lose `receiver`, or make `set` return `true` unconditionally, so inherited accessors read the wrong object and failed writes look successful. They also implement only `get` and `set`, then claim to provide “secure private properties” while overlooking `defineProperty`, deletion, enumeration, and target aliases. Another failure cluster is an untested `ownKeys`: a virtual key duplicates a real key, or remains reported after the target is frozen.

Models also wrap `Map`, `Date`, or private-field instances in one generic logging proxy and discover incompatible-receiver or private-brand errors only at runtime. A generated fix may bind every function to the target, changing method identity, `this`, and the boundary returned by fluent APIs. Logging code that handles property keys with `` `${key}` `` can also throw for some `Symbol` keys and break the original operation.

**Ask your AI to check**

- List every object operation used by callers and map it to its trap, `Reflect` default, and any policy bypass.
- For every trap, state its `receiver`, return type, and relevant invariants, then test those claims with non-configurable properties and a non-extensible target.
- Search for `Map`, `Set`, `Date`, class private fields, `Symbol` keys, and target aliases; explain whether the proxy changes brand checks, object identity, or the access boundary.

**Review checklist**

- Verify `receiver` with an inherited object and getters or setters, and assert that failed writes aren't disguised as success.
- Test assignment, property definition, deletion, presence, string keys, `Symbol` keys, and all own keys separately.
- Repeat the tests after freezing or preventing extensions, then cover pre/post-revocation behavior and branded built-ins.

**Prompt vocabulary**

Use the exact terms `proxy trap`, `target`, `handler`, `receiver`, `forwarding`, `property descriptor`, `proxy invariant`, `internal slot`, `revocable proxy`, and `brand check`. Ask for an operation matrix and invariant tests; “put a proxy around this object” doesn't define correct behavior.

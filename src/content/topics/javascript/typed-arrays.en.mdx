...
Generated binary parsers often call `new DataView(input.buffer)`, discarding the nonzero offset and local length of a `Uint8Array` or Node.js `Buffer`. Other output reads protocol fields through multibyte typed arrays and therefore depends on native byte order, or swaps `slice()` with `subarray()` so caller mutations later corrupt cached data. Models also trust a length declared in the header, calling `getUint32()` or creating a payload view before checking the remaining bytes.

**Ask your AI to check**

- List every view's `buffer`, `byteOffset`, `byteLength`, element type, and owner, then mark overlapping regions.
- Trace parsing field by field with a nonzero-offset input, stating which length check precedes each read.
- Classify every copy or constructor operation as value conversion, byte copy, shared view, or byte reinterpretation.

**Review checklist**

- Run parsing tests with a truncated header, false length, reversed byte order, and sentinel bytes around the input view.
- Check that every external multibyte field explicitly chooses signedness, width, and byte order.
- Mutate each source and result once, then verify ownership after `slice()`, `subarray()`, transfer, and shrinking.

**Prompt vocabulary**

Use the exact terms `backing buffer`, `ArrayBufferView`, `byte offset`, `element length`, `endianness`, `length-tracking view`, `fixed-length view`, `detached buffer`, and `aliasing`. Ask for a byte layout and ownership table; "turn these bytes into an array" says too little about conversion semantics.

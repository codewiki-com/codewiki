...
模型常让 `[Symbol.iterator]()` 返回保存在实例上的同一个生成器，导致两个消费者争用游标；也常把已创建的生成器对象缓存为“可复用数据”。
另一些生成实现会展开没有可信上限的输入、手动调用 `next()` 却忽略 `done`，或在资源生成器中遗漏 `finally` 和 `return()` 清理路径。

**要求 AI 检查什么**

- 标出每个值是 iterable、iterator 还是 iterator result，并说明谁拥有推进状态。
- 跟踪正常完成、`break`、消费方异常和显式 `return()` 时是否执行同一套资源清理。
- 证明每个展开、`Array.from()` 和剩余解构的输入都有有限上界。

**审查清单**

- 对同一个可迭代集合交错启动两个循环，确认游标相互独立；对单次源则确认接口明确禁止复用。
- 让序列合法地产出 `undefined`，并验证消费者只依据 `done` 判断结束。
- 在第一个产出后退出或抛错，断言订阅、句柄或锁已经释放且只释放一次。

**提示词词汇**

使用精确术语 `iterable protocol`（可迭代协议）、`iterator protocol`（迭代器协议）、`iterator result`（迭代结果）、`single-pass iterator`（单次迭代器）、`generator state`（生成器状态）、`IteratorClose`（迭代器关闭）、`yield delegation`（`yield*` 委托）和 `eager materialization`（立即物化）。
要求模型画出调用方与每个暂停点之间的 `next(value)` 时间线，比笼统要求“检查生成器”更容易暴露状态错误。

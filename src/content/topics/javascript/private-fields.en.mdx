---
title: JavaScript private fields
...
Models put `obj.#token` outside its class and wrap it in `try...catch`, making the file fail to parse instead of handling a runtime error. They also use `obj['#token']`, `Object.hasOwn()`, or object spread to "inspect" a private field, but those operations see only ordinary properties. During a callback refactor or `Proxy` addition, a model may preserve the method body while changing its receiver. Generated static factories often use `this.#counter` in an inheritable method and test only the base call. Another common leak is a public getter that returns the private array itself, or a `toJSON()` method that re-exposes a token that was meant to stay inside the boundary.

**Ask your AI to check**

- List the declaring class and every access site for each private name, then state which private brands each receiver is expected to carry.
- Run detached callbacks, proxy calls, and derived static calls through their real paths, reporting the exact receiver and exception type.
- Audit spread, cloning, freezing, and serialization boundaries, distinguishing omitted private state from leaked mutable references.

**Review checklist**

- Confirm that no class-external `#name` access makes the file unparseable and that no string key `"#name"` is mistaken for a private field.
- Exercise methods with a wrong receiver, a proxy, and a derived class; confirm that binding and state ownership match the design.
- Inspect every public return value and serialized record for secrets or internal mutable objects, and test the brand after reconstruction.

**Prompt vocabulary**

Use the exact terms `private element`, `private field`, `private name`, `private brand`, `brand check`, `receiver`, `wrong receiver`, `lexical access`, and `static private field`. Ask for a table with "declaring class, access site, actual receiver, expected brand" rather than asking only for an encapsulation review.

...
解构赋值（destructuring assignment）是一组 JavaScript 语法，用左侧的<Term id="destructuring-pattern">解构模式（destructuring pattern）</Term>描述怎样从右侧值中取出数据。数组模式按迭代顺序取值，对象模式按属性键取值。它不会改变源数组或源对象，但模式中的赋值目标和默认表达式可能产生其他副作用。
...
数组和对象模式看起来相似，读取规则却不同。`const [first] = value` 要求右侧实现可迭代协议，而 `const { first } = value` 查找名为 `first` 的属性。数组解构不是“读取数字属性 0”的简写；自定义<Term id="iterable">可迭代对象（iterable）</Term>、字符串、`Set` 和生成器同样能被数组模式消费。
...
> [!PITFALL]
> 生成代码常把 `{ count = 10 }` 理解成“缺少或无效时使用 10”。事实上，只有 `undefined` 会触发默认表达式，`null`、`0`、`false` 和 `''` 都会原样保留。
...
模型经常根据一个示例 JSON 生成过深的参数模式，却没有保护可选的中间对象；它也常误以为默认值会替换 `null`。另一个真实风险是用对象剩余属性排除当前已知的密码字段，再把余下内容直接写入日志或响应。生成的重命名还可能把属性键误当成局部变量，或在赋值表达式外漏掉必需的括号。

**让 AI 检查**

- 列出每个嵌套模式可能遇到的 `undefined` 与 `null`，并说明哪一层会先抛错。
- 对每个默认表达式给出触发矩阵，至少覆盖缺失、`undefined`、`null`、`0`、`false` 和空字符串。
- 跟踪对象剩余属性的键集合与嵌套引用，确认新敏感字段不会被自动转发。

**审查清单**

- 用缺失字段、显式 `undefined`、显式 `null` 和错误类型运行输入边界测试。
- 核对每个 `{ sourceKey: localName }`，确认后续代码读取的是实际创建的绑定。
- 对外输出只构造允许字段，并测试修改副本的嵌套值是否会影响源对象。

**提示词词汇**

使用准确术语：`destructuring pattern`（解构模式）、`binding pattern`（绑定模式）、`assignment pattern`（赋值模式）、`default initializer`（默认初始化表达式）、`rest element`（剩余元素）、`computed property key`（计算属性键）、`iterator closing`（迭代器关闭）和 `shallow copy`（浅拷贝）。要求 AI 区分“属性缺失”和“属性值为 `null`”，也要明确输出是按允许列表还是排除列表构造。

...
Models often insert `sort()`, `reverse()`, or `splice()` into a data pipeline without acknowledging that the input changes; with UI state or cached data, the mutation surfaces far from the call. They also emit Boolean comparators such as `(a, b) => a.score > b.score`, erase valid zeroes with `filter(Boolean)`, omit the initial `reduce()` value on empty inputs, or assume spread syntax copied nested objects. The signature failure in async code is an `async` callback passed to `forEach()`, followed by an immediate return of results that aren't ready.

**Ask your AI to check**

- List whether each array method mutates its receiver, what it returns, and which object references its result shares with the input.
- Test the comparator with equal, forward, and reversed pairs, and confirm that its results cover negative, zero, and positive values.
- Add cases for empty arrays, sparse arrays, valid falsy values, duplicate sort keys, and Promise rejection to the generated pipeline.

**Review checklist**

- Find `sort()`, `reverse()`, `splice()`, `fill()`, and assignments inside callbacks; verify every side effect and owner.
- Check the `reduce()` seed, the business meaning of each filter predicate, and whether searches can short-circuit.
- Confirm who awaits asynchronous callbacks, how failures propagate, and whether batched requests have a capacity limit.

**Prompt vocabulary**

Use `mutating method`, `copying method`, `shallow copy`, `callback return value`, `short-circuiting`, `sparse array`, `empty slot`, `comparator`, and `initial accumulator`. Ask the model to account separately for input identity, element identity, return value, empty input, and the async completion condition; "optimize this array code" is too vague to expose its assumptions.

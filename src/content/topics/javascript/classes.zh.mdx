...
模型经常从 Java、C# 或 TypeScript 搬来并不存在于纯 JavaScript 的规则，例如重载多个 `constructor`，或直接写 `protected` 和 `abstract`。更隐蔽的错误是把方法直接交给回调、让基类构造函数调用覆盖方法、在可继承静态方法中使用 `this.#field`，以及用一层展开运算符声称私有对象已经隔离。这些代码有的无法解析，有的只在创建子类或处理真实嵌套数据时失败。

**让 AI 检查**

- 列出每个成员实际存放的位置：实例自有属性、原型、类构造函数或私有槽位，并用属性描述符验证公开成员。
- 跟踪派生实例的初始化顺序，标出 `super()`、基类构造函数、基类字段、派生类字段和覆盖方法第一次运行的时点。
- 把每个方法既作为 `object.method()` 调用，也作为脱离接收者的回调调用；对访问静态私有字段的方法再通过子类调用一次。

**审查清单**

- 确认所有派生构造函数都在读取 `this` 前调用 `super()`，且基类构造函数不分派到依赖派生字段的方法。
- 检查传给 `map`、事件系统、定时器和解构赋值的方法是否仍有正确接收者与稳定的移除身份。
- 检查返回值、对象展开和序列化边界是否泄漏私有状态中的嵌套可变对象或秘密字段。

**提示词词汇**

使用准确术语：`prototype chain`（原型链）、`own property`（自有属性）、`instance field`（实例字段）、`private field brand check`（私有字段标记检查）、`derived constructor`（派生构造函数）、`method receiver`（方法接收者）、`static initialization block`（静态初始化块）。要求模型给出成员存储位置和初始化时间线；只说“把它改成面向对象”不会暴露这些选择。

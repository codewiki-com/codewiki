...
短路求值（<Term id="short-circuiting">short-circuiting</Term>）决定右侧表达式是否执行。`left && right` 在左侧为假值时停止，`left || right` 在左侧为真值时停止，`left ?? right` 只在左侧为 `null` 或 `undefined` 时执行右侧。这个差别对合法的 `0`、`false` 和空字符串尤其重要。

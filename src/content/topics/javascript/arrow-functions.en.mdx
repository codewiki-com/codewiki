...
Models often normalize every anonymous function to an arrow, breaking dynamic `this` in object methods or turning a factory still called with `new` into a non-constructible function. They also generate separate inline arrows in `addEventListener()` and `removeEventListener()`, so cleanup fails; when adding logging, they change an expression body to a block and forget `return`. In class code, a model may replace a prototype method with an arrow field without updating tests that patch the prototype or the subclass design.

**Ask your AI to check**

- List every arrow that reads `this`, `arguments`, `super`, or `new.target`, and identify the exact outer environment that supplies each name.
- Pair every registration API with its unregistering site and compare both the function identity and any matching options.
- Find every expression body changed to a block body and prove that all value-producing paths execute an explicit `return`.

**Review checklist**

- Exercise receivers through the real call forms, including method calls, detached calls, and explicit `call()` or `bind()` paths.
- Search construction calls and `prototype` access to confirm that no target was accidentally changed to a non-constructible arrow.
- Test callback registration, duplicate registration, removal, and an event emitted after removal.

**Prompt vocabulary**

Use the exact terms `arrow function expression`, `lexical this`, `dynamic receiver`, `expression body`, `block body`, `implicit return`, `rest parameter`, `function identity`, and `constructible`. Ask the model for a table of call forms and function identities; "make the functions more modern" doesn't state a semantic constraint.

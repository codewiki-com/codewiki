...
Models generate `Symbol('id')` separately in writer and reader modules, then mistake matching descriptions for one key. They also call Symbol properties “non-enumerable” or “private,” validate input with `Object.keys()`, and copy it with object spread—letting unchecked enumerable Symbol keys enter the result. Another failure is generating `` `${key}` `` for logging; when proxy or reflection code receives a real Symbol key, the log throws first and changes the original operation.

Models also replace every key with `Symbol.for()` to get “cross-module consistency,” sometimes using user input as the registry key. That increases the collision surface without solving JSON or message transport, and generated tests that use only string keys miss the boundary difference.

**Ask your AI to check**

- List each Symbol's creation site, owner, and consumers, and explain how every consumer obtains the same identity.
- For every enumeration, copy, merge, and serialization operation, list its own/inherited, enumerable/non-enumerable, and string/Symbol key selection.
- For each well-known Symbol hook, give its trigger, allowed return type, state ownership, and a failing test.

**Review checklist**

- Test key reconstruction with two Symbols that have the same description but different identities.
- Add enumerable and non-enumerable Symbols plus an inherited string key to input, then inspect copy and validation boundaries.
- Run logging, error paths, JSON output, and reflection with a Symbol key to catch skipped checks and accidental exceptions.

**Prompt vocabulary**

Use `Symbol identity`, `description`, `property key`, `registered symbol`, `global symbol registry`, `well-known symbol`, `enumerable own symbol key`, and `protocol hook`. Ask for a key-selection matrix and an identity-ownership table; “use a Symbol to hide metadata” conceals visibility and boundary problems.

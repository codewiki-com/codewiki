...
Models often generate `text.slice(0, limit)` to truncate a username, splitting an emoji or combining sequence. They also use `[...text]` and claim the code now handles “characters,” although it still splits a family emoji. Other common failures pass external data directly as a `replaceAll()` replacement string, causing `$&` or `$1` to be reinterpreted, or substitute `split(',')`, a tag regex, or a simplistic email regex for a real format parser.

Generated normalization code also changes semantics too aggressively. It may apply `trim().toLowerCase()` indiscriminately to usernames, file paths, tokens, and display text without defining a locale, Unicode normalization form, or whether the protocol allows those distinctions to collapse. It may also put `localeCompare(a, b) === -1` in a sort function and rely on an unspecified result magnitude.

**Ask your AI to check**

- For every `length`, `at()`, `slice()`, `substring()`, and `padStart()`, name the counting unit and demonstrate the boundary with a surrogate pair, combining mark, and zero-width-joiner sequence.
- For every replacement call, state the search-value type, first-or-all behavior, and whether `$&` or `$1` inside replacement data remains literal.
- List the business contract behind each normalization step, distinguishing protocol identifiers, user-visible comparison, search, sorting, and security-sensitive keys.

**Review checklist**

- Run empty-string, missing-delimiter, repeated-match, and end-of-string boundary tests.
- Verify counting, slicing, reversing, and truncation with `"😀"`, `"e\u0301"`, and a family emoji.
- Confirm that formal grammars such as CSV, HTML, and URLs use dedicated parsers and that locale-aware transformations don't define authorization or unique keys.

**Prompt vocabulary**

Use precise terms: `immutable string`, `UTF-16 code unit`, `Unicode code point`, `grapheme cluster`, `half-open range`, `replacement string`, `replacer function`, `Unicode normalization`, and `locale-aware comparison`. Ask the model for a counting-unit table and boundary-test matrix. “Handle Unicode correctly” is still underspecified.

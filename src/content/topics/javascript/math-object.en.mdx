...
Models often generate `Math.round(amount * 100) / 100` and claim the amount is now exact, or mechanically add `Number.EPSILON` to every approximate comparison. They also use `Math.random()` for verification codes, pass degrees directly to `Math.sin()`, spread an unbounded array into `Math.max()`, or convert `BigInt` to `Number` before a `Math` call and silently lose precision.

**Ask your AI to check**

- For every numeric value, state its type, unit, allowed range, missing-value rule, and error budget, then mark every expression that can produce `NaN` or an infinity.
- Produce an endpoint table for every rounding and random formula, covering at least negatives, midpoints, empty collections, minima, and maxima.
- Distinguish display rounding, storage precision, reproducible pseudorandomness, and cryptographic randomness, and explain which contract the chosen API satisfies.

**Review checklist**

- Check all external inputs with `Number.isFinite()`, `Number.isSafeInteger()`, and domain bounds instead of relying on implicit numeric conversion.
- Run generated code against negative values, negative zero, `NaN`, infinities, very large arrays, and both ends of every interval.
- Search for `Math.random()`, `Math.round()`, spread into `min` or `max`, and `BigInt`-to-`Number` conversion, then verify each use against its intent.

**Prompt vocabulary**

Use the exact terms `rounding direction`, `ties toward positive infinity`, `binary floating-point`, `absolute tolerance`, `relative tolerance`, `half-open interval`, `seeded PRNG`, `cryptographically secure randomness`, and `rejection sampling`. Ask for a numeric contract and an endpoint table; "optimize this math code" is too vague to expose the assumptions.

...
Generators often put a full CRUD interface beside an implementation struct
even when the current consumer reads one piece of data.
They also see `value.Method()` compile and wrongly assign `value` to an interface that requires a pointer-receiver method;
the mistake may surface only when generated mocks or another package accept the implementation.
Another real failure is returning `(*CustomError)(nil)` on success, causing upstream code to log a successful request as failed or panic while formatting it.

Models also reach for `any` and a large type switch when the design is uncertain.
The result looks extensible, but every new type requires editing the central switch,
and a misspelled dynamic type isn't discovered until runtime.
When the requirement describes shared behavior, ask for a narrow interface and have the compiler prove each implementation.

**Ask your AI to check**

- List the methods each interface consumer calls, then remove methods the consumer never uses.
- Compute the method sets of both `T` and `*T`, then generate compile-time assertions for the intended implementations.
- Trace every successful return from an `error`-returning function and confirm that no nil concrete pointer is converted to the interface.

**Review checklist**

- Compile real implementations and test doubles at the interface boundary; don't rely on automatic addressing in method calls.
- Test matching, mismatching, and nil inputs for type assertions, and confirm that failure doesn't panic unexpectedly.
- For interface equality, map keys, and `error` returns, check whether the dynamic value can be non-comparable or a typed nil.

**Prompt vocabulary**

Use the exact terms `implicit interface satisfaction`, `method set`, `dynamic type`,
`dynamic value`, `typed nil`, `comma-ok type assertion`, and `consumer-defined interface`.
Ask the model to show interface assignments and failure paths instead of merely saying that a type

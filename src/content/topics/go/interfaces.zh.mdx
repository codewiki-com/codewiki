...
生成器常在实现结构体旁创建包含整套 CRUD 方法的接口，
即使当前调用方只读一项数据。
它也会看到 `value.Method()` 能编译，便错误地把 `value` 赋给要求指针接收者方法的接口；
这种问题往往在生成 mock 或把实现传入另一个包时才暴露。
另一种真实故障是成功路径返回 `(*CustomError)(nil)`，让上层把成功请求记成失败，甚至在日志格式化时 panic。

模型还喜欢用 `any` 和大段类型开关绕过不确定的设计。
结果看似支持扩展，新增类型却必须修改中央开关，
而拼错的动态类型直到运行时才出现。
当需求描述的是一种共享行为时，应让模型生成窄接口，并要求编译器证明每个实现。

**让 AI 检查**

- 列出每个接口的消费方法，并删除调用方从未调用的方法。
- 分别计算 `T` 与 `*T` 的方法集，再为预期实现生成编译期断言。
- 追踪每条返回 `error` 的成功路径，确认没有把 nil 具体指针转换成接口。

**审查清单**

- 在接口边界编译真实实现和测试替身，不依赖方法调用的自动取地址。
- 对类型断言测试匹配、不匹配和 nil 输入，并确认失败不会意外 panic。
- 对接口相等、map 键和 `error` 返回检查动态值是否可能不可比较或为带类型的 nil。

**提示词词汇**

提示中使用准确术语：`implicit interface satisfaction`（隐式接口满足）、
`method set`（方法集）、`dynamic type`（动态类型）、`dynamic value`（动态值）、
`typed nil`（带类型的 nil）、`comma-ok type assertion`（双结果类型断言）和
`consumer-defined interface`（使用方定义的接口）。
要求模型给出接口赋值和失败路径，而不只是说某类型“实现了这个行为”。

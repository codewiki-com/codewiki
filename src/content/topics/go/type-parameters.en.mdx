...
Models often declare `[T any]` and then use `<`, `+`, indexing, or field selection on `T`, producing code that can't compile. They also put `int` in a union but omit `~int`, unexpectedly rejecting domain types such as `UserID`; or they see `comparable` and generate `map[any]V` without considering that a dynamic slice key panics. Training examples written for older Go versions may still claim that methods can never declare extra type parameters, which is false in Go 1.27.

**Ask your AI to check**

- For each type parameter, list its inference sources, full type set, and the constraint basis for every operation in the body.
- Compile and run tests with a predeclared type, a named type sharing its underlying type, a zero value, and an interface containing a slice.
- Distinguish receiver type parameters, Go 1.27 method type parameters, and interface methods, and state the code's minimum Go version.

**Review checklist**

- Confirm that every type parameter preserves a real relationship instead of acting as one-use syntax decoration.
- Confirm that each constraint supports the required operations without excluding named types that should be accepted.
- Test failed inference, nil containers, and dynamic interface values rather than only integer literals.

**Prompt vocabulary**

Use the exact terms `type parameter`, `type argument`, `constraint`, `type set`, `type term`, `underlying type`, `constraint satisfaction`, `strict comparability`, `type inference`, `instantiation`, `receiver type parameter`, and `generic method`. Ask the model for an evidence chain covering declaration, inference, constraint checking, instantiation, and runtime dynamic values.

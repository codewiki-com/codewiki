---
title: 错误处理
description: Go 把错误作为值显式返回；这里讲清错误契约、错误包装、errors.Is、errors.As、多错误以及 typed nil 陷阱。
track: go
section: basics
...
错误也是 API 契约的一部分。一个包可能只承诺成功或失败，也可能公开稳定的<Term id="sentinel-error">哨兵错误（sentinel error）</Term>、返回有文档说明的具体类型，或者提供判断函数。调用方只能依赖包文档明确承诺的属性。
...
> [!PITFALL]
> 对每个依赖错误都使用 `%w`，可能会意外地把依赖的哨兵错误和类型变成自己的公开 API。
...
> [!PITFALL]
> `error` 接口只要包含具体类型就是非 nil，即使其中保存的具体指针是 nil。
...
> [!PITFALL]
> 生成的辅助函数和手写辅助函数有时会因为无效输入、文件不存在或网络失败而 panic，但调用方本可以正常处理这些情况。
...
生成的 Go 代码经常写出 `fmt.Errorf("operation failed: %v", err)`，保留了文本，却切断了错误链。其他常见错误包括根据 `err.Error()` 分支、用 `==` 比较被包装的哨兵错误、把 nil 具体指针作为 `error` 返回，以及在每个栈帧重复记录同一次失败。模型还会为了省去在生成签名中逐层传递错误而使用 `panic`，或者包装数据库驱动的错误，却没有决定这是否属于本包契约。

**让 AI 检查**

- 列出每个返回错误，并说明调用方可以依赖哨兵错误、具体类型、判断函数，还是只能依赖非 nil 失败。
- 跟踪每个包装器，证明 `%w`、`errors.Is` 和 `errors.As` 在跨包边界后仍保留预期原因。
- 查找 typed nil 返回、字符串匹配、重复日志、秘密泄漏，以及把预期失败变成 panic 的路径。

**审查清单**

- 运行一个成功用例和每种有文档说明的失败用例，其中要包括被包装的原因和 nil 自定义错误指针。
- 确认判断只依赖稳定的错误身份或类型，不依赖消息文案或合并错误的顺序。
- 确认只有一个拥有结果的边界报告失败，而且日志字段不含凭据或敏感载荷。

**提示词词汇**

使用准确术语：`error contract`（错误契约）、`sentinel error`（哨兵错误）、`custom error type`（自定义错误类型）、`error chain`（错误链）、`wrap with %w`（用 `%w` 包装）、`errors.Is`、`errors.As`、`errors.Join`、`typed nil`（带类型的 nil）和 `reporting boundary`（报告边界）。要求模型把供人阅读的上下文与供程序检查的分类信息分开。

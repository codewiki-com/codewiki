---
title: reflect.Type 与 reflect.Value
description: Go 反射在运行时检查类型和值；理解 Type、Value、Kind、可设置性和 nil 边界，才能安全编写动态代码。
track: go
...
类型的<Term id="method-set">方法集（method set）</Term>决定反射能找到哪些方法。`T` 与 `*T` 的方法集不同，所以值可能找不到只属于指针接收者的方法。`Call` 还要求实参数量和类型准确匹配，并会传播被调用函数的 panic。
...
`Port` 通过全部检查，所以写入原结构体。普通 `int` 不能赋给 `string`，未导出字段即使可寻址也不能设置。生产代码通常还会用标签建立允许修改的字段清单，避免外部输入直接选择 Go 字段名。
...
方法值的 `Type` 已经绑定接收者，所以 `NumIn()` 只计算显式的 `orderID` 参数。不存在的方法返回无效 `Value`，因此先检查 `IsValid()`。即使签名匹配，被调方法本身仍可能 panic，调度边界必须另行决定是否恢复。
...
`err` 是<Term id="typed-nil">带类型的 nil（typed nil）</Term>：动态类型为 `*Problem`，动态指针为 nil，所以接口本身不等于 `nil`。两个切片长度都为零，但只有一个切片为 nil。序列化或补丁语义若区分「缺失」与「空集合」，这项差异必须保留。

## 陷阱

> [!PITFALL]
> 在检查有效性前调用 `Type()`、`Elem()` 或 `Interface()`。`ValueOf(nil)`、失败的 `FieldByName` 以及 nil 指针的 `Elem()` 都可能产生无效 `Value`。

**修复方法：** 每次使用可能返回无效值的 API 后立即检查 `IsValid()`；确认 `Kind` 合适后，才能调用 `IsNil()` 或 `Elem()`。

> [!PITFALL]
> 把 `CanAddr()` 当成 `CanSet()`，或认为同包代码能通过反射修改未导出字段。可寻址值仍可能受可见性限制，直接 `Set` 会 panic。
...
> [!PITFALL]
> 用值接收者查找只存在于指针方法集中的方法。`MethodByName` 会返回无效 `Value`，随后直接 `Call` 就会 panic。
...
模型经常从 `reflect.ValueOf(input).Elem()` 开始，遗漏 input 为 nil、非指针或带 nil 指针的路径。它们还会把 `Kind` 相同误当成类型可赋值，把 `ConvertibleTo` 当作业务校验，或者在 `CanAddr()` 为真时尝试修改未导出字段。这些代码通常能够编译，只在错误输入上 panic。

动态接口中的风险更大。生成代码可能把用户提供的名称直接交给 `FieldByName` 或 `MethodByName`，把管理字段或危险方法意外暴露给请求方。另一类错误是为「提速」缓存请求中的 `reflect.Value`，从而延长对象生命周期并引入并发访问问题。

**让 AI 检查**

- 列出每个 `reflect.Value` 在各分支中的 `IsValid`、`Kind`、`CanInterface` 和 `CanSet` 前置条件，并指出可能 panic 的调用。
- 对每次 `Set`、`Convert` 和 `Call` 写出来源与目标的精确 `reflect.Type`，再检查数值范围、参数数量和返回值数量。
- 追踪来自请求、配置或模型输出的字段名与方法名，确认它们先经过固定允许清单和授权检查。

**审查清单**

- 用 nil 接口、带类型的 nil、错误 `Kind`、未导出字段、缺失字段和不匹配参数覆盖失败路径。
- 确认外部输入不能选择任意导出字段或方法，日志与错误也不会泄露反射读取的敏感值。
- 对照 Go 1.27 文档核对生成的符号，并针对重复元数据发现运行有代表性的基准。

**提示词词汇**

使用准确术语：`reflection`（反射）、`reflect.Type`（运行时类型描述）、`reflect.Value`（运行时值句柄）、`Kind`（种类）、`addressable`（可寻址）、`settable`（可设置）、`invalid Value`（无效值）、`typed nil`（带类型的 nil）、`method set`（方法集）和 `assignable versus convertible`（可赋值与可转换）。要求模型「列出所有 panic 前置条件」并说明外部名称如何通过 `allowlist`（允许清单）映射；只说「安全使用反射」不够具体。

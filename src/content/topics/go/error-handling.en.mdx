...
Generated Go often uses `fmt.Errorf("operation failed: %v", err)`, which preserves the text but breaks the chain. Other common failures are branching on `err.Error()`, comparing a wrapped sentinel with `==`, returning a nil concrete pointer as `error`, and logging the same failure at every stack frame. Models also reach for `panic` to avoid threading an error through a generated signature, or expose a database driver's error by wrapping it without deciding whether that is part of the package contract.

**Ask your AI to check**

- List every returned error and state whether callers may rely on a sentinel, a concrete type, a predicate, or only non-nil failure.
- Trace each wrapper and prove that `%w`, `errors.Is`, and `errors.As` preserve the intended cause across package boundaries.
- Find typed-nil returns, string matching, duplicate logging, leaked secrets, and panic paths for expected failures.

**Review checklist**

- Run one success case and each documented failure case, including a wrapped cause and a nil custom-error pointer.
- Verify that decisions use stable error identity or type, never message wording or joined-error order.
- Confirm that one owning boundary reports the failure and that its log fields contain no credentials or sensitive payloads.

**Prompt vocabulary**

Use `error contract`, `sentinel error`, `custom error type`, `error chain`, `wrap with %w`, `errors.Is`, `errors.As`, `errors.Join`, `typed nil`, and `reporting boundary`. Ask the model to separate human-readable context from machine-inspectable classification.

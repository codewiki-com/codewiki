...
模型经常从 `reflect.ValueOf(input).Elem()` 开始，遗漏 input 为 nil、非指针或带 nil 指针的路径。
它们也会把 `Kind` 相同误当成类型可赋值，把 `ConvertibleTo` 当作业务校验，或在 `CanAddr()` 为真时修改未导出字段。
动态 API 中更危险的版本是把用户提供的名称直接交给 `FieldByName` 或 `MethodByName`，无意中扩大了可修改状态和可调用方法。

旧语料还会让模型生成 Go 1.27 已弃用的 `reflect.PtrTo`，而当前名称是 `reflect.PointerTo`。
另一个常见错误是对任何 `Value` 调用 `IsNil()`，导致整数、结构体或 invalid 输入在错误处理路径再次 panic。
这些问题往往能通过编译，却只在少见输入或权限边界上暴露。

**让 AI 检查**

- 列出每个 `reflect.Value` 在各分支中的 `IsValid`、`Kind`、`CanInterface` 和 `CanSet` 前置条件，并指出任何可能 panic 的调用。
- 对每次 `Set`、`Convert` 和 `Call` 写出来源与目标的精确 `reflect.Type`，再检查数值范围、参数数量和返回值数量。
- 追踪所有来自请求、配置或模型输出的字段名与方法名，确认它们先经过固定允许清单和授权检查。

**审查清单**

- 用 nil 接口、带类型的 nil、错误 `Kind`、未导出字段、缺失字段和不匹配参数运行失败路径测试。
- 确认外部输入不能选择任意导出字段或方法，日志和错误也不会泄露反射读到的敏感值。
- 在 Go 1.27 文档中核对生成的 API 名称与弃用状态，并对重复的元数据发现运行有代表性的基准。

**提示词**

提示中应使用 `reflection`（反射）、`reflect.Type`（运行时类型描述）、`reflect.Value`（运行时值句柄）、`Kind`（种类）、`addressable`（可寻址）、`settable`（可设置）、`invalid Value`（无效值）、`typed nil`（带类型的 nil）、`method set`（方法集）和 `assignable versus convertible`（可赋值与可转换）。
还应明确要求“列出所有 panic 前置条件”和“外部名称只能通过 allowlist 映射”；仅说“安全使用反射”不足以约束生成结果。

...
模型常把“方法不修改状态”机械地翻译成值接收者，忽略结构体含锁、指针或切片，也忽略接收者变更对接口满足关系的影响。它还会因为 `x.M()` 可编译就把 `T` 赋给接口，或把嵌入解释成带重写的继承。另一类隐蔽错误是先创建值接收者方法值，再修改原对象，却期待回调读取新状态。

**让 AI 检查**

- 列出每个方法的接收者类型，并说明复制哪些字段、共享哪些底层对象。
- 分别证明 `T` 与 `*T` 满足哪些接口，并编译实际使用的断言。
- 跟踪每个方法值在何时保存接收者，以及保存的是值副本还是指针。

**审查清单**

- 对每个修改方法执行调用后状态测试，并对切片、映射和指针字段做别名测试。
- 运行 `go vet ./...`、`go test -race ./...`，并检查含锁类型是否被按值复制。
- 展开每个提升方法的完整选择器路径，检查歧义、`nil` 嵌入指针和接口方法集。

**提示词词汇**

使用 `value receiver`（值接收者）、`pointer receiver`（指针接收者）、`method set`（方法集）、`addressable value`（可寻址值）、`implicit address-taking`（隐式取地址）、`method value`（方法值）、`method expression`（方法表达式）和 `promoted method`（提升方法）。要求 AI 给出“接收者复制与接口满足矩阵”，比笼统地要求“检查 Go 方法”更容易暴露错误。

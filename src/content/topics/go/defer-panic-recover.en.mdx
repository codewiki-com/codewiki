...
Models often add `recover` mechanically at the top of a function without checking whether the panic occurs in a child goroutine, and without capturing a stack; the result either catches nothing or turns a programming bug into an empty 500 response. Another common failure is to write `defer file.Close()` and later call `log.Fatal` in the same function, assuming cleanup still runs. Generated batch code also puts `defer` inside an unbounded loop or discards a writer's close error.

Older training material also gets applied to Go 1.27 loop variables. Variables declared by a `for` clause or `range` short declaration are distinct per iteration under the Go 1.22-or-later rules. A variable declared outside the loop and reused by assignment can still be shared by closures. Review the `go.mod` language version and the exact loop form before mechanically adding or removing `item := item`.

**Ask your AI to check**

- List every exit path from the function, state which deferred calls run, and identify any `os.Exit`, `log.Fatal`, or non-returning call.
- Trace each panic path across goroutine boundaries to the first deferred function that calls `recover` directly, then state where control returns after recovery.
- Check the error contract for every `Close`, `Flush`, `Commit`, and `Rollback`, including whether the original error remains inspectable.

**Review checklist**

- Test cleanup order on normal return, ordinary error, and panic paths, and compare the observed output or side effects.
- For every `go` statement, confirm that panic policy, result reporting, and stack capture live inside the new goroutine.
- Check whether a `defer` in a loop keeps files, locks, connections, or memory alive until the outer function ends.

**Prompt vocabulary**

Use the precise terms `deferred call`, `argument evaluation at defer time`, `LIFO order`, `panic unwinding`, `direct call to recover`, `same-goroutine recovery`, `re-panic`, and `recovery boundary`. Asking for an exit-path and goroutine-ownership trace exposes boundary mistakes more reliably than asking to "add error handling."

...
Models often produce a zero-value prefix by writing `make([]T, n)` followed by a loop that calls `append`. They also describe `dst := src` as a copy. Another common failure is a helper that appends internally without returning the new slice, or a caller that ignores the return. Generated concurrent code may have several goroutines append to one slice; small test inputs can look fine despite races on the header and backing array.

**Ask your AI to check**

- Mark the backing-array owner for every slice and list every pair of slices whose ranges overlap.
- Trace each `append` with both `len == cap` and `len < cap` inputs, confirming that every returned value is used.
- Check the JSON shape of zero results and whether locks, single ownership, or disjoint indices protect every concurrent write.

**Review checklist**

- Compare every `make` length and capacity with the code's subsequent write pattern.
- Mutation-test assignments, return values, and subslices that claim independence, then observe whether the source data changes.
- Run `go test -race ./...` on shared-slice paths and test empty, single-element, and capacity-boundary inputs.

**Prompt vocabulary**

Use the exact terms `slice header`, `backing array`, `length`, `capacity`, `aliasing`, `full slice expression`, `append reallocation`, `shallow copy`, and `nil versus empty slice`. Ask the model to show `len`, `cap`, and sharing after every operation. That exposes more than a vague request to "check whether the slice is safe."

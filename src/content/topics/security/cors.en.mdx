...
Models often use `origin: true`, unconditional reflection, or a broad regular expression to silence a development error, then copy that policy onto production endpoints with cookies. They also put preflights behind authentication so they return `401`, add CORS headers only to successful responses, or omit `Vary: Origin` for a dynamic allowlist. Another dangerous pattern treats “the frontend is restricted by CORS” as authorization, leaving non-browser calls free to bypass the supposed boundary.

**Ask your AI to check**

- List each route's allowed origins, methods, request headers, exposed response headers, and credentials mode, and identify whether the gateway or application enforces them.
- Trace `OPTIONS`, successful responses, and every error path, confirming that actual responses receive a consistent, minimal set of CORS headers.
- Generate browser integration tests for two allowed origins, one denied origin, missing `Origin`, and literal `null`; label which assertions instead belong to authentication, authorization, or CSRF.

**Review checklist**

- Search for wildcards, request-origin reflection, `includes()`, unbounded regular expressions, and permissive defaults when an environment variable is empty.
- Confirm that credentialed requests use exact origins, dynamic responses merge `Vary: Origin`, and cookie policy is configured separately.
- Observe the preflight and actual request in a real browser, and verify that a CORS-hidden response never lets the server skip its security controls.

**Prompt vocabulary**

Use the exact terms `origin tuple`, `same-origin policy`, `CORS-safelisted request`, `preflight request`, `credentialed request`, `origin reflection`, `opaque response`, and `Vary: Origin`. Ask the model to map the browser, cache, and server trust boundaries instead of merely asking it to “fix the CORS error.”

...
Generated fetch endpoints often check `hostname.includes("trusted.example")`, reject only IPv4 strings, and then call a client that resolves DNS again. Models also enable default redirects, spread inbound headers into the outbound request, or claim that replacing an HTTPS hostname with its approved IP “pins DNS” while silently breaking correct TLS hostname handling. In agent and retrieval code, a model may treat tool-returned URLs as trusted even though retrieved pages, MCP data, email, or prompt text supplied them.

**Ask your AI to check**

- Trace every value that can affect scheme, host, port, resolved address, redirect, method, header, and request body from source to socket.
- Show how the approved DNS answer is bound to the actual connection while TLS still verifies the original hostname.
- Enumerate failure tests for mixed DNS answers, IPv4-mapped IPv6, redirect-to-private targets, oversized bodies, and unavailable policy services.

**Review checklist**

- Confirm that callers choose a server-defined service or pass through one centralized, default-deny outbound policy.
- Confirm that every DNS answer and redirect hop is checked and that the connection cannot perform a second uncontrolled lookup.
- Confirm that headers, credentials, time, bytes, decompression, concurrency, and egress routes have explicit bounds.

**Prompt vocabulary**

Use the exact terms `server-side request forgery (SSRF)`, `destination allowlist`, `special-purpose address`, `DNS rebinding`, `time-of-check/time-of-use (TOCTOU)`, `address pinning`, `redirect revalidation`, `egress filtering`, and `instance metadata service`. Ask for a source-to-socket data-flow review rather than a generic “secure this URL” pass.

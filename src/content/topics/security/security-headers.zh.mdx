...
模型常从旧博客拼出一套看似完整的头：全局复用 CSP nonce，用 `'unsafe-inline'` 修复构建错误，给所有路由复制同一 CSP，并把 HSTS 预加载直接用于尚未审计的子域名。它还可能在应用层正确设置头，却忽略 CDN 覆盖、错误响应和静态托管路径，导致评审看到的配置与浏览器收到的结果不同。

**让 AI 检查**

- 列出每个页面实际需要的脚本、样式、连接、frame、表单与浏览器功能，并把每项依赖映射到最小 CSP 或 Permissions Policy 指令。
- 跟踪 nonce 从密码学随机生成、请求上下文、模板属性到最终响应头的完整数据流，并检查缓存是否会复用它。
- 生成覆盖 `200`、重定向、`401`、`404`、`413`、`429` 与 `502` 的部署测试，比较应用、网关和浏览器观察到的最终头。

**审查清单**

- 搜索 `unsafe-inline`、`unsafe-eval`、宽泛通配符、固定 nonce、重复响应头和空配置时的允许默认值。
- 确认 HSTS 只从 HTTPS 发送，`includeSubDomains` 与预加载经过域名审计，并且证书续期和回滚都有负责人。
- 在真实浏览器中验证 CSP 阻止样本、frame 嵌入、MIME 不匹配和被禁用功能，不以 `curl` 输出替代执行结果。

**提示词词汇**

使用准确术语：`Content Security Policy`（内容安全策略）、`fetch directive`（获取指令）、`CSP nonce`（CSP 随机数）、`frame-ancestors`（祖先 frame 限制）、`HSTS max-age`（HSTS 有效期）、`MIME sniffing`（MIME 嗅探）、`referrer policy`（来源信息策略）和 `policy owner`（策略所有者）。要求模型给出“最终响应头矩阵”和“逐项放宽理由”，比笼统地要求“添加安全头”更容易审查。

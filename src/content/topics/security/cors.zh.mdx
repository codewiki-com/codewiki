...
模型经常用 `origin: true`、任意反射或过宽正则表达式消除开发环境报错，并把同一配置复制到带 Cookie 的生产端点。它还会把预检交给认证中间件而返回 `401`，只在成功响应上加 CORS 头，或在动态允许源时漏掉 `Vary: Origin`。另一个危险模式是把“CORS 已限制前端”写成授权理由，使非浏览器调用直接绕过边界。

**让 AI 检查**

- 列出每个路由的允许源、方法、请求头、响应头和 credentials mode，并指出配置是在网关还是应用层生效。
- 追踪 `OPTIONS`、成功响应和每条错误路径，确认实际响应都返回一致且最小的 CORS 头。
- 为两个允许源、一个拒绝源、缺失 `Origin` 和字面值 `null` 生成浏览器集成测试，并说明哪些检查属于认证、授权或 CSRF。

**审查清单**

- 搜索通配符、请求头反射、`includes()`、无边界正则以及环境变量为空时的开放默认值。
- 确认凭证请求使用精确源，动态源响应合并 `Vary: Origin`，Cookie 策略另有明确配置。
- 在真实浏览器中观察预检与实际请求，并验证被 CORS 隐藏的响应不会因此跳过服务端安全控制。

**提示词词汇**

使用准确术语：`origin tuple`（源元组）、`same-origin policy`（同源策略）、`CORS-safelisted request`（CORS 安全列表请求）、`preflight request`（预检请求）、`credentialed request`（凭证请求）、`origin reflection`（源反射）、`opaque response`（不透明响应）、`Vary: Origin`。要求模型给出浏览器、缓存与服务端三方的信任边界，而不只是“修复 CORS 报错”。

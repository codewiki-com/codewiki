...
模型常生成 `findById(req.params.id)`，并把认证中间件当成对象级授权；它也会用对象展开更新模型，让 `role`、`tenantId` 或状态字段进入批量赋值。另一类错误是只解码 JWT 载荷、不验证签名和声明，或者把 CORS 配置误写成服务端访问控制。

资源控制同样容易出现看似完整的错误实现。生成代码会在每个进程创建内存限流器、信任任意 `X-Forwarded-For`，只限制请求次数却不限制页大小和并发作业。日志助手还可能序列化整个请求，把 Authorization header 和嵌套令牌一起写入日志。

**让 AI 检查**

- 列出每个由客户端控制的对象 ID、动作和属性，并追踪它们到最终数据库读写处的授权证据。
- 生成主体 × 租户 × 对象所有者 × HTTP 方法的否定测试矩阵，并标明预期的 `401`、`403` 或 `404`。
- 检查所有请求体、查询、批量操作和下游调用的独立预算，以及多实例部署中的原子计数方式。

**审查清单**

- 验证凭据后只创建最小化主体，不把未验证声明或客户端角色传给策略层。
- 对每个对象和字段执行默认拒绝的授权，并测试旧版本与替代路由是否经过同一控制。
- 对错误、响应与日志执行字段允许列表，再用真实嵌套载荷测试秘密和个人数据不会出现。

**提示词词汇**

使用准确术语：`authentication`（认证）、`authorization`（授权）、`principal`（主体）、`deny by default`（默认拒绝）、`least privilege`（最小权限）、`object-level authorization`（对象级授权）、`property-level authorization`（属性级授权）、`mass assignment`（批量赋值）、`rate limiting`（速率限制）和 `request budget`（请求预算）。要求模型给出策略输入、执行位置和否定测试，而不是笼统地「添加安全检查」。

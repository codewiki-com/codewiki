...
Models often generate a convincing Express or frontend middleware stack but place controls at the wrong boundary: they use a regex to “sanitize” every input, omit object-level authorization after authentication, treat a CORS allowlist as server permission, or concatenate strings into `innerHTML`. They also reuse a fixed CSP nonce, accept `isAdmin` or `tenantId` from the request body, log all request headers, and copy deprecated packages or response-header settings.

**Ask your AI to check**

- List the full path of every client-controlled value from its source to an HTML, SQL, filesystem, URL, or command sink, and identify the actual control at each sink.
- For every endpoint, list the principal, action, target object, and rejection side effects; then generate cross-user, cross-tenant, and alternate-method counterexample tests.
- Distinguish the boundaries enforced by authentication, authorization, CORS, CSRF, CSP, and input validation, and identify any configuration being used as a universal defense.

**Review checklist**

- Run accepted and rejected tests with quotes, markup, encoded variants, and oversized values; confirm that the correct context always interprets the data.
- Replay every object operation as another valid user and tenant; confirm that reads, writes, logs, and downstream calls are all denied.
- Inspect the cookies and headers that a browser actually receives, and test cross-origin, framing, missing-token, and error-response paths.

**Prompt vocabulary**

Use the precise terms `trust boundary`, `untrusted input`, `contextual output encoding`, `parameterized query`, `object-level authorization`, `deny by default`, `same-origin policy`, `CSRF token`, `Content Security Policy`, and `negative-path test`. Asking for a data flow and execution points produces more verifiable results than “make it more secure.”

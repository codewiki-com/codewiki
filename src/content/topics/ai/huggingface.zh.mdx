...
生成代码经常省略 `model` 或 `revision`，于是 Pipeline 使用默认模型，或部署在仓库更新后改变行为。
它也会混合不同仓库的 tokenizer 与权重，把 `trust_remote_code=True` 当成兼容性修复，或继续生成
已被 `huggingface_hub` 1.x 忽略的 `resume_download` 和 `local_dir_use_symlinks` 参数。

另一类错误发生在输入与设备边界。生成代码可能对任意长度文本关闭截断，在 CPU 上强制半精度，
同时使用 `device_map` 和手动 `.to()`，或在每个 Web 请求内重新构造 Pipeline。代码可以通过语法
检查，却在长输入、冷缓存、无 GPU 或并发启动时失败。

**让 AI 检查**

- 列出每个 `from_pretrained` 和 `pipeline` 调用的仓库 ID、提交哈希、任务、tokenizer 来源与远端代码设置。
- 在空缓存和离线模式下分别执行启动测试，并报告实际下载的文件、总大小与缺失制品。
- 用最长允许输入和超长输入运行批量测试，记录截断策略、张量形状、设备、数据类型和峰值内存。

**审查清单**

- 确认模型卡、许可证、目标任务和本地评测证据都对应部署的提交哈希。
- 确认 tokenizer、配置、处理器和权重来自同一经过审核的快照，远端代码默认关闭。
- 确认 token 不进入源码或日志，并在新进程、空缓存、离线环境中验证启动路径。

**提示词词汇**

使用准确术语：`repository ID`（仓库 ID）、`commit hash`（提交哈希）、`revision pinning`
（固定修订版本）、`model card`（模型卡）、`artifact allowlist`（制品允许列表）、
`local_files_only`（仅使用本地文件）、`trust_remote_code`（信任远端代码）和
`tokenizer-model compatibility`（tokenizer 与模型兼容性）。要求 AI 给出“仓库、修订版本、文件、
加载类、设备”的清单，比“写一段 Hugging Face 推理代码”更容易暴露隐含默认值。

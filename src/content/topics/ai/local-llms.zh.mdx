...
生成的本地 LLM 客户端经常沿用旧版 llama.cpp 的 `./main` 或 `./server` 命令，
也会把 Ollama 流误当成一个 JSON 文档。它还可能按模型文件大小分配全部内存，忽略 KV 缓存和并发，
或者把服务绑定到所有网卡却没有补上认证、授权与限流。另一个隐蔽错误是吞掉终止原因，
让被长度限制截断的 JSON 或代码进入下游。

**让 AI 检查**

- 对照锁定版本的官方文档，列出每个命令、端点、请求字段和终止字段，并标出旧接口。
- 根据目标模型、量化、上下文和并发解释内存组成，明确哪些数字只是权重下限，哪些必须实测。
- 跟踪一次流式断线和一次长度截断，说明客户端如何标记不完整结果、重试以及阻止下游副作用。

**审查清单**

- 核对模型摘要、许可证、聊天模板、运行器版本与评测记录是否属于同一发布制品。
- 在最长输入和目标并发下测量峰值内存、排队时间、首词元延迟与端到端延迟。
- 确认服务监听地址、网络访问控制、日志删减以及每种非正常终止路径。

**提示词词汇**

使用准确术语：`local inference`（本地推理）、`weight quantization`（权重量化）、
`context window`（上下文窗口）、`KV cache`（键值缓存）、`prefill`（输入预填充）、
`decode`（逐词元生成）、`time to first token`（首词元时间）、`tokens per second`（每秒词元数）、
`artifact digest`（制品摘要）和 `CPU/GPU offload`（CPU/GPU 卸载）。要求给出目标运行器版本、
硬件、上下文和并发；只说「优化本地模型」无法约束答案。

...
Models frequently generate LangChain 0.x imports alongside 1.x `create_agent`, producing code that is internally inconsistent before it runs. They also guess that every pipe passes strings, call `ainvoke` without `await`, treat a fake test model as production-ready, or create an in-memory history dictionary with no tenant boundary. In agent code, the costliest failure is often outside the model call: a generated retry wraps a non-idempotent tool and duplicates a real action.

**Ask your AI to check**

- List the concrete input and output type at every runnable edge, including message and dictionary keys.
- Verify every imported symbol against the installed LangChain 1.x packages and flag anything that requires `langchain-classic`.
- Trace retries, thread identifiers, authorization checks, and side effects through one full model–tool–model cycle.

**Review checklist**

- Run the pipeline with malformed, empty, overlapping, and provider-error inputs; assert intermediate shapes as well as the final answer.
- Interleave two session IDs and restart the state backend to test isolation and persistence separately.
- Stub the model into repeated identical tool calls and confirm that authorization and idempotency live inside the tool boundary.

**Prompt vocabulary**

Use `Runnable input/output contract`, `LCEL sequence`, `RunnableParallel`, `RunnableBranch`, `chat message`, `tool schema`, `agent loop`, `checkpointer`, `thread_id`, `idempotency key`, and `provider capability`. Ask for an execution trace with concrete state transitions; “make this LangChain code robust” does not identify which boundary must be verified.

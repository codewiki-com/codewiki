...
**生成代码在这里常犯的错误：** 模型会复用旧教程中的 `client.completions.create(...)`、已经过时的模型 ID 或其他供应商的消息角色格式。即使代码能发出请求，它也常把 `content` 当成纯文本，只处理第一个工具调用，漏掉助手工具块，或把 SDK 自动重试扩展到退款等业务副作用。针对 Claude Sonnet 5，生成代码还可能照搬非默认 `temperature`、`top_p` 或 `top_k`，而当前模型会对此返回 400。

**让 AI 检查：**

1. “列出这段代码能收到的每种 `content` 块和 `stop_reason`，指出没有显式处理的分支。”
2. “从初始用户消息追踪到 `tool_use`、授权检查、执行、`tool_result` 和下一次请求，核对 ID 与历史是否完整。”
3. “把模型 HTTP 重试与有副作用的工具重试分别画出来，指出每个边界的幂等键和持久结果在哪里。”

**审查清单：**

- 模型 ID、API 版本和 SDK 入口都来自当前配置与官方文档，没有旧式 Completions 调用。
- 内容块、停止原因、流内错误和多工具调用都有测试，未知变体不会被误当作文本。
- 密钥不进入客户端或日志，工具执行再次做认证、授权、输入验证和幂等控制。

**提示词词汇：** 使用 “Messages API（消息 API）”“content block（内容块）”“stop reason（停止原因）”“client tool（客户端工具）”“tool result（工具结果）”“Server-Sent Events（服务器发送事件）”“transport retry（传输重试）” 和 “idempotency key（幂等键）”。这些词能让生成任务指向协议边界，而不是模糊地要求“接入 Claude”。

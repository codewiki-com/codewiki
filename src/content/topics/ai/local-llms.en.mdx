...
Generated local-LLM clients often retain old llama.cpp `./main` or `./server` commands and parse an Ollama stream
as one JSON document. They also allocate memory from the model file size while ignoring the KV cache and concurrency,
or expose a service on every interface without adding authentication, authorization, and rate limits.
A quieter failure drops the stop reason and passes length-truncated JSON or source code downstream.

**Ask your AI to check**

- Compare every command, endpoint, request field, and terminal field with the pinned version's official documentation; identify stale interfaces.
- Explain memory components for the target model, quantization, context, and concurrency; separate weight floors from values that require measurement.
- Trace one stream disconnect and one length truncation; show how the client marks incomplete output, retries, and prevents downstream side effects.

**Review checklist**

- Confirm that the model digest, license, chat template, runner version, and evaluation record describe one release artifact.
- Measure peak memory, queue time, time to first token, and end-to-end latency at maximum input and target concurrency.
- Verify the listen address, network access controls, log redaction, and every abnormal completion path.

**Prompt vocabulary**

Use the exact terms `local inference`, `weight quantization`, `context window`, `KV cache`, `prefill`, `decode`,
`time to first token`, `tokens per second`, `artifact digest`, and `CPU/GPU offload`. State the target runner version,
hardware, context, and concurrency. "Optimize the local model" is too vague to constrain an answer.

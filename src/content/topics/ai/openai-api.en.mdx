...
**What generated code gets wrong here:** Models mix fields from legacy Completions, Chat Completions, and Responses, such as passing `messages` to `responses.create` or reading `choices[0].message.content`. They also assume `output[0]` is text, ignore `incomplete` status, omit new `instructions` on continuation, and treat frontend environment variables as key protection. Retry wrappers generated to “improve reliability” sometimes catch every exception while repeating both the model request and business side effects.

**Ask your AI to check:**

1. “Using the current Responses API, list every request field this code sends and every response field it reads, then mark any field imported from an older API.”
2. “Enumerate the response statuses, output item types, and stream terminal events this path can receive, and identify branches that accept incomplete output.”
3. “Trace retries from the API call into database, email, or payment code, and identify the side-effect boundary, idempotency key, and durable result.”

**Review checklist:**

- The key exists only in a controlled server runtime, model and prompt versions come from configuration, and API fields match current official documentation.
- Code checks response status before dispatching output by type; multi-turn calls send trusted instructions again and validate conversation ownership.
- A stream commits only after its completion event, errors are classified, and every side effect has independent authorization and idempotency control.

**Prompt vocabulary:** Use exact terms: `Responses API`, `response item`, `output_text`, `previous_response_id`, `conversation state`, `terminal event`, `Retry-After`, and `idempotency key`. Ask the AI to draw state transitions across request, response, validation, and commit. “Make the API call more robust” is too vague to expose which operations it plans to retry.

...
Generated Java often wraps an entire method in `catch (Exception error)` and returns `null`, `false`, or an empty list. That merges parse errors, I/O failures, and programming defects into an apparently successful result. Models also produce hand-written `finally` cleanup, omit the `cause` when wrapping, or catch every exception in retry helpers, thereby retrying deterministic argument errors and swallowing `InterruptedException`.

Another concrete failure is getting the type relationship wrong: catching a parent before its child or generating `catch (IOException | FileNotFoundException error)`. The first makes the later handler unreachable; the second places related types in one multi-catch. Neither compiles. Even when code does compile, verify that its exception mapping does not expose file paths, SQL text, tokens, or request content.

**Ask your AI to check**

- List the exception subclasses each `catch` actually covers, and say whether this layer recovers, translates, or propagates each one.
- Trace every exception wrapper, confirm the original is retained as the `cause`, and check whether logging still includes suppressed exceptions.
- Inspect all blocking calls and retry loops, confirming that broad catches do not swallow `InterruptedException` and non-retryable failures are not retried.

**Review checklist**

- Exercise success, checked failure, runtime defect, resource-close failure, and thread interruption as separate tests.
- Confirm every resource closes after partial initialization and body failure, including reverse order and the contents of `getSuppressed()`.
- Inspect user-facing errors and log fields for enough correlating context without sensitive data exposure.

**Prompt vocabulary**

Use the exact terms `checked exception`, `unchecked exception`, `exception propagation`, `stack unwinding`, `exception translation`, `cause chain`, `suppressed exception`, `try-with-resources`, and `interrupt status`. Ask for each failure path's exception type, catch boundary, resource state, and caller-visible result; “add error handling” does not constrain those semantics.

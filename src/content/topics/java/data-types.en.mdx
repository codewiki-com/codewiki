...
Generated Java often maps nullable JSON numbers to wrappers and then implicitly unboxes them in comparisons or arithmetic. Happy-path samples pass, while a missing field throws `NullPointerException`. Models also mistake a `long` result variable for overflow protection, turn monetary decimals into cents with `double` and a cast, or slice identifiers by `char` even when they may contain supplementary code points. Another common patch is to add a cast solely to silence the compiler, hiding the absent range check.

**Ask your AI to check**

- List every primitive conversion and implicit unboxing site, then state its input bounds, precision loss, and null behavior.
- Annotate the intermediate type of every integer expression and verify that widening occurs before any operation that can overflow.
- Generate focused tests with zero, boundary values, out-of-range values, NaN, missing fields, and supplementary Unicode code points.

**Review checklist**

- Match each declared type to the business range, and decide whether narrowing must reject or accept a truncated result.
- Inspect wrapper equality and every unboxing path, especially conditional expressions, collection iteration, and overloaded calls.
- Check decimal construction and rounding for money, and whether text logic counts code units, code points, or graphemes.

**Prompt vocabulary**

Use the exact terms `primitive type`, `reference type`, `widening conversion`, `narrowing conversion`, `binary numeric promotion`, `autoboxing`, `unboxing`, `wrapper identity`, and `UTF-16 code unit`. Ask for a conversion table and boundary tests; “check type safety” is too vague to expose these failures.

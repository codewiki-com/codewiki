...
模型经常生成能够展示语法、却没有完整边界行为的控制流：数组循环使用 `<= length`，`switch` 表达式漏掉 `default`，或者在块规则中写 `break value` 而不是 `yield value`。它还可能把用于 `null` 防护的 `&&` 改成 `&`，让右侧方法调用照常发生。另一类错误是先用 `int` 完成乘法，再把损坏的结果赋给 `long`。

**让 AI 检查**

- 列出每个局部变量的声明、作用域与所有到达首次读取的控制流路径，确认明确赋值成立。
- 展开每个含 `&&`、`||`、`?:` 或方法调用的表达式，按 Java 从左到右与短路规则标出实际求值顺序。
- 为每个循环和 `switch` 枚举空输入、边界输入、提前退出与未覆盖路径，并给出能触发它们的测试。

**审查清单**

- 用空集合、单元素、最大索引和刚越过阈值的值测试循环边界。
- 检查算术中间结果的类型，并确认溢出是允许回绕还是必须拒绝。
- 确认箭头与冒号 `switch` 没有意外混用，且每个表达式路径都产生结果或明确抛出异常。

**提示词词汇**

使用准确术语：`definite assignment`（明确赋值）、`lexical scope`（词法作用域）、`left-to-right evaluation`（从左到右求值）、`short-circuit evaluation`（短路求值）、`guard clause`（卫语句）、`switch expression exhaustiveness`（`switch` 表达式完整性）、`abrupt completion`（突然完成）。要求 AI 给出控制流路径和边界表，比“检查 Java 语法”更容易暴露语义错误。

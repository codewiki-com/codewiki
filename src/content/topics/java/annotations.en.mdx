...
Models often create a custom runtime annotation without `@Retention(RUNTIME)`, then treat reflection's `null` as permission to skip validation. They also call `getAnnotation()` for a repeatable type, assume `@Inherited` propagates through interfaces and overridden methods, or read a type-argument annotation as if it annotated the method declaration. Generated processor configuration commonly preserves an old build assumption by adding a JAR to the class path without explicitly enabling processing for Java 25.

**Ask your AI to check**

- For every custom annotation, list the consumer, reading phase, `@Target`, `@Retention`, and policy when it is absent.
- For every reflection query, state whether it needs direct, inherited, repeatable-container, or type-use semantics, and name the corresponding API.
- Run a clean build with the target `--release` and real build configuration to prove the processor generates the expected file or diagnostic.

**Review checklist**

- Test queries with zero, one, and multiple annotations, including superclass, interface, and overriding-method boundaries.
- Confirm that real consumers enforce authorization, transactions, and validation, with a safe default when configuration is missing.
- Inspect processor paths, explicit enablement options, generated directories, and incremental-build caches.

**Prompt vocabulary**

Use precise terms: `annotation interface`, `annotation element`, `meta-annotation`, `retention policy`, `declaration annotation`, `type-use annotation`, `directly present`, `indirectly present`, `repeatable container`, and `annotation-processing round`. Ask the model for a consumer-and-visibility matrix; “add an annotation” doesn't define verifiable behavior.

...
**What generated code gets wrong here.** Models often generate public setters and mutable collection getters, then claim the object is “encapsulated.” They also build inheritance hierarchies for code reuse even when subtypes don't preserve the substitution contract. More subtle failures include calling overrides from base constructors, generating `equals()` without a consistent `hashCode()`, and bypassing an existing polymorphic boundary with a growing `instanceof` chain.

**Ask your AI to check.** Ask it to list every class invariant and map each one to the constructors and mutating methods that preserve it. Ask it to start from one base-type caller and compare every subtype's preconditions, results, exceptions, and side effects. Ask it to compile with `javac -Xlint:all` and generate tests for nulls, boundaries, equality, and substitute implementations.

**Review checklist.**

- Does each piece of mutable state have one clear owner, and does any public method leak a collection, array, or partially constructed `this`?
- Can every subtype replace its base without strengthening preconditions or weakening postconditions?
- Do `equals()`, `hashCode()`, copy rules, and thread-safety promises match the object's identity and lifetime?

**Prompt vocabulary.** Use `class invariant`, `encapsulation boundary`, `subtype polymorphism`, `dynamic dispatch`, `Liskov substitution`, `composition over inheritance`, `defensive copy`, and `object identity versus value equality` instead of “check OOP best practices” to constrain the review precisely.

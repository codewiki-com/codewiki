...
Models often produce control flow that demonstrates syntax but has incomplete boundary behavior: an array loop uses `<= length`, a `switch` expression omits `default`, or a block rule writes `break value` instead of `yield value`. They may replace a null-guarding `&&` with `&`, causing the right-hand call to run anyway. Another recurring error multiplies as `int` first and assigns the damaged result to `long`.

**Ask your AI to check**

- List each local variable's declaration, scope, and every control-flow path to its first read; confirm definite assignment on all of them.
- Expand expressions containing `&&`, `||`, `?:`, or method calls and mark their actual evaluation order under Java's left-to-right and short-circuit rules.
- Enumerate empty input, boundary input, early exit, and uncovered paths for every loop and `switch`, then supply a test that reaches each one.

**Review checklist**

- Test loop bounds with empty collections, one element, the maximum valid index, and a value just beyond the threshold.
- Inspect the type of every arithmetic intermediate and decide whether overflow may wrap or must be rejected.
- Confirm that arrow and colon `switch` forms are not mixed accidentally and that every expression path yields a result or explicitly throws.

**Prompt vocabulary**

Use the exact terms `definite assignment`, `lexical scope`, `left-to-right evaluation`, `short-circuit evaluation`, `guard clause`, `switch expression exhaustiveness`, and `abrupt completion`. Asking for a control-flow path and boundary table exposes semantic mistakes more reliably than “check the Java syntax.”

...
Models add a second abstract method to `@FunctionalInterface` to make a lambda "more complete," breaking every lambda call site. They also generate `default boolean equals(Object other)` or `default String toString()`, although an interface cannot use a default to override those public instance methods of `Object`. Other patches call a static interface factory as if implementations inherited it, or assert that adding a `default` is always compatible without checking same-signature methods in implementation classes and other superinterfaces.

**Ask your AI to check**

- List the declaration site and abstract/default status of every same-signature instance method in the interface graph, then derive the winner using class precedence and the most-specific-interface rule.
- Cleanly compile every implementation and lambda call site with `javac -Xlint:all -Werror`, then separately run compatibility tests that pair old implementation binaries with the new interface.
- List every interface field, default-method side effect, and exception translation, and state the owner, concurrency boundary, and lifetime of the resulting state.

**Review checklist**

- Run the same contract tests against at least two real implementations and one test double without branching on concrete classes in the tests.
- Check for new abstract methods, conflicting defaults, mutable fields, and default declarations that conflict with `Object` methods.
- Record separate source, binary, and behavioral compatibility conclusions for a public interface change; reject a bare claim of "backward compatible."

**Prompt vocabulary**

Use precise terms: `interface contract`, `nominal subtyping`, `default method`, `most-specific interface`, `class-wins rule`, `functional interface`, `single abstract method`, `SAM conversion`, `source compatibility`, and `binary compatibility`. Ask for a method-candidate table and a complete compiler command; "check polymorphism" is too vague to expose a conflict.

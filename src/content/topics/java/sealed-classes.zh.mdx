...
生成器经常把 `sealed` 根类型与获准子类分到未命名模块的不同包，或者忘记给普通直接子类加 `final`、`sealed` 或 `non-sealed`。它还会为“保险”添加 `default`，从而破坏新增变体触发重新审查的效果；另一种常见错误是把插件接口密封后，又声称第三方仍可任意实现。

生成的模式 `switch` 也常忽略 `null` 和分离编译。它可能正确覆盖当前源代码中的变体，却没有测试旧消费者遇到新实现类的部署组合。对 `non-sealed` 分支，模型还可能逐个匹配当前实现，误以为这些实现构成封闭集合。

**让 AI 检查**

- 列出每条直接继承边，并验证两端是否满足具名模块或未命名模块的位置规则。
- 找出所有消费该根类型的模式 `switch`，说明 `default`、`case null` 与每个 `non-sealed` 分支的影响。
- 模拟新增一个获准直接子类型，重新编译所有消费者，并测试新旧二进制混合部署。

**审查清单**

- 每个直接获准子类型都明确终止、继续密封或刻意开放分支。
- 无 `default` 的封闭层次 `switch` 覆盖全部直接分支，并具有明确的空值策略。
- 构建按承诺的 `--release` 与模块路径编译，且测试覆盖分别部署的生产者和消费者。

**提示词词汇**

使用精确术语 `sealed class`（密封类）、`sealed interface`（密封接口）、`permits clause`（许可子句）、`permitted direct subclass`（获准直接子类）、`non-sealed branch`（非密封分支）、`exhaustive pattern switch`（穷尽模式 switch）和 `separate compilation`（分离编译）。要求生成器给出“直接子类型表”和“层次演进测试”，比笼统地要求“使用 sealed”更容易暴露边界错误。

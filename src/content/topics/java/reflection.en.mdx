...
Models often generate `getDeclaredMethod(name, args[i].getClass())`, immediately breaking primitives, interface parameters, `null`, and overloads; they then patch it with `setAccessible(true)` and request global `--add-opens`. They also scan and invoke every same-named member without filtering `static`, bridge, or synthetic methods, or feed arbitrary request values to `Class.forName()`, creating unauthorized type and operation selection. Proxy templates may call `proxy.toString()` in the handler log and recurse, then swallow the business exception outside `InvocationTargetException`.

**Ask your AI to check**

- List the name, exact parameter types, inheritance rule, allowed modifiers, and ambiguity handling for every reflective lookup.
- Draw an access matrix containing caller module, target module, `exports`, `opens`, and the actual startup arguments.
- For every `invoke()`, trace the final public form of lookup failure, access failure, bad arguments, target exceptions, and proxy exceptions.

**Review checklist**

- Test member selection with primitives, interface implementations, `null`, overloads, inherited methods, and bridge methods.
- Run modular tests without extra `--add-opens` and confirm failure becomes a clear deployment diagnostic rather than a silent skip.
- Restrict loadable classes and invocable methods, then check that caches, logs, and proxy object methods don't leak class loaders or sensitive values.

**Prompt vocabulary**

Use the exact terms `reflection`, `member lookup`, `declared member`, `reflective access`, `deep reflection`, `opens directive`, `InvocationTargetException`, `dynamic proxy`, and `bridge method`. Ask for a signature allowlist, module-access matrix, and exception-translation table instead of vaguely requesting “more robust reflection.”

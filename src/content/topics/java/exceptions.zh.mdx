...
生成的 Java 代码经常用 `catch (Exception error)` 包住整段方法，再返回 `null`、`false` 或空列表。这会把解析错误、I/O 失败和程序缺陷合并成一个看似成功的结果。模型还会生成手写 `finally` 关闭逻辑、包装异常时遗漏 `cause`，或在重试助手里捕获所有异常，从而重试确定性的参数错误并吞掉 `InterruptedException`。

另一个具体故障是把类型关系写错：先捕获父类再捕获子类，或生成 `catch (IOException | FileNotFoundException error)`。前者让后续处理器不可达，后者把有继承关系的类型放入多重捕获；两种代码都无法编译。即使代码通过编译，也要验证异常映射是否暴露文件路径、SQL 文本、令牌或请求内容。

**让 AI 检查**

- 列出每个 `catch` 实际覆盖的异常子类，并说明当前层对每一种是恢复、转换还是传播。
- 跟踪每次异常包装，确认原始失败通过 `cause` 保留，并检查日志是否还输出被抑制异常。
- 检查所有阻塞调用和重试循环，确认 `InterruptedException` 不会被宽泛捕获吞掉，且不可重试失败不会被重试。

**审查清单**

- 对成功、检查型失败、运行时缺陷、资源关闭失败和线程中断分别运行测试。
- 确认每个资源在部分初始化与主体失败时仍会关闭，并验证逆序关闭及 `getSuppressed()`。
- 检查面向用户的错误与日志字段，确保既有可关联的上下文，又不泄露敏感数据。

**提示词词汇**

使用准确术语：`checked exception`（检查型异常）、`unchecked exception`（非检查型异常）、`exception propagation`（异常传播）、`stack unwinding`（栈展开）、`exception translation`（异常转换）、`cause chain`（原因链）、`suppressed exception`（被抑制异常）、`try-with-resources`（资源自动关闭语句）、`interrupt status`（中断状态）。要求 AI 给出每条失败路径的异常类型、捕获边界、资源状态和调用方可见结果；“加上错误处理”无法约束这些语义。

---
title: 面向对象编程
description: Java 对象把状态与行为组织在明确边界内；理解类不变量、动态分派、可替换性与组合，才能设计可靠的对象模型。
track: java
...
<TLDR>
  <TLDRCell label="what">
    Java 面向对象编程用类定义对象的状态、行为和边界，再通过引用操作对象。对象不是字段的容器，而是负责维护自身契约的协作单元。
  </TLDRCell>
  <TLDRCell label="trap">
...
对象边界解决的是状态由谁负责的问题。若余额可以由任意调用方直接改写，每个调用点都必须重复检查规则；若账户只公开 `deposit()` 和 `withdraw()`，校验就集中在拥有余额的对象内。这种把表示隐藏在 API 后面的做法叫作<Term id="encapsulation">封装（encapsulation）</Term>。
...
<TryToBreak items={['把开户余额改为 -1', '存入 0 分', '连续取款直到余额恰好为 0']} />
...
> [!PITFALL]
> 为每个字段机械生成 getter 和 setter，会把对象退化成可由外部任意改写的数据袋。setter 各自校验也可能允许跨字段的非法组合，例如结束时间早于开始时间。
...
> [!PITFALL]
> 子类会继承可见 API、生命周期假设和覆盖点，而不只是几行实现。若子类必须拒绝父类允许的输入，或改变结果单位和副作用，它就破坏了替换契约。
...
**生成代码在这里容易出什么错。** 模型常生成公开 setter 和可变集合 getter，却声称对象“已封装”；它也会为了代码复用建立不满足替换契约的继承层次。更隐蔽的失败包括在父类构造器中调用覆盖方法、只生成 `equals()` 不生成一致的 `hashCode()`，以及用不断扩大的 `instanceof` 链绕过已有的多态边界。

**让 AI 检查这些内容。** 要求它列出每个类不变量，并逐条标明构造器和修改方法如何保持不变量；要求它从一个基类型调用方出发，对每个子类型比较前置条件、结果、异常与副作用；要求它用 `javac -Xlint:all` 编译，并生成覆盖空值、边界值、相等性和替代实现的测试。

**审查清单。**

- 每份可变状态是否只有明确的所有者，公开方法是否泄漏了集合、数组或未完成构造的 `this`？
- 每个子类型能否在不增加前置条件、不削弱后置条件的情况下替代基类型？
- `equals()`、`hashCode()`、复制规则和线程安全承诺是否与对象的标识及生命周期一致？

**提示词词汇。** 使用 `class invariant`（类不变量）、`encapsulation boundary`（封装边界）、`subtype polymorphism`（子类型多态）、`dynamic dispatch`（动态分派）、`Liskov substitution`（里氏替换）、`composition over inheritance`（组合优于继承）、`defensive copy`（防御性复制）和 `object identity versus value equality`（对象标识与值相等），比“检查 OOP 最佳实践”更能限定审查目标。

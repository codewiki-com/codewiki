...
Generated routers often default missing or unrecognized selectors to the newest version, which turns typos into silent contract upgrades. Models also duplicate complete handlers per version, spread persistence objects into public JSON, and call any added response property backward-compatible without checking strict clients. Lifecycle middleware is especially error-prone: older training examples still produce `Deprecation: true`, `Deprecation-Date`, or an ISO string in `Sunset`.

**Ask your AI to check**

- Enumerate every observable request and response difference between the supported versions, including defaults, errors, headers, ordering, and side effects.
- Trace missing, malformed, conflicting, deprecated, and unsupported selectors through routing and cache behavior.
- Compare lifecycle headers with RFC 9745 and RFC 8594, then show the exact serialized values in a test.

**Review checklist**

- Run provider and representative consumer contract tests for every supported version.
- Confirm that version selection happens before expensive or state-changing work and never silently falls through.
- Verify that telemetry identifies remaining consumers without logging credentials or sensitive payloads.

**Prompt vocabulary**

Use the terms `breaking change`, `backward compatibility`, `version selector`, `tolerant reader`, `request adapter`, `response presenter`, `contract test`, `Deprecation Structured Field Date`, `Sunset HTTP-date`, and `Vary cache key`. Ask for a compatibility matrix and counterexamples; “add API versioning” is too vague to define a safe policy.

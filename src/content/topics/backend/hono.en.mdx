...
Models often copy `app.listen()` into a Hono application without adding the correct Node.js runtime
adapter, or read `process.env` in Cloudflare code, so local type checks pass and deployment fails.
They also assert the result of `c.req.json()` as an interface, register authentication after routes,
or call `next()` from middleware without awaiting it or returning a response. A generated RPC client
may derive its type from an application value that discarded chained return types, leaving routes out,
or it may drift from the server version actually deployed.

**Ask your AI to check**

- List the middleware each route actually traverses, in registration order, including short-circuit branches and the response phase after `await next()`.
- Identify the runtime validator for every request input and prove that business handlers read only the result of `c.req.valid()`.
- Separate the application core from its Node, Cloudflare, Bun, or Deno adapter, listing every runtime-specific API and binding.

**Review checklist**

- Exercise success, unauthorized, validation-failure, not-found, and exception responses with real `Request` objects, asserting status, relevant headers, and body.
- Confirm every protected route is registered after authentication middleware and every middleware either returns a `Response` or correctly awaits `next()`.
- Verify RPC types come from the final chained route, client and server versions are compatible, and runtime contract tests remain in place.

**Prompt vocabulary**

Use the exact terms `Web Standard Request/Response`, `Hono Context`, `middleware order`,
`onion model`, `runtime validation`, `short-circuit response`, `Bindings`, `Hono RPC`, and
`typed client`. Ask for a route-to-middleware map and a runtime-boundary inventory; “write a Hono API”
doesn't constrain these critical behaviors.

...
Models mix Laravel 10 or earlier `app/Http/Kernel.php` and `RouteServiceProvider` registration patterns into the streamlined Laravel 13 skeleton. They also combine `$request->all()` with broad `$fillable` declarations, mistake route model binding for authorization, or access unloaded relationships inside resource loops. Another real failure is injecting the request into a singleton: ordinary feature tests pass, but Octane or a long-running queue worker leaks the preceding request's state.

**Ask your AI to check**

- Use `composer.lock` and the actual directory structure to list file paths, registration points, and APIs copied from older Laravel releases.
- Starting at each route, trace every input field through validation, authorization, domain rules, mass assignment, and database constraints.
- List every container binding and its lifetime, then say whether it resets after an Octane request or queue job.

**Review checklist**

- Request every write endpoint as an unauthenticated user, another tenant, with malformed JSON, and with extra sensitive fields.
- On the real resource serialization path, assert status, public fields, database changes, and query count.
- Exercise route listing, configuration caching, and long-running process behavior on the target version to catch old-skeleton assumptions and cross-request state.

**Prompt vocabulary**

Use the exact terms `request lifecycle`, `service container binding`, `zero-configuration resolution`, `scoped binding`, `route model binding`, `Form Request`, `mass assignment`, `eager loading`, and `object-level authorization`. Asking for an input-to-SQL data flow and a binding-lifetime table exposes boundary errors better than “write Laravel CRUD.”

...
**What generated code gets wrong here.** Models often reproduce an old tRPC setup from a different major version, mix client integration APIs, or import the runtime router into browser code. More serious failures are procedures without runtime input schemas, `protectedProcedure` used as a substitute for resource authorization, blind `data: input` writes that accept server-owned fields, and cache invalidation aimed at the wrong router path.

**Ask your AI to check.** Ask it to trace one procedure from serialized request through parsing, middleware, authorization, resolver, output mapping, and client handling. Ask it to list every field controlled by the caller versus derived from context, and to compile client and server with the pinned tRPC and TypeScript versions. Also ask it to replay an older request shape against the new router and explain the observed error.

**Review checklist**

- Every externally reachable procedure has bounded runtime input validation and an intentional public output shape.
- Authentication comes from trusted context, while tenant and object authorization stay attached to the data access or state transition.
- HTTP-path tests cover serialization, batching or partial failure where used, stable error codes, and compatibility with deployed clients.

**Prompt vocabulary.** Use `router type` (router 类型), `procedure builder` (procedure 构建器), `runtime input parser` (运行时输入解析器), `context narrowing` (上下文类型收窄), `object-level authorization` (对象级授权), `output validator` (输出验证器), `server-side caller` (服务端调用器), and `rolling-deployment compatibility` (滚动部署兼容性). These terms force a model to separate static inference from runtime trust boundaries.

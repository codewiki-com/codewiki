...
模型会把 WSGI 签名写成 `async def application(environ, start_response)`，或者让原始 ASGI callable 直接 `return Response(...)`，混淆框架 API 与底层协议。它也常只读取第一个 `http.request`、把 header 列表转成字典、忽略 `lifespan` 与 `websocket` scope，或在中间件修改一个消息后忘记继续 `await send(message)`。更隐蔽的问题是协程里残留同步数据库调用，以及创建无人持有、关闭时也不取消的后台任务。

**让 AI 检查**

- 列出每个入口与中间件支持的 scope 类型，并给出每种类型允许的入站、出站事件顺序。
- 追踪所有可从 `async def` 到达的 I/O 调用，标记哪些真正可等待、哪些在线程池运行、哪些仍会阻塞事件循环。
- 为分片请求、重复响应头、客户端断开、取消、启动失败和每 worker 资源清理生成测试，不接受只调用业务函数的测试替代协议测试。

**审查清单**

- 核对 callable 签名、`str`／`bytes` 边界、请求体终止条件，以及响应只开始一次。
- 让非目标 scope 原样透传，并验证 middleware 没有丢失 body 分片、重复头或最终 `more_body`。
- 在真实服务器进程中观察阻塞调用、断开清理、lifespan 次数和适配器线程行为。

**提示词词汇**

使用准确术语：`WSGI environ`（WSGI 环境）、`start_response callable`（响应开始 callable）、`response iterable`（响应可迭代对象）、`ASGI connection scope`（ASGI 连接 scope）、`receive event`（接收事件）、`send event`（发送事件）、`more_body`（后续响应体标记）、`lifespan protocol`（生命周期协议）、`backpressure`（背压）、`sync-to-async adapter`（同步转异步适配器）。要求模型输出事件序列和资源所有权，而不是泛泛地「改成异步」。

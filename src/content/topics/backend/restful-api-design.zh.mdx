...
description: 用资源、HTTP 方法、状态码、条件请求和稳定错误契约设计可理解、可重试且能安全演进的 RESTful API。
track: backend
section: http-apis
...
REST（Representational State Transfer，表述性状态转移）是一种网络应用架构风格，不是 JSON 格式、路由命名规范或特定框架。RESTful API 将可寻址的领域对象或能力建模为<Term id="resource">资源（resource）</Term>，客户端与服务端通过统一接口交换资源的<Term id="representation">表示（representation）</Term>。一个订单是资源，订单列表也是资源；JSON 只是它们可能采用的一种表示。
...
URI 标识资源，HTTP 方法表达对该资源的操作。集合和成员通常形成 `/orders` 与 `/orders/{orderId}` 这样的关系；查询参数用于筛选、排序、分页或选择表示，而不是隐藏另一套方法系统。路径采用复数还是单数属于团队约定，一致性比把某种拼写宣称为 REST 定律更重要。
...
第一个处理器把集合 URI 与成员 URI 分开，并让方法决定操作。创建成功返回 `201` 和 `Location`，不受支持的方法返回 `405` 和 `Allow`。
...
<TryToBreak items={['读取不存在的订单', '向成员 URI 发送 POST', '向未知集合发送请求']} />

</Depth>

这个内存处理器只演示可观察的 HTTP 决策，不是生产服务器。真实实现要验证 `sku` 与 `quantity`，根据已认证主体检查创建权限，并避免把数据库记录直接当响应表示。`Allow` 还应根据实际资源能力生成，而不是散落在多个分支中。
...
`POST` 在 HTTP 规范中不保证幂等，但应用可以通过<Term id="idempotency-key">幂等键（idempotency key）</Term>识别同一次业务尝试。服务端还要保存请求指纹；同一个键若配上不同输入，应被拒绝，而不是静默返回第一次操作的结果。
...
> [!PITFALL]
> 生成代码常把 `PUT` 实现成任意字段合并，又断言所有 `PATCH` 天然幂等。补丁是否幂等取决于媒体类型和操作，例如「把值设为 5」与「给值加 1」的重复效果不同。
...
> [!PITFALL]
> 模型常生成 `return { ...row }`，把内部备注、成本、软删除标志或其他租户的标识一起暴露。数据库新增列还会在没有 API 评审时改变公开响应。
...
模型会从函数名机械生成 `/getOrder`、`/updateOrderStatus` 和全 `POST` 路由，或者把部分字段合并标成完整 `PUT`。更危险的失败包括让 `GET` 触发付款或删除、把 ORM 记录整体展开到 JSON、用未经租户限定的内存 `Map` 保存幂等键，以及读取 `ETag` 后执行无条件数据库更新。生成的游标也常只包含非唯一时间戳，测试数据小时看不出重复和漏项。

**让 AI 检查**

- 为每条路由输出「方法、资源、是否安全、是否幂等、成功状态、失败状态、重试后果」表，并标出相互矛盾的实现。
- 追踪每个响应字段来自哪个持久化字段，以及在哪一步执行资源级授权；列出任何直接展开对象的位置。
- 模拟超时后重试、两个客户端携带同一旧 `ETag` 更新，以及相同排序值跨分页边界这三种时间线。

**审查清单**

- 对安全方法执行副作用审计，并重复发送幂等方法来验证最终效果而非响应相等。
- 在实际事务边界测试条件更新和幂等创建，包括并发、崩溃与多实例执行。
- 对完整 HTTP 响应做契约测试，覆盖状态码、响应头、媒体类型、错误结构和敏感字段缺失。

**提示词词汇**

使用准确术语：`resource`（资源）、`representation`（表示）、`uniform interface`（统一接口）、`safe method`（安全方法）、`idempotency`（幂等性）、`conditional request`（条件请求）、`strong ETag`（强实体标签）、`lost update`（丢失更新）、`Problem Details`（问题详情）、`stable total order`（稳定全序）。要求模型说明资源所有权、原子边界和可观察 HTTP 契约；「把它改成 RESTful」不足以约束结果。
...
服务端可以在 `GET` 或 `HEAD` 响应中发送 `ETag`。客户端随后发送 `If-None-Match`；当前表示仍匹配时，服务端用 `304 Not Modified` 避免传输响应体。缓存是否可以保存或复用响应还受 RFC 9111 的 `Cache-Control` 等规则约束，不能仅凭出现了 `ETag` 就推断。
...
<Term id="problem-details">问题详情（Problem Details）</Term>为 HTTP API 提供 `application/problem+json` 错误格式。核心字段包括作为问题类型标识的 `type`、面向人的 `title` 与 `detail`、对应此次问题的 `status`，以及标识具体发生实例的 `instance`。API 可以增加稳定扩展字段，例如字段级验证错误或可追踪请求标识。
...
<Term id="openapi">OpenAPI</Term>可以描述路径、方法、参数、请求体、响应、媒体类型和安全方案，是评审与生成工具的共同输入。它应列出每个重要失败分支以及 `Location`、`ETag`、`Allow`、`Retry-After` 等有行为意义的响应头，而不只是成功 JSON 模式。
...
- [RFC 5789：HTTP PATCH 方法](https://www.rfc-editor.org/rfc/rfc5789.html)
- [RFC 9457：HTTP API 问题详情](https://www.rfc-editor.org/rfc/rfc9457.html)
- [OpenAPI 规范](https://spec.openapis.org/oas/latest.html)
- [REST 架构风格：博士论文原文](https://ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm)

...
模型经常把 `app.listen()` 复制到 Hono 应用中，却没有为 Node.js 引入正确的运行时适配器；
或者在 Cloudflare 代码里读取 `process.env`，导致本地类型检查通过、部署后才失败。它也常把
`c.req.json()` 直接断言成接口，把认证中间件登记在路由后面，或在中间件中调用 `next()`
却既不 `await` 也不返回响应。生成的 RPC 客户端还可能基于丢弃了链式返回值的应用类型，
从而缺少路由，或者与实际部署的服务端版本漂移。

**让 AI 检查**

- 列出每条路由实际经过的中间件，按注册顺序标明短路分支和 `await next()` 后的响应阶段。
- 对每个请求输入指出运行时验证位置，并证明业务处理器只读取 `c.req.valid()` 的结果。
- 区分应用核心与 Node、Cloudflare、Bun 或 Deno 适配层，列出每个运行时专用 API 与绑定。

**审查清单**

- 用真实 `Request` 覆盖成功、未授权、验证失败、未找到和异常响应，同时断言状态、响应头与正文。
- 确认所有受保护路由在认证中间件之后注册，所有中间件要么返回 `Response`，要么正确 `await next()`。
- 验证 RPC 类型来自最终链式路由，客户端与服务端版本兼容，并保留运行时契约测试。

**提示词词汇**

使用准确术语：`Web Standard Request/Response`（Web 标准请求／响应）、`Hono Context`
（Hono 上下文）、`middleware order`（中间件顺序）、`onion model`（洋葱模型）、
`runtime validation`（运行时验证）、`short-circuit response`（短路响应）、`Bindings`
（运行时绑定）、`Hono RPC`（Hono RPC）和 `typed client`（类型化客户端）。要求模型输出
路由到中间件的映射和运行时边界清单；“写一个 Hono API”无法约束这些关键行为。

...
模型经常混用 Axum 0.7 与 0.8 的路由语法，把消费正文的 `Json<T>` 放错参数位置，或者给所有错误返回 `200` JSON。它还会在 `async fn` 中调用同步 ORM，把 `MutexGuard` 带过 `.await`，为每条记录启动不受控任务，并在超时后假设写入已取消。认证示例尤其容易只检查 `Authorization` 是否以 `Bearer ` 开头，却不验证签名、签发者、受众和资源所有权。

**让 AI 检查**

- 根据锁文件列出 Rust、Axum、Tokio 与 Tower 的实际版本，并指出每个示例 API 对应的版本文档。
- 画出每个 `.await` 前后仍然存活的锁守卫、借用和共享状态，同时标出可能阻塞运行时线程的调用。
- 对每个超时、重试和 `tokio::spawn` 说明任务所有者、取消动作、副作用是否可能已提交，以及谁负责等待结束。

**审查清单**

- 用畸形路径、错误媒体类型、无效 JSON、边界数量和第二个租户请求真实 `Router`，断言状态码与公开响应字段。
- 搜索同步 I/O、无界通道、无界正文收集和循环内 `spawn`，为每一处写出容量限制。
- 从关闭信号追踪到监听器、请求任务、后台任务与阻塞工作，确认有截止时间并记录未完成工作。

**提示词词汇**

使用准确术语：`async runtime`（异步运行时）、`request extractor`（请求提取器）、`FromRequestParts`（请求部件提取）、`Tower layer`（Tower 层）、`shared state`（共享状态）、`blocking boundary`（阻塞边界）、`cancellation safety`（取消安全）、`backpressure`（背压）、`graceful shutdown`（优雅关闭）。要求模型输出请求生命周期和任务所有权表，而不是笼统地「让代码更异步」。

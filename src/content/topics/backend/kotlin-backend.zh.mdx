...
模型经常在 Ktor 路由中生成 `GlobalScope.launch` 来发送通知，导致响应已经成功而任务仍可能在重启时丢失。它还会把 `call.receive<T>()` 或 `@Serializable` 当成业务校验，在可空路径参数后补 `!!`，或者把阻塞 JDBC 调用放进 `suspend` 函数便宣称它是非阻塞的。另一类常见错误是依次写 `async { a() }.await()` 和 `async { b() }.await()`，代码看似并发，实际仍按顺序等待。

**让 AI 检查**

- 列出每条路由接收的不可信值、运行时校验、领域转换和所有可能的 HTTP 响应。
- 为每个 `launch`、`async` 和自建 `CoroutineScope` 标出父 `Job`、所有者、取消路径与服务关闭行为。
- 识别所有阻塞库调用，并说明其调度器、超时、连接池及请求取消后的实际行为。

**审查清单**

- 用畸形 JSON、缺失字段、边界数值与未授权身份通过真实 HTTP 测试调用端点。
- 检查取消与异常是否传播到同级子任务，响应后副作用是否经过持久机制。
- 确认响应 DTO 不含数据库字段、秘密、堆栈信息或原始异常消息。

**提示词词汇**

使用准确术语：`request boundary`（请求边界）、`structured concurrency`（结构化并发）、`parent Job`（父 `Job`）、`cooperative cancellation`（协作式取消）、`blocking boundary`（阻塞边界）、`runtime validation`（运行时校验）、`idempotency key`（幂等键）、`transactional outbox`（事务 outbox）。要求模型画出所有权与故障传播路径；「把它改成异步」不足以说明线程和生命周期约束。

...
生成的 Django 代码经常把结构检查误当作授权：按 `pk` 找到对象后，就把它返回给任意已登录用户。它还倾向于把 `model_to_dict()` 的结果或所有模型字段暴露为 JSON，造成批量赋值和数据泄露路径。其他常见问题包括为了让 POST 通过而添加 `@csrf_exempt`、在 `async def` 中调用同步 ORM、在 QuerySet 循环中隐藏 N+1 查询，以及修改模型却没有迁移或上线计划。

**让 AI 检查**

- 跟踪一个写请求经过 URL 解析、中间件、认证、对象授权、验证、事务边界，直至最终响应的完整过程。
- 列出每个 QuerySet 求值点和每次关联字段访问，然后预测并测量多行数据对应的查询数。
- 对比模型与迁移状态，找出旧应用版本仍在提供服务时无法安全上线的数据库模式变更。

**审查清单**

- 发送格式错误的 JSON、缺失字段、边界值、跨租户对象 ID、过期的并发更新，并把同一写请求发送两次。
- 启用 CSRF 检查，让视图经过中间件；不要直接调用视图函数，也不要只依赖 Django 测试客户端的默认行为。
- 检查生成的设置和响应中是否包含密钥、`DEBUG`、过宽的主机或来源信任、内部字段，以及错误消息造成的意外披露。

**提示词词汇**

使用准确术语：`URLconf`（URL 配置）、`path converter`（路径转换器）、`middleware ordering`（中间件顺序）、`object-level authorization`（对象级授权）、`QuerySet evaluation`（QuerySet 求值）、`N+1 query`（N+1 查询）、`select_related`、`prefetch_related`、`database constraint`（数据库约束）、`transaction.atomic`、`ASGI`、`async-safety`（异步安全）、`CSRF` 和 `deployment check`（部署检查）。还要明确认证机制和数据所有权规则；「让这个 Django 端点更安全」没有说明其中任何一层边界。

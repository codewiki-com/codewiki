...
Models readily produce tidy CRUD routes while copying in `200` error envelopes, arbitrary mass assignment, and unbounded `limit` parameters. They also claim that `Idempotency-Key` makes a `POST` safe while generating a fresh key inside every retry or never persisting the key with the result. For concurrent updates, generated code often reads an `ETag`, compares it in process, and issues an unconditional database write. That looks conditional but still loses updates.

**Ask your AI to check**

- Enumerate success and failure responses for every operation, then check method semantics, status, `Content-Type`, `Location`, `ETag`, and caching headers.
- Trace one idempotency key through a retry after timeout, proving that request fingerprinting, uniqueness, transaction commit, and cached response share one scope.
- Draw timelines for two concurrent writers and for an insert between pages; identify whether version comparison is atomic and whether ordering has a unique tie-breaker.

**Review checklist**

- Attack the generated input boundary with unknown fields, unauthorized resources, oversized pages, invalid cursors, and unsupported media types.
- Inject a lost response after commit and confirm that retrying does not charge, create, or emit a side effect twice.
- Generate contract tests from both OpenAPI and the implementation, then compare status codes, headers, schemas, and empty bodies.

**Prompt vocabulary**

Use the exact terms `resource`, `representation`, `safe method`, `idempotent method`, `problem details`, `strong ETag`, `conditional request`, `lost update`, `stable total order`, and `opaque cursor`. Ask for a failure matrix and concurrency timeline. “Generate a best-practice API” is too vague to constrain retries, authorization, or races.

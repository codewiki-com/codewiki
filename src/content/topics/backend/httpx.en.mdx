...
Models often instantiate `AsyncClient` for every FastAPI request or loop iteration, preventing connection reuse, and they describe `timeout=5.0` as a complete five-second deadline. Other common failures parse `.json()` before checking status or forget `aclose()` on early-return and cancellation paths when `stream=True`. Generated retry wrappers also retry every method and multiply retries across an SDK, proxy, and job queue without an idempotency key or shared budget.

**Ask your AI to check**

- List the creator, sharing scope, and close path of every `Client` and `AsyncClient`, and identify any created in a hot loop.
- Draw the timeout budget across pool, connect, write, read, and the overall deadline, then calculate the maximum attempts from nested retry layers.
- Trace success, status error, transport error, and cancellation for every streaming call and every side-effecting request.

**Review checklist**

- Confirm an async call chain contains no synchronous HTTP client and that concurrency and connection limits have explicit bounds.
- Confirm status and media type are handled before body parsing, and exception logs do not leak tokens, cookies, or sensitive query parameters.
- Confirm streams always close and only operations with idempotent semantics, backoff, an attempt cap, and an overall deadline are retried.

**Prompt vocabulary**

Use the exact terms `client lifetime`, `connection pooling`, `transport error`, `phase timeout`, `overall deadline`, `streaming response cleanup`, `bounded concurrency`, and `idempotency key`. Ask the model for a resource-ownership table, failure-state matrix, and retry budget instead of merely saying “add error handling.”

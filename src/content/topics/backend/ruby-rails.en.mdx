...
Models often generate `Order.find(params[:id])` plus strong parameters but omit account scoping or policy authorization. They use `permit!` to silence `Unpermitted parameter`, or return `order.attributes` and leak internal columns into the API. Other failures hide in lifecycle code: an `after_save` sends a notification before commit and retries duplicate it, while a serializer reads unloaded associations in a loop and creates N+1 queries. Models also splice initializer, asset-pipeline, or deployment snippets from old tutorials into a Rails 8.1 application without checking its real tree or `Gemfile.lock`.

**Ask your AI to check**

- Starting from every write route, trace the authenticated principal, object lookup, authorization, permitted fields, domain rules, transaction, and database constraints.
- List every association read by views and serializers, then report the actual SQL count for a fixed data shape.
- Compare file paths and APIs against `Gemfile.lock`, `bin/rails routes`, and the current application tree, marking anything copied from another Rails version.

**Review checklist**

- Request every write endpoint as an unauthenticated user, another tenant, with malformed input, and with extra sensitive fields.
- Assert the status, complete public response, database changes, absent fields, and external calls together.
- Run migrations, request tests, and query counts on the target version and deployment mode; code shape alone is not evidence.

**Prompt vocabulary**

Use the exact terms `request lifecycle`, `resource routing`, `strong parameters`, `object-level authorization`, `mass assignment`, `Active Record transaction`, `eager loading`, and `N+1 query`. Ask for an input-to-SQL data flow, the boundary between transactions and external effects, and an endpoint-by-endpoint authorization matrix; "generate Rails CRUD" is too vague to expose these gaps.

...
Generated Django code often mistakes shape checks for authorization: it resolves an object by `pk` and returns it to any logged-in user. It also tends to expose `model_to_dict()` or every model field as JSON, creating mass-assignment and data-leak paths. Other recurring failures are `@csrf_exempt` added to silence a failing POST, sync ORM calls inside `async def`, a QuerySet loop that hides N+1 queries, and model edits without migration or rollout planning.

**Ask your AI to check**

- Trace one write request through URL resolution, middleware, authentication, object authorization, validation, transaction boundaries, and the final response.
- List every QuerySet evaluation point and every related-field access, then predict and measure the query count for multiple rows.
- Compare models with migration state and identify schema changes that cannot safely roll forward while the previous application version is still serving.

**Review checklist**

- Send malformed JSON, missing fields, boundary values, a cross-tenant object ID, a stale concurrent update, and the same write twice.
- Run the view through middleware with CSRF checks enabled instead of calling the view function directly or relying only on Django's default test client behavior.
- Inspect generated settings and responses for secrets, `DEBUG`, broad host or origin trust, internal fields, and accidental disclosure through error messages.

**Prompt vocabulary**

Use `URLconf`, `path converter`, `middleware ordering`, `object-level authorization`, `QuerySet evaluation`, `N+1 query`, `select_related`, `prefetch_related`, `database constraint`, `transaction.atomic`, `ASGI`, `async-safety`, `CSRF`, and `deployment check`. Name the authentication mechanism and data ownership rule; “make this Django endpoint secure” does not specify either boundary.

...
Models often generate middleware that calls `decode`, checks `exp`, and never verifies a signature. They also call a verification function without supplying `issuer`, `audience`, or an algorithm allowlist. Other implementations trust `kid` or `jku` directly and introduce path traversal, injection, or SSRF. Refresh endpoints commonly read an old token, delete it, then write a new one, leaving a concurrent replay window.

**Ask your AI to check**

- Enumerate every data flow from the raw token to the request identity, and identify the exact step after which the header and payload become trusted.
- Produce a validation-policy matrix for every JWT kind, including algorithm, key source, `typ`, `iss`, `aud`, required claims, and time rules.
- Test refresh rotation with two concurrent refresh requests and one replay of the old token; verify atomic consumption and whole-family revocation.

**Review checklist**

- Confirm that algorithms and keys come from server trust configuration, unknown `kid` values fail closed, and token-provided URLs are never fetched arbitrarily.
- Confirm that signature, `iss`, `aud`, `exp`, `nbf`, and token purpose all pass before an identity reaches authorization code.
- Confirm that logs, URLs, exceptions, and trace attributes omit complete tokens, while logout and security events revoke refresh capability.

**Prompt vocabulary**

Use the exact terms `JSON Web Token`, `JOSE header`, `claim validation`, `algorithm allowlist`, `issuer-bound key`, `audience restriction`, `cross-JWT confusion`, `refresh token rotation`, `replay detection`, and `fail closed`. Ask the AI to state trust boundaries and rejection paths; “make JWT more secure” does not constrain an implementation.

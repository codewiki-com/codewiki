...
> [!PITFALL]
> Returning `{ ...row }` directly can expose internal notes, costs, soft-delete flags, or another tenant's identifiers. Adding a database column can then change the public response without an API review.

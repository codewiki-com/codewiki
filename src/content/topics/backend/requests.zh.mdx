---
title: Requests
description: 用 Python Requests 构造 HTTP 请求、处理响应、复用会话，并为超时、重试、流式读取和 TLS 验证设定清晰边界。
track: backend
section: http-apis
...
`Session` 保存默认请求头、Cookie、认证和适配器配置，并通过 urllib3 复用同一主机的底层连接。这个<Term id="connection-pool">连接池（connection pool）</Term>只有在响应体被读完或响应明确关闭后才能顺利回收连接。会话因此同时具有配置状态和网络资源生命周期。
...
<TryToBreak items={['返回 HTML 的 404 响应', '没有响应体的 204 响应', '在响应头之后停住的服务端']} />
...
`Retry` 的 `total` 表示首次尝试之后允许的重试次数上限，而不是总调用次数。即使 HTTP 方法按规范具有<Term id="idempotency">幂等性（idempotency）</Term>，客户端仍要确认服务端实现、请求体和前置条件确实允许重放。
...
> [!PITFALL]
> Requests 默认不会超时。传入一个数字会同时设置连接和读取超时，但读取超时衡量的是套接字多久没有收到字节，并不是整个下载必须在该秒数内结束。
...
> [!PITFALL]
> `response.json()` 可以成功解析 `500` 的错误响应，也可能在 `204`、HTML 网关错误或截断内容上失败。JSON 可解析与业务成功是两个独立事实。
...
> [!PITFALL]
> 长期全局 `Session` 可能把 Cookie、认证或默认请求头带到不相干的调用。`stream=True` 的响应若既没读完也没关闭，还会占住连接，削弱池的复用能力。
...
模型常生成没有 `timeout` 的一行式调用，再用 `except Exception: return None` 把超时、DNS 失败、TLS 错误和 HTTP 错误压成同一个结果。它也会先执行 `response.json()` 再执行 `raise_for_status()`，把所有方法加入重试集合，或为解决本地证书问题添加 `verify=False`。更隐蔽的版本是把带管理员令牌的全局 `Session` 用于用户提供的 URL，或者开启 `stream=True` 后只读一小段却没有关闭响应。

**要求 AI 检查**

- 列出每个请求的连接超时、读取超时、总截止时间和最大尝试次数，并计算最坏情况调用预算。
- 按 HTTP 方法说明每次重试为何可安全重放，并指出服务端幂等性机制和 `Retry-After` 行为。
- 追踪 URL、重定向、代理、Cookie 与 `Authorization` 的信任边界，标出每个响应在哪条路径上关闭。

**审查清单**

- 失败路径能区分传输失败、HTTP 错误、解码错误和领域错误，且日志已限制大小并脱敏。
- 所有调用都有超时；流式响应在上下文中消费；会话不会跨认证租户泄漏状态。
- TLS 验证保持开启；不可信地址及其每个重定向目标都经过验证；重试仅覆盖允许重放的操作。

**提示词词汇**

使用准确术语：`prepared request`（已准备请求）、`connect timeout`（连接超时）、`read timeout`（读取超时）、`total deadline`（总截止时间）、`connection pool`（连接池）、`idempotency`（幂等性）、`retry budget`（重试预算）、`response streaming`（响应流式读取）、`certificate verification`（证书验证）和 `SSRF allowlist`（SSRF 允许列表）。要求模型输出失败矩阵和资源生命周期，而不是笼统地「添加错误处理」。

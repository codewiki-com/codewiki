...
Models often join `file.originalname` into a path, check only `file.mimetype`, and return a public URL before scanning. Another common mistake is reading the complete request into memory merely to use `async/await`, or generating a presigned URL and trusting the client's completion callback. Generated chunking examples also keep sessions in a process-local `Map`, omit part index, length, and digest checks, and never expire abandoned sessions.

**Ask your AI to check**

- Enumerate every byte, time, field-count, decompressed-output, and concurrency limit from the proxy through the format parser, and identify where each value is actually counted.
- Trace the trust source for the original filename, declared type, object key, checksum, and scan result, proving that no client value directly triggers publication.
- Enumerate state and cleanup behavior after initialization failure, interrupted streaming, repeated parts, repeated completion, scanner outage, and database commit failure.

**Review checklist**

- Test fail-closed behavior with an empty file, spoofed `Content-Type`, valid signature plus corrupt structure, oversized stream, and decompression bomb.
- Verify that unauthenticated users, the wrong owner, and clients holding expired or reused presigned capabilities cannot publish an object.
- Inject failure at every I/O boundary and confirm that no unscanned public file, dangling database record, or unreclaimable temporary data remains.

**Prompt vocabulary**

Use the exact terms `multipart/form-data boundary`, `streaming byte limit`, `backpressure`, `server-generated object key`, `quarantine-to-available transition`, `content signature`, `full decoder validation`, `presigned upload capability`, and `idempotent finalize`. Ask the AI for a state machine and failure matrix; “write a secure upload endpoint” is too vague to expose the missing transitions.

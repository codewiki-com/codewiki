...
Models often generate `let (data, _)` and decode immediately, discarding the HTTP status and headers. They interpolate paths, queries, and tokens into one force-unwrapped string, make a fresh session on every retry, or apply three rounds of exponential backoff to every POST. Authentication code is particularly prone to a refresh storm: concurrent 401 responses each refresh the token, then one business response is incorrectly delivered to every waiter. Another common patch swallows the real error with `try?` and prints the entire body "for debugging."

**Ask your AI to check**

- List the path for transport errors, non-HTTP responses, each accepted status, and decoding failure at every call site.
- Produce a session-ownership table that says when configuration is copied, who owns each delegate, when invalidation occurs, and how cancellation propagates.
- For every automatic retry, state the method, endpoint idempotency contract, attempt limit, total deadline, `Retry-After` handling, and duplicate-write protection.

**Review checklist**

- Test success, 404, 429, malformed JSON, connection failure, and cancellation with a controlled `URLProtocol`, without touching a live service.
- Search for URL string interpolation, force unwraps, `try?`, per-call sessions, and logs containing authentication headers or raw bodies.
- Trace task ownership during concurrent 401 responses, view disappearance, and app backgrounding; rule out refresh storms, orphaned work, and crossed responses.

**Prompt vocabulary**

Use the exact terms `URLSessionConfiguration`, `URLRequest`, `transport error`, `HTTP status validation`, `task cancellation`, `idempotency key`, `Retry-After`, `URLProtocol test double`, and `delegate invalidation`. Ask for a status matrix, ownership table, and failure timeline. "Write a robust networking layer" is too vague to expose bad assumptions.

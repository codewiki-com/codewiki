...
模型经常混用不同 Flask 版本的片段，例如生成已删除的 `before_first_request`、`FLASK_ENV` 或旧 JSON 配置键。它还会把 `request.json` 当成已验证模型，把 `app.run(debug=True)` 留在部署入口，或者在后台函数中继续引用 `request` 代理。另一类错误是把 `async def` 解释成自动增加 WSGI 并发量，并在视图里调用 `asyncio.create_task()` 后立即返回。

**让 AI 检查**

- 按 Flask 3.1.3 检查每个导入、配置键和生命周期钩子，并列出已删除或弃用的写法。
- 为每条路由列出允许方法、输入来源、验证规则、成功状态码、错误状态码和授权检查。
- 跟踪 `request`、`session`、`g` 与 `current_app` 的每次使用，确认没有越过上下文生命周期或执行单元。

**审查清单**

- 通过测试客户端发送错误媒体类型、畸形 JSON、错误路径转换值和未授权请求，检查完整响应。
- 确认路由、蓝图、扩展和错误处理器都在应用工厂返回前完成注册，而且测试实例之间没有状态泄漏。
- 检查生产入口不启动开发服务器或调试器，并验证代理、密钥、Cookie 和请求大小配置来自受控部署环境。

**提示词词汇**

使用准确术语：`application factory`（应用工厂）、`blueprint registration`（蓝图注册）、`request context`（请求上下文）、`application context`（应用上下文）、`context-local proxy`（上下文局部代理）、`view return conversion`（视图返回值转换）、`teardown callback`（清理回调）、`WSGI worker model`（WSGI 工作单元模型）。要求 AI 给出请求生命周期和失败响应矩阵，比「生成一个 Flask API」更容易暴露边界错误。

...
Models often generate a resolver that loads by `args.userId` without tying that ID to the principal in context. They also put DataLoader in module scope and reuse memoized values across tenants. Another common result awaits one query for every list item, or adds only a fixed depth limit while missing aliases, expanded fragments, pagination multipliers, and costly fields. Bulk-added `!` markers copied from ORM types turn one recoverable data gap into a missing response subtree through null bubbling.

**Ask your AI to check**

- List every route from each root field to a data source, and identify where object-level authorization runs and which authenticated principal it uses.
- Compute field cost for an operation with aliases, fragments, and the maximum page size, then report its actual downstream call count.
- List the failure sources for every Non-Null field and show the propagation path when any resolver returns `null` or throws.

**Review checklist**

- Execute the same operation for two tenants in sequence and confirm that both context and DataLoader are request-scoped.
- Record resolver and database call counts with at least three list rows so a one-row fixture can't hide N+1 behavior.
- Inject a mid-write failure and a disconnect after commit into mutations, then verify transaction boundaries and idempotent retry.

**Prompt vocabulary**

Use the exact terms `schema validation`, `selection set`, `field resolver`, `resolver context`, `object-level authorization`, `null bubbling`, `request-scoped DataLoader`, `operation cost`, and `trusted document`. Ask for an authorization matrix, resolver call graph, and Non-Null propagation path; "make GraphQL secure" doesn't define a review boundary.

...
Models often infer a polished OpenAPI document from a controller but write `201` where the code returns `200`, omit `Location`, use the wrong error media type, or miss authentication. They also copy a database model into the public schema, exposing internal fields, and make every property optional to silence type errors. Another common failure is generating a success-only mock and test, so the specification and implementation jointly miss authorization failures, unknown fields, and boundary values.

**Ask your AI to check**

- List each operation's actual statuses, media types, and required response headers, then compare every item with the published contract.
- Trace one consumer task through its request schema, success response, and every recoverable failure; identify behavior that exists only in code.
- Classify a specification change from the request producer, request consumer, response producer, and response consumer perspectives instead of giving one broad compatibility verdict.

**Review checklist**

- Run valid, invalid, and unauthorized requests against the real HTTP service, save the exchanges, and execute contract verification.
- Check whether public schemas accidentally contain database fields, internal enums, or resource identifiers with no authorization semantics.
- Diff against the last published contract and test the migration assumption with one client that is still in use.

**Prompt vocabulary**

Use the exact terms `API-first`, `API contract`, `provider verification`, `consumer-driven contract`, `schema conformance`, `breaking change`, `tolerant reader`, and `request/response compatibility`. Ask for operation IDs and exact JSON Pointers; "check API consistency" is too vague to produce an auditable result.

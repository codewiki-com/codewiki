...
Models often emit a one-line call without `timeout`, then use `except Exception: return None` to collapse timeouts, DNS failures, TLS errors, and HTTP responses into one result. They also call `response.json()` before `raise_for_status()`, put every method into the retry set, or add `verify=False` to cure a local certificate problem. A subtler variant uses one global `Session` carrying an administrator token for user-supplied URLs, or reads only a prefix from `stream=True` without closing the response.

**Ask your AI to check**

- List the connect timeout, read timeout, overall deadline, and maximum attempts for every call, then calculate the worst-case call budget.
- Explain why each retry is replay-safe by HTTP method, naming the server-side idempotency mechanism and `Retry-After` behavior.
- Trace trust boundaries for URLs, redirects, proxies, cookies, and `Authorization`, and identify where every response closes on every path.

**Review checklist**

- Failure paths distinguish transport, HTTP, decoding, and domain errors, and logs are bounded and redacted.
- Every call has timeouts, streamed responses are consumed in context, and sessions do not leak state across authentication tenants.
- TLS verification remains enabled, untrusted targets and every redirect are validated, and retries cover only replayable operations.

**Prompt vocabulary**

Use the exact terms `prepared request`, `connect timeout`, `read timeout`, `total deadline`, `connection pool`, `idempotency`, `retry budget`, `response streaming`, `certificate verification`, and `SSRF allowlist`. Ask for a failure matrix and resource lifecycle instead of vaguely asking to "add error handling."

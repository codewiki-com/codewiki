...
Models commonly join three similar names into a plausible manifest that cannot run: the Pod's `runtimeClassName`, the RuntimeClass name, and the CRI handler all become one guessed value, followed by a fictitious `wasm32` node architecture. They also combine the old `wasm32-wasi` target, a Preview 1 command module, and a Preview 2 HTTP component, or label raw Wasm bytes as a standard OCI filesystem layer that requires a tar archive. On security, generated configuration often claims that “Wasm is secure by default” while mounting host directories, injecting every secret, and permitting arbitrary egress.

**Ask your AI to check**

- List whether the artifact is a core module or component, its WASI target and world, every import, and how the target runtime satisfies or denies each one.
- Trace actual names from Pod `runtimeClassName` through RuntimeClass `metadata.name`, `handler`, containerd CRI configuration, and the shim binary.
- Parse the OCI manifest and recalculate every blob's size and SHA-256 digest, then check whether both the registry and runtime support each media type.

**Review checklist**

- Execute the artifact in the target runtime and retain failing tests for missing imports and denied directory or network access.
- Confirm that scheduling is blocked on nodes without the Wasm shim, then prove that a real Pod starts and its probes work on an eligible node.
- Check shim and engine versions, artifact digests, and outer Pod permissions; do not substitute the Wasm sandbox for supply-chain and cluster controls.

**Prompt vocabulary**

Use the exact terms `core Wasm module`, `WebAssembly component`, `WASI world`, `WIT imports and exports`, `ambient authority`, `OCI descriptor`, `containerd shim`, `CRI runtime handler`, and `RuntimeClass`. Ask the AI for a compatibility matrix across build, artifact, and runtime; “deploy it as a Wasm container” does not identify any interface version or permission boundary.

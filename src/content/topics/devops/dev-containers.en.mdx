...
Models often treat `remoteUser` as Docker's `USER`, name a nonexistent `vscode` account, or generate
conflicting `image`, `build`, and Compose settings. They copy the obsolete Compose `version` field,
use floating `latest` tags, install dependencies in `postStartCommand`, and hide creation failures
with `|| true`.

The security mistakes are concrete. Generated configurations put tokens in `containerEnv`, mount
an entire SSH directory or the Docker socket, and add `privileged: true` to get past a debugging
problem. Generated multi-service files also point the application at `localhost`, omit database
health checks, or publish the database port to the host without a reason.

**Ask your AI to check**

- List every host path, socket, device, credential agent, and port the environment can access, then propose the smallest required set.
- Expand lifecycle commands across first creation, resume, and reattachment, naming the execution location, user, wait point, and failure behavior.
- Resolve mutable image, Feature, and language dependency references; provide a digest or lockfile and the CI command that verifies it.

**Review checklist**

- Run `devcontainer build` or `up` from a clean cache, then run version checks and project tests as the remote user.
- Search for `privileged`, `capAdd`, Docker sockets, broad bind mounts, and plaintext credentials; justify every occurrence.
- Rebuild and restart the environment; confirm lifecycle commands are repeatable and Compose dependencies become healthy.

**Prompt vocabulary**

Use `devcontainer.json` (开发容器配置), `Feature resolution` (Feature 解析),
`containerUser` (容器用户), `remoteUser` (远程用户), `lifecycle command` (生命周期命令),
`waitFor` (连接等待点), `bind mount` (绑定挂载), `OCI digest pinning` (OCI 摘要固定), and
`frozen lockfile` (冻结锁文件). Name the implementation and version too. "Generate a secure Dev
Container" does not constrain the permission model.

...
模型经常把 `remoteUser` 当成 Docker 的 `USER`，引用基础镜像中不存在的 `vscode` 用户，
或同时生成互相冲突的 `image`、`build` 和 Compose 配置。它还会照搬旧示例中的 Compose `version` 字段、
使用浮动的 `latest`、把安装放进每次运行的 `postStartCommand`，以及用 `|| true` 掩盖创建失败。

安全问题更具体：生成配置会把令牌写入 `containerEnv`，挂载整个 SSH 目录或 Docker 套接字，
并为调试问题直接加入 `privileged: true`。生成的多服务配置也常把应用连接地址写成 `localhost`，
遗漏数据库健康检查，或把数据库端口无条件发布给宿主机。

**让 AI 检查**

- 列出环境能读写的每个宿主机路径、套接字、设备、凭据代理和端口，并说明最小化方案。
- 按首次创建、恢复和重新连接三条路径展开生命周期命令，标明执行位置、用户、等待点和失败传播。
- 解析镜像、Feature 与语言依赖的可变引用，给出锁文件或摘要，并说明怎样在 CI 中验证。

**审查清单**

- 在干净缓存上执行 `devcontainer build` 或 `up`，再以远程用户运行版本检查和项目测试。
- 搜索 `privileged`、`capAdd`、Docker 套接字、宽泛绑定挂载与明文凭据，逐项确认必要性。
- 重建并重启环境，验证生命周期命令不会重复破坏状态，Compose 依赖按健康状态就绪。

**提示词词汇**

提示中使用 `devcontainer.json`（开发容器配置）、`Feature resolution`（Feature 解析）、
`containerUser`（容器用户）、`remoteUser`（远程用户）、`lifecycle command`（生命周期命令）、
`waitFor`（连接等待点）、`bind mount`（绑定挂载）、`OCI digest pinning`（OCI 摘要固定）和
`frozen lockfile`（冻结锁文件）。还要写明实现工具与版本；只说“生成一个安全的开发容器”无法限定权限模型。
